...
> **RETIRED 2026-08-27 to `reference`.** This file is the audit and decision record: the
> settability matrix (§1–§8), owner rulings D1–D8 (§10, §12, §14), the storage-model option space
> (§13), and the Arc 1 execution record (§12.5–§14.9). All remaining work continues in
> `resource-surface-consolidation-2026-08-27.md`, whose §"What already landed" names the five commits.
> Every row here is terminal — `✓` done, `⚠` premise falsified, or `✗ SUPERSEDED` with the
> successor row named. The prose-form open items in §8's "Row status" and "Also-flagged and SF
> rows" blocks are superseded wholesale: gaps 2/3/4 → P2.1–P2.3, row 5b → P2.4, row 5c → P4.6,
> SF-1 → P4.1, SF-2 → P2.2, SF-3 → P4.2, SF-4 → P4.3, gate severity → P4.4, framework passthrough
> → P4.5, category type → P4.7, `create_prompt` bridge → P4.8.
>
...
| T1.2 | ✗ SUPERSEDED (2026-08-27 · continues as `resource-surface-consolidation-2026-08-27.md` P1.1) | Loaded prompts record their source root                                                                                                                                                                                   | T1.1                                                                                                          | a prompt loaded from an overlay reports the overlay root, one from the bundle reports the package root                             |
| T1.3 | ✗ SUPERSEDED (2026-08-27 · continues as `resource-surface-consolidation-2026-08-27.md` P1.2) | `update` writes to the source root; falls back to copy-on-write into the workspace when the source root is the bundle and a workspace exists                                                                              | T1.2                                                                                                          | update to an overlay-resident prompt rewrites the overlay file and creates nothing under `server/resources/prompts` (closes T1-F1) |
| T1.4 | ✗ SUPERSEDED (2026-08-27 · continues as `resource-surface-consolidation-2026-08-27.md` P1.3) | `delete` of a bundled prompt refuses with the shadow named as the alternative                                                                                                                                             | T1.2                                                                                                          | delete of a bundled prompt under a distinct workspace returns a refusal and removes no file                                        |
| T1.5 | ✗ SUPERSEDED (2026-08-27 · continues as `resource-surface-consolidation-2026-08-27.md` P1.4) | Receipt reports the write destination root; `category_ship_status` + `readCategoryShipStatus` retired in the same change                                                                                                  | T1.3                                                                                                          | `rg "readCategoryShipStatus\|resolveCategoryShipStatus"` returns zero; receipt names the root (closes T1-F3)                       |
| T1.6 | ✗ SUPERSEDED (2026-08-27 · continues as `resource-surface-consolidation-2026-08-27.md` P1.6) | grep -c prompt.yaml` equals the on-disk count)                                                                                                                                                                            | Migrate the 83 personal prompts to a workspace outside the repo; delete `server/resources/prompts/.gitignore` | T1.5                                                                                                                               | `git ls-files server/resources/prompts \| grep -c prompt.yaml` equals the on-disk count (closes P7-F4, D5) |
...
| #     | Status                                                                                       | Change                                                                                                                                             | Depends | Verification                                                                |
| ----- | -------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- | ------- | --------------------------------------------------------------------------- |
| T1-F5 | ✗ SUPERSEDED (2026-08-27 · continues as `resource-surface-consolidation-2026-08-27.md` P1.5) | Personal prompt store moves outside every checkout, so the served catalog does not depend on which worktree launched the session (folds into T1.6) | T1.3    | prompt count served from a fresh worktree equals the count served from main |
...
| ----- | -------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------ |
| T1-F6 | ✗ SUPERSEDED (2026-08-27 · continues as `resource-surface-consolidation-2026-08-27.md` P1.0) | Does the personal overlay need a dedicated env var, or is relocating state/logs alongside resources acceptable? Decide before the operator migration, not after | flips when the operator's `MCP_WORKSPACE` change is shown not to move `state.db` unintentionally |
...
| #     | Status                                                                                       | Change                                                                                          | Verification                                                                     |
| ----- | -------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------- |
| T1.14 | ✗ SUPERSEDED (2026-08-27 · continues as `resource-surface-consolidation-2026-08-27.md` P5.3) | `spawnMcpServer` strips `NODE_ENV`/`JEST_WORKER_ID` from the child env like the new helper does | a server spawned through the shared helper answers `initialize` from inside jest |
...
| #     | Status                                                                                       | Change                                                                                                                                                                                                                                                                     | Verification                                                                                    |
| ----- | -------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- |
| T1.15 | ✗ SUPERSEDED (2026-08-27 · continues as `resource-surface-consolidation-2026-08-27.md` P5.2) | Inventory reports load FAILURES, not just successes — a count of 120 against 123 on disk is still a silent subset. Needs a failure count threaded through `CategoryPromptsResult` (`modules/prompts/types.ts:69`), which today carries only `promptsData` and `categories` | startup line reads `120 (17 categories, 3 failed)` against the current main tree                |
| T1-F7 | ✗ SUPERSEDED (2026-08-27 · continues as `resource-surface-consolidation-2026-08-27.md` P5.1) | Repair the three prompts that fail schema validation, and the five with dropped inline gates                                                                                                                                                                               | a startup against the main tree logs zero `[ERROR] Invalid YAML` and zero `Dropped inline gate` |
