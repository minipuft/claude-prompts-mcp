...
**Status**: **COMPLETE 2026-08-11.** All 68 rows closed: 62 ✓, 6 ⊘ (4 refused on evidence, 2
superseded). Every tier — 0, 1, 2, 3, 4, 5, 6, IT, E2E, V, ORD — is terminal. The last open row,
5.7, was retired 2026-08-11 once its condition was measured true.

> **The status block that stood here was falsified by its own document** and had been for weeks:
> it read _"Tiers 0–2 complete except 0.1 and 2.3 (partial)… Tiers 3, 4, IT unstarted"_ while the
> body below recorded all three as closed, plus four tiers that did not exist when it was written.
> Kept as a marker, not deleted: this plan's central diagnosis is **Untrusted Inventory**, and its
> own header was the longest-lived instance of it. Trust the row marks; re-measure the prose.

| Measure                | Baseline  | Tier 2    | Final         | Δ          |
| ---------------------- | --------- | --------- | ------------- | ---------- |
| knip unused files      | 57        | 22        | —             | −35        |
| knip duplicate exports | 3         | 1         | —             | −2         |
| depcruise modules      | 461       | 436       | —             | −25        |
| `validate:all` members | 11        | 15        | **34**        | +23 guards |
| Unit tests             | 1631 pass | 1696 pass | **2124 pass** | +493       |
| vocab-guard exceptions | —         | 42        | **19**        | −23        |
| depcruise errors       | 0         | 0         | **0**         | —          |

Gate at completion (2026-08-11): `validate:all` **34/34 PASS** · `typecheck` 0 ·
`typecheck:tests:ratchet` PASS · `validate:arch` 0 · `lint:ratchet` 0 regressions · unit
**2124/2124** · `build` 0 · `verify:mcp` **12/12** against a live server. `test:all` was proven
green 3 consecutive runs at Tier ORD.

**Retirement status**: `reference`, not `done`. `plans:retire:check` refuses to archive a finished
plan that other documents still cite — three do (`stepstate-lifecycle-migration.md` and both
`validation-mechanism-architecture-2026-08-05` files). Archiving it would break them.
...
intent:
  ...
  source_spec   : plans/reference/technical-debt/arg-gate-pipeline-fixes.md (STALE — see 0.4)
...
| 0.4 | ✓      | Correct stale status column: mark item 2.4 done, cite `gate-set-resolver.ts:273/277/282`                                                                                                                                                                      | `plans/reference/technical-debt/arg-gate-pipeline-fixes.md` | 0.1                                | `rg -n "2\.4" plans/reference/technical-debt/arg-gate-pipeline-fixes.md` shows completed marker                                                                             |
...
| ID  | Status | Step                                                                                                                        | Files                                                                             | Depends | Verification                                                                                        |
| --- | ------ | --------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------- | ------- | --------------------------------------------------------------------------------------------------- |
| 4.1 | ✓      | **NEW** audit — row per ~40 site: `file:line`, what it guards, consumer probe, verdict LOAD-BEARING / SPECULATIVE, evidence | `plans/reference/technical-debt/compat-site-audit.md`                             | 3.10    | Every `rg -n "backward compat\|Kept for\|for compatibility" src` hit has a row; zero blank verdicts |
...
| `plans/reference/technical-debt/arg-gate-pipeline-fixes.md` | Yes — step 0.4                                                                                                                     |
...
- [x] **Pattern**: "Orphaned enforcement" — a project inventing a guard mechanism and never wiring
      it. Found 3× during Tiers 0-2 (guard scripts, identity no-op renames, stale plan status
      column), then **twice more**: `validate:metadata` inert since the build became a bundle
      (Tier V), and the vocab guard's blanket `tests/` exemption (IT.10). **5 sightings — past the
      3-sighting bar.** Captured 2026-08-11.
- [x] **Correction (logged)**: `rg -c` counts _matching lines_, not occurrences or assertions.
      Reporting it as "12 assertions" was wrong; actual was 9. Any count feeding a plan must come
      from a probe measuring the thing claimed.
- [x] **Memory**: knip is installed but unconfigured in this repo — its raw output is not a work
      list. (Superseded by 0.3: measured, the config buys exactly one line; the report was already
      trustworthy. The reusable lesson is the re-measurement, not the config.)
- [x] **Skill**: `/refactoring` pre-flight adding "is the inventory reconciled?" before consuming
      any generated list. **Second occurrence arrived** — 5.7's own site inventory was truncated by
      `head` and shipped a half-retirement until the guard caught it.
- [x] **NEW (5.7)**: **A retirement condition with no detector is still permanent.** 5.7's
      condition was this plan's best-written one — a tag comparison, no judgement — and it still
      went unretired for 10 days after coming true. Stating a condition makes an exemption
      _retirable_; it does not make anyone notice it became so. `validate:table-contracts` grew a
      satisfied-exception check for exactly this; the vocab guard has no equivalent. Candidate:
      teach `auditExceptions` to evaluate a `closedBy` predicate, not just assert one exists.
...
| 5.7 | ✓      | **DONE 2026-08-11 — the condition fired, and the scope was 6 fold families, not 2.** Deferred 2026-07-30 because v2.1.0 shipped 7 framework files naming `methodologyGates`. **Condition measured true**: v3.0.0 was tagged 2026-07-31 — one day after the rename landed — and ships `frameworkGates` in all 8 framework YAMLs; v3.2.1 is two releases further on. The row's Files column named 2 folds, but **19 allowlist entries shared its retirement condition** — see 5.9. All retired in one commit with their tests and entries. Original step: retire the two folds added in pass 5 (`FrameworkSchema.methodologyGates`, `GatePassCriteriaSchema.methodology`) **only** once no shipped or workspace resource uses the old spelling. A fold with no retirement condition is a parallel system | `framework-schema.ts`, `gate-schema.ts`, **+13 more — see 5.9**                                                                                                                                                                                                                                                                                                                                                                                                                                   | 5.2-5.6 | **DONE** — `validate:no-methodology-vocab` green at **19 exceptions (was 42)**; folds A and C each negative-verified to fail exactly their own test, no others |
...
| 5.9 | ✓      | **NEW (2026-08-11) — records what 5.7 actually covered.** Six fold families shared `RETIREMENT.RENAME_MAJOR`: **A** framework-YAML `methodologyGates`; **B** gate-YAML `methodology` plus the `methodology_compliance` enum preprocess; **C** `gates.methodologyGates` plus the two legacy config-section adoptions; **D** the `methodology_gates`/`methodology_elements` authoring payload; **E** the `{METHODOLOGY}` template placeholder. 5.7 named A and B only — the **fourth** time in this plan a row omitted its own surface (after 5.2 omitting `core-config.ts`, 5.3 omitting the contract layer, and pass 4 omitting `server/scripts/`). Only family **D** was measurably outside the declared contract                                                                                     | `template-variables.ts`, `core-config.ts`, `infra/config/index.ts`, `config-input-validator.ts`, `config-operations.ts`, `config-utils.ts`, `config.schema.json`, `resources/schemas/framework.schema.json`, `framework-lifecycle-processor.ts`, `framework-file-writer.ts`, `resource-manager/core/{router,types}.ts`, `framework-manager/core/types.ts`, `framework_builder/script.py`; **deleted** `framework-authoring-keys.ts`                                                               | 5.7     | **DONE** — typecheck 0, unit 2124/2124, `validate:all` 34/34                                                                                                   |
...
**SUPERSEDED 2026-08-11: the condition came true and 5.7 is closed. See "5.7 retirement" below.**

**5.7 blocked, with evidence (as of 2026-07-30).** `git grep -l 'methodologyGates' v2.1.0 -- server/resources/methodologies`
...
#### 5.7 retirement (2026-08-11) — the condition fired, and nothing noticed for 10 days

**v3.0.0 was tagged 2026-07-31**, one day after the rename landed, and ships `frameworkGates` in
all 8 framework YAMLs. v3.1.0, v3.1.1, v3.2.0 and v3.2.1 followed. The condition above was
therefore true almost immediately — and the row sat ⊘ DEFERRED for ten days regardless.

That gap is the finding, not the delay. This plan's whole answer to permanent exemptions was
**state a condition that can be checked**, and 5.7's was exemplary: a tag comparison, no judgement
required. It still did not retire, because **stating a condition does not create a detector.**
`cleanup-standards.md` already separates retirability from detection — this is that distinction
observed on the plan's own best-written condition. `validate:table-contracts` grew a satisfied-
exception check for exactly this reason; the vocab guard has no equivalent for a `closedBy` whose
condition has come true.

**Scope: 6 fold families, not the 2 the row named.** Full list in row 5.9. The row's Files column
omitted its own surface for the fourth time in this plan.

**Semver, raised and overridden.** Retiring families A/B/C/E removes accepted input spellings from
in-contract formats (framework YAML, gate YAML, `config.json`), which by this repo's Public API
Contract is breaking and calls for a major. Family **D** alone is undeclared — `methodology_gates`
and `methodology_elements` appear in no contract JSON and no generated artifact (probe
sensitivity-checked: the same search finds `framework_gates` in both). The owner elected to ship
the full retirement in a **minor**, on the basis that the project has no user base. Recorded in
`CHANGELOG.md` as Breaking rather than presented as non-breaking.

**Tests inverted, not deleted.** Every fold test now asserts the old spelling is INERT.
`.passthrough()` keeps an unknown key on the parsed object, so "the fold is gone" and "the fold
silently still runs" are externally identical — only an assertion on the value separates them.
Deleting the tests would have removed the only evidence the retirement took effect.

**Two defects the retirement surfaced**, neither a fold:

| #   | Defect                                                                                                                                                                                                                | Found by                    |
| --- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------- |
| D10 | **The guard was RED at HEAD** — 7 non-allowlisted hits, all self-referential (its own test file, its `.d.ts`, a sibling script naming it). Row 0.8 widened the scan to the git-tracked set and it began seeing itself | baseline run before editing |
| D11 | `framework-compliance/gate.yaml`'s allowlist entry was closed by `FOLD_DOCUMENTED` — but its comment records a dropped `keyword_count` entry, **not a fold**. A mis-stated condition, surfaced by coming due wrongly  | retiring `FOLD_DOCUMENTED`  |

**Verification.** `validate:all` **34/34 PASS**; `validate:no-methodology-vocab` green at **19 accepted exceptions, down from 42**, all
load-bearing. typecheck 0, `typecheck:tests:ratchet` PASS, `lint:ratchet` 0 regressions, unit
**2124/2124**. Folds A and C were each negative-verified by reinstatement: **1 test failed, and it
was the one written for that fold** — distinct mutations failing distinct tests.

> **A third vacuous probe.** The family-C negative verification first reported `2124 passed` while
> its mutation script had aborted on a moved anchor — which reads as "the mutation survived" and
> actually meant "the mutation never applied." Prettier had reflowed the target line. Re-run
> against the real text, 1 test failed. The plan's standing rule held again: **a probe reporting
> "no difference" needs its own output checked before the result is believed.**
