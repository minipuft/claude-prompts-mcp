...
| 0.8  | ✓      | **The vocab guard cannot see 57 git-tracked files.** Two causes: ripgrep skips dot-paths without `--hidden`, and `.gitignore` lists `CLAUDE.md` (which is nonetheless tracked). Four unreachable files hold 11 occurrences, and at least two are **stale, not exempt** — `CLAUDE.md` documents a `resource_manager (methodology)` resource type that the schema enum does not contain, and `feature_request.yml` offers "Methodologies" in a user-facing dropdown. Widen the scan, then triage what surfaces. **Second, independent instance measured 2026-08-06 (3.1)**: a consumer search for `validate:tool-schemas` reported "run by nothing" until `--hidden` was added, at which point `.github/workflows/ci.yml` appeared. The defect is not specific to this guard — it is the default of the tool every guard shells out to, and it silently converts "no hits" into "no such thing"                                                                                                                                                                                                                                                                                                                                                                                                                                                | 0.7     | ✓ **Both authored counts confirmed on re-measure (57 unreachable; 4 files / 11 occurrences)** — the first row in this plan whose numbers did not drift. Fixed by scoping the scan to `git ls-files` rather than adding `--hidden`: the tracked set is the definition the walk was approximating, so it also closes the opposite defect (18 **untracked** files were being scanned, which is how the 2026-08-09 `.ignore` file turned this gate red). All 11 occurrences were **stale documentation, not exemptions** — every one corrected, **zero new allowlist entries**. Live: plants in `.claude/` and in gitignored-but-tracked `CLAUDE.md` each redden the guard; plants in an untracked file and in `plans/**` correctly do not. Counterfactual: the old scan's file list contained **neither** target. 13 Jest tests on the pure scope function, M1/M2 reddening 2 and 3 distinct tests                                                                                                                 |
| 0.9  | ✓      | **Delete the dead dependency-cruiser rule `methodology-via-loader-only`.** Its `to.path` is `methodologies/` (no such directory anywhere), and both `pathNot` exemptions name files under `src/engine/frameworks/methodology/`, which does not exist. The rule is structurally incapable of firing and depcruise reports nothing about it                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | —       | ✓ Rule deleted. `validate:arch` byte-identical before and after: 3 warnings, **445** modules (authored 444 — drift, one module added since authoring), 1828 dependencies. **Undeclared dependency found**: this row's `Depends: —` was wrong. All **5** of `.dependency-cruiser.cjs`'s vocabulary occurrences ARE this dead rule, so 0.9 removes 5 of 0.8's 11 and had to run first — otherwise 0.8 triages lines that are about to be deleted                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| 0.10 | ✓      | **`plans:retire:check` fails on another workstream's plan, blocking `validate:all` for everyone.** `plans/adaptive-chain-runtime-p2-complexity-telemetry-2026-08-11.md` declares `status: done` while its own implementation-notes file still cites it; the convention says a finished plan something points at is `reference`, not `done`. Committed in `2e84bb3c` (chains telemetry), so **not this tier's failure** and deliberately not fixed here — but it is a red step in the CI-whole wrapper, which is exactly the "gate nobody can pass" condition that blocks every future tier. Owner is the adaptive-chain-runtime workstream; one frontmatter word (`done` → `reference`). **This row is itself now a second citation** — naming the blocking plan is required by Phase 6, and the checker counts any reference, so writing the row necessarily strengthened the finding it reports. Removing the citation would not clear the check, since the notes file still cites it                                                                                                                                                                                                                                                                                                                                                      | —       | `npm run validate:all` reaches 0 failures, or the row is reassigned to whoever owns that plan                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| 0.11 | ⚠      | **Root cause of 0.2 found, and it was never about that one file.** `validate:format` runs `npx --prefix server prettier --check`, which resolves `server/.prettierrc.json` (`printWidth: 100`). A bare `npx prettier --write` at the repo root resolves **no config at all** (measured: `--find-config-path` errors for `plans/**`) and so formats at the default 80. Markdown tables are padded to the print width, so the two invocations disagree on every table-bearing plan file, each one reverting the other — which is exactly 0.2's "`--write` does not converge, ran 3×, failed each time". 0.2 closed the symptom by adding one file to `.prettierignore`; the trap is still armed for every other plan file, and it costs a full 53 s suite run to notice. Fix: a `plans/**` prettier config, or a documented `format:plans` script that uses the gate's own invocation                                                                                                                                                                                                                                                                                                                                                                                                                                                          | 0.2     | ⚠ **PREMISE FALSIFIED 2026-08-11 — there is no config divergence; this row was a mis-attribution.** Measured directly: `npx prettier --check` and `npx --prefix server prettier --check` return the SAME verdict on the same plan file — both CLEAN on a clean one, both dirty on a dirty one — and it is the same binary either way (3.9.6). Proof the theory was backwards: adding an identical `.prettierrc.json` at the repo root CHANGED the gate's behaviour (68 files went dirty), which can only happen if the gate was never resolving `server/.prettierrc.json` for repo-root files to begin with. The real cause of the oscillation was row **3.5** — the ledger hook re-appending non-conforming markdown after each format — and it is fixed. I diagnosed this from a correlation (`bare --write → gate fails`, `prefixed --write → gate passes`) without controlling for the concurrent writer that ran between the two. Closed as resolved-by-3.5; no config and no `format:plans` script needed |
...
| 3.2  | ⚠      | **PREMISE FALSIFIED 2026-08-11 — the script is NOT dead; it is wired to CI and both of this row's options were wrong.** `.github/workflows/renovate-config-validator.yml` runs it as its last step: `node server/scripts/validate-renovate-extraction.js < "$extraction"`, against Renovate dry-run JSONL. The row concluded "run by nothing" because it searched for the **npm script name** (`validate:renovate-extraction`, colon) while the consumer invokes the **script file** (`validate-renovate-extraction.js`, hyphen) — this plan's own "probe the property, not a token that co-occurs with it" anti-pattern, third instance. It also cannot become a `SUITE` member as the Verification demanded: it reads stdin, and `runStep` spawns with `stdio: ['ignore', …]`. And it must not be deleted — it is a fail-closed supply-chain policy gate (automerge posture, TypeScript version holds, MCPB pinning) with 11 self-test cases. **No action needed on the script.** The residual — that the npm alias is never the entry point — is closed by 3.3 probing both spellings. ~~The checker's proof-that-it-can-fail passes on every run while the check itself never executes against real inputs. Same shape as 1.5's vacuous guard, reached by a different route: unwired rather than pattern-drifted. Wire it or delete it~~ | 3.1     | ~~Either the check is a `SUITE` member and passes, or the script and its `:self-test` are both gone~~ — **neither is achievable or desirable** (stdin-driven; live CI gate). Superseded: 3.3's `validate:suite-membership` now declares it outside `SUITE` with `runBy: ['.github/workflows/renovate-config-validator.yml']`, asserted on every run, so the wiring is verified rather than assumed                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| 3.3  | ✓      | **Nothing asserts that a `validate:*`/`verify:*` script is either in the suite or knowingly excluded.** ~~50 such scripts exist; 30 are in `SUITE`, 17 are `:self-test` (owned by Jest), leaving 5 outside~~ — **measured 2026-08-11: 55 keys, 19 `:self-test`, 36 non-self-test, 33 in `SUITE` after this row, 7 outside**. The row's outsider list was wrong in both directions: it omitted `verify:claims` and `validate:all`, and of its five only ONE was genuinely unrun. Authored list: `verify:mcp`, `validate:tool-schemas`, `verify:package-artifact` and `validate:build` need a build so cannot sit in a pre-build suite, and `validate:renovate-extraction` (3.2) has no reason at all. 0.3 governs which _mechanism_ a new check uses; nothing governs whether it is ever _run_. 3.1 added a test that every declared step exists — the converse is unguarded                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | 3.2     | ✓ `scripts/validate-suite-membership.js` + `:self-test`, wired into `SUITE` (now 33). Reports **two distinct** failures: UNWIRED (in neither `SUITE` nor the exception list) and FALSE REASON (a declared exception whose named consumers stopped referencing it — the check silently died and the reason became a lie). Stale-entry hygiene delegated to 4.1's shared harness. **The trap was real and it bit this very plan**: a consumer is matched by npm name **or** by the script file the name resolves to, because `.github/workflows/renovate-config-validator.yml` invokes `validate-renovate-extraction.js` by path — probing only the npm name is what produced 3.2's false conclusion. File set is `git ls-files` filtered to `.github/**` + `.husky/**`, per 0.8. 7 self-test rules; 3 live plants (new orphan script → UNWIRED; falsified `runBy` → FALSE REASON; deleting the renovate exception → reports it as unwired **while naming the consumer the plan's search missed**)                |
| 3.4  | ✓      | **The real orphan was `validate:build`, which 3.3 had excused.** The row asserted it "needs a build so cannot sit in a pre-build suite", implying something runs it post-build. Measured 2026-08-11: **nothing runs it anywhere** — zero references outside its own `package.json` line, no CI, no hook, no doc. It was `npm run build && npm run start:test`, and `.github/workflows/ci.yml:274` already runs `npm run start:test` inline after its own build step, so the alias was redundant as well as unrun. Deleted. Note the inversion worth keeping: the plan **flagged the wired script (3.2) and excused the dead one** — both errors came from the same probe, and an excuse is not evidence                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | 3.3     | ✓ `validate:build` absent from `package.json`; `validate:suite-membership` green with 6 exceptions, none naming it; CI's build+smoke coverage unchanged                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| 3.5  | ✓      | **The plan-hygiene validation-ledger hook emits markdown that `validate:format` rejects, so the format gate fails on the run AFTER every validation run.** The hook appends `- <date> · <cmd> · ran` bullets under `## Validation runs` with no blank line after the heading; prettier wants one. Observed 3× in this tier alone — format the file, run the suite, the suite's own ledger write makes it dirty again. The standing remedy has been to add one more file to `.prettierignore` (row 0.2, and commit `26fc516d` for the sqlite plan), which is a treadmill: it exempts the file rather than fixing the writer, and every new plan needs its own exemption. Fix the hook's output, or ignore `plans/**-implementation-notes.md` as a class with a stated reason                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | 0.11    | ✓ Fixed at the writer, in `~/.claude/hooks/validation/validation-flush.py` — **outside this repo**. `_append` had two branches: the one that CREATES the section emitted `\n\n` after the header, the one that APPENDS to an existing section emitted `\n`. The create branch was right; the branches simply disagreed, and only the append branch ever runs after the first time. It also split one bullet list into two. Verified end-to-end: format clean → run a validation command → hook appends → `validate:format` GREEN, `.prettierignore` untouched. 4 structural cases (empty section / with entries / empty-then-heading / no section, each double-appended) checked against real prettier; reverting the fix turns them dirty. New test `test_ledger_keeps_the_notes_file_markdown_clean` asserts the blank line AND contiguity — the pre-existing test used `assertIn`, which is blind to both; 33/33 pass, and the mutation reddens exactly the new test                                         |
...
| 4.2  | ✓      | **The vocab allowlist's 35 entries carry no structural `closedBy`.** Their retirement conditions are grouped prose comments above blocks of entries, so the harness's form check is switched off for that one gate — the only gate where it is. The truth check runs; the form check cannot. Give each entry a `closedBy` field, or accept that this gate's exemptions can never be checked for having an exit                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | 4.1     | ✓ All **35** entries (authored count confirmed) carry `closedBy`, and the accessor is wired. Retirement conditions became **named shared constants** (`RETIREMENT.RENAME_MAJOR` ×18, `PINS_FOLD` ×7, `NOTE_USEFUL` ×3, plus 7 singletons) rather than 35 copied strings — entries that retire together now share one condition, which is the fact the grouped prose was conveying and the gate could not read. Falsified live: blanking one entry's `closedBy` reports `has no closedBy` and exits **1**; restored, exits 0. Confirmed this was the only gate missing the accessor — the other five all passed it                                                                                                                                                                                                                                                                                                                                                                                               |
| 4.3  | ✓      | **Form and truth are separate checks and no gate had both.** `validate-table-contracts` checked `closedBy` non-empty and never whether the exception was still needed; `no-llm-client` checked the reverse. 4.1 gave every gate both, but nothing states the pair as a requirement for a _new_ exception surface — the same inflow gap 0.3 closed for guard mechanism and 3.3 opens for suite membership                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | 4.1     | ✓ A **check**, not a convention — `claude/require-exception-audit` in the ESLint plugin, per 0.3's rule that single-file AST work belongs there rather than in `scripts/`. It fires when a gate script declares `closedBy` but never calls `auditExceptions`. **Probes the property, not the name**: matching `ALLOWLIST`/`ACCEPTED_*`/`ALLOWED_*` identifiers hits `ALLOWED_METRIC_LABELS`, `ACCEPTED_PACKAGE_NAMES` and `ALLOWED_PREFIX_TOKENS`, none of which are gate exceptions — measured 0 false positives on those. **Documented blind spot**: a list carrying neither `closedBy` nor an audit is invisible, because nothing distinguishes it from an ordinary constant; widening to names trades a stated gap for silent false positives. Falsified: 0 hits on a conforming gate, 1 when its audit call is removed, 0 restored. **1 true positive repo-wide** — row 4.4                                                                                                                                |

| 4.4 | ✓ | **The rule's first catch, and it is real: `scripts/validate-db-claim-order.js` declares `closedBy` on every `ACCEPTED_INHERITORS` entry and never audits them.** Form adopted, truth missed — the author took the half that is visible while reading a sibling gate and missed the half you only make if you know `lib/exception-hygiene.js` exists. That asymmetry is the argument for 4.3 being a rule instead of a convention. The guard landed mid-plan from the db-claim-order workstream and is **uncommitted**, so it was deliberately not edited here | 4.3 | `npx eslint scripts/validate-db-claim-order.js` reports no `require-exception-audit`; its four inheritor entries are classified on every run |
| 4.5 | ✓ | **`lint:ratchet` absorbed a brand-new rule's violation without reporting anything.** Adding `claude/require-exception-audit` introduced 1 real error and the ratchet still printed `OK … (no regressions)`, because the baseline is a **ceiling with slack**, not a measurement. Second sighting this session: Tier 0 measured 3193 → 3197 while green, and 2 of those were real `no-undef` errors in a file this plan had just edited. Consequence: a new gate can be added, fire correctly, and enforce nothing until someone regenerates the baseline — so `validate:all` green is not evidence that a newly added rule is clean | 4.3 | ✓ **Premise was wrong and the truth is worse.** Not ceiling slack — `lint:ratchet` ran `eslint src`, declaring `eslintTarget: 'src'`. It never linted `scripts/` at all, and plain `lint` (`eslint .`), which does, is **not a suite member**. So 93 errors sat outside `src/` reported by nothing, and **both ESLint rules this plan added as gates** (`require-guard-mechanism-verdict` from 0.3, `require-exception-audit` from 4.3) are scoped to `scripts/` and were therefore enforced by nothing. Compound and self-concealing: 13 of the 15 `scripts/` errors were a CONFIG gap — `scripts/**/*.ts` had no parser block, so every TypeScript gate failed to parse and was silently unlinted entirely, which is why `require-exception-audit` could not fire on `validate-table-contracts.ts` or `validate-no-phantom-columns.ts`, two of the six surfaces it governs. Each defect hid the other. Fixed: TS parser + Node globals for `scripts/`, `.d.ts` exempt from `no-unused-vars`, one real `preserve-caught-error` fix → **15 → 0**. Target widened to `src,scripts,eslint-rules`; baseline regenerated (3200/1020) without absorbing foreign regressions. Added a divergence guard so a baseline measured over a different file set stops the run instead of mis-comparing. Falsified: a planted `closedBy`-without-audit file in `scripts/` now fails the ratchet (`baseline=0 current=1`); it was invisible before. The first plant was misnamed and matched no rule scope — the mutation fired without reaching the branch it was aimed at |
| 0.13 | ✓ | **6 chain/formatting unit tests fail on uncommitted work from the adaptive-chain-runtime workstream, so the plan Gate's `npm test` half cannot pass.** `response-assembler-chain-cta` (3), `step-execution-stage` (4 — counted once per suite run), `response-formatting-stage` (1): each expects `"Chain execution complete"` from a `response-assembler.ts` that is mid-refactor, alongside 8 modified pipeline stages and `modules/chains/manager.ts`. **Not this tier's failure and not fixed here.** Recorded because the Gate reads as unmet until it clears | — | `npm run test:ci` reports 0 failures, or the row is reassigned to the adaptive-chain-runtime workstream |

| 0.14 | ✓ | **RESOLVED 2026-08-11 by `05b6efcb`.** That third workstream was the shim-debt sweep row 5.7; its uncommitted guard edits are now committed and `validate:no-methodology-vocab` is green at 19 accepted exceptions, down from 42. **A third workstream's in-flight `methodology` → `framework` rename has the vocab guard red, and it edited the guard itself.** `scripts/validate-no-methodology-vocab.js` carries **-80/+58 uncommitted** against `5af52f56` with no commit touching it: allowlist entries are being deleted as their folds go, which is what this guard's retirement rule asks for. But one entry (`infra/config/index.ts` / `methodologyGates`) was removed while its fold is still live at `src/infra/config/index.ts:384`, so the guard exits 1. Rows 0.8 and 4.2 survive intact in their edit (RETIREMENT x19, closedBy x21, tracked-set scan x5). **Not this plan's failure** — an ordering issue inside their rename | - | `npm run validate:no-methodology-vocab` exits 0, with each fold and its entry removed in the same commit |
| 0.15 | ✓ | **RESOLVED 2026-08-11 by `05b6efcb`.** The assertion was right and the fold was the lagging half: `FrameworkSchema`'s `methodologyGates` transform is deleted, so the test passes (unit 2147/2147). It was observed mid-flight, when the inverted assertion had landed but the fold had not yet been removed. **`tests/unit/frameworks/framework-gates-field.test.ts` fails against that same rename.** It asserts the pre-rename `methodologyGates` fold is gone (`expect undefined`) while the fold still returns the 4 CAGEERF gates — the test landed ahead of the removal. Drift measured across three consecutive `test:ci` runs inside one task: 11 failures/2127 tests → 2/2124 → 1/2124, with `pass-criteria-framework-fold.test.ts` present in run 1 and deleted by run 3. The Gate's `npm test` half is red for this, not for 0.13 | 0.14 | `npm run test:ci` reports 0 failures |
| 0.16 | ✓ | **RESOLVED 2026-08-11.** Both shim-debt plan files pass `prettier --check`. **`plans/technical-debt/shim-debt-sweep-2026-07-29*.md` fail `validate:format`** — another session's staged/modified plan files carrying single-quoted YAML frontmatter titles where Prettier wants double. Both prettier invocations agree they are dirty, so this is ordinary unformatted content and NOT 0.11's imagined divergence. I ran `--write` on the notes file before realising it was foreign and staged: a one-character quote change, recorded rather than hidden | - | `npm run validate:format` exits 0 |
...
**Gate status (2026-08-11): BLOCKED — by two rows, neither belonging to this plan.**
`npm run validate:all` is **1 of 34** failing: `plans:retire:check`, on the adaptive-chain-runtime
plan's frontmatter (row **0.10**). `npm test` has **6 failures** in chain/formatting suites, on that
same workstream's uncommitted refactor (row **0.13**). Every step and suite this plan's own tiers
touch is green; the "ported rule observed failing on a planted violation" half of the Gate is met
for all of them. Do not read the Gate as passing until 0.10 and 0.13 clear.
...
## Execution record — Tier 0.8 + 0.9 (2026-08-11)

### The first row whose authored counts survived re-measure

Every prior tier in this plan corrected a count on execution (0.3's 4.4×, 0.6's 34, 0.7's 37,
1.4's seven, 4.1's three surfaces). 0.8 asserted **57** unreachable tracked files and **4 files /
11 occurrences**; all three measured exactly. Recorded because the habit this plan built — distrust
the number — is a heuristic, not a law, and a plan that is right is evidence about how it was
written: 0.8's numbers came from a probe run at authoring time, not from recall.

The counts that DID drift were the ones nobody re-derived: `validate:arch` reported **445** modules
against the row's 444, and `SUITE` now has **32** steps against 3.1's 30 (two added by the
adaptive-chain-runtime workstream).

### Fixing the mechanism, not the symptom

The row said "widen the scan", and the obvious widening is `--hidden`. That would have been wrong
in a way the row could not see, because it names only half the defect:

| Direction  | What the old walk did                                                                                | Cost                                                                           |
| ---------- | ---------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------ |
| Too narrow | Skipped 57 tracked files — dot-paths need `--hidden`, and `.gitignore` lists the tracked `CLAUDE.md` | 11 live occurrences went unreported while the gate said OK                     |
| Too wide   | Scanned 18 **untracked** files                                                                       | The 2026-08-09 `.ignore` file turned the gate red on files that ship to nobody |

`--hidden` fixes only the first, leaves `CLAUDE.md` invisible, and drags in 5,405 files under
`.git/`. `git ls-files` **is** the definition the walk was approximating — "shipped content" — so
scoping to the tracked set closes both directions at once and neither can recur: an untracked file
cannot enter the set, and no ignore rule can remove a tracked one.

A detail that decided the implementation: `--glob` does **not** filter paths passed as arguments,
only paths ripgrep discovers by walking. Measured — `rg --glob '!CLAUDE.md' methodolog CLAUDE.md`
still prints its 3 hits. Keeping the globs would have left an exclusion list that excluded nothing,
so they became regexes tested in JS.

### All 11 were stale documentation, not exemptions

The row allowed for either outcome ("fixed or allowlisted"). None needed an allowlist entry:

| Occurrence                                                                                                       | Verdict                                                                                                                                                                                                                             |
| ---------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `CLAUDE.md` ×3 — `resource_manager (methodology)`, `MethodologyValidator`, resource-format and doc-pointer prose | Stale. The schema enum is `['prompt','gate','framework','checkpoint']` — no `methodology`; `MethodologyValidator` does not exist, the class is `FrameworkValidator`; `docs/guides/gates.md` holds 0 "methodology" and 4 "framework" |
| `.claude/rules/mcp-contracts.md` ×1 — "methodology overlays"                                                     | Stale. The source comment it describes says "framework overlays"                                                                                                                                                                    |
| `.github/ISSUE_TEMPLATE/feature_request.yml` ×2 — user-facing dropdown                                           | Stale, and the most visible of the eleven: it offered "Methodologies" to anyone filing an issue                                                                                                                                     |
| `server/.dependency-cruiser.cjs` ×5                                                                              | All five are row 0.9's dead rule — deleted, not fixed                                                                                                                                                                               |

**The undeclared dependency.** 0.9's `Depends` column said `—`, but 0.9 removes 5 of 0.8's 11
occurrences. Run in the other order, 0.8 would have triaged five lines that were about to be
deleted. Neither row's text mentioned the other; the overlap only appeared because Phase 1
re-measured _where_ the occurrences were instead of trusting the count.

### Falsification

Four plants, chosen to discriminate rather than to confirm — two must redden, two must stay green:

| Plant                                      | Class                  | Result                            |
| ------------------------------------------ | ---------------------- | --------------------------------- |
| `.claude/rules/mcp-contracts.md`           | dot-path directory     | exit 1 ✓                          |
| `CLAUDE.md`                                | tracked but gitignored | exit 1 ✓                          |
| new untracked file under `server/scripts/` | untracked              | exit 0 ✓ (correctly out of scope) |
| `plans/**` (already saturated — 25 hits)   | deliberate exclusion   | exit 0 ✓                          |

Counterfactual on the old scan: its file list contained **neither** reddening target, so both
plants were structurally unreachable rather than merely unmatched.

Unit: 13 Jest tests on `inScopeFiles`. M1 (drop the `plans/` exclusion) reddened **2**; M2
(re-introduce dot-path blindness) reddened **3** — distinct mutations, distinct tests.

### Criteria → tests (`all_criteria_mapped: yes`)

| #   | Criterion                           | Evidence                                               |
| --- | ----------------------------------- | ------------------------------------------------------ |
| C1  | scan reaches dot-paths              | plant 1 + 3 unit tests + M2                            |
| C2  | scan reaches tracked-but-gitignored | plant 2 + counterfactual                               |
| C3  | untracked files stay out of scope   | plant 3 + `scope is the input set` tests               |
| C4  | deliberate exclusions still applied | plant 4 + 6 unit tests + M1                            |
| C5  | all 11 occurrences resolved         | guard exits 0; 35 exceptions all load-bearing          |
| C6  | depcruise rule gone                 | `grep` → 0                                             |
| C7  | `validate:arch` unchanged           | 3 warnings / 445 modules / 1828 deps, before and after |

### Gate: BLOCKED by 0.10, not by this tier

`validate:all` reports **1 of 32** steps failing: `plans:retire:check`, on a plan owned by the
adaptive-chain-runtime workstream and committed in `2e84bb3c`. Every step this tier touches is
green. Recorded as row 0.10 rather than fixed here — a foreign red step is still a blocked gate,
and 3.1's runner is what made it visible at all: under the old `&&` chain the format failure would
have aborted the run and hidden it.

## Execution record — Tier 3.2 + 3.3 (2026-08-11)

### The row was wrong about which script was dead, and the probe explains both halves

3.2 said `validate:renovate-extraction` is "not in the suite, not in CI, not in either hook".
It IS in CI — the last step of `.github/workflows/renovate-config-validator.yml`. 3.3 said
`validate:build` "needs a build so cannot sit in a pre-build suite", which reads as a legitimate
exclusion. Nothing runs `validate:build` at all.

One probe produced both errors. Asking "is this check run by anything" was implemented as "does
the **npm script name** appear in CI", and the CI step invokes the **script file**:

```
node server/scripts/validate-renovate-extraction.js < "$extraction"
```

`validate:renovate-extraction` and `validate-renovate-extraction.js` differ by one character, and
the search for the first cannot find the second. My own first measurement in this tier repeated the
mistake exactly, and was only caught by opening the workflow to see what a "wireable" check looks
like. The false negative is silent in both directions: a wired check reads as dead, and a dead one
keeps whatever excuse it was given, because nobody re-checks an excuse.

**This is the third instance of the same class in this plan** — 0.8's dot-path blindness, 0.8's
`validate:tool-schemas` consumer search, and now this. The first two were about the file set the
probe covered; this one is about the token the probe matched. Same failure, different axis: the
probe answered a question adjacent to the one being asked.

### Measured vs authored

| Quantity                     | Authored                  | Measured 2026-08-11                        |
| ---------------------------- | ------------------------- | ------------------------------------------ |
| `validate:*`/`verify:*` keys | 50                        | **55**                                     |
| `:self-test`                 | 17                        | **19** (20 after this tier)                |
| in `SUITE`                   | 30                        | **32** (33 after this tier)                |
| outside `SUITE`              | 5                         | **7**                                      |
| of those, genuinely unrun    | 1 (`renovate-extraction`) | **1 (`validate:build`) — a different one** |

The outsider list also omitted `verify:claims` (run by `npm-publish.yml`) and `validate:all`
itself, which cannot be a member of the suite it runs. Two of the drifts (`SUITE` 30→32, self-tests
17→19) are the adaptive-chain-runtime workstream's, not decay.

### What the checker asserts that a membership list would not

`validate:suite-membership` separates two failures deliberately, because collapsing them is how an
exception list rots:

| Mode         | Meaning                                                                                                                 |
| ------------ | ----------------------------------------------------------------------------------------------------------------------- |
| UNWIRED      | in neither `SUITE` nor the exception list — a gate someone wrote and never connected                                    |
| FALSE REASON | a declared exception whose `runBy` files no longer reference it — the check stopped running and its excuse is now false |

FALSE REASON is the load-bearing half. Six checks legitimately sit outside `SUITE` because they
need a build, a tarball, or piped stdin; each names the workflow that runs it, and that claim is
re-asserted on every run rather than trusted. 4.1's rule — form and truth are separate checks —
applied to suite membership: `closedBy` is the form, `runBy` is the truth.

### Falsification

Self-test: 7 rules, all falsifiable, auto-discovered by the Jest harness (19 → 20 self-tests, 23
tests in that suite). Live plants against the real repository:

| Plant                                                           | Expected                               | Result                                                                    |
| --------------------------------------------------------------- | -------------------------------------- | ------------------------------------------------------------------------- |
| new `validate:orphan` script in `package.json`                  | UNWIRED                                | ✓ reported                                                                |
| `verify:mcp` `runBy` pointed at a workflow that does not run it | FALSE REASON                           | ✓ reported, naming the file that actually runs it                         |
| delete the `renovate-extraction` exception                      | UNWIRED **and** name the real consumer | ✓ "it IS referenced by `.github/workflows/renovate-config-validator.yml`" |

The third plant is the acceptance test for this tier: it reproduces the exact mis-measurement that
caused 3.2 to be authored, and the checker contradicts it in one line.

### Gate: still blocked by 0.10, plus new row 3.5

`validate:all` is 33 members; every step this tier touches is green. Two failures remain, neither
this tier's: `plans:retire:check` (row 0.10, adaptive-chain-runtime's plan frontmatter) and
`validate:format` (row 3.5 — the ledger hook re-dirties the notes file on every validation run, so
the gate cannot stay green across two consecutive runs by construction).

## Execution record — Tier 3.5 (2026-08-11)

### The fix was one character, in another repository

`_append` in `~/.claude/hooks/validation/validation-flush.py` wrote `SECTION + "\n" + line`
where the sibling branch six lines below wrote `f"\n\n{SECTION}\n\n{line}\n"`. Markdown requires a
blank line after a heading, so every ledger write left the notes file failing `prettier --check`.

Three properties made it survive:

1. **The two branches disagree, and only one is exercised in steady state.** The create branch is
   correct and runs once per plan; the append branch is wrong and runs on every validation command.
2. **The gate that catches it runs in a different repository from the code that causes it.** The
   symptom is `validate:format` failing in `claude-prompts-mcp`; the cause is a hook in
   `~/.claude/hooks`. Nothing links them, so the fix kept being applied at the symptom.
3. **Its own test could not see it.** `test_appends_ledger_line_with_outcome` asserts
   `assertIn("`npm run typecheck` · ok", text)`. The substring is present with or without the
   blank line, so the test passed against the defect for the life of the hook.

### The workaround was a treadmill, and it had already run three times

Row 0.2 added `plans/reference/sqlite-layer-remediation-2026-08-03.md` to `.prettierignore`.
Commit `26fc516d` is titled "stop prettier oscillating on the sqlite remediation plan". Row 0.11
diagnosed a _second_, real cause (config resolution: root prettier at width 80 vs
`server/.prettierrc.json` at 100) and is still open. Each response exempted one more file.

Every one of those was a correct local action. The pattern only becomes visible when the same
symptom is fixed a third time — which is the argument for recording remediations as rows rather
than doing them: three `.prettierignore` entries in three commits look like three unrelated
chores, and three rows in one table look like an unfixed cause.

**0.11 remains open and is NOT this.** Two independent causes produced one symptom: a writer
emitting non-conforming markdown (3.5, fixed here) and two prettier invocations disagreeing on
width (0.11, still live for anyone formatting a plan by hand from the repo root).

## Execution record — Tier 4.2 + 4.3 (2026-08-11)

### 4.2 — grouping was the information, and the constant kept it

The 35 entries had their retirement conditions as prose comments above blocks. Converting to a
`closedBy` string per entry would have satisfied the gate and destroyed the one fact the prose
carried: that 18 of them retire in a single event. Named constants (`RETIREMENT.RENAME_MAJOR` and
six others) keep it, and promote it — "these retire together" is now a shared reference the gate
reads, rather than a paragraph a reader has to scroll to.

The count was 35, exactly as authored. Worth noting how nearly it was mis-measured: a regex for
`{ file: '…', match: '…' }` over the allowlist block returns **38**, because three commented-out
example entries live inside the explanatory prose. Matching the shape of a declaration is not the
same as matching a declaration — the same token-vs-property error this tier's own subject is about,
committed while measuring it.

### 4.3 — a convention would not have worked, and there is evidence

The row allowed "a stated convention, or a check". The deciding evidence arrived on its own:
`scripts/validate-db-claim-order.js`, written by another workstream while this plan was open,
carries `reason` **and** `closedBy` on every entry of `ACCEPTED_INHERITORS` — and never calls
`auditExceptions`.

That author was clearly following the convention. They adopted the half that is **visible in a
sibling file's data** and missed the half that lives in **a module you have to know exists**. A
stated convention addresses only people who read the statement; this author already behaved like
someone who had. So the check is the answer, and 4.4 records the instance.

**The probe choice mattered more than the rule.** The obvious implementation matches identifiers
that look like exception lists — and `ALLOWED_METRIC_LABELS`, `ACCEPTED_PACKAGE_NAMES` and
`ALLOWED_PREFIX_TOKENS` are not exceptions, they are domain vocabulary. `closedBy` is the property
itself: the repo's own marker for "this exemption names what retires it". Measured 0 false
positives, 1 true positive.

The blind spot is stated in the rule header rather than closed: a list with neither `closedBy` nor
an audit is indistinguishable from an ordinary constant. A narrow check with a documented gap beats
a broad one whose false positives train readers to ignore it.

### What the ratchet did not tell me

Adding the rule introduced one genuine error, and `lint:ratchet` reported
`OK: 3208 errors, 1028 warnings (no regressions)`. The baseline is a ceiling with slack, so a new
rule can fire correctly and enforce nothing. This is the second sighting in this session — Tier 0's
run moved 3193 → 3197 while green, and two of those were real `no-undef` errors in a file that tier
had just written. Recorded as row 4.5, because "the suite is green" is being used throughout this
plan as evidence, and here it is evidence of less than it appears.
