...
| 4.2  | ✓   | `server/package.json`                                                                                                                                                             | Register `validate:phase-header-drift` + self-test in `validate:all`                                                                                                                                                                                                                                                                                                                                                 |      4 | 4.1     | `validate:all` runs the full suite and passes (39 steps when authored; 44 as of 2026-08-19 — the count drifts, the pass does not)                   | A gate CI does not run is not a gate                                                                                                                                                                                                                                                                                                                                                                 |
...
| Suite green                            | `validate:all` + `test:ci` | 39 steps / 2480+ tests when closed; 44 steps as of 2026-08-19              |
...
Reviewed 2026-08-19 while verifying this plan's `reference` status. A `reference` plan may not
carry unmarked boxes (`cleanup-standards.md` — a status outlives what it described), so each is
now terminal or stamped.

- [x] Capture the pattern: _a check and the instruction it grades must read one source; when they
      cannot, the check must not block on what the instruction omitted._ — shipped as the
      advisory branch in row 3.1 and enforced by `validate:phase-header-drift` (row 4.1), which
      is the pattern in executable form rather than prose.
- [x] Record that this defect reproduced on the authoring chain itself — recorded in the sibling
      implementation notes; that reproduction is what row 3.1 was written against.
- [ ] (as of 2026-08-19 · flips when each gate family's declared criteria are compared against
      what the model actually receives) Consider whether other gate families (inline gates,
      framework compliance) declare criteria the model never receives. **Still genuinely open**
      and deliberately not marked done. Adjacent evidence has since accumulated elsewhere:
      `script-tools-verification-2026-08-17.md` F10 found the `script_tool` gate family shells
      its tool id instead of resolving a tool, and returns `passed: true` when it cannot run at
      all. This box wants a live home rather than a retired one.
