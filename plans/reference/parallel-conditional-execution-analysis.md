---
title: "Parallel & Conditional Execution - Current System Analysis"
date: 2025-10-19
status: reference
tags: []
---
...
**Current Parser**: `server/src/execution/parsers/command-parser.ts`
...
**Current Regex Patterns**: `server/src/execution/parsers/command-parser.ts:132-138`
...
**File**: `server/src/execution/parsers/symbolic-operator-parser.ts` (new)
...
export class ParallelExecutor {
  ...
  private aggregateResults(results: StepResult[]): ParallelExecutionResult {
...
export class ExecutionStateManager {
  ...
  trackParallelGroup(groupId: string, stepIds: string[]): void {
  ...
  handleStepFailure(groupId: string, stepId: string, error: Error): void {
    const state = this.getState(groupId);
...
**File**: `server/src/execution/parsers/symbolic-operator-parser.ts` (enhancement)

```typescript
export class SymbolicCommandParser {
  private readonly CONDITIONAL_PATTERN =
    /\?\s*["'](.+?)["']\s*:\s*(\w+)(?:\s*:\s*(\w+))?/;
...
export class ConditionEvaluator {
  ...
  private evaluatePresenceCondition(condition: string, result: any): boolean {
...
export class StepSelector {
  ...
  async selectNextStep(
    ...
    if (conditionMet) {
      this.logger.info(
        `Condition "${conditionalOp.condition}" is TRUE, executing: ${conditionalOp.trueBranch}`,
      );
      return conditionalOp.trueBranch;
    } else {
      if (conditionalOp.falseBranch) {
        this.logger.info(
          `Condition "${conditionalOp.condition}" is FALSE, executing: ${conditionalOp.falseBranch}`,
        );
        return conditionalOp.falseBranch;
      } else {
        this.logger.info(
          `Condition "${conditionalOp.condition}" is FALSE, no false branch, stopping`,
        );
        return null;
      }
    }
