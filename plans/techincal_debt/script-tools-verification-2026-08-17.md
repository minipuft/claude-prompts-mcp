...
| 6.3 | ☐ (as of 2026-08-19 · flips when `test:ci` reads `tests/integration`, or plan gates name the suite that reads their files) | plan gates, all tiers           | Every tier in this plan declared `npm run test:ci` (or a scoped subset) as its gate while writing only integration tests, which `test:ci` does not read. Decide once: either `test:ci` includes `tests/integration`, or plan gates name the suite that reads the files they write                                                                       | —      | —       | the gate command's path list intersects the tier's Files column                              |
