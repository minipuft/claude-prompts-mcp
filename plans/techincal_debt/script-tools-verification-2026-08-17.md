...
| 6.4 | ✓   | create_gate/user-message.md:229, gates.md:15,347, gate-schema.ts, gate-primitives.ts | **Corrected — but NOT to 6.1's outcome.** The row assumed the doc would become true once 6.1 landed. F11 says otherwise: no live path runs `script_tool` at all, so "Enforced" is false regardless of how well the runner works. All four sites now read **Not enforced**, and `gates.md` carries a WARNING naming Stage 20 as the only live consumer of `pass_criteria`. Fixing this row to say "Enforced" would have replaced one false claim with a better-argued one                                                                                     | 3      | 6.1, F11 | ✓ claim matches measured behavior (drive below)                                                                          |
...
| 6.7 | ☐ (as of 2026-08-19 · flips when a declared `script_tool` criterion either executes during a run or is rejected at load) | 20-gate-review-stage.ts, gate-shell-verify-runner.ts | **F11 — decide the type's fate.** A `script_tool` gate is inert end to end: it renders a heading with an empty body and enforces nothing, silently. Two honest exits, and this row wants one of them chosen, not deferred: (a) wire the type into gate review beside `shell_verify`, which needs the script-tool runtime threaded to Stage 20 and `resolveShellVerificationCoverage` taught to count it; or (b) retire the type — breaking, since gate YAML is contract surface, so it needs a major bump and the `llm_self_check` treatment (keep it accepted, reject it loudly at load). Documented honestly in the meantime, and pinned by a characterization test that turns red the moment (a) happens | 60 | — | a `script_tool` gate either runs its tool or is refused at load; `gates.md` moves with it |
...
**Still open**: row 6.7 only (F11 — wire `script_tool` into gate review, or retire it). Every
other row across all six tiers is terminal. The post-fix live drive named in Done criteria is
