...
| #   | St                                                                                             | File                                | Change                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | ~Lines | Depends | Verify                                                                                       |
| --- | ---------------------------------------------------------------------------------------------- | ----------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ | ------- | -------------------------------------------------------------------------------------------- |
| 6.1 | ☐ (as of 2026-08-19 · flips when a `script_tool` gate naming a registered tool runs THAT tool) | gate-validator.ts:388               | **The `script_tool` gate does not run a script tool.** It passes `command: criteria.script_tool_id` as a STRING, and `resolveCommand` turns a string into `sh -c "<value>"` — so `script_tool_id: "my_analyzer"` shells that name rather than resolving it against the script-tool registry. Route it through `ScriptExecutor`, or delete the gate type                                                                                                                                                                                                                                                                                                                          | 40     | —       | a gate naming a registered tool runs THAT tool; a gate naming `echo hi` does not run a shell |
| 6.2 | ☐ (as of 2026-08-19 · flips when an unrunnable script_tool check stops scoring 1.0)            | gate-validator.ts:374               | Missing `script_tool_id` returns `passed: true, score: 1.0` — a verification that could not run reports as verified. Fail closed, or return a distinct non-scoring outcome                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | 8      | 6.1     | test asserts an unrunnable check does not score 1.0                                          |
| 6.3 | ☐ (as of 2026-08-19 · flips when a local gate runs `test:integration`)                         | package.json:78-86, .husky/pre-push | **ESCALATED 2026-08-19 — this shipped a red `main`.** No local gate runs integration or e2e tests. `test:ci` is `test:unit`; `validate:all` calls `test:ci`; the pre-push full route runs unit tests only. So `npm run validate:all` + pre-push can both pass while `tests/integration/**` is broken, and CI is the first thing that ever runs it. That is not hypothetical: commit `d6cd6b73` passed 44/44 locally, passed pre-push, and failed CI on 5 assertions in `tests/integration/reference/script-reference-resolution.test.ts`. CLAUDE.md's "every local route remains a subset of CI" holds, but the subset is narrow enough that a whole test tier is post-push-only | 15     | —       | a local gate fails when only an integration test is broken                                   |
| 6.4 | ☐ (as of 2026-08-19 · flips when the create_gate row matches 6.1's outcome)                    | create_gate/user-message.md:229     | Documents `script_tool` as "**Enforced** — runs registered script with JSON via stdin". False per 6.1. Correct it, or remove the row if 6.1 deletes the type                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | 3      | 6.1     | claim matches behavior                                                                       |

| 6.5 | ☐ (as of 2026-08-19 · flips when a prompt can render a literal `{{script:...}}` without executing it) | script-reference-resolver.ts:55 | **A prompt cannot contain a literal `{{script:...}}`.** `preResolve` is a regex over raw template text and is `{% raw %}`-unaware, so it executes references inside a raw block; and anything that escapes the regex still reaches Nunjucks as an expression and fails to parse. `{% raw %}` is the documented escape for `{{ }}` (template-syntax.md §Escaping), so it should hold here too. Found by the post-fix live drive — the demo's own explanatory table broke the prompt twice before being rewritten to avoid braces entirely | 25 | — | a prompt renders `{% raw %}{{script:word_count}}{% endraw %}` as literal text, executing nothing |
| 6.6 | ☐ (as of 2026-08-19 · flips when the doc examples match the regex the resolver uses) | docs/reference/template-syntax.md:105,108 | **The doc's own `{{script:...}}` examples do not work.** Both are written spaced — `{{ script:word_count text=content }}` — and the resolver regex requires `{{script:` with no space, so neither is ever resolved; Nunjucks then fails to parse them. Pre-existing, unrelated to this initiative's changes | 2 | — | copy each example into a prompt and render it |
...
**Still open**: Tier 6 (four rows, all discovered here), and the post-fix live drive named in Done
criteria. The pre-fix live drive is recorded in the notes; the post-fix one needs an MCP restart
to pick up the rebuilt `dist/`.
