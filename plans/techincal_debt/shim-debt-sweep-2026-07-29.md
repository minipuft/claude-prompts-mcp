...
| 3.10 | ⊘      | **SUPERSEDED BY 5.8.** Blocked here because a zero-tolerance guard could not pass — the surface was ~1658 hits. Passes 1-5 took it to 418, all of it contract surface, so the guard is now writable but must **allowlist** rather than demand zero. Do not implement both rows                                                                      | `scripts/validate-no-framework-vocab.js`, `package.json`                                     | 3.9      | n/a — see 5.8                                                                                                       |
