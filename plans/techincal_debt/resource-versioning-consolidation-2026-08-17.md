...
| 1.5 | ✓   | `src/mcp/tools/index.ts:1078`                                                                                                                        | Apply the same annotations in `reregisterToolsWithUpdatedDescriptions`                                                                                        | ~10    | 1.4     | Switch framework, re-list tools over Streamable HTTP, confirm annotations persist     | STDIO pins one server; HTTP builds one per request. Annotations set only at first registration vanish on the HTTP path |
...
| #   | St  | File                                                                 | Change                                                                                                                  | ~Lines | Depends | Verify                                                                                     | Justification                                                   |
| --- | --- | -------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- | ------ | ------- | ------------------------------------------------------------------------------------------ | --------------------------------------------------------------- |
| 5.0 | ☐   | —                                                                    | Main-thread ruling on OQ-E1 before any edit in this tier                                                                | ~0     | 4.3     | Ruling recorded in `implementation-notes.md` with rationale                                | Durable rows recorded as object projections outlive this change |
| 5.1 | ☐   | `src/modules/versioning/tree-snapshot.ts` (NEW)                      | Recursive read of a resource directory to `{relativePath: contents}`, with an exclusion list and a 1 MB refusal ceiling | ~110   | 5.0     | Unit test on `resources/prompts/examples/create_framework/` (74,336 B, the corpus maximum) | Ceiling gives 13× headroom over the measured max                |
| 5.2 | ☐   | `snapshot-contract.ts`                                               | Add a `format` discriminator so tree-era and object-era rows are told apart on read                                     | ~30    | 5.1     | Old-format fixture row still restores through the object path                              | Without a discriminator the two eras are indistinguishable      |
| 5.3 | ☐   | the three contract implementations                                   | Project and restore through the tree walker; retain object restore per the OQ-E1 ruling                                 | ~90    | 5.2     | New test: delete a file under `tools/{id}/`, roll back, assert it is restored              | F7                                                              |
| 5.4 | ☐   | `.claude/rules/sqlite-persistence.md`, `docs/reference/mcp-tools.md` | Record the format discriminator, the ceiling, and the measured percentiles                                              | ~35    | 5.3     | Docs describe current state, no historical breadcrumbs                                     | Docs and code move in lockstep                                  |
...
| Tree snapshot restores `tools/{id}/`                           | integration                      | same file                                                                   | Multi-file restore is the whole point of item E                                                                                   |
| Object-era row still restores after item E                     | integration, fixture row         | same file                                                                   | OQ-E1's acceptance criterion; a seeded old-format row is the only way to test a durable-row path                                  |
| 1 MB ceiling refuses rather than truncates                     | unit                             | `tests/unit/versioning/`                                                    | Pure function over a directory; measured against the corpus max of 74,336 B                                                       |
...
| F7 closed              | Task 5.3                                                                                          | A file deleted under `tools/{id}/` is restored by rollback                                                             |
...
| Safety property intact | Fault-injection test                                                                              | A persistence failure during `commitEdit` leaves the file unmodified                                                   |
...
| Live drive             | `npm run build && npm run verify:mcp`, then the real client flow                                  | create gate → update ×2 → history → `dry_run` rollback → rollback → reload → inspect, on both transports               |
...
| `.claude/rules/sqlite-persistence.md` | Record the Tier 5.2 format discriminator, the 1 MB ceiling, and the measured percentiles               |
...
- [ ] Pattern: _a shared tool surface can hide divergent semantics per resource type_ — the tool contract was uniform while three implementations disagreed. Candidate for `/knowledge-capture` if it recurs.
- [ ] Pattern: _a projection-based snapshot creates a resolved-versus-authored problem that a byte-based one does not have._ First sighting; record, do not codify.
- [ ] Memory: the durable-table constraint means every snapshot-format change needs a dual-read plan, not a migration. Worth a project memory entry if OQ-E1 resolves to the default.
- [ ] Skill: `/testing` may deserve the red-on-mutation-removal obligation as an explicit step, given the conformance file records a second instance of an assertion passing without its mutation.
...
it. A rollback refused for any reason no longer writes version rows. Destructive actions are denied
by one guard, tools advertise destructive hints to clients, and `dry_run` previews rollback and
delete.
