...
| S3  | Information disclosure | SUSPECTED          | The prompt write receipt prints `Resource root:` from configuration rather than from the actual write target — pre-fix it named the real root while writing to `/tmp`. Containment closes the escape; the receipt is still unverified (row 2.6)        |
| S4  | Injection              | SUSPECTED          | Chain step-output re-render. Direct argument SSTI is REFUTED (`{{ 7*7 }}` renders literally), but whether captured step output is re-rendered downstream was not settled (row 2.5)                                                                     |
