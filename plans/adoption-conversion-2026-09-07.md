...
**Status**: ACTIVE — Tier 0 baseline captured 2026-09-07; **rebaselined 2026-09-13** — Tier 1 waits for the first ledger line whose window starts on or after the README install-path fix merges
**Owner**: minipuft
**Created**: 2026-09-07
...
**⚠ Superseded 2026-09-13 — the funnel broke earlier than this page.** Every documented install path ended with a command naming a prompt the package did not ship: `tech_evaluation_chain` (Claude Code, Codex), `research_chain` (Claude Desktop), and `review`, `code_review` and `analyze` elsewhere in the README. The first three lived only in the owner's personal store, which the owner's shell serves over the bundle, so the break was invisible from the one machine that checked it. Line 1 therefore stands as the record of a funnel broken at install, not as a comparison base. The reading above is kept because it is what the plan believed, and the tutorial defects it lists are still real — they are just not the first break. Fixed by `plans/readme-install-path-2026-09-13.md`.
...
## Tier 1 — Fix the page the funnel breaks on (BLOCKED on the rebaseline line)
...
**Rebaselined 2026-09-13 (owner ruling).** The block moves from ledger line 2 to the **rebaseline line**: the first ledger line whose `window_start` is on or after the merge date of `plans/readme-install-path-2026-09-13.md`. That fix lands inside the old window on purpose. It corrects a defect rather than trying a page, and holding every installer's first command broken to protect a comparison would only protect a comparison of a broken funnel.

| #   | Status                                                                                                                                                                | Task                                                                                                                                                                                                                                                                                                                   | Falsifier                                                                                                   |
| --- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- |
| 1.0 | ✓ (verified 2026-09-14 · #276 merged as `c68205bd` at 2026-09-14T09:36Z, so the rebaseline line is the first ledger line whose `window_start` is 2026-09-14 or later) | **Every README install path ends in a command that runs on a fresh install.** Executed by that plan; this row exists so the rebaseline condition has a row to read                                                                                                                                                     | the merge date of that plan's change, which fixes the rebaseline line                                       |
| 1.1 | ☐ (as of 2026-09-07 · flips when the ledger holds 2 lines)                                                                                                            | Second snapshot lands (Sunday 2026-09-13). Confirms the script runs unattended on the owner's box and gives the first Δ. ⚠ The first-Δ half is superseded by the 2026-09-13 rebaseline: line 2 measures the pre-fix funnel                                                                                             | `wc -l` on the ledger reads 2                                                                               |
| 1.2 | ☐ (as of 2026-09-13 · flips when the tutorial opens with prerequisites and a working install link, and every run example is a real command a reader can paste)        | **Held until the rebaseline line exists.** Rewrite `docs/tutorials/build-first-prompt.md` for a reader with nothing installed: prerequisites + install link first; `resource_manager` authoring demoted below the file pattern; real commands (not pseudo-syntax in `bash` fences); "hooks" named; `{{now}}` explained | a reader with a fresh checkout follows the page top-to-bottom and reaches "Run it" without leaving the page |
| 1.3 | ☐ (as of 2026-09-13 · flips when the README's Claude Desktop / MCPB install path is demoted or removed)                                                               | **Held until the rebaseline line exists.** README: demote the MCPB path. 58 all-time asset downloads across every release; keep it in `docs/`, drop it from the top-level install choice                                                                                                                               | `validate:readme` green; the README's install section names no `.mcpb` above the fold                       |
| 1.4 | ☐ (as of 2026-09-13 · flips when a "next page" exists at the tutorial's end that the paths table can see)                                                             | **Held until the rebaseline line exists.** Give the tutorial ONE next step, not five. The paths table shows readers stop after it; a single link to the gates guide makes continuation measurable as `docs/guides/gates.md` uniques                                                                                    | `docs/guides/gates.md` appears in the top paths at ≥ 20 uniques in a window after the change                |
