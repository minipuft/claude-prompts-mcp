...
| DEV-T5-1 | **Main moved before the PR opened.** #275 rewrote `.husky/pre-push` without scope branches and bundled a 40th prompt. Merged `origin/main` in rather than rewriting a pushed branch: took main's pre-push whole, because this branch's README-check additions lived inside the docs and hooks branches #275 deleted, and recomputed the shipped count from the same `git ls-files` set the README gate reads — 51 prompts across 9 categories, in the README and the CHANGELOG entry. The first resolution attempt refused its own assertion and aborted: `merge.conflictStyle` is `diff3`, and the regex could not separate the ours, base and theirs sections, so the retry parsed conflict markers line by line. Rows 3.7–3.9 moved to a successor plan executed by one follow-up PR |
