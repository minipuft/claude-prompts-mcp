...
| P4-F3    | Step-targeted gates enter the run-wide review accumulator before the step loop — targeting scopes guidance INJECTION only; review PARTICIPATION is unscoped, so a gate "on step 3" still reviews every step. Pre-existing for target_step_number, unchanged by mutation                                                                              | P5 (visibility/scoping is exactly this axis)                                            | OPEN — found P4 T4 (DEV-T4-12)                                                                       |
