...
**Status**: P0 IMPLEMENTED — code landed; template application still pending the operator MCP reconnect (live server predates the writer fix; update rolled back safely 2026-08-09). P1 IMPLEMENTED 2026-08-09 — all 6 tiers via subagents (sonnet T1-2/T5-6, opus T3-4), all gates green (typecheck, lint:ratchet, tests:ratchet, test:ci minus 2 pre-existing foreign failures, validate:arch, verify:mcp 12/12); pending final live drive after reconnect. Deviations + semantics rulings in the P1 implementation-notes file. P2-P6 unopened.
