---
title: "STAGED — not yet applied. Target: `implementation_plan/verification` → `user-message.md`."
date: 2026-08-09
status: backlog
tags: []
---

# STAGED — not yet applied. Target: `implementation_plan/verification` → `user-message.md`.

# Apply via `resource_manager(prompt, update, id:"implementation_plan/verification", user_message_template: <body below>)` AFTER the live MCP server restarts — the running stdio process predates the nested-id writer fix in `file-operations.ts` and would roll the write back.

# Body starts at the `<!-- TEMPLATE BODY BEGINS -->` marker; send everything after it and nothing before. Verify afterward with `resource_manager(prompt, inspect, id:"implementation_plan/verification", detail:"full")`.
...
Before emitting the implementation table (Phase 3), verify EVERY file path the design references against the actual filesystem. This catches plan-author drift — wrong paths, wrong line numbers, shim files mistaken for real implementations, fields assumed to exist that don't.

Your response MUST contain three CAGEERF-aligned sections. The phase-guard verification stage **enforces** their presence, minimum length, and freedom from placeholder terms (`TODO`, `TBD`, `placeholder`, `to be determined`, `will be added`). Missing or under-filled sections fail the gate and block advancement to Phase 3.

The header text below is the `section_header` declared in `resources/frameworks/cageerf/phases.yaml`, and the section splitter matches it literally. Do NOT emit the phase _ids_ (`context_establishment`, `systematic_analysis`, `goal_definition`) as headers — the splitter finds no sections and the guard fails on a response that looks complete. Emit ONLY the headers in the first column.

| Section header | CAGEERF phase         | What goes here                                                                                                                                       | Min length | Enforced by                                         |
| -------------- | --------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- | --------------------------------------------------- |
| `## Context`   | Context Establishment | Restate the design references being verified — the file paths, line numbers, and symbols Phase 2 cited                                               | 100 chars  | Phase guard (required, min_length, no-placeholders) |
...
### Verification protocol (per file in design's `read-before-implementing` + every file the plan_table will reference)
...
[1-3 sentences naming what Phase 2 design claimed: which files, which line numbers, which symbols, which fields the plan said exist. This section is what the verification will check against — the "before" state.]
...
revised_paths_block: |
  [If yes: corrected paths and line numbers Phase 3 plan_table MUST use instead.
...
The CAGEERF phase guards, evaluated by the phase-guard verification stage, deterministically check that the `## Context`, `## Analysis`, and `## Goals` sections are present, ≥100/100/80 characters, and free of placeholder terms. Missing or stub responses fail the gate and force a retry.

Pasting raw tool output under `## Analysis` does three things:
1. **Satisfies the phase guard** (real bash outputs easily exceed 100 chars and contain no `TODO`/`TBD`)
