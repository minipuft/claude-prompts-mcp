...
**Tier 0.5b reconciled 2026-08-11**: 24 of 33 rows resolved, 9 open — and **none of the 9 gates the
...
| A2  | ☐                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | Read-only actions first (SAFE, no isolation needed): `inspect`, `guide`, `analyze_type`, `analyze_gates`, `compare`, `history` across prompt/gate/framework                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | —       | scenarios green; no write to any tracked path                                                                                            |
...
| A5  | ☐                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | `checkpoint` resource_type (4th type, currently unexercised)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | A1      | scenario green or a recorded reason it cannot be                                                                                         |
...
| B2  | ☐ **UNBLOCKED 2026-08-11** | `system_control framework switch` round-trip: switch → read back → confirm the active framework changed. The blocker is gone twice over: A1 landed 2026-08-09, and Tier 0.5.10 added the `workspace: isolated` mode whose server runs on its own `MCP_RUNTIME_ROOT` **and** its own workspace, so a `switch` writing `kv_state key='framework'` cannot reach the owner's `state.db`. Write it as an isolated-workspace row. Assert the READ-BACK names the new framework — `ok:` cannot see a switch that reported success and changed nothing, which is the same blindness that hid the framework-application defect | A1, B1  | the read-back reflects the switch, not the default; owner's framework unchanged after the run |
...
| C5  | ◐ **1 of 3 arms — 2026-08-11**              | `skip` landed as `gate-action-skip-bypasses-shell-verification`, which also **falsified row 0.5.17**: skip works, and the "same gate at attempt 1/3" that looked like a defect was a sibling framework gate. `retry` and `abort` remain. Assert the RUN OUTCOME, not the next response — the intermediate reply names whichever gate speaks next, which is the ambiguity that produced the false finding                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| C6  | ☐                                           | Malformed `gate_verdict` string must reject, not silently pass — this is the failure mode the structured shape exists to remove                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | C4      | `error_contains` a parse-failure message                                           |
