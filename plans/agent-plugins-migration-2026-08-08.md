...
| D12 | ☐                                                                                                                                                                                                                                                                                                                                                           | **The registry's `gate` operator does not match its own examples.** Measured 2026-08-12 in BOTH engines (JS `RegExp` and Python `re` agree, so this is not a cross-language issue): all three `examples` for `gate` score 0/3 against `pattern.typescript`, because the pattern requires leading `\s+` and the examples start at position 0. The other seven operators are 2/2, 3/3, 1/1. Harmless inside a real command (`>>p :: 'x'` has the whitespace) but the `examples` field feeds hook hints and docs, so a user copying one verbatim gets silence. Either anchor the examples in a full command or relax the pattern                                                                                                                                                                                   | a check asserts every operator's `examples` match its own `pattern` — the registry's self-consistency, which nothing currently tests                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| E11 | ☐ **The ✓ marks in Tier 1 could not be trusted, and nothing detects that class**                                                                                                                                                                                                                                                                                                                         | Executing E10 found **seven rows across Tiers 1 and 0.5 marked ✓ whose deliverables were never committed** — 1.1, 1.2, 1.3 (all three named files untracked), 1.4, 1.5, and the source halves of two 0.5 fixes. Row 1.7 separately asserts "`skills/` generated output **is committed**"; it was 0 tracked, 2 present. A ✓ in this plan has meant "I made the edit", not "the edit is in the repository", and the two diverged silently for three days because every gate read the working tree. E7's `typecheck:committed` closes the compile half; **nothing closes the general case** — a plan row naming a file that is untracked at HEAD is detectable and undetected                                                                                                                                                                                                                                                                                                                                                                                                                                         | a check reads each ✓ row's named files and fails when one is untracked at HEAD, OR the plan records that ✓ carries no such guarantee                                                                                                                                                                                                                                                                                                                                  |
