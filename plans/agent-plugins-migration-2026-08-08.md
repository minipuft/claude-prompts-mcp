...
| 4.5   | ☐ (as of 2026-08-14 · flips when a plugin installed from the git marketplace is measured to have a working server, or a bundling step is added that gives it one) | **NEW — a marketplace install may still have no runtime, and this is unmeasured.** A fresh clone carries 21 tracked files and **no** `node_modules` (gitignored), and Codex installs a plugin by copying its source — the skill records that `node_modules/` is _preserved_ in the copy, not installed. The dev-marketplace install works because its source is a symlink to a checkout that has `node_modules`; a git-sourced install has nothing to preserve, so `./bin/start-mcp.mjs`'s import of `../node_modules/claude-prompts/dist/index.js` would fail. **Not claimed as measured** — verifying it means installing `codex-prompts@minipuft`, which collides with the enabled dev install on this machine. If confirmed, the options are committing the installed dependency or publishing a bundled release archive for the marketplace to point at                                                                                                                                                                                                                                                                                                                                       | 4.0.1      | install from the git marketplace on a clean machine, then complete an MCP initialize handshake against that install                                  |
