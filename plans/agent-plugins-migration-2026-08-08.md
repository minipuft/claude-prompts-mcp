...
**TIER 2 CLOSED and TIER 3 LANDED-EXCEPT-ITS-OWNER-GATE, 2026-08-13.** `validate:all` **37/37**,
`test:ci` **2480/2480**. The renderer exists and the zero-diff property is proven in its strongest
form — it WROTE both published manifests and `git diff` was empty. Tier 2's authored gate named
four comparison trees and only one is a render; Tier 3's rows 3.2 and 3.3 are marked ⚠ because
their shared premise ("renders supersede the sync path") is false in both directions. Tier 3's own
Gate is owner-only and is **not** claimed here.

**Still open elsewhere**: rows 1.6–1.9, 3.2/3.3 (⚠, premises rewritten), 3.5 (new), findings
0.5.23–0.5.25 and E12 (carried forward, not blockers), and tiers 4–7 — which ARE the release.
This plan stays `status: active`.
...
### Tier 2 — Renderer + drift check — ✓ LANDED 2026-08-13

| #   | Status                              | File                                                                           | Change                                                                                                                                                                                                                                                                                                                                                                                                                   | ~Lines authored → measured | Depends | Verify                                                                                                                                |
| --- | ----------------------------------- | ------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------- | ------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| 2.1 | ✓ **2026-08-13**                    | scripts/render-distributions.mjs (NEW) · scripts/render-targets.json           | **Scope corrected — the four-target premise was falsified.** Only ONE target has a file-copy relationship with this repo; see the row below. The renderer projects canonical `plugin.json`/`mcp.json` → `.claude-plugin/plugin.json` + `.mcp.json` (drop `$schema`, drop `type`, drop `MCP_RUNTIME_ROOT`, `${PLUGIN_ROOT}`→`${CLAUDE_PLUGIN_ROOT}`, version last). `renderKind` per target; an unknown value is an error | ~250 → ~330                | 1.1–1.4 | ✓ **zero-diff**: `--write` against the untouched hand state produces `git diff` EMPTY on both files — stronger than compare-equal     |
| 2.2 | ✓ **2026-08-13**                    | same file, `--check` mode · `validate:render-drift` in SUITE                   | render in memory, byte-compare vs published target, name the first differing line. Byte comparison not deep-equal: a semantic match would pass while the next `--write` still emits a diff                                                                                                                                                                                                                               | ~40 → ~55                  | 2.1     | ✓ **3 distinct seeded mutations → 3 distinct single-file failures**: published byte, canonical byte, deleted output                   |
| 2.3 | ✓ **2026-08-13 — premise INVERTED** | server/scripts/sync-versions.js · release-please-config.json · .prettierignore | **The gap ran the other way.** Row asserted the RENDERED manifests needed covering; measured, `.claude-plugin/plugin.json` was already at `validate-versions.js:22` and predates this plan. The real defect: root `plugin.json` — the CANONICAL manifest — had **no version writer at all**. Reproduced by simulating a bump: rendered went to 9.9.9, canonical stayed 3.2.1, and BOTH gates red                         | ~20 → ~14                  | 2.1     | ✓ re-simulated after the fix — canonical and rendered move together, `validate:render-drift` green under a bump                       |
| 2.4 | ✓ **2026-08-13 — found by 2.2**     | server/tests/unit/scripts/validation-suite-runner.test.ts                      | **Red at HEAD before this tier started, attributed to `e582adc0`.** Its "declares only steps package.json defines" assertion regexed the runner's SOURCE TEXT for `{ script: '…'` on one line; `e582adc0` gave each entry a second field and Prettier wrapped them, so **zero** entries matched while the property was unchanged. Now imports `SUITE` — the runner guards its entry point, so importing is safe          | — → ~20                    | —       | ✓ falsified: a SUITE step with no npm script reds the test **and names it**. `toBeGreaterThan(0)` guard kept — it is what caught this |

**Gate**: ✓ PASSED 2026-08-13 — `validate:all` **37/37**, `test:ci` **2480/2480**, typecheck 0
errors, `lint:ratchet` + `typecheck:tests:ratchet` no regressions. The zero-diff property is
demonstrated at its strongest form: the renderer WROTE both published files and `git diff` was
empty, so the check is not merely comparing equal — it reproduces the hand state byte-for-byte.

**The gate's authored subject was four trees; only one of them exists.** Measured 2026-08-13:

| Authored                                     | Measured                                                                                                                                                                                                                           |
| -------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| zero-diff vs `gemini-prompts` working tree   | `gemini-prompts/hooks/lib` is a **symlink into `node_modules/claude-prompts/hooks/lib`**; its `hooks/hooks.json` and `hooks/gate-enforce.py` both DIFFER from ours because Gemini CLI's events differ (`BeforeAgent`/`BeforeTool`) |
| zero-diff vs `opencode-prompts` working tree | tracks **no** `hooks/` and **no** `server/` — an independent TypeScript port (`src/`, `index.ts`)                                                                                                                                  |
| `hookMapping` + `consumes` render into both  | `.claude/rules/extension-alignment.md` states the actual contract: downstream repos "consume it as an npm dependency and register thin adapters"; OpenCode is "a behavioral port… nothing is shared with the TS rewrite"           |

A renderer emitting into those two repos would not reproduce a hand state — it would invent one,
and the gate would compare against something nobody ships. `render-targets.json` now records
`renderKind: "npm-dependency"` with the measurement inline, and their per-release update is the
dependency-range bump `extension-publish.yml`'s `sync-downstream` job already performs.

**Why row 1.3's Verify passed while asserting this** — the substitution checked that every
`consumes` path _exists in this repo_. The claim was that the target _consumes it that way_. Same
...
| #     | Status                                              | File                                                                                                              | Change                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | ~Lines authored → measured      | Depends | Verify                                                                                                                                                                                                                        |
| ----- | --------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------- | ------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 3.1   | ✓ **2026-08-13**                                    | .github/workflows/extension-publish.yml                                                                           | **Step, not a job.** The "push with `--auto` + MERGED verify" this row describes ALREADY EXISTS as the `sync-downstream` job (:280), and it pushes version metadata, not renders. What was missing is the drift check on the publish path: both jobs that stage `.claude-plugin/plugin.json` + `.mcp.json` into an installer-facing artifact (`plugin-dist` → dist branch, `claude-code-plugin` → archive) validated only that the files EXIST, never that they are current                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | ~80 → ~22 (2 jobs)              | 2.1     | ✓ both jobs carry `Verify rendered manifests match their canonical source`, before `npm ci` so it fails fast                                                                                                                  |
| 3.2 ⚠ | **PREMISE FALSIFIED — do NOT delete**               | .github/workflows/downstream-sync.yml                                                                             | Row says "DELETE — renders supersede". Measured 2026-08-13, **twice wrong**: (a) it syncs nothing — it is report-only, parsing a dispatch payload and running `validate-versions --distribution --skip-npm`, so there is nothing for a render to supersede; (b) it has a **live external dispatcher** — `opencode-prompts/.github/workflows/release-please.yml:34-40` fires `event-type: downstream-release` at `minipuft/claude-prompts-mcp` on every opencode release. Deleting it drops the only signal that an opencode release diverged from upstream                                                                                                                                                                                                                                                                                                                                                                                    | −59 → **0**                     | —       | superseded — see row 3.5 for the question that remains                                                                                                                                                                        |
| 3.3 ⚠ | **PREMISE FALSIFIED — do NOT delete**               | server/scripts/synchronize-downstream-lock.js                                                                     | Row says "retire absorbed targets; delete if empty". Measured: **147 lines and load-bearing.** Called at `extension-publish.yml:384` to regenerate downstream lockfiles, and `validate-release-workflow.js:52` **fails when the reference is absent**. It exists because package.json/lockfile desync left opencode-prompts unable to `npm ci` for months. `rg` for it returns 4 non-plan hits, not 0                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | −100 → **0**                    | —       | ✓ `validate:release-workflow` PASSED with the script in place — the gate the row would have broken                                                                                                                            |
| 3.4   | ✓ **no change required**                            | validate-renovate-extraction.js                                                                                   | Row anticipated "delete+add". Neither happened: 3.2 deletes nothing and 3.1 added steps to an existing workflow rather than a new file. Inventory re-measured — 7 workflows on disk + 1 action file = the 8 declared entries, `EXPECTED_COUNTS['github-actions'] = 8` agrees                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | ~6 → **0**                      | 3.1–3.2 | ✓ `validate:renovate-extraction:self-test` PASSED. NOTE: the bare script reads **stdin** (`readFileSync(0)`) and is fed Renovate JSONL in CI — running it alone throws by design and is not in the SUITE                      |
| 3.5   | ✓ **2026-08-13 — deleted, both halves**             | .github/workflows/downstream-sync.yml · server/scripts/validate-versions.js · opencode-prompts release-please.yml | **Row 3.2's question, answered by measuring rather than by relocating.** The chain was dead in three independent ways: `downstream-sync.yml` reports **`total_count: 0`** runs since it was written; `UPSTREAM_DISPATCH_TOKEN` is **absent from opencode-prompts' secrets** so the dispatch could never authenticate, with `continue-on-error` swallowing it; and the receiver's own check was `continue-on-error` too. `--distribution` had exactly one caller — that workflow — so ~130 lines of live code had never executed. **Superseded by the fleet auditor** (`minipuft/repository-standards`), which covers all three downstream repos including `minipuft-plugins`, ran successfully **on schedule 2026-08-10**, exits non-zero on drift, and compares the resolved `package-lock` version rather than the declared range                                                                                                           | −59 −130 −17 (3 files, 2 repos) | —       | ✓ zero live references remain (`rg` clean); `validate:renovate-extraction:self-test`, `validate:release-workflow`, `validate:github-action-pins` (42 refs / 10 files) all green                                               |
| 3.6   | ✓ **2026-08-13 — rebuilt where it runs, not filed** | .github/workflows/extension-publish.yml · server/scripts/validate-release-workflow.js                             | **The one assertion 3.5's deletion would have dropped.** `assertMarketplaceSource` checked that the listing resolves this repo by its real current name and not a **rename redirect** — its own comment said "this check is the only thing that would notice" — and the fleet auditor never opens `marketplace.json`. Filing it as a row was the wrong call: it traded a guard for a promise. Rebuilt in the `sync-downstream` job that already edits the marketplace entry, which runs every release and blocks. The URL is **derived** from canonical `plugin.json.repository`, never hardcoded — the previous guard hardcoded it, and a second literal is exactly how `fleet.json` ended up on the pre-rename slug. `ref` must be `dist`, the branch `publish-dist` force-pushes. Measured before rebuilding: the live listing is correct (3.2.1, `claude-prompts-mcp.git`, `dist`) — nothing was broken, the protection was simply absent | +14 / +18                       | —       | ✓ falsified twice: 3 self-test cases (missing url assertion · missing ref assertion · slug re-hardcoded) each fail alone, AND the shell logic run against a seeded pre-rename listing REJECTS it while accepting the live one |

**Gate**: ⚠ **BLOCKED — owner-only, not passed by this execution.** The criterion is "test-tag
release dry-run; OWNER installs Claude Code plugin from dist branch", and neither half is
self-servable: publishing a test tag mutates a public release surface and the install is a manual
owner check. What WAS verified is every automated gate the tier touches — `validate:all` 37/37,
`validate:release-workflow`, `validate:github-action-pins` (44 refs across 11 files), and the
workflow YAML parsing with both new steps present in `plugin-dist` and `claude-code-plugin`.
...
| 4.0.1 | ✓ 2026-08-14                                                                                                                         | **RESOLVED — the vendored tarball is gone, not refreshed.** The premise was that codex-prompts needed its `file:` tarball bumped to current; measured 2026-08-14, the tarball was never committable at all — `package.json` declared `file:vendor/claude-prompts-3.1.2.tgz` while `.gitignore` excluded `vendor/*.tgz`, so **the repo's only dependency was not in the repo** and a fresh clone could not install it. The failure hides behind a warm npm cache: a clone on a machine that had once packed the tarball resolves it from `~/.npm/_cacache` and succeeds, while the same clone with an empty cache fails outright. Its own README named the exit condition ("until `claude-prompts@3.1.0` is on npm … after publish, switch the dependency") and 3.1.0/3.1.1/3.2.0/3.2.1 have all since published — the workaround outlived its stated retirement by four versions because nothing watched for the condition to come true. Now `"claude-prompts": "^3.2.1"` from the registry (`49c2161`)                                                                                                                                                                                                                                              | —          | ✓ cold-cache install pulls 3.2.1 with `dist/index.js`; adapter tests 27/27; the launcher completes an MCP initialize handshake                       |
...
| 4.4   | ☐ (as of 2026-08-14 · flips when `fleet.json` lists `minipuft/codex-prompts` and a scheduled `Fleet Drift Audit` run has covered it) | **Keeping codex-prompts means auditing it, and the blocker to that is now removed.** It is the only consumer absent from `fleet.json`, which is why it drifted three minors unobserved. The obstacle this row recorded — the auditor reads a resolved `node_modules/claude-prompts` lock version and a vendored `file:` tarball has none — no longer applies: 4.0.1 moved it to the registry, and its lockfile now resolves `https://registry.npmjs.org/claude-prompts/-/claude-prompts-3.2.1.tgz`. Adding it should be a `fleet.json` entry with the `node-consumer` profile its siblings use, in `minipuft/repository-standards`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | 4.0.1      | a `schedule` run of `Fleet Drift Audit` reports codex-prompts                                                                                        |
| 4.5   | ⚠ ☐ CONFIRMED by measurement (as of 2026-08-14 · flips when a git-sourced install completes an MCP initialize handshake)             | **CONFIRMED 2026-08-14 — a marketplace install from git has no server at all.** Measured without spending a Codex token: a clone of local HEAD (tracked files only, no `node_modules` — exactly what a git marketplace serves) was installed under a renamed plugin id through a throwaway marketplace, so the enabled dev install two live sessions were using was never touched. **Codex ran no install step**: the cache copy contains the 9 tracked entries and no `node_modules`. Starting the declared server from that install fails with `ERR_MODULE_NOT_FOUND … /node_modules/claude-prompts/dist/index.js imported from bin/start-mcp.mjs`. Positive control on the same probe: the dev install, which has `node_modules`, returns a full initialize result. Same command, opposite outcomes. **This affects the currently published 0.1.0 too**, so the marketplace listing serves a non-functional plugin today. Note both halves break, not just the server: `hooks/lib` is a committed symlink into `node_modules`, and Codex drops symlinks in the copy, so `_codex_bootstrap.py`'s fallback resolves to the same missing directory — an `npx`-based `.mcp.json` (the gemini pattern) would fix the server and leave the hooks broken | 4.0.1      | a git-sourced install completes an MCP initialize handshake AND its hooks resolve `hooks/lib` — both halves, not just the server                     |
...
| E12 | ☐ (as of 2026-08-13 · flips when a check reads the TARGET's shape, not just the source path — e.g. `render-targets.json` asserting a downstream consumption mode that is verified against that repo's tracked tree)                                                                                                                                                                                                                                                                                                                                                                                                                                                | **A `consumes` list is a claim about a RELATIONSHIP, and every gate on it read only one end.** Tier 2 found `render-targets.json` asserting that `gemini-prompts` and `opencode-prompts` consume `hooks`/`server/dist` as file copies. Neither does — gemini symlinks `hooks/lib` into `node_modules/claude-prompts/` and carries its own differing adapters; opencode tracks neither directory. Row 1.3's substituted Verify passed throughout because it resolved each path **in this repo**, which is available and cheap, while the claim was about the other repo, which is neither. This is E7 and E11's shape a third time: an available stand-in silently becomes the definition. It binds Tier 4 and Tier 5, both of which are scoped from these same target entries                                                                                                                                                                                                                                                                                                                                      | a check that reads the downstream tree, OR the entries carry their measured consumption mode and nothing derives scope from `consumes` alone (partially done 2026-08-13 — `renderKind` records the mode; nothing verifies it)                                                                                                                                                                                                                                         |
| E13 | ✓ **2026-08-13 — fixed in Tier 2.4**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | **A gate this initiative built broke a test, and the gate that would have caught it does not run tests.** `e582adc0` gave every SUITE entry a `reads`/`converse` field; Prettier wrapped the entries onto multiple lines; `validation-suite-runner.test.ts`'s source-text regex `{ script: '…'` then matched **zero** of 37 while the property it asserts was unchanged. It sat red at HEAD. `validate:all` runs checkers and not Jest, so nothing local reported it — the same "green gate reading something else" class as E7, arriving via the substrate work itself                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | ✓ the assertion imports `SUITE` instead of scanning source; falsified by a step with no npm script, which reds the test and names it                                                                                                                                                                                                                                                                                                                                  |
...
## Execution record — Tier 2 + Tier 3, 2026-08-13

**Closed**: 2.1, 2.2, 2.3, 2.4 (new), 3.1, 3.4. **Marked ⚠**: 3.2, 3.3 — both premises falsified.
**New rows**: 2.4, 3.5. **New findings**: E12 (open), E13 (closed).
**Result**: `validate:all` **37/37** · `test:ci` **2480/2480** · typecheck 0 errors ·
`lint:ratchet` and `typecheck:tests:ratchet` no regressions.

### What the re-measurement changed before any code was written

Section A obliges re-measuring what the tier asserts. Tier 2's gate named four comparison trees;
three of them do not exist as renders. Tier 3's four rows rested on "renders supersede the sync
path"; measured, the sync path does not sync content and the renderer does not target the repos
it was to supersede. Six of the eight rows across both tiers moved:

| Authored                                                    | Measured 2026-08-13                                                                                      |
| ----------------------------------------------------------- | -------------------------------------------------------------------------------------------------------- |
| zero-diff vs 4 trees                                        | 2 files, 1 target — the other two consume this repo via npm                                              |
| 2.3: cover the RENDERED manifests                           | already covered since before the plan; the CANONICAL manifest had no version writer                      |
| 3.2: `downstream-sync.yml` DELETE, renders supersede        | report-only, syncs nothing, and has a live dispatcher in `opencode-prompts`                              |
| 3.3: `synchronize-downstream-lock.js` −100, delete if empty | 147 lines, called at `extension-publish.yml:384`, and `validate-release-workflow.js:52` fails without it |
| 3.4: inventory reflects delete+add                          | neither occurred; inventory already agrees with disk (7 workflows + 1 action = 8 declared)               |
| 3.1: render-distributions JOB, push with `--auto`           | that job exists (`sync-downstream`); the gap was a pre-publish drift check, ~22 lines across 2 jobs      |

### One probe answered a different question, and it nearly deleted a live workflow

Probing "who dispatches `downstream-release`" with `rg` over this repository returned one hit —
an ASCII diagram in `docs/guides/release-process.md:129` — which reads as "nothing dispatches it,
the workflow is dead". The trigger is `repository_dispatch`, which is **cross-repo by
construction**, so a repo-scoped probe cannot answer it. Re-probing the sibling repos found
`opencode-prompts/.github/workflows/release-please.yml:34-40` firing it at
`minipuft/claude-prompts-mcp` on every opencode release.

Two probes in one session measured the wrong end of a cross-repo relationship — this one and the
`consumes` check in E12. The shape is worth naming: **when a claim spans two repositories, a probe
that runs in one of them is measuring a correlate.** An earlier `rg` in the same session also
missed `.github/**` entirely, because ripgrep skips dotted directories by default — same class,
smaller blast radius.

### Substitution

Row 2.1's Verify ("zero-diff render vs current .claude-plugin/, .mcp.json, gemini-prompts,
opencode-prompts working trees") could not be run as written; two of its four subjects are not
renders. **Substituted with a stronger check on the subjects that do exist**: rather than
comparing rendered bytes to published bytes, the renderer WROTE both files and `git diff` was
required to be empty. Compare-equal proves the check agrees with itself; write-then-diff proves
the renderer reproduces the committed hand state.

Row 3.4's Verify ("gate green") names `validate:renovate-extraction`, which reads **stdin** and
throws when run bare — a vacuous invocation, not a gate. Substituted with its `--self-test` plus a
direct inventory re-measurement against the workflow directory.

### The gate that graded its own author

`validate:render-drift` was added to the SUITE with `reads: ['file']`. The substrate mechanism
built in the previous session re-derived it and reported `[file, spawn]` — because the renderer's
failure hint contained the literal `npm run`, which is indistinguishable by shape from a command a
