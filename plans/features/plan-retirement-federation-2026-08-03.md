...
| F4.5 | ✓ 2026-08-14                                                        | Migrate this repo to the shared version; delete the local copy                 | `scripts/`, `server/package.json`, `release-please.yml` | F4.4    | ✓ local script deleted; shared self-test OK; `plans:retire:check` reports 0 invalid and `validate:all` is 37/37. The 0/0 half was blocked by a defect the migration EXPOSED rather than caused — the shared walker reported the gitignored `.validation-log.md` sidecar as a plan with no frontmatter, which the deleted local copy had a suffix-specific guard against. Fixed upstream (`repository-standards` PR #7, released as `v1.2.1`) by skipping gitignored files, which is principled rather than a workaround: retirement's safety rests on git history, so an ignored file cannot be a retirable plan. Consumed here by the v1.2.0 → v1.2.1 bump |
| F4.6 | ✓ 2026-08-15 (generalization proven; one apply residual owner-held) | Onboard **cloudySky** — 131 plans, 49 `done`, no remote                        | cloudySky `plans/`, package scripts                     | F4.5    | ✓ config present in canonical cloudySky; check live and falsified — a seeded citer exits 1 and removing it returns 0; **15 archived / 43 referenced / 0 remaining**, matching this row's predicted numbers exactly. RESIDUAL: `--apply` refuses to move anything while `plans/ocean-surface-cone-remediation-2026-07-21.md` is `done` with uncommitted changes — another session's work, one commit away, owner-held                                                                                                                                                                                                                                        |
...
**Gate**: ✓ **CLOSED 2026-08-15 (owner ruling on the residual).** The criterion was that cloudySky
retire its own finished plans by running the shared script, and that a deliberately misclassified
plan there exit 1 — proving the check _measures_ in a repo with a different layout and no CI, not
merely that it runs where it was written.

**The generalization proof is the strongest evidence this initiative produced, and it was not a
seeded one.** Onboarding cloudySky immediately surfaced **30 plans marked `done` that were still
cited** — by `src/**/*.ts`, by `docs/`, and by sibling plans — 57 citations in total. The retired
hardcoded `LINK_SOURCES` named `server/src` and `server/scripts`, neither of which exists there, so
the old scan would have found nothing, reported a clean queue, and archived all 30 into a gitignored
directory **in the one repo with no remote to restore from**. That is precisely the silent-then-
destructive path §"The danger this tier exists to contain" was written about, caught on its first
real run.

Reclassification **cascaded** — 34 across three passes. Each plan moved to `reference` stops
co-moving, so its citations begin blocking their own targets. Worth knowing before onboarding
another repo: convergence is iterative, not single-pass, and a first-pass count understates the
work.

Final state: **15 archived / 43 referenced / 0 remaining**, matching this row's predicted numbers
exactly, with a seeded citer exiting 1 and its removal returning 0.

**Residual, owner-held**: `--apply` has not moved anything, because the fail-closed guard refuses the
whole batch while `plans/ocean-surface-cone-remediation-2026-07-21.md` is `done` with uncommitted
changes. That guard doing its job in a repo with no remote is itself part of the proof. It is one
commit away and the owner has taken it. **The mechanism question this tier existed to answer is
closed; what remains is an operational step, not an unknown.**

**Gate state — RESOLVED 2026-08-15.** Both consumer migrations originally ran in independent clones
because their canonical checkouts held other sessions' uncommitted edits. `claude-prompts-mcp`
landed via #209 and is live (F4.5). cloudySky is now onboarded in its canonical checkout, and the
one file still holding `--apply` back is another session's uncommitted plan — the guard refusing to
proceed is the designed behaviour, not a blocker to route around.
