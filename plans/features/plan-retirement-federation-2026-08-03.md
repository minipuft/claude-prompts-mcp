...
| F4.5 | ☐ (as of 2026-08-14 · flips when `plans:retire:check` reports 0 invalid plans in this repo — the migration itself is MERGED and the local copy is deleted; what blocks the 0/0 half of the Verify is `repository-standards` PR #7) | Migrate this repo to the shared version; delete the local copy                 | `scripts/`, `server/package.json`, `release-please.yml` | F4.4    | ✓ local script deleted; shared self-test OK — ✗ shared check reports 1 invalid, and the cause is a defect the migration EXPOSED rather than caused: the shared walker reports the gitignored `.validation-log.md` sidecar as a plan with no frontmatter, which the deleted local copy had a suffix-specific guard against. Fixed upstream in PR #7 by skipping gitignored files (principled: retirement's safety rests on git history, so an ignored file cannot be a retirable plan). Closes here on a `v1.2.1` bump |
