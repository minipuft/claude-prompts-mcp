...
| A.2 | ☐ (as of 2026-08-30 · OQ-A2 ruled, see below · flips when `>>a --> >>b :: 'x' ==> >>c` compiles through `compileWorkflowIR` with byte-identical `chain_run_nodes`) | `04-parsing-stage.ts`, `symbolic-operator-parser.ts`, `workflow-ir/compiler.ts` | A `-->` command parses to an IR (`n1…nN`, linear edges, per-step args/gates/`==>` mapped onto node fields) and enters `compileWorkflowIR` — the string path stops building `chainSteps` on its own                                                                                                       | Byte-identical `chain_run_nodes` for `>>a --> >>b` before/after (snapshot); existing symbolic-chain tests green                                   |
