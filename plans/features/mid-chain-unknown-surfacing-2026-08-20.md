...
| A.2 | ⚠ (as of 2026-08-30 · OQ-A2's `::` mapping is falsified by measurement, see the Tier A continuation record and DEV-TA2-1 · flips when a ruling exists that keeps per-step `::` resolvable against the gate registry)           | `04-parsing-stage.ts`, `symbolic-operator-parser.ts`, `workflow-ir/compiler.ts` | **Blocked on a re-ruling, not on work.** The `==>` and `* N` halves of OQ-A2 are implementable as ruled. The `::` half is not: routing per-step criteria to run-level `gates[]` provably LOSES registered-gate resolution, so `>>a :: code-quality --> >>b` would stop binding the registered gate and start creating a temp gate whose criterion is the literal string. Shipping A.2 under the current ruling regresses a protected surface and turns `symbolic-inline-gate-attribution.test.ts` red — which is the row's own Verify clause | Byte-identical `chain_run_nodes` for `>>a --> >>b` before/after (snapshot); existing symbolic-chain tests green                                   |
...
| A.4 | ☐ (as of 2026-08-30 · flips when the `-->` append syntax is documented alongside A.1's three-inputs-one-IR half — A.3 has LANDED, so both halves are now writable)                                                             | `docs/reference/workflow-ir.md`, `chain-schema.md`, `mcp-tools.md`              | Document the three inputs → one IR; `budget` in YAML; append syntax                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Docs/code lockstep                                                                                                                                |
