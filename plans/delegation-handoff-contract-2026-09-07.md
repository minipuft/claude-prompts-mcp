...
| 2.9  | ✓   | `pipeline/stages/16-response-capture-stage.ts` `runHandoffEvidencePhase`                                                                                                                           | LIVE-DRIVE FINDING: a `gate_verdict`-only call (no `user_response`) at a delegated node with a pending review advanced and COMPLETED the run on this branch and on `main` — the phase returned early on an empty reply, and the pending-review PASS path advanced the step with nothing captured. Fix: an empty reply is exempt only when the node already holds a real captured output (`chainSessionStore.isStepComplete`, checked at capture time); otherwise the empty reply is classified (`trailer`) and refused under `required`                                                                                                               | ~20    | 2.4      | integration: verdict-only at the delegated node → refused; two-call pattern (reply then verdict) → accepted; live drive                                              | guarantee B is about the RUN advancing, not about which parameter carried the advance                                            |
| 2.10 | ✓   | `operators/chain-operator-executor.ts:92,108,372,553`                                                                                                                                              | LIVE-DRIVE FINDING (pre-existing, on `main` too): under the default CAGEERF framework the resume that captures step 1 renders delegated step 2 through `renderGateReviewStep` (phase-guard review pending, attempt 0) — 'Structural Review Required' with the full step-2 template and NO brief, NO handoff. The parent has nothing to hand a worker, so the feature is hollow in default-framework chains. Fix: a gate-review render whose target step is delegated and not a retry carries the normal delegated render (brief + handoff) plus the review CTA                                                                                        | ~40    | —        | integration with a pending phase-guard review at first render; live drive                                                                                            | the success signal (`run_in_background: false` in the handoff) failed live until this                                            |
| 2.11 | ✓   | `pipeline/stages/19-phase-guard-verification-stage.ts:199-215,283`, `operators/chain-operator-executor.ts:1274`                                                                                    | FOUND WHILE FIXING 2.10 (pre-existing, on `main`): the phase-guard review grades step N's `user_response` (stage 19 reads it after stage 16 advanced `currentNodeId` to N+1) but stamps no reviewed-step index, so `resolveReviewStep` falls through to `current_step` = N+1 — the review quotes step N+1's task, lists step N's missing sections as 'Improvements Needed', and gates step N+1's advance. Row 2.10 renders the delegated brief in that response so the parent can still hand off; this row fixes the attribution itself                                                                                                               | ~40    | —        | integration: step N fails guards → the review quotes step N                                                                                                          | out of this arc's scope; changes every phase-guard render                                                                        |

**Tier 2 receipt (✓ 2026-09-08, `feat/delegation-handoff`)** — rows 2.1–2.8 flipped on their Verify commands, row 3.4 (hook) with them:
`typecheck` clean; `lint:ratchet` 3093/970 no regression; `typecheck:tests:ratchet` 367 no regression;
`validate:arch` 0 errors / 17 warnings (two fewer than the agent's draft — vocabulary moved to `shared/types`);
`validate:domain-ownership` 15 capabilities / 5 modules; `validate:module-catalog` 36 boundaries;
`validate:python` 281 passed; `test:unit` 229 suites / 3048 passed (1 skipped, pre-existing); `test:integration` 63 suites / 807 passed.
`execute` = 14, `captureStep` = 14. Deviations 7–16 in the notes.

**Tier 2 addendum receipt (✓ 2026-09-08, rows 2.9–2.10)** — verdict-only resume at a delegated node refused (`carries no worker reply`), two-call pattern accepted, gate-review render of a delegated step carries brief + handoff + review CTA; live drive: brief HIT (`EXECUTION BRIEF`, `HANDOFF RESULT`, `run_in_background: false`, token `n2`), verdict-only / prose-only / wrong-node refused, conforming resume completes 2/2. Deviations 18–21 in the notes.

**Row 2.11 receipt (✓ 2026-09-09, `fix/phase-guard-attribution`)** — the phase-guard review names the step it graded:
`StepCaptureService.captureRealResponse` records `context.state.session.capturedStep = {nodeId, ordinal}` when it writes a
...
Row 3.5 receipt: ✓ 2026-09-08 — ledger carries D8–D14, open-decisions block closed, O5 killed by D14 before it was opened.

**Tier 3 receipt (✓ 2026-09-08)** — 3.1 chains-lifecycle +51 (why delegate, trailer block, `### Resuming a delegated step`, layering), 3.2 overview Execution Domain row + layering paragraph, 3.3 CHANGELOG Added + Changed; root prettier clean; `rg delegation_skipped docs` → 0. Noted, not fixed: the Execution Domain table numbers ResponseCapture as stage 08 while the stage-order diagram says 16 (pre-existing).

Tier 3 gate: `scripts/classify-validation-scope.js` → full (mixed change); pre-push runs the full route.

## Tier 4 — Detached mode (same release as Tiers 1–3 — R4; its own PR off `main` — R5)

| #   | St                                                                                                                                        | File                                                                                                                                                                  | Change                                                                                                                                                                                                                                                                                                                                                                                                         | ~Lines | Depends  | Verify                                                                                                                                                     | Justification                                       |
| --- | ----------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ | -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------- |
| 4.1 | ✓                                                                                                                                         | `modules/workflow-ir/node-schema.ts`, `operators/types.ts`, `workflow-ir/compiler.ts`, `yaml-prompt-loader.ts`, stage 04, stage 06                                    | `await?: 'node' \| 'run'` declared, carried at all three strippers, `run` implies delegated (stage 06)                                                                                                                                                                                                                                                                                                         | ~20    | —        | IR schema snapshot regenerated; `typecheck:tests:ratchet`                                                                                                  | YAML/IR field first (non-goal: symbolic syntax)     |
| 4.2 | ✓                                                                                                                                         | `infra/database/sqlite-engine.ts` + `table-contracts.ts` + `run-registry.ts`                                                                                          | `SCHEMA_VERSION` 30 → 31; `chain_run_nodes` += `spawned_at INTEGER` (one column, not two — notes deviation 25)                                                                                                                                                                                                                                                                                                 | ~30    | 4.1      | schema tests; ephemeral tables drop/recreate                                                                                                               | detached needs a node lifecycle the run can query   |
| 4.3 | ✓                                                                                                                                         | `modules/chains/manager.ts` (`markNodeSpawned`, `completeHeldRun`, `transitionRunStatus` guard), stage 18, `shared/types/chain-session.ts`                            | detached node marked spawned at render, reported on late result; `completed` refused while any spawned detached node is unreported                                                                                                                                                                                                                                                                             | ~80    | 4.2      | integration: close refused until the late result lands                                                                                                     | the run-level obligation (D4)                       |
| 4.4 | ✓                                                                                                                                         | stage 16 `runResumeAdmission` + NEW `delegation/detached.ts` (`resolveDetachedReport`) + `StepCaptureService`                                                         | a trailer naming a non-current `await: run` node routes the result to that node instead of the current one                                                                                                                                                                                                                                                                                                     | ~40    | 4.3      | integration with fake worker                                                                                                                               | reuses the token as the router; no new parameter    |
| 4.5 | ✓                                                                                                                                         | `delegation/brief.ts`, `renderer.ts`, `strategy.ts` + `hooks/`                                                                                                        | detached handoff: "do not wait, report later with this node token"; Claude `run_in_background: true`; the Claude Code hook holds a detached spawn to that pin                                                                                                                                                                                                                                                  | ~20    | 4.1      | unit                                                                                                                                                       | —                                                   |
| 4.6 | ✓                                                                                                                                         | tests + `docs/concepts/chains-lifecycle.md`                                                                                                                           | detached flow end to end; docs section; the HTTP arm of the live drive (a fake worker over Streamable HTTP)                                                                                                                                                                                                                                                                                                    | ~230   | 4.4, 4.5 | full suite + HTTP drive of one detached chain                                                                                                              | —                                                   |
| 4.7 | ☐ (as of 2026-09-22 · flips when the owner's live drive of one detached chain in Claude Code lands, after merge — R5)                     | — (owner's drive)                                                                                                                                                     | the Claude Code arm of 4.6's live drive, split out: one `await: run` chain driven in Claude Code with a real background worker                                                                                                                                                                                                                                                                                 | —      | 4.6      | owner's live drive                                                                                                                                         | the server cannot spawn a worker; only a client can |
| 4.8 | ☐ (as of 2026-09-22 · flips when the planner rules the four design questions in notes deviation 34 — R8 returned under its own stop rule) | `modules/prompts/chain-step-resolution.ts` (the sibling of #268's check), `registry.ts`, `scripts/validate-prompts.ts`, `resource-manager/prompt/utils/validation.ts` | R8 (supersedes R7): a detached step opens no gate review at spawn; when gates apply, its review opens against the late report, the verdict is accepted through the existing `gate_verdict` path routed to the node, and the close guard holds completion while it is outstanding. NOT built: the single `pendingGateReview` slot is occupied by the current step's review when the report lands (deviation 34) | ~120   | 4.6      | unit twins + one `resource_manager` write + `validate:prompts` fixture                                                                                     | OQ5 ruled (R7)                                      |
| 4.9 | ✓                                                                                                                                         | stage 16 `16-response-capture-stage.ts` + `tests/integration/chain/delegation-detached.integration.test.ts`                                                           | a chain whose first step is delegated (`await: run` or `delegated: true`) starts: the run-opening call is a brief, not a resume, so resume admission is skipped on it (`b461d680`, changelog restored `b025dc9e`)                                                                                                                                                                                              | ~10    | 4.6      | integration twins (detached-first, blocking-first; delegated-second and non-delegated-first are the existing cases) + a shipped-defaults drive of all four | the opening call has no resume to admit             |

**Tier 4 receipt (✓ 2026-09-22, `feat/delegation-detached-mode`, commits `aa4c691f`..`e8acd265`)** — rows 4.1–4.5 and
the HTTP arm of 4.6 flipped on their Verify:
4.1 — `await` rides `workflowNodeSchema` (so `ChainStepSchema`, the IR, `resource_manager`'s `chain_steps`/`chain_step_data`
and `remainder` nodes), carried by the loader, the stage-04 projection and `compileNode`; a remainder node refuses it by
name. `validate:tool-schemas`: 8 additive enum lines on four node shapes, snapshot regenerated. Mutations: compiler drop,
stage-06 read, schema enum, loader drop, stage-04 drop — each 1 red.
4.2 — v31 paragraph beside v29/v30; `spawned_at` round-trips through `StepMetadata.spawnedAt` and survives a cold load
(`chain-run-storage` case, raw column + NULL control); `ddl-copy-parity.test.ts` now covers `chain_run_nodes`. Mutation:
insert binds NULL → 1 red.
4.3/4.4 — `unreportedDetachedNodeIds` is the one derivation of "owed", read by the guard, `isRunComplete`/`isRunHeldOpen`,
the held-run render and the router. Mutations (tests across `detached.test.ts`, `delegation-renderer.test.ts`,
`delegation-detached.integration.test.ts`, `chain-run-storage`): guard off 3 red · no spawn mark 4 · no routing 3 ·
already-reported accepted 2 · unknown token passes 2 · no continue-past 3 · no held completion 1 · placeholder counted as a
report 5 · held counted complete 2 · review-pending ignored 1.
4.5 — Claude detached pin 2 red; hook mode ignored 3 red, mode not recorded 1 red.
4.6 — HTTP drive (`~/.cache/rsc-tail/t4-drive.mjs`, built hermetic server, Streamable HTTP, every stream message read):
23/23 PASS — `resource_manager` authors `await: run` into YAML; the brief renders with `run_in_background: true` and
"Do NOT wait"; a no-`user_response` resume moves past it; a wrong token is refused by name with no `step_complete` on the
stream; the late result lands on its node (`step_complete` fires — positive control); a second report is refused as
already reported; a detached LAST step holds the run open (no completion notification, no completion wording), refuses a
non-reporting resume by node, and closes on the report (`chain/complete` fires — positive control); the blocking control
pins `run_in_background: false` and still refuses an empty resume. Deviations 25–32 in the notes.
...
- `delegation/detached.ts` (Tier 4) — the router a late result goes through and the words a held run says;
  `handoff-contract.ts` owns the token and the trailer parser, which the router imports rather than restates.
- `tests/integration/chain/delegation-detached.integration.test.ts` — the detached flow through the real
  stages; the evidence suite owns blocking resumes and stays unchanged as the control.
...
| OQ4 | ✗ KILLED 2026-09-22 (no consumer asked; `execution.delegation.evidence` is the one dial · revives if a workflow author asks for a per-run evidence mode) | 4.1      | no per-run override of evidence mode via workflow `budget`                                                                                                                                                                                                                                                                                                                                  | `budget.delegationEvidence`                                                                                |
| OQ5 | RULED 2026-09-22 (owner) — refuse at load, R7                                                                                                            | 4.7      | keep the combination unsupported by docs only: a detached step whose step gate blocks renders a review the parent must answer before the worker has produced anything                                                                                                                                                                                                                       | refuse `await: run` on a step carrying a blocking gate when the prompt loads, naming the step and the gate |
