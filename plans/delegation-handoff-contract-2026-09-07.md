...
| 4.8 | ☐ (as of 2026-09-22 · flips when the planner re-rules R7's predicate against notes deviation 33)                      | `modules/prompts/chain-step-resolution.ts` (the sibling of #268's check), `registry.ts`, `scripts/validate-prompts.ts`, `resource-manager/prompt/utils/validation.ts` | R7: `await: run` on a step carrying a blocking step gate is refused at load, naming the step and the gate — NOT implemented: measured, a gate's declared mode has no runtime reader and every detached step holds a review under shipped defaults, gate or not (deviation 33) | ~120   | 4.6      | unit twins + one `resource_manager` write + `validate:prompts` fixture | OQ5 ruled (R7)                                      |
