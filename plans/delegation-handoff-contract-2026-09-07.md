...
| #   | St                                                                                     | File                                                                                             | Change                                                                                                                             | ~Lines | Depends  | Verify                                                       | Justification                                     |
| --- | -------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------- | ------ | -------- | ------------------------------------------------------------ | ------------------------------------------------- |
| 4.1 | ☐ (as of 2026-09-08 · flips when the row's Verify passes on `feat/delegation-handoff`) | `modules/workflow-ir/node-schema.ts:196`, `operators/types.ts:39`, `workflow-ir/compiler.ts:158` | `await?: 'node' \| 'run'` declared, carried, passed through                                                                        | ~20    | —        | IR schema snapshot regenerated; `typecheck:tests:ratchet`    | YAML/IR field first (non-goal: symbolic syntax)   |
| 4.2 | ☐ (as of 2026-09-08 · flips when the row's Verify passes on `feat/delegation-handoff`) | `infra/database/sqlite-engine.ts:236,872` + `table-contracts.ts`                                 | `SCHEMA_VERSION` 28 → 29; `chain_run_nodes` += `await_mode TEXT`, `reported_at INTEGER`                                            | ~30    | 4.1      | schema tests; ephemeral tables drop/recreate                 | detached needs a node lifecycle the run can query |
| 4.3 | ☐ (as of 2026-09-08 · flips when the row's Verify passes on `feat/delegation-handoff`) | `modules/chains/manager.ts:914-922` + run registry                                               | detached node marked spawned at render, reported on late result; terminal transition refused while any detached node is unreported | ~80    | 4.2      | integration: close refused until the late result lands       | the run-level obligation (D4)                     |
| 4.4 | ☐ (as of 2026-09-08 · flips when the row's Verify passes on `feat/delegation-handoff`) | stage 16 `runHandoffEvidencePhase`                                                               | a trailer naming a non-current `await: run` node routes the result to that node instead of the current one                         | ~40    | 4.3      | integration with fake worker                                 | reuses the token as the router; no new parameter  |
| 4.5 | ☐ (as of 2026-09-08 · flips when the row's Verify passes on `feat/delegation-handoff`) | `delegation/brief.ts`, `strategy.ts`                                                             | detached brief: "report later with this node token"; Claude `run_in_background: true`                                              | ~20    | 4.1      | unit                                                         | —                                                 |
| 4.6 | ☐ (as of 2026-09-08 · flips when the row's Verify passes on `feat/delegation-handoff`) | tests + `docs/concepts/chains-lifecycle.md`                                                      | detached flow end to end; docs section                                                                                             | ~230   | 4.4, 4.5 | full suite + live drive of one detached chain in Claude Code | —                                                 |
