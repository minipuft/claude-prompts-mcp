...
| P1.5  | ☐ (as of 2026-08-29 · flips when a client restart serves ≥119 prompts from a root outside every checkout, with `↳ over bundled base` beneath it)                                                                    | Move the personal store outside every checkout. **Staged 2026-08-29**: 84 prompts copied to `~/.claude/resources/prompts`, byte-verified. **Two blockers before the `.mcp.json` flip, both measured.** (1) `.mcp.json` launches `${CLAUDE_PLUGIN_ROOT}/server/dist/index.js` — MAIN's build, which predates P1.0a. Flipping now serves **81** prompts from the personal store alone and silently drops the 39 bundled: the exact defect P1.0a fixes, still live on main. The fix must reach main and be rebuilt FIRST. (2) `.mcp.json` is tracked and shipped, so a hardcoded `/home/minipuft/...` would reach every user — the flip needs a user-scoped override or `${VAR:-default}` expansion, not a literal                            | P1.0    | a restart serves ≥119 from the external root                                                                                              |
...
| P1.5e | ☐ (as of 2026-08-29 · flips when the plugin server's own log names the personal root, after the next client restart; the catalog half is already verified)           | Restart both consumers and verify. **Catalog: verified live** — 120 prompts over the bundled base, HTTP-confirmed. **Claude Code: mechanism proven, effect pending** — a session cannot restart itself, so this row cannot close from inside the run that set it up                                                        | P1.5c P1.5d | catalog done; plugin server pending restart                           |
...
**Gate P1.5-X — PARTIAL (as of 2026-08-29), blocked on P1.5e's Claude Code half.** The catalog
consumer is verified live, and no config or source file in the repo names a personal path. The
plugin consumer's mechanism is proven — `settings.json` `env` demonstrably reaches MCP subprocesses —
but the flip takes effect on the next client restart, which cannot happen from inside the session it
would restart. Do not read the five ✓ rows as this gate passing.
