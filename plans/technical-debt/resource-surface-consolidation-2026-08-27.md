...
| P1.5  | ☐ (as of 2026-08-28 · flips when a fresh worktree launched with `MCP_WORKSPACE` at the personal store serves the same prompt count as main)              | Move the personal store outside every checkout, so the served catalog stops depending on which worktree launched the session. P1.0 ruled the mechanism: `MCP_WORKSPACE` at the store, `MCP_RUNTIME_ROOT` pinned so `state.db` does not follow it. **Owner action** — it relocates the operator's own library                                                                                                                                                                               | P1.0    | startup inventory from a fresh worktree matches main's count                                                                              |
| P1.6  | ☐ (as of 2026-08-28 · flips when the **main checkout** reports 123 from `git ls-files server/resources/prompts \| grep -c prompt.yaml`)                  | ⚠ **premise corrected.** Migrate the personal prompts; delete `server/resources/prompts/.gitignore`. **Re-take the backup first** — the 2026-08-19 one captured 121, disk is now 123. Authored falsifier compared tracked to on-disk, which is **vacuously equal (39 == 39) inside any worktree**, since a worktree checks out only the tracked ones — it reported success for work nobody had done. Re-anchored to the absolute count in main                                             | P1.5    | in main: tracked `prompt.yaml` count is 123                                                                                               |
