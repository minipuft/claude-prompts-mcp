...
- **A script tool whose extension maps to no runtime is refused instead of run as a shell script.** Auto-detection covers `.py`, `.js`, `.mjs`, `.cjs`, `.ts`, `.sh`, `.bash`; anything else — including a file with no extension — now fails with a message naming the extension and the fix. Declaring `runtime:` explicitly still reaches every runtime including `shell`, so the capability is unchanged and only the silent default is gone. The `shell` runtime was never the defect: it builds an argv array (`bash script.sh`), the same posture as `python script.py`. Priced as breaking on the same reasoning as the gate-criterion refusal above — script tool YAML is resource-format surface — though no tool in this repository relied on it.
- **A script tool's `env` may no longer set keys that decide what its interpreter resolves to**, and its `workingDir` must stay inside the tool's own directory. The interpreter is looked up on the PATH the tool supplies, so an author-set `PATH` chose which `python3` ran; `workingDir` reached `path.join`, which resolves `..` silently. The refused keys are `PATH`, `LD_*`, `DYLD_*`, `NODE_OPTIONS`, `PYTHONPATH`, `PYTHONHOME`, `PYTHONSTARTUP`, `BASH_ENV`, `ENV`, `IFS`, `SHELLOPTS`, `BASHOPTS`, `PERL5LIB`, `PERL5OPT`, `RUBYLIB`, `RUBYOPT`; ordinary variables are untouched. The same rule bounds a gate's `shell_env`, alongside a new `MCP_SHELL_VERIFY_ALLOWED_DIRS` for `shell_working_dir`.

- **A gate's `shell_command` is now an argv array; a bare string fails to load.** `shell_command: ["npm", "test"]` replaces `shell_command: "npm test"`, and the shipped `test-suite` gate is migrated. A string was joined into `sh -c '<string>'`, so the shell parsed whatever the gate author wrote — which made `MCP_SHELL_VERIFY_ALLOWLIST` a check on text rather than on a command, defensible only by enumerating shell metacharacters. With argv, a prefix entry such as `npm *` is structurally sound and the metacharacter rule no longer applies to that shape. This does **not** put a shell out of reach: `["sh", "-c", "..."]` remains expressible and remains bounded by the allowlist, where the operator can see it. The inline `:: verify:"npm test"` operator is unchanged and still takes a string — it is typed into an invocation rather than carried in a file, so it keeps the metacharacter rule.

- **`category_ship_status` is removed from the prompt mutation receipt.** It reported whether the target category was allow-listed by `server/resources/prompts/.gitignore` — a file that no longer exists, now that the bundled prompt tree is fully tracked. With nothing to parse, the field answered `ships: true` unconditionally and the warning it fed could not fire, so what shipped was a constant wearing the shape of a check. Removing it takes the ~200-line `.gitignore` parser with it. The receipt still names where the write landed: `resource_root` resolves through the same call `FileOperations` writes through, and a test now binds every written path to it rather than leaving the agreement to inspection.
...
- **A blocking unknown now interrupts the run instead of quietly bending it.** A `blocking:true` observation already inserted an investigation step; it now also returns a structured `chain_interrupt` — the unknown, the steps it affects, what remains of the plan, and the exits that run accepts — in both the response text and `structuredContent`. The affected steps are DECLARED links only (`target_step_id`), never a text scan: a heuristic there would be a server-authored claim about your plan, and every other part of this policy reacts only to what was declared. A run may also declare `budget.pauseOnBlocking` (Workflow IR budget, or a YAML chain's chain-level `budget:`) and HOLD instead, issuing no step until `gate_action:"resume"` or `gate_action:"accept_alternative"` clears it — `gate_action` gains those two members, and no `gate_verdict` clears this hold because no gate produced it. The paused verb list is a different list, not a longer one: it never offers "answer the step", because a paused run issued none. The scope is blocking-only; whether a non-blocking unknown should ever interrupt was considered and killed for want of evidence.
- **`prompt_engine(remainder:…)` rewrites the rest of a running chain.** When a discovery invalidates the shape of a plan rather than one step of it, the caller authors the alternative as Workflow IR nodes and the server validates it against the same schema, validator and caps a `workflow` submission meets — `mode:"replace"` discards everything strictly after the current node, `mode:"append"` extends it, and the current node is never touched. `maxNodes` counts executed nodes PLUS the submission, so rewriting the tail repeatedly cannot buy back spent budget; one accepted remainder per unknown id, plus a per-run ceiling. Every refusal is named. The server still authors no step content: the model declares, the server validates. A `chain_id` call whose `command` begins with `-->` is the string spelling of an append and takes the same path — the only command form that may accompany `chain_id`, with `==>` inside the fragment delegating the step that follows it. A contributed node carries `{id, promptId, stepName, args, delegated}` and refuses every other node field by name, because it has no parse-time step for the rest to live on and an accepted-then-dropped field reads exactly like a supported one. Runs record what happened: `chain_run_nodes.origin='remainder'` with the motivating unknown id, and `interrupts_raised` / `remainders_accepted` on the run's terminal `execution_records` row. Schema v26 adds the two counters, v27 adds `chain_run_nodes.delegated` and `args_json`.
- **`delegated` and `inlineGateCriteria` are declarable step fields.** A chain step can now ask for context isolation without naming a model tier it does not care about (`delegated: true`, the `==>` operator's own field), and can carry raw `::` gate tokens for per-step resolution where YAML could previously express only pre-resolved `inlineGateIds`. Both reach YAML `chainSteps`, the Workflow IR, and `-->` chains through the one derived schema.
- **One step vocabulary across YAML chains, the Workflow IR, and `-->` chains.** A `chainSteps[]` entry and an IR `nodes[]` entry are now validated by a single Zod schema (`workflowNodeSchema`), with `ChainStepSchema` derived from it, so the two can no longer drift — they previously agreed field-for-field by hand, with nothing failing when one side gained a field. Two identity differences remain and are deliberate: `id` is required on a node and minted from `stepName` for a YAML step, and `stepName` is required in YAML and defaults to `promptId` on a node. A parity test fails on anything else. Three capabilities follow from the unification: a YAML chain step may declare `args` (merged over the run's invocation arguments for that step only), and a YAML chain may declare chain-level `edges` (dependency constraints, linearized into step order at load, with a missing endpoint or a cycle failing the load by name) and `budget` (the IR's shape, so `maxInsertions` and `declaredCostCeiling` now reach template chains rather than being submission-only). Step-level `delegation` is no longer a schema field: it has no runtime reader — only the skills-sync exporter reads it off raw YAML — so it is stripped before validation instead, and YAML carrying it keeps loading.
- **`GET /api/v1/catalog/prompts` lists the served prompt catalog** for authenticated catalog adapters, returning metadata-only summaries under a `prompts` key. Only the per-prompt item route `/api/v1/catalog/prompts/:promptId` existed, so an adapter could fetch any prompt it could already name and could not discover one — the agent-workbench prompt authority turns a non-200 listing into `catalog_unavailable`, projects an empty catalog, and the downstream client shows no prompts at all. Measured 2026-08-30 against the running service: every `/v1/prompts/{id}` call succeeded while `/v1/catalog` returned zero entries. The existing read-token guard already covered the prefix, so the collection authenticates and fails closed exactly as the item route does.
- **Typed `prompt_engine.inputs`.** Structured prompt arguments now bypass the command-string grammar, preserving nested objects, arrays, quotes, and backslashes. Explicit inline arguments win over `inputs`, which win over legacy `options`, which win over prompt defaults.
- **Prompt creation preview, receipts, and optimistic concurrency.** `resource_manager(resource_type:"prompt", action:"validate")` runs canonical creation validation without writing or versioning. Successful create/update calls return addressable roots, affected files, refresh/load verification, and current version; `expected_version` refuses stale prompt updates before versioning or writing.
- **Reference-grounded creative workflows.** `>>design_muse` turns supplied visual evidence into an approval-gated design direction, while `>>practice_capture` preserves references, techniques, reactions, and unknowns through the existing Practice Library writer. Practice capture defaults to evidence retention only; framework promotion is evaluated separately and never applied without confirmation.
- **Semantic module governance.** Colocated `module.yaml` descriptors now name source-boundary responsibilities and lifecycle, a guarded scaffold creates new boundaries, and Dependency Cruiser data generates a deterministic module catalog. Validation checks descriptor coverage, generated-catalog drift, and architecture output through structured JSON instead of parsing CLI prose.
- **Cross-client chain handoff.** `prompt_engine(chain_id, handoff:true)` mints a single-use token; `prompt_engine(claim_token:"…")` from another client (Codex, OpenCode, a different Claude Code conversation) transfers the run's ownership and resumes it in the same call. The token is the only key — run numbers are a per-server sequence and collide across servers sharing one `state.db`, so a claim by chain name could select the wrong run. Claim rewrites the row's owner and burns the token in one statement; the donor's copy is retired on its next persist. A claim never rewrites workspace scope and refuses a run with no persisted blueprint, naming the reason. This is the deliberate path that replaces the automatic cross-client recovery removed by the session-scoping fix. Schema v25 adds `chain_runs.handoff_token` (ephemeral tables; live runs are dropped on first start).
...
- **Delegated steps run on the host's general agent; the plugin no longer ships `agents/chain-executor.md`.** A delegated step's EXECUTION BRIEF is self-contained (template, gates, history, Result Contract), so the shipped agent only restated it — and contradicted it on the verdict shape, mandating the retiring `GATE_REVIEW:` string where the brief asks for the `Proposed Gate Review:` block the server detects. With no `agentType` declared, a Claude Code handoff now names `general-purpose`; Codex, Gemini, OpenCode and Cursor handoffs carry no `agent_type` parameter, so the client's default agent applies. Agent names pass through exactly as written: the render no longer prefixes bare names with `claude-prompts:`, which had made every host-catalog agent (`Explore`, `~/.claude/agents/*`) unreachable from a brief. The brief's Result Contract now states the worker boundary — one step, no chain tools, no chain metadata in the reply — which the deleted agent's tool list used to enforce for Claude Code alone. **Migration**: a chain that wrote `agentType: chain-executor` explicitly should delete the key or name a real host agent; exported skills for delegating chains re-render their delegation column, so `skills_sync diff` reports them once.
- **`>>create_prompt` delegates validation and persistence to `resource_manager`.** Its script is now a thin field-mapping adapter, removes author-controlled file-path inputs, accepts complete inline script-tool definitions, previews canonically, and requires confirmation before creation. Existing prompt maintenance uses inspect → dry-run update → approval → version-checked update → reload → render smoke test.
...
- **Prompt-id normalization existed twice and the two copies disagreed.** `normalizePromptId` and an inline chain in the command parser differed by one step, and they diverged on nested chain-step ids — `deep_analysis/initial_scan` collapsed to `deep_analysisinitial_scan` — while the draft service compares that output against every loaded prompt id, nested ones included. One owner now lives in `shared/utils/resource-ids.ts`, which also states the id convention: ids that appear in the `>>` / `-->` command grammar are `snake_case`, every other id is `kebab-case`. The parser's extra step was proven unreachable before removal — the capture group feeding it already excluded every character it stripped. `npm run validate:prompts` enforces the convention and takes `--root`, so it also checks a prompt library outside the repository.
- **A create with a spaced category always reported failure for a write that was correct.** `My Category` is written to `my-category/` on disk, and the loader then derives `category` back from that directory — so post-write verification compared the reloaded slug against the value the caller authored and answered `❌ Post-write verification failed (mismatched: category)` every time. The write was never wrong; the comparison was against the wrong side. Both sides are now normalized through one `slugifyCategoryDirectory`, replacing two inline copies of the same expression.
- **A failed prompt mutation opened by announcing success.** The response led with `✅ **Prompt Created**` and carried `❌ **Post-write verification failed**` fourteen lines down, under the headline and the receipt, while `isError` was `true` the whole time — so any client, log excerpt or truncated transcript that reads the opening line reported a success. The headline is now chosen after verification runs, and a failed one leads with the failure and states the reason on the next line. It still says the prompt was written, because it was; what failed is the verification that the registry serves it.
- **The claims-conformance suite wrote into the packaged resource tree while passing.** Its isolated server set `MCP_WORKSPACE` to a temp copy, but the spawn helper defaulted `MCP_RESOURCES_PATH` to the package tree — and that outranks `MCP_WORKSPACE` in path resolution, so every mutation landed in the shipped tree and left seven prompt directories behind on a fully green run. A caller that redirects the workspace now suppresses the default, and a jest teardown fails any run that adds a file to the packaged tree, so a future leak reds an otherwise-passing suite.
- **Prompts that fail schema validation were dropped at load with only a log line.** Frameworks have been validated against the loader's schema since they shipped; prompts had no equivalent, so an invalid prompt and a malformed inline gate both left the server starting successfully with a quietly smaller catalog. `npm run validate:prompts` now runs the loader's own schema and gate normalizer over a prompt tree and fails on either, and takes `--root` so it can check a personal library outside the repository. It found two gates in the bundled `practice_capture` prompt that had never loaded; both are repaired.
- **Editing a resource that ships with the server destroyed the parts of it that are not fields.** A prompt or framework served from the bundled tree has no directory under your own resources root, so an update landed on an empty directory and rebuilt the resource from its in-memory model — which holds its fields and nothing about its subtree. Measured 2026-08-30: editing only the `description` of `planning/implementation_plan` replaced all five of its chain steps with 42–55 byte scaffold stubs (`discovery/user-message.md`, 3852 bytes → 50), and the served step then returned the stub; the same edit to `examples/create_framework` dropped all four files under its `tools/` directory. Both reported `✅ Prompt Updated`. A bundled framework was worse off still: its update refused outright with `Failed to load framework files … may be corrupted`, which was false — it loads and serves correctly, it simply was not where the writer looked. An update now copies the whole source subtree into your root before editing, so the preservation logic that was always correct is handed the prior state instead of an empty directory, and the receipt says the resource has been forked and will no longer track bundled updates. A loaded prompt records the root it came from, and `inspect` reports it alongside whether editing would copy.
- **Deleting a bundled resource refused for a reason that was not true.** `delete` searches only the writable root, so a prompt, gate or framework that ships with the server was reported as `Prompt not found` / `Gate directory not found: <path>` — for resources the same server had just inspected and served, naming paths that were never meant to exist. Each now states where the resource actually lives, where your own root is, and that updating it is the way to change it. This covers `focus`, `liquescent`, `radiant` and `verify`, which ship but are missing from the hardcoded built-in framework list, so they previously fell through to a missing-directory message. Deleting your own copy of something that also ships with the server still succeeds, and now says the bundled version is being served again rather than leaving it to look like a failed deletion. Framework `delete` also resolved the wrong root entirely — it hardcoded the package tree instead of asking for the frameworks directory, so deleting a framework you had just created in a personal library reported it missing.
- **A caller-supplied path segment could steer a resource write outside the resources root.** A prompt's `category`, and a gate's or framework's `id`, went from the tool payload into `path.join` unchecked, so a value containing `../..` walked out of the root and the write landed there with caller-controlled content — reported as success. Measured 2026-08-30 against a built server, each with a passing benign control beside it: `category: '../../ESCAPED'` wrote `prompt.yaml` and `user-message.md` outside the root under a `✅ Prompt Created` headline; `id: '../../ESCAPED_GATE'` and `id: '../../ESCAPED_FW'` did the same for their types. The gate `delete` path shared the join, aiming a recursive removal. Only the prompt `id` was guarded, by a validator reached from the draft service and never from `category`; `validateCategoryName` existed with **zero call sites**, which is how a surface with a validator named for the job stayed unvalidated. Every join whose segments come from a payload now resolves through `resolveContainedPath`, `validateCategoryName` is wired and requires a single directory name, and framework `phasesFile` / `judgePromptFile` references are contained too — those are read back to the client, so an uncontained join disclosed a file rather than writing one. Fixing the three resource types was not fixing the class: enumerating every file that resolves a resources root and writes found a fourth site the type-by-type pass missed entirely, the HTTP `create_category` handler, which joined a request-body field straight into `mkdir`. `validate:contained-resource-writes` now performs that enumeration on every run, so a new resource type cannot ship unguarded and green.
- **The e2e suite now decides its own environment instead of inheriting the developer's.** Five places spawned a server with `...process.env`, each scrubbing a different subset — one removed jest's two boot-blocking markers, one also removed `NODE_OPTIONS`, one blanked `MCP_RUNTIME_ROOT` and no other path variable. With `MCP_RESOURCES_PATH` exported, which is a supported way to point the server at a personal prompt library, a child read that library instead of the test's fixture: `bundled-resource-fallback.e2e.test.ts` booted against 121 personal prompts while asserting about a workspace holding one, and its `> 1` assertion passed either way. All five route through one helper, and `validate:hermetic-child-env` fails a sixth.
- **The SQLite resource index described a different catalog than the server served, and nothing compared the two.** `resource_index` is what every Python hook reads, so the gap was not internal: a live server served 119 prompts while the index held 78, and `>>strategicImplement` — bundled, loaded, executable — answered `Unknown prompt` at the prompt router. Gates, frameworks and styles indexed as zero, so `get_valid_styles_from_db` and `get_valid_frameworks_from_db` returned empty lists. Three independent causes, all now fixed: the indexer walked a single resource root (it never learned about the bundled tree the loaders overlay), it stopped after two directory levels (missing 20 chain-step prompts nested a third level down), and it keyed rows on the YAML `id:` field where the loader path-qualifies a nested id — so hooks handed out ids `prompt_engine` rejects. Startup now compares the index against the served catalog and names any disagreement in both directions, which is the check whose absence let these diverge silently.
- **A prompt id defined in two categories is reported by path instead of silently losing one.** The index is keyed `(id, type)` with no category, so the second definition overwrote the first and the loser became unreachable through every id lookup. The warning names both files and distinguishes the two causes: a matching category is one resource being overridden by precedence, which is the overlay contract; the id alone is two resources, which is a defect no precedence rule resolves.
- **The startup inventory line names its subtractions.** `prompts: 120 (16 categories, 3 invalid)` reconciles against the 123 files on disk; previously a count of successes alone made a catalog missing four prompts indistinguishable from a complete one. Reasons with a zero count are omitted.
- **`mergePromptResults` keyed `promptsData` on the bare prompt id while keying `convertedPrompts` on `category/id`**, so the two arrays disagreed about the catalog — 119 against 120 — and a consumer's answer depended on which one it happened to read. Both now use `category/id`.
- **A workspace resource directory now overlays the bundled tree instead of replacing it.** Path resolution returned the first existing candidate and stopped, so once a workspace had `resources/<type>/` the shipped definitions were never read. The consequence differed per type and only one of the three was loud: a workspace holding a single framework made the server exit at startup with `FATAL: Framework 'cageerf' not found`; a workspace holding a single prompt served that one prompt and silently dropped the other 39; an empty workspace `styles/` served zero styles. The bundled directory is now the lowest-precedence contributing root for prompts, gates, frameworks and styles, so a same-id workspace entry still wins — the behavior `--help` has always described. Gate reads additionally resolved through a package-only search that consulted neither `MCP_RESOURCES_PATH` nor the workspace, while gate writes have honored both; both sides now resolve the same root, which also means a workspace gate overriding a bundled id takes effect rather than being shadowed by it.
- **The startup inventory names the bundled base it merged.** A count drawn from two roots was previously reported against one, and for gates the reported root was one the gates had not been read from — `gates: 25 — <workspace>/resources/gates` for a directory holding a single gate.

- **Skills Sync refuses to write through a symlinked skill directory.** `export` and `sync` now check that each resource's output directory resolves inside the client's base directory; one that resolves elsewhere (e.g. `~/.codex/skills/dev-workflow -> ~/.claude/skills/dev-workflow`) is skipped with a recorded failure naming the real target. Previously only the client base directories were compared, so a per-skill link passed the collision guard and the second client's pass overwrote the first client's render — `diff` then reported the first export as locally modified against a file the exporter itself had clobbered (measured 2026-08-27).
- **`resource_manager` no longer drops handler `structuredContent` at MCP registration.** Prompt validation drafts, inspect metadata, write receipts, and version conflicts now reach clients as machine-readable data instead of text only.
- **Reloaded argument key order no longer creates phantom version bridges.** Version comparison now matches JSON-persisted structure while ignoring object key order, so a loader round trip remains one version row per edit.

- **Plugin hooks now resolve Python 3 across supported platforms.** Claude Code hook commands delegate to a shipped Node runner that prefers `py -3` on Windows and `python3` on macOS/Linux, with deterministic fallbacks and unchanged hook exit codes. A stock python.org Windows install no longer depends on the `python3.exe` WindowsApps alias. ([#224](https://github.com/minipuft/claude-prompts-mcp/issues/224))
- **One client's chain can no longer surface in another client's conversation.** Several MCP server processes — launched by different clients (Claude Code, Codex, OpenCode) — can share one `state.db`, and the hooks' chain reader selected the newest active chain owned by ANY live server PID. A compact recovery, prompt-submit reminder, or Stop-event block could therefore act on a chain a different client was running, complete with a missing-blueprint error when the local server could not resume it. Recovery is now keyed by the chain the conversation itself recorded (PostToolUse tracking in `hooks-state.db`); `state.db` refreshes that specific chain, a chain whose owning server exited expires instead of resurrecting from the stale snapshot, and the unscoped scan is gone — `load_active_chain_state` without a chain id returns nothing. Hooks also resolve `state.db` the way the server resolves its write path (`{MCP_RUNTIME_ROOT || MCP_WORKSPACE}/runtime-state/`, legacy `{workspace}/server/runtime-state/` as fallback); the old hook-side path could read a database the session's own server never writes, which made the injected chain always someone else's.
- **Hook session rows now clear when their run resolves.** The PostToolUse tracker wrote a row on every parsed response but nothing ever deleted one: a gate PASS returns "Execution complete." with no chain markers, the parser yields nothing, and the stale pending-gate row survived until the 24-hour sweep — where compact recovery would re-inject a long-resolved gate. Rows now clear on a verdict or cancel submission whose response carries no further chain state, and on an explicit completion marker with nothing pending. "Step N of N" alone does not clear — the final step in flight and "Chain complete (N/N)" parse to the same numbers, so the marker text is the discriminator.
