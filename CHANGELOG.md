...
- **Prompt creation preview, receipts, and optimistic concurrency.** `resource_manager(resource_type:"prompt", action:"validate")` runs canonical creation validation without writing or versioning. Successful create/update calls return addressable roots, affected files, category ship status, refresh/load verification, and current version; `expected_version` refuses stale prompt updates before versioning or writing.
