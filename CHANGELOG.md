...
- **`resource_manager` refuses a parameter the resource type does not read, instead of ignoring it and reporting success.** `update` with `resource_type:"framework"` (or `"gate"`, or `"category"`) and `unset:["description"]` answered "updated successfully", saved a new version, and left the file byte-identical — `unset` is a prompt parameter and nothing else reads it. Removing a field from a gate, framework or category is still not implemented; the call now says so by name and names the type that does implement it, before any write or version snapshot. The same holds for every other type-specific parameter the tool publishes — around sixty of them were accepted and dropped for the three types that do not read them. A call that sent one of these parameters used to succeed while ignoring it; it now fails, naming the parameter and the resource types that read it. Scripts must drop the parameter for that resource type.
...
- **`cpm rollback` no longer writes a key the entry file does not use.** A version snapshot records the spelling the server's own writer accepts, and the writer translates it on the way out; the CLI merges the snapshot straight into the YAML, so wherever the two spellings differ it added a second key beside the real one. Rolling back a framework wrote `tool_descriptions:` next to `toolDescriptions:` and reported `toolDescriptions` as "not restored" while writing it under a name nothing reads; rolling back a prompt wrote the `systemMessage` and `userMessageTemplate` BODIES into `prompt.yaml`, which carries only the `systemMessageFile` / `userMessageTemplateFile` pointers the loader reads. The framework key is now renamed on the way in and genuinely restored; the two prompt bodies are reported as not restored, like a gate's `guidance` already was. A new end-to-end check runs both rollback paths over the same version and fails on any future divergence.
