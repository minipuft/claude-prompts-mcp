...
## [5.0.0](https://github.com/minipuft/claude-prompts-mcp/compare/v4.0.1...v5.0.0) (2026-09-15)
...
- **Two framework fields were written to disk and never read back.** `judge_prompt` was loaded from its file and dropped before the framework was reassembled, and `execution_type_enhancements` was written to `phases.yaml` and read by nothing — so an update that carried either could not preserve it, and neither could ever be reported. Both now survive the round trip. They were declared authorable and covered by tests asserting the written file, which is what hid this: checking that a key reaches the file cannot show that nothing will ever read it back.
