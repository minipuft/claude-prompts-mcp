...
- **Unified output-shape block in chain step prompts**: Chain step prompts now emit a single post-task block that combines (a) the methodology contract — required section headers + length/forbidden-term hints — when a framework is active (CAGEERF, ReACT, 5W1H, SCAMPER), with (b) the existing delivery markers (Summary, Gate Coverage, GATE_REVIEW). The contract reads from the same `phases.yaml` `processingSteps[].section_header` + `guards` SSOT that already drives post-emission phase-guard validation at stage 09b — so the model sees the same contract it is later judged against. Gate-review prompts surface the contract too (with delivery markers suppressed since gate reviews use the `GATE_REVIEW: PASS|FAIL` CTA convention). Consolidates what would otherwise be two parallel output-shape mechanisms (`buildOutputContractBlock` + `buildResponseFormatSection`) into one renderer.
