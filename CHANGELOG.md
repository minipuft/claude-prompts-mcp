...
- **Updating a prompt stored as a single file (`{category}/{id}.yaml`) no longer leaves it defined twice.** The writer only ever produces the directory layout (`{id}/prompt.yaml` plus message files), and its lookup for a prompt already on disk skipped single-file matches, so an update wrote a fresh `{id}/` directory beside the untouched file — one definition updated, one going stale, both on disk. An update of a single-file prompt now converts it to directory layout in the same write: the flat file is deleted, the directory is written, and the preview names the deleted file alongside the files it creates.
