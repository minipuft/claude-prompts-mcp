...
- **A structural review now quotes the step it actually graded.** A phase-guard review grades the output of the step just captured, but it is created after the run has advanced to the next step, and it recorded no reviewed-step identity — so the review showed the NEXT step's task under `## Original Task Instructions` above the previous step's missing sections, and a client following it revised the wrong step. The review now carries the graded step's node id and ordinal and renders that step. When the review is about step N while the run stands on step N+1, the response also keeps the render of N+1 instead of discarding it, so a delegated N+1 still arrives with the execution brief and handoff token its resume requires.
