...
- **A config path set with `MCP_CONFIG_PATH` or `--config` that cannot be used now stops the server instead of starting it on defaults.** A missing file, a directory, an unreadable file, or malformed JSON used to produce a server that started, served, and ignored every setting in the file you named. It now exits non-zero before serving anything, on STDIO and Streamable HTTP, and prints to stderr which variable or flag was set, the path it resolved to, and what is wrong with it. Fix the path, or unset the variable (or drop the flag) to fall back to the default `config.json`, whose path the message names.
