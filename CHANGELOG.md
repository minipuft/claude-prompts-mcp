...
- **Rendered gate guidance no longer grows an extra blank line between gates.** Keeping `guidance.md`'s trailing newline for write-back (above) meant every gate's guidance text now carried one into the guidance the model reads too, widening the gap between two consecutive gates' sections from one blank line to two. Rendering now trims for display, independently of the verbatim value the write-back path still uses.
||||||| e52e8eb1
