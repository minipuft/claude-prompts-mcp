...
- **The Claude Code plugin's saved state now survives a plugin update.** Gate and framework settings, argument history and prompt version history were saved inside the plugin's install folder, which Claude Code replaces on every update. The plugin now keeps them in the plugin data folder Claude Code preserves across updates, and its hooks read them from there. State already saved in an install folder is not moved. Prompts you create through `resource_manager` are not covered by this change: they are still written inside the install folder.
