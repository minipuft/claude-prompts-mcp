...
- **Editing a style's `guidance.md` while the server is running now reaches the next render, instead of needing a restart.** Style hot reload was built but never wired in, so `#style_id` kept serving the guidance loaded at startup; it now watches the same directories the style manager reads from, workspace and bundled alike, matching how prompts, gates and frameworks already reload live.
