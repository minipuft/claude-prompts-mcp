...
- **`resource_manager` now refuses a chain whose step names an unregistered prompt.** It previously saved the chain with a warning, and the run failed at that step one invocation later; nested step ids (`deep_analysis/initial_scan`) were skipped by the check entirely, which is the canonical form for a chain's own steps. The refusal names each bad step by position (`step 2 references unknown promptId 'run_smoke_tests'`), writes nothing and consumes no version; steps of the form `<chainId>/<step>` stay accepted, because the same call scaffolds them. Startup and hot reload now log one warning per broken chain instead of loading it silently, and `validate:prompts` fails CI on one in the bundled tree.
