...
- **The prompts the README tells you to run now ship with the server.** `tech_evaluation_chain`, `research_chain` and `review` were the first command of every documented install path but were not in the package, so a fresh install answered each with prompt-not-found. They ship now, along with the eight prompts their chains call, in `development` and a new `analysis` category: 50 bundled prompts across 9 categories.
