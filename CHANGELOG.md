...
- **A workspace holding both `config.jsonc` and `config.json` now refuses to start, and refuses `cpm config`, instead of silently preferring one.** Starting the server against a workspace set with `MCP_WORKSPACE`/`--workspace` now checks for this ambiguity before doing anything else, and refuses naming both paths; `cpm config list`/`get`/`set`/`validate`/`reset` and `cpm init` refuse the same way: `Two config files in one directory: <path> and <path>. Keep one — config.jsonc is the 5.0 name, config.json is still read.` An explicit `MCP_CONFIG_PATH`/`--config` path is unaffected — it names the file it means, whatever else sits beside it. Delete whichever file you are not using.
...
- **`system_control(action:"config", operation:"get")` returns one key's effective value and where it came from.** Send `config: { operation: "get", key: "<dot.path>" }`; the reply reports the value as JSON and a `source` — `file` (the config file sets it), `default` (the built-in default; the file does not set it), or `environment` (an environment variable overrides the file and default). An undeclared key, or a `get` with no key, is refused by name rather than answered with a listing. See [Config Operations](docs/reference/mcp-tools.md#config-operations).
- **Your config file can now be `config.jsonc`, with `//` and `/* */` comments and a trailing comma before `}`/`]`.** Nothing else beyond JSON is accepted. A workspace resolves `config.jsonc` first, then `config.json`, then the packaged default; an explicit `--config`/`MCP_CONFIG_PATH` path is read in whichever dialect its own extension names. `config.json` is still read, and still strictly — a comment inside one is a parse error. `cpm init` now writes `config.jsonc`: `$schema` and `"version": 5` are live, and every other setting is listed commented-out with its description, default and permitted values, generated from the schema. Every example line ends with a comma, so uncommenting one setting (together with the braces of the section it sits in) is enough on its own. `init` still skips, unchanged, when either file already exists.
- **The generated `config.schema.json` now carries a fetchable `$schema` address.** `cpm init`, the server's `--init`, and `cpm config reset` write `"$schema": "https://cdn.jsdelivr.net/npm/claude-prompts@5/config.schema.json"` instead of the relative `"./config.schema.json"`, so an editor with JSON Schema support validates a config file wherever it lives, not only next to an installed copy of the schema. The URL resolves once the first `5.x` release is published; a file still carrying the old relative value keeps loading exactly as before, since neither the server nor `cpm` ever reads a config's own `$schema` line.
- **A reference page now owns the configuration file.** [`docs/reference/configuration.md`](docs/reference/configuration.md) documents the file lookup order, how a single value resolves across defaults, the file, environment variables and CLI flags, what the `$schema` line validates, the CLI and MCP read/write surface, and the Python hooks' independent lookup chain — collecting what was previously scattered across roughly ten guides with no single owner.
...
- **`cpm config set`, and a persisted `gates`/`framework` toggle from `system_control`, now edit your config file in place instead of rewriting the whole document.** The edit changes only the one key's own characters, so comments, key order and every other line's formatting survive. Setting a key that exists only as a commented-out example in the generated template inserts the live key and leaves the commented example where it was; nothing tries to uncomment it.
- **`cpm config reset --force` now writes back whichever dialect your workspace already used**: the commented template to a `config.jsonc`, or the minimal `{$schema, version}` document to a `config.json`. Either way it backs up the old file first, as `<file>.backup.<timestamp>`, and never renames it.
- **Every config message now names the file it acted on.** `cpm config get` on a missing key reports `Key '<key>' not found in <file>`; `cpm config validate` reports `<file> is valid` or `<file> validation failed:`; `system_control(action:"config", operation:"validate")` reports `✅ <file> matches its schema.`; a persisted toggle reports `Persisted <key>=<value> to <file>.`; `cpm validate --config` names its result by the file's own name. Help text says `config.jsonc`.

### Fixed

- **Enabling or disabling the framework system now takes effect in the workspace that asked for it.** Over Streamable HTTP one server process serves several workspaces, and `system_control(action:"framework", operation:"enable"|"disable")` read the calling workspace's state to decide whether the toggle changed anything, then wrote the launch workspace's. The caller was answered "Framework System Enabled" while its own workspace stayed disabled, and an unrelated project's setting flipped instead. `operation:"reset_metrics"` on the `analytics` action had the same split. A toggle or reset written under a workspace is also read back after a restart now — startup loaded only the launch workspace's row and answered every other workspace with defaults, which also lost a `switch` performed by one of them.
- **A gate FAIL in advisory or informational mode now reports the step the run actually moved to.** Both modes clear the pending review and advance the step, but the handler doing that work was started and not waited for, so the response was assembled from a snapshot taken before either finished: it reported the run still holding on the step it had moved off, and a failure to clear or advance vanished with nothing logged. The blocking mode and the PASS path were already correct. Separately, a chain session blueprint whose write failed at the end of an execution was reported as persisted — the handler meant to catch that failure could never see it.
- **Hooks now read the config file in your workspace, not only the one the server ships with.** `hooks/lib/config_loader.py` looked only at `server/config.json`, so `hooks.expandedOutput` set in a workspace config never reached a hook — the setting silently did nothing. Hooks now resolve `MCP_WORKSPACE`, then `${CLAUDE_PLUGIN_DATA}`, then the packaged `server/` directory, trying `config.jsonc` then `config.json` in each and using the first file found; an unreadable or malformed file still falls back to defaults rather than making a hook refuse to run.
- **`npx claude-prompts --init` now writes the config file the README has long said it scaffolds.** The flag created the starter prompts and stopped there; `config.jsonc` only appeared if you separately ran `cpm init`. It now calls the same config generator `cpm init` does, right after the workspace is created, and leaves an existing `config.jsonc` or `config.json` alone.
- **`cpm` version history now finds `state.db` where the server actually writes it.** `resolveStateDbPath` walked up from the resource directory looking for an existing `runtime-state/` directory and ignored `MCP_RUNTIME_ROOT` entirely, so under the Claude Code plugin — which sets `MCP_RUNTIME_ROOT` to a directory `cpm` never discovers on disk — `cpm history`/`compare`/`rollback` read a different database from the one the server wrote, or found none at all. It now checks `MCP_RUNTIME_ROOT`, then `MCP_WORKSPACE`, the same precedence and resolution `PathResolver.getRuntimeRoot()` uses, before falling back to the old ancestor walk when neither is set.
- **A `:: verify:"…" loop:true` reply now says so when the loop's state could not be armed or cleared, instead of reporting as if it had been.** Both failures were caught inside `VerifyActiveStateStore`, logged as a warning, and discarded, so a write that failed to arm `verify-state.db` still let verification proceed, and a clear that failed to run on pass or escalation still returned the normal "passed" or "attempts exhausted" reply — in both cases with the Stop hook left reading a stale or missing record. The write and the clear now throw, and the pipeline's single error boundary reports the failure to the caller instead of the reply that would otherwise have claimed success.
- **`skills_sync pull` and `clone` now report how many files they wrote.** Both commands write real files — `pull` writes edited prose and frontmatter back to canonical YAML, `clone` scaffolds a new resource from an external `SKILL.md` — but the run report's `written` count stayed 0 for both and the MCP reply's `Files written` line was left out entirely rather than print a false `0`, so the only way to know what either command did was to list the directory yourself. `pull` now counts distinct files written and breaks the count out by client the same way `export`/`sync` already do; `clone` counts the distinct files it wrote across its base document, prose files, and any companion gates or chain steps.
- **Updating a framework no longer resets its version or rewrites files the update did not change.** `resource_manager` framework `update` wrote `version: 1.0.0` over the stored value on every call, so a description edit took CAGEERF from 2.0.0 to 1.0.0. It also wrote `enabled: true`, so editing a disabled framework turned it back on. It also rewrote `framework.yaml` and the phases file every time, dropping their comments and reflowing flow-style YAML, even when the update named neither. An update now keeps the stored `version` and `enabled`, and writes only the files whose content it changes. The others stay byte-identical, comments included. `rollback` follows the same rule.
...
- **A change that lands while a hot reload is already running is now served, instead of waiting for the next unrelated change.** The server dropped any hot-reload event that arrived during a running prompt reload, so a prompt edit, or a folder created after startup, that landed inside that window was logged and discarded, and the previous content kept being served until something else triggered a reload. Reloads still run one at a time; every event that arrives during one now produces exactly one more reload when it finishes, which reads the disk as it then is.
