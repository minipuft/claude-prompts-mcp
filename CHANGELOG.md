...
- **Updating a framework no longer resets its version or rewrites files the update did not change.** `resource_manager` framework `update` wrote `version: 1.0.0` over the stored value on every call, so a description edit took CAGEERF from 2.0.0 to 1.0.0. It also rewrote `framework.yaml` and the phases file every time, dropping their comments and reflowing flow-style YAML, even when the update named neither. An update now keeps the stored `version`, and writes only the files whose content it changes. The others stay byte-identical, comments included. `rollback` follows the same rule.
