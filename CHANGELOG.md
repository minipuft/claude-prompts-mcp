...
- **The `strategicImplement` prompt is now `strategic_implement`.** Type `>>strategic_implement` wherever you typed `>>strategicImplement`, in commands, chains and templates. The old spelling is no longer recognised and there is no alias: it answers "Unknown prompt" and suggests the new name. Every other prompt id in the >> command language is already snake_case, and this was the last bundled prompt that was not. If you export skills, the exported skill folder is renamed too, to `strategic_implement/`.
- **`cpm rename --json` and `cpm move --json` report `oldPath`/`newPath` instead of `oldDir`/`newDir`.** A prompt can now be a single file, so the moved thing is a path, not always a directory. Scripts reading the old keys need the new names.
...
- **`cpm rename`, `move`, `toggle` and `link-gate` no longer leave a partial write in place when a mutation throws instead of returning a failure.** `runValidatedMutation` rolled back a validation failure but not an exception raised inside the mutation it wraps, so a bug there could leave the resource tree holding whatever had been written before the throw. A throw is now rolled back the same way a validation failure is, and the original error still propagates afterward — the helper restores the tree; it does not decide how the error is reported.
- **Renaming or deleting a chain with `cpm` now moves or removes its steps' history, and a renamed chain still finds its steps.** Chain steps record their history as `chain/step`, and only the chain's own history moved with it, so after a rename or delete the steps' history stayed under ids nothing serves. A renamed chain also kept `promptId: chain/step` in its `chainSteps`, which then named steps that no longer existed. Both now follow the new id. References from other prompts are still reported and left unchanged.
- **Version history is looked up by each resource's own type and id, never from its path.** `skills-sync` read a resource's history from its directory path, which made any gate in a workspace under a directory named `prompts` read as a prompt, so its export recorded another resource's version number. `cpm rename` now moves history only after the renamed files pass validation; a rename that validation rolled back used to leave the history under the new id. A failed `cpm create` no longer deletes history stored under the id it tried to use, which can be a deleted prompt's history that `resource_manager` keeps.
- **`cpm` now lists, validates and edits nested chain steps and single-file prompts, and `cpm delete` on a single-file prompt removes that file, not its category.** `cpm` read only `{category}/{id}/prompt.yaml`, so the 15 chain steps shipped under `resources/prompts` (`deep_analysis/deep_dive` and the rest) and any `{category}/{id}.yaml` prompt were invisible to `list` and `validate`, which now check 51 bundled prompts instead of 36. Every command now finds a prompt under the id the server serves it as. `delete` removes a single-file prompt's file alone and a directory prompt's directory as before. `history`, `compare` and `delete` read and remove a nested step's own version history rather than that of a top-level prompt with the same last segment. `rename` and `move` validate the target before writing, so a refused target leaves the prompt untouched rather than half-renamed. `rename` renames a nested step or a single-file prompt in place but refuses to change anything but the last segment of an id, and `move` refuses a nested step, which moves with its chain.
- **Editing one field of a prompt through `resource_manager` no longer deletes the fields it did not touch.** An `update` that changed only a prompt's `description` rebuilt `prompt.yaml` from the fields the writer knows about, so it deleted the `artifacts` declaration that attaches artifact-scoped gates, a chain's `edges` and `budget`, and any extra key the prompt schema allows. It also reordered keys inside `arguments`. An update now starts from the file on disk and changes only the keys it was asked to change. Comments and YAML layout are still rewritten whenever `prompt.yaml` itself is written.
- **Editing one field of a gate or category through `resource_manager` no longer deletes the fields it did not touch.** `GateFileWriter` and `CategoryFileWriter` had the same defect just fixed for prompts: an update rebuilt `gate.yaml`/`category.yaml` from the fields each writer models, so a description-only edit dropped any key outside that set — a hand-authored key `GateDefinitionSchema`'s `.passthrough()` (or `CategorySchema`'s unrejected-unknown-key parsing) otherwise allows. Both writers now start from the file on disk and overlay only the keys the call decides, sharing the overlay logic the prompt fix introduced. Comments and YAML layout are still rewritten whenever either file is written.
...
- **`cpm rename` now finds a reference living in a `.md` template, including a nested chain step's own template.** `scanReferences` read only each resource's own entry YAML, so a `>>id` reference in a `systemMessageFile`/`userMessageTemplateFile` template was invisible — a real rename reported 1 remaining reference where there were 4. It now reads every file the loader would read for each resource and matches on an id boundary, so scanning for `foo` no longer misreports `foo_bar`.
- **`cpm create` refuses a single-file prompt's id instead of shadowing it.** `resourceExists` checked only the directory form (`{cat}/{id}/prompt.yaml`), so `cpm create prompt x` next to an existing single-file `{cat}/x.yaml` created `x/`, which the loader then served in the file's place. `create` now refuses by name, citing the existing path, when either form already exists. `move` and `rename` were unaffected — both already check both forms through `occupantOf`.
- **A `resource_manager` `update` patching `user_message_template` (or `system_message`) on a hand-authored, inline-template prompt now changes what the prompt renders.** `userMessageTemplate`/`systemMessage` are not `prompt.yaml`-resident keys — their text lives in a separate `.md` file for every prompt the writer itself produces — so a call supplying only one of them left `prompt.yaml` untouched. That is correct when the file already points at its `.md` file, but a hand-authored prompt that declares the template inline (no `*File` pointer) has no pointer for the write to leave alone: the write rewrote the `.md` file and the stale inline value kept rendering, because the loader prefers a `*File` pointer over the inline key only when one is present. The write now also converts the file to pointer form — the shape every writer-produced prompt already has — whenever it finds the field declared inline; a prompt already in pointer form is untouched, unchanged from before.
- **`resource_manager update` on an id that is neither loaded nor quarantined is now refused as unknown, instead of reaching patch or diagnosis logic meant for a real prompt.** A bare id that only exists as a nested chain step's composite id (e.g. `plan_table`, addressable only as `implementation_plan/plan_table`) fell through to the same path a quarantine repair uses — an empty draft defaulted to `category: "general"` — and a `patch` against it failed with `anchor_not_found`, naming an anchor that could never have existed rather than the id that does not resolve. The refusal now fires before any patch or diagnosis runs, and names the composite id when the bare id matches exactly one loaded nested step (or lists every match when it matches several).
- **The `cpm.scope.source` telemetry attribute now reports the request's real identity source instead of the constant `"default"`.** It read `ExecutionContext.state.scope.source`, a field that duplicated `state.identity` and whose only writer was deleted as test-only dead code at P4.52 — so every execution reported `"default"` regardless of whether identity was resolved from a token, a header, or a launch default. It now reads `state.identity.context?.identitySource`, the field `IdentityResolutionStage` actually populates. The duplicate `state.scope` field is removed.
