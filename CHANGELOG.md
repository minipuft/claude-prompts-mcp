...
## [5.0.0](https://github.com/minipuft/claude-prompts-mcp/compare/v4.0.1...v5.0.0) (2026-09-18)


### ⚠ BREAKING CHANGES

* **config:** every config key resolves the same from a 4.x file, a 5.0 file, or over MCP ([#325](https://github.com/minipuft/claude-prompts-mcp/issues/325))
* **frameworks:** framework tool text adds to each tool's description ([#323](https://github.com/minipuft/claude-prompts-mcp/issues/323))
* **config:** one generated authority describes the 5.0 config.json ([#322](https://github.com/minipuft/claude-prompts-mcp/issues/322))
* **config:** configuration is read-only over MCP ([#312](https://github.com/minipuft/claude-prompts-mcp/issues/312))
* **runtime:** a workspace, resources directory or workspace config.json the server cannot use now stops it from starting instead of being silently created, replaced or ignored.
* **runtime:** a config path set with `MCP_CONFIG_PATH` or `--config` that cannot be used now stops the server instead of starting it on defaults.
* **mcp-tools:** every mutation intent gets a verb the tool can refuse ([#262](https://github.com/minipuft/claude-prompts-mcp/issues/262))
* **server:** three gate/resource YAML contracts change, each with the migration in the error message.
* **server:** the HTTP transport now binds 127.0.0.1 by default and the four /api/v1/tools/ routes require MCP_TOOLS_WRITE_TOKEN. Deployments reaching the server across a network must set MCP_HTTP_HOST, and any client calling the tool routes must present the new token.
* **gates:** a gate criterion that cannot be enforced is refused at load. `shell_verify` without `shell_command` and `script_tool` without `script_tool_id` fail gate loading, naming the missing field. Both previously auto-passed at review time, so a gate declaring a check it could not perform reported as verified -- the strongest available evidence for the one outcome that carries none. Gate YAML is contract surface, so this is priced as breaking even though nothing in this repository or the three downstream consumers relies on the old permissiveness.
* **hooks:** delete the blocking subagent-gate-enforce hook (S3)
...
* **chains:** cross-client chain handoff — mint and claim a run across servers ([49efdcf](https://github.com/minipuft/claude-prompts-mcp/commit/49efdcf1d5d5c1adfaf32867dce03d70f5e4c991))
* **chains:** mid-chain unknown surfacing and adaptive consolidation ([#254](https://github.com/minipuft/claude-prompts-mcp/issues/254)) ([19ce795](https://github.com/minipuft/claude-prompts-mcp/commit/19ce7950e86494e63fd260b985001f16f5569fb1))
* **config:** configuration is read-only over MCP ([#312](https://github.com/minipuft/claude-prompts-mcp/issues/312)) ([b31d920](https://github.com/minipuft/claude-prompts-mcp/commit/b31d9207fdae579a4a0a5a61229b7adcb4253d21))
* **config:** every config key resolves the same from a 4.x file, a 5.0 file, or over MCP ([#325](https://github.com/minipuft/claude-prompts-mcp/issues/325)) ([8f9960e](https://github.com/minipuft/claude-prompts-mcp/commit/8f9960eb43540546b43a57f9d49dac3d7f773d15))
* **config:** one generated authority describes the 5.0 config.json ([#322](https://github.com/minipuft/claude-prompts-mcp/issues/322)) ([914b068](https://github.com/minipuft/claude-prompts-mcp/commit/914b068cc5848bad1b3a3327613395e112ee0694))
* **config:** the server reports config.json schema mismatches ([#288](https://github.com/minipuft/claude-prompts-mcp/issues/288)) ([e84bf39](https://github.com/minipuft/claude-prompts-mcp/commit/e84bf39439aede6f8687c8eb372678bbaeb9cde1))
* **execution:** delegated steps hand off a self-contained execution brief ([8287132](https://github.com/minipuft/claude-prompts-mcp/commit/82871329f189a25c4f27d34f1c7578621e80e4fe))
* **execution:** record delegation_skipped at step capture (S8) ([4983c87](https://github.com/minipuft/claude-prompts-mcp/commit/4983c8738e6ccdf22457394b2343393c6c7327fd))
* **frameworks:** land the phase-guard declaration contract and schema v24 ([9970764](https://github.com/minipuft/claude-prompts-mcp/commit/9970764eb4d9a9c99642c6b1a47e64855ee5bf91))
* **gates:** gates attach by artifact; evidence, export, and schema follow ([#300](https://github.com/minipuft/claude-prompts-mcp/issues/300)) ([ea3865a](https://github.com/minipuft/claude-prompts-mcp/commit/ea3865af208850bbe4fff6245da6fe5812e25921))
* **gates:** gates become checks and reminders; verdicts bind to recorded evidence ([#297](https://github.com/minipuft/claude-prompts-mcp/issues/297)) ([056d5de](https://github.com/minipuft/claude-prompts-mcp/commit/056d5def4b249f0938b2aa641f7510bc4f773435))
* **gates:** run script_tool criteria, and refuse ones that cannot run ([078eb7e](https://github.com/minipuft/claude-prompts-mcp/commit/078eb7e0a14c6c94505720e49ddb1d45e6e7fe8e))
* **mcp-tools:** add argument_updates parameter for prompt updates ([ce93c8a](https://github.com/minipuft/claude-prompts-mcp/commit/ce93c8ac4568f40096e4484da862d205b12f84db))
* **mcp-tools:** every mutation intent gets a verb the tool can refuse ([#262](https://github.com/minipuft/claude-prompts-mcp/issues/262)) ([a0af327](https://github.com/minipuft/claude-prompts-mcp/commit/a0af3278b2a3cd886b42df14d8e89d66012671fb))
* **mcp-tools:** inspect reports what the resource file actually declares ([#273](https://github.com/minipuft/claude-prompts-mcp/issues/273)) ([56138c3](https://github.com/minipuft/claude-prompts-mcp/commit/56138c35733a0809ed9436d5acf9faab6e53e574))
* **mcp-tools:** make prompt authoring verifiable ([390ae26](https://github.com/minipuft/claude-prompts-mcp/commit/390ae26ebcce613397529bf2e39506d49d9056fa))
* **mcp-tools:** move session cancel to prompt_engine, add cross-workspace history reads ([efe9a60](https://github.com/minipuft/claude-prompts-mcp/commit/efe9a605d75795b27d1d7a61dd775da975165cb2))
* **mcp-tools:** reach skills-sync through system_control and adopt its orphans ([5e08469](https://github.com/minipuft/claude-prompts-mcp/commit/5e084697a68f9be340576202c3edec33c393e89e))
* **mcp-tools:** the resource surface matches what the server reads and writes ([#265](https://github.com/minipuft/claude-prompts-mcp/issues/265)) ([320d3ce](https://github.com/minipuft/claude-prompts-mcp/commit/320d3ce626442dc00be7779428e72e7d598fe2ff))
* **prompts:** add authenticated catalog detail endpoint ([#244](https://github.com/minipuft/claude-prompts-mcp/issues/244)) ([4d822bd](https://github.com/minipuft/claude-prompts-mcp/commit/4d822bd074e46a72510c46f83817b35d16cb222d))
* **prompts:** add reference-grounded creative workflows ([f218b11](https://github.com/minipuft/claude-prompts-mcp/commit/f218b115470ed1147300584482c7140a6b1a9a93))
* **prompts:** expose workflow catalog metadata ([f6e7adc](https://github.com/minipuft/claude-prompts-mcp/commit/f6e7adce88309107b93f1835274b1643d735711c))
* **prompts:** strategicImplement plans and judges; strategic_worker owns each row ([#275](https://github.com/minipuft/claude-prompts-mcp/issues/275)) ([81748d6](https://github.com/minipuft/claude-prompts-mcp/commit/81748d6a1daa70ac2bd8f682d73feb493c8f115b))
* **prompts:** the bundled deep analysis chain asks how far its findings can be trusted ([#277](https://github.com/minipuft/claude-prompts-mcp/issues/277)) ([aaecac4](https://github.com/minipuft/claude-prompts-mcp/commit/aaecac4e0fa485a8c82dfdd63f513e4bc38c2c1f))
* **resources:** compile template chains faithfully and ship gate manifests for OpenCode parity ([2e7f965](https://github.com/minipuft/claude-prompts-mcp/commit/2e7f9656aaa5aa98b4a02c1066f53078e0aceeca))
* **scripts:** add validate:registry-coherence so a write that never registers reds ([457d312](https://github.com/minipuft/claude-prompts-mcp/commit/457d312123b67f10bff2355ca06a8f0917e357cf))
* **scripts:** assert tool-parameter coverage in validate:conformance-coverage ([7513d52](https://github.com/minipuft/claude-prompts-mcp/commit/7513d527711c588178faa0c9a9334a929690f948))
* **scripts:** have the validation runner record its own verdict and attribute failures ([b1cd430](https://github.com/minipuft/claude-prompts-mcp/commit/b1cd4305f60e00ec8f09b0d6542d937e610659a1))
* **scripts:** wire git hooks in linked worktrees, and detect when they are not ([#239](https://github.com/minipuft/claude-prompts-mcp/issues/239)) ([29961b2](https://github.com/minipuft/claude-prompts-mcp/commit/29961b2477b5a3c21b1cd063b6d5ce61ef7e6fe5))
* **semantic:** govern guidance and module boundaries ([6f4b230](https://github.com/minipuft/claude-prompts-mcp/commit/6f4b23096136753b76c2b2e3929c76d4cb7a83c8))


### Fixed

* adaptive-chain-runtime residuals sweep ([ba03839](https://github.com/minipuft/claude-prompts-mcp/commit/ba03839ec552be178fdd1f6628efdde9d529f26b))
* **chains:** sync cpm rollback onto go-forward version numbering ([2e15f0c](https://github.com/minipuft/claude-prompts-mcp/commit/2e15f0c5305c63414643a849e4bdc9742d917d50))
* **ci:** a plan footer asserts progress, not same-PR retirement ([#263](https://github.com/minipuft/claude-prompts-mcp/issues/263)) ([12d96ed](https://github.com/minipuft/claude-prompts-mcp/commit/12d96ed492b0850cd1536d873b8a107cdabe5cbb))
* **ci:** run integration at pre-push, and make git-backed checks hook-safe ([639fe26](https://github.com/minipuft/claude-prompts-mcp/commit/639fe268feb49bdbdf5d67c019d0eee757a98fe0))
* close both conformance-gate blockers from the test roadmap ([ee6c5e3](https://github.com/minipuft/claude-prompts-mcp/commit/ee6c5e32191c43c98927b58767c01eb0cf96b208))
* **config:** architecture checks enforce the boundaries they name ([#316](https://github.com/minipuft/claude-prompts-mcp/issues/316)) ([bbd5f69](https://github.com/minipuft/claude-prompts-mcp/commit/bbd5f692bce324cc399ac9c413a8d1d345d455ba))
* **contracts:** array element types match what each schema accepts ([#328](https://github.com/minipuft/claude-prompts-mcp/issues/328)) ([79b9fa3](https://github.com/minipuft/claude-prompts-mcp/commit/79b9fa36e3f2a4e5be1a65b2d2428f78a754a650))
* **contracts:** every tool's input schema matches its contract ([#318](https://github.com/minipuft/claude-prompts-mcp/issues/318)) ([37fafdd](https://github.com/minipuft/claude-prompts-mcp/commit/37fafdddc111d0fc129bd97fcd46176cf93096f2))
* **contracts:** give both suggestion matchers one scoring contract and a word floor ([b5b1904](https://github.com/minipuft/claude-prompts-mcp/commit/b5b1904c380558d567abad56463574bba3d26a6e))
* **deps:** declare @jest/globals so the knip ratchet measures real debt ([3d2572a](https://github.com/minipuft/claude-prompts-mcp/commit/3d2572abacf5bff4df0ea54d6dcd327fd159fdaf))
* **docs:** correct four dead commands in CONTRIBUTING and gate the class ([cd7d18b](https://github.com/minipuft/claude-prompts-mcp/commit/cd7d18bf3f76f3852cd18d053c3665789a76d642))
* **docs:** correct four dead commands in CONTRIBUTING and gate the class ([#233](https://github.com/minipuft/claude-prompts-mcp/issues/233)) ([91f5058](https://github.com/minipuft/claude-prompts-mcp/commit/91f50588d0dad58ec48826dab8995222edcf955b))
* **docs:** file the plan pair under the frontmatter status vocabulary ([ecb0c2d](https://github.com/minipuft/claude-prompts-mcp/commit/ecb0c2def98fec94541e100856da78ed5bfd9c62))
* **execution:** advisory names the real delegated step, not the synthetic review step (S10) ([19a537b](https://github.com/minipuft/claude-prompts-mcp/commit/19a537bebc2b0f63c579e383711ad07f56d24352))
* **execution:** commit the declared-sections closure efe9a605 depends on ([4fa54e5](https://github.com/minipuft/claude-prompts-mcp/commit/4fa54e573bbdd9b2503a199f27a7806e5f144e15))
* **execution:** honor execution.confirm for inline script references ([d6cd6b7](https://github.com/minipuft/claude-prompts-mcp/commit/d6cd6b7339756f3a20a70840665a14cab96b0f29))
* **frameworks:** a framework created or kept in the workspace can be the active framework ([#308](https://github.com/minipuft/claude-prompts-mcp/issues/308)) ([1534763](https://github.com/minipuft/claude-prompts-mcp/commit/15347639d8fb0659e01811b87588e57e9e6ed847))
* **frameworks:** framework companion files keep the names framework.yaml declares ([#295](https://github.com/minipuft/claude-prompts-mcp/issues/295)) ([e19dcba](https://github.com/minipuft/claude-prompts-mcp/commit/e19dcba275b2d52b4a048d84b368b227c55310d5))
* **frameworks:** framework tool text adds to each tool's description ([#323](https://github.com/minipuft/claude-prompts-mcp/issues/323)) ([9a7bfb1](https://github.com/minipuft/claude-prompts-mcp/commit/9a7bfb1b0b2b327ae72b2269897a1b8d897699b4))
* **frameworks:** re-register on update and reload instead of claiming it (G2, G4) ([d5eaa6a](https://github.com/minipuft/claude-prompts-mcp/commit/d5eaa6a1c8cead3ca3b68a66e44278b9dc8fa5e7))
* **frameworks:** the [@id](https://github.com/id) override recognizes frameworks created after startup ([#305](https://github.com/minipuft/claude-prompts-mcp/issues/305)) ([6fc40cb](https://github.com/minipuft/claude-prompts-mcp/commit/6fc40cbe7a2f6f44db606c0eccd45b00dd4fb8ad))
* **gates:** a gate with no activation block is opt-in; export hooks are opt-in per prompt ([#286](https://github.com/minipuft/claude-prompts-mcp/issues/286)) ([b3e8df3](https://github.com/minipuft/claude-prompts-mcp/commit/b3e8df3182e0058fc374d03f3f561653b2dbdaaa))
* **gates:** a guidance-only gate edit leaves gate.yaml alone ([#294](https://github.com/minipuft/claude-prompts-mcp/issues/294)) ([9e87964](https://github.com/minipuft/claude-prompts-mcp/commit/9e87964c492a1931ae8910396457821d630644c6))
* **gates:** add Stage 1.5 existence gate to GateSetResolver (P6-F14) ([084b662](https://github.com/minipuft/claude-prompts-mcp/commit/084b662757b049920daad83ecf2eb70ca34024db))
* **gates:** guidance.md stays byte-identical when a gate change leaves guidance alone ([#284](https://github.com/minipuft/claude-prompts-mcp/issues/284)) ([340b341](https://github.com/minipuft/claude-prompts-mcp/commit/340b3415c1998093797a25d97c221b9966f4c69b))
* **gates:** preserve gate.yaml fields the caller did not supply on update ([dc1c5f7](https://github.com/minipuft/claude-prompts-mcp/commit/dc1c5f7543b91d2761b87305ca33c3c00f873700))
* **gates:** resolve script_tool_id to a tool, and let raw blocks hold ([c6f111f](https://github.com/minipuft/claude-prompts-mcp/commit/c6f111fcd6202d2792ddddc01e95d22bd770b73e))
* **gates:** stop the default framework gate from undoing every resolver veto ([794f460](https://github.com/minipuft/claude-prompts-mcp/commit/794f460a31b273fe389f5c82d533b00a5c36144e))
* **gates:** the documentation gates reach the documentation prompts ([#313](https://github.com/minipuft/claude-prompts-mcp/issues/313)) ([9cdc3ad](https://github.com/minipuft/claude-prompts-mcp/commit/9cdc3adc8943d151aea4b650b45f5d50b9d1914b))
* **hooks:** accept every contract resolution verb while a gate is pending ([60e161e](https://github.com/minipuft/claude-prompts-mcp/commit/60e161e8d83f79622ae86a28e3ab0727737601c8))
* **hooks:** carry the &gt;&gt; resolution line on the channel clients actually read ([488e5bb](https://github.com/minipuft/claude-prompts-mcp/commit/488e5bb1adb49168875b778a9417a1c5329b483d))
* **hooks:** clear session rows when their run resolves ([6c1f923](https://github.com/minipuft/claude-prompts-mcp/commit/6c1f923df67e480efa2ed785b8b748def894e2ca))
* **hooks:** delete the blocking subagent-gate-enforce hook (S3) ([fcdfe7d](https://github.com/minipuft/claude-prompts-mcp/commit/fcdfe7d9718008fe787a266f0c7db9f9df7feb5f))
* **hooks:** resolve Python across platforms ([#241](https://github.com/minipuft/claude-prompts-mcp/issues/241)) ([dd70238](https://github.com/minipuft/claude-prompts-mcp/commit/dd70238355dc44dff738cebcf10a948671dc59dc))
* **hooks:** route &gt;&gt; resolution through the channel clients read ([9d8c92d](https://github.com/minipuft/claude-prompts-mcp/commit/9d8c92de031b7f3c5198d55bb51ef556be736634))
* **hooks:** scope chain recovery to the owning conversation and read the server's real state.db ([e53be94](https://github.com/minipuft/claude-prompts-mcp/commit/e53be94ccdb4601c78cf1a83eeb33f219ced2225))
* **hooks:** withhold systemMessage on SDK hosts and echo it as assistant text ([0bc86c1](https://github.com/minipuft/claude-prompts-mcp/commit/0bc86c169d4eb913320f580df7ae030584eb06a5))
* **mcp-tools:** a chain step cannot scaffold outside its parent prompt's folder ([#289](https://github.com/minipuft/claude-prompts-mcp/issues/289)) ([53bed1f](https://github.com/minipuft/claude-prompts-mcp/commit/53bed1fa47ea9087ee0577df25245d5d9f11eaf6))
* **mcp-tools:** a framework error names the folder the framework resolves from ([#293](https://github.com/minipuft/claude-prompts-mcp/issues/293)) ([a14491c](https://github.com/minipuft/claude-prompts-mcp/commit/a14491c9ef641ea4148682ad348ce4c040916918))
* **mcp-tools:** a gate or framework diff names the files its write changes ([#290](https://github.com/minipuft/claude-prompts-mcp/issues/290)) ([fed35e2](https://github.com/minipuft/claude-prompts-mcp/commit/fed35e2fb0cde239b84ed80aa6b29453e9e75d52))
* **mcp-tools:** a new prompt, gate or framework starts at version 1 ([#296](https://github.com/minipuft/claude-prompts-mcp/issues/296)) ([61b43c3](https://github.com/minipuft/claude-prompts-mcp/commit/61b43c3f50a4e0ca99f706f76224e38141f452a5))
* **mcp-tools:** a prompt preview shows the diff that lands on disk ([#285](https://github.com/minipuft/claude-prompts-mcp/issues/285)) ([975093b](https://github.com/minipuft/claude-prompts-mcp/commit/975093bfe79b00d82084aff98f340dd1d1e6fbe2))
* **mcp-tools:** a tool reply reports only what the server actually did ([#310](https://github.com/minipuft/claude-prompts-mcp/issues/310)) ([b6ac608](https://github.com/minipuft/claude-prompts-mcp/commit/b6ac6083904018ff037577c6b99c63646dd97846))
* **mcp-tools:** apply the database port to the framework tool built after it (G5) ([63ee8ed](https://github.com/minipuft/claude-prompts-mcp/commit/63ee8ed4c159417c147e9bf4f23c5cc3d0db9b8f))
* **mcp-tools:** deny destructive resource actions once and preview them ([d57a9c5](https://github.com/minipuft/claude-prompts-mcp/commit/d57a9c5574f94df14689b82fad0f3cd0da3fb3d7))
* **mcp-tools:** every field a tool handler reads reaches it over MCP ([#299](https://github.com/minipuft/claude-prompts-mcp/issues/299)) ([4d323d6](https://github.com/minipuft/claude-prompts-mcp/commit/4d323d60ba657db3ea50f917cd95bed73dfb6fb0))
* **mcp-tools:** every prompt resource type now loads from the workspace ([#298](https://github.com/minipuft/claude-prompts-mcp/issues/298)) ([5a5c50e](https://github.com/minipuft/claude-prompts-mcp/commit/5a5c50e2e4fbed709d90281ea747a89a326e4a52))
* **mcp-tools:** fix resource_manager prompt write-path settability defects ([5c3198b](https://github.com/minipuft/claude-prompts-mcp/commit/5c3198b5391a68911acaefadb211313f741b1660))
* **mcp-tools:** register a created gate so the process that made it can use it ([b7102dd](https://github.com/minipuft/claude-prompts-mcp/commit/b7102dd9bcf20af5073e848ca06e9ac80c32d46b))
* **mcp-tools:** register skills_sync in the action inventory and stamp plan tags ([860aeca](https://github.com/minipuft/claude-prompts-mcp/commit/860aeca93df52b7425beb7761d7ae79923cfd2f2))
* **mcp-tools:** the model in Claude Code sees every tool result's readable text ([#283](https://github.com/minipuft/claude-prompts-mcp/issues/283)) ([868f36e](https://github.com/minipuft/claude-prompts-mcp/commit/868f36ec23d3e048b73449d496300626f8b59691))
* **mcp-tools:** updating a single-file prompt leaves exactly one definition of it ([#292](https://github.com/minipuft/claude-prompts-mcp/issues/292)) ([648e88e](https://github.com/minipuft/claude-prompts-mcp/commit/648e88e68a37b5c1a4570031e143bb1130a4dda5))
* **parsers:** attribute inline gate tokens to their chain segment (S9) ([e693164](https://github.com/minipuft/claude-prompts-mcp/commit/e693164729896607c1068f8cf20f1f53388aafae))
* **parsers:** resolve empty symbolic args and prompt-scoped chain tools ([2f84df6](https://github.com/minipuft/claude-prompts-mcp/commit/2f84df6558ce4c49adafba988316c83159818e35))
* **pipeline:** re-evaluate pending gate review after same-request advance (P5-F6) ([d5e14d7](https://github.com/minipuft/claude-prompts-mcp/commit/d5e14d77ce4bb6ca03d40058475f2408628eea48))
* **prompts:** every README install path ends in a command a fresh install can run ([#276](https://github.com/minipuft/claude-prompts-mcp/issues/276)) ([c68205b](https://github.com/minipuft/claude-prompts-mcp/commit/c68205bd2eb9c8c122c55aec1e6ce182d8346265))
* **prompts:** strategic_worker export excludes its auto-attaching gates ([#282](https://github.com/minipuft/claude-prompts-mcp/issues/282)) ([d1f3f71](https://github.com/minipuft/claude-prompts-mcp/commit/d1f3f71dd9e800d110c03030a24ea68c02a59a1e))
* **resources:** close the false-claim shape in the paths the first pass missed ([b9c1b6e](https://github.com/minipuft/claude-prompts-mcp/commit/b9c1b6e9b8da7fd66f1ff69f3cde9002ab90e018))
* **resources:** compile the else-branch for export and say when a placeholder survives ([ecc8537](https://github.com/minipuft/claude-prompts-mcp/commit/ecc8537b2fdba06607fc4d066222c7541fab875e))
* **resources:** emit skill artifacts that state only what clients can honour ([9b0fcfc](https://github.com/minipuft/claude-prompts-mcp/commit/9b0fcfcf23697ecfd9a6f0be9f0d613bb41254a1))
* **resources:** every prompts-folder walk follows the server's rules ([#324](https://github.com/minipuft/claude-prompts-mcp/issues/324)) ([01cbb36](https://github.com/minipuft/claude-prompts-mcp/commit/01cbb36893ad24b779be9f4095adea8c213c13ab))
* **resources:** make the framework draft validator check the shape it requires (G1, G3) ([295271a](https://github.com/minipuft/claude-prompts-mcp/commit/295271a198dc43e3f5b3e5da5338f94df6e9e3a8))
* **resources:** refuse skill export through a symlink that leaves the client's base dir ([#256](https://github.com/minipuft/claude-prompts-mcp/issues/256)) ([df2e9f1](https://github.com/minipuft/claude-prompts-mcp/commit/df2e9f1441c133c9b64780a91223fdc7136a47c6))
* **resources:** resolve export gate activation through the engine, not a local copy ([519ec10](https://github.com/minipuft/claude-prompts-mcp/commit/519ec1055faa3b566c00a876a1a4bb1598dc13aa))
* **resources:** skills-sync diff works without a manifest and export names what it removes ([#309](https://github.com/minipuft/claude-prompts-mcp/issues/309)) ([05b6115](https://github.com/minipuft/claude-prompts-mcp/commit/05b61158d8949aff7402d84e1ebbe93f61f2ba9a))
* **resources:** tell a tool that failed to load from a tool that was deleted ([def97ce](https://github.com/minipuft/claude-prompts-mcp/commit/def97ceab61c50a046eed367684249b4022cc69c))
* **resources:** the resource surface keeps one copy of each rule it enforces ([#319](https://github.com/minipuft/claude-prompts-mcp/issues/319)) ([83196fc](https://github.com/minipuft/claude-prompts-mcp/commit/83196fcdd6462e7217f664bd2861fcfdd284f665))
* **resources:** the server reports which copy of a resource actually serves ([#315](https://github.com/minipuft/claude-prompts-mcp/issues/315)) ([f2e07a4](https://github.com/minipuft/claude-prompts-mcp/commit/f2e07a45d089ba4cb863e984e75b23854de7f722))
* **runtime:** a failure that was silently dropped now reaches the caller ([#311](https://github.com/minipuft/claude-prompts-mcp/issues/311)) ([6bc3622](https://github.com/minipuft/claude-prompts-mcp/commit/6bc3622731f3534d6d7d44adf047048e5f1b122e))
* **runtime:** a prompt created through the Claude Code plugin survives a plugin update ([#281](https://github.com/minipuft/claude-prompts-mcp/issues/281)) ([aed06d0](https://github.com/minipuft/claude-prompts-mcp/commit/aed06d02882176a49c62943e86e916d80509be82))
* **runtime:** a prompt hot reload refreshes which prompts are exported as skills ([#304](https://github.com/minipuft/claude-prompts-mcp/issues/304)) ([5163dff](https://github.com/minipuft/claude-prompts-mcp/commit/5163dff99c1b27b4e31ad23b1676e120dd5b4e4e))
* **runtime:** a prompt or framework edit reaches the running server ([#314](https://github.com/minipuft/claude-prompts-mcp/issues/314)) ([24977b9](https://github.com/minipuft/claude-prompts-mcp/commit/24977b955fdddec6d34ee6335a2546f764aaba04))
* **runtime:** a server runs only on the settings it was given, including after a restart ([#279](https://github.com/minipuft/claude-prompts-mcp/issues/279)) ([d2af7a8](https://github.com/minipuft/claude-prompts-mcp/commit/d2af7a8a56f44a1b12fca5252826bcbf0837bd47))
* **runtime:** a STDIO server writes only its real warnings to stderr ([#307](https://github.com/minipuft/claude-prompts-mcp/issues/307)) ([e50095a](https://github.com/minipuft/claude-prompts-mcp/commit/e50095acbeb356a8abc8e62443437bd131723e29))
* **runtime:** every run, server or check, uses only the configuration it was given ([#278](https://github.com/minipuft/claude-prompts-mcp/issues/278)) ([c141a04](https://github.com/minipuft/claude-prompts-mcp/commit/c141a04896039b2ebfcdc8e0ede8c067c310e8de))
* **runtime:** hot reload serves folders created after startup ([#329](https://github.com/minipuft/claude-prompts-mcp/issues/329)) ([53cbcbd](https://github.com/minipuft/claude-prompts-mcp/commit/53cbcbda21db4b7e462d76f266e0084113600342))
* **runtime:** hot reload watches every folder a style, framework or script loads from ([#302](https://github.com/minipuft/claude-prompts-mcp/issues/302)) ([af4ff5d](https://github.com/minipuft/claude-prompts-mcp/commit/af4ff5d6a6bb7545de7e1b43d6207afc1ccf1c83))
* **runtime:** load the same roots on reload that startup loads ([#258](https://github.com/minipuft/claude-prompts-mcp/issues/258)) ([5ba103c](https://github.com/minipuft/claude-prompts-mcp/commit/5ba103ccb33a2a083ff0341984fa009be0be818c))
* **runtime:** read skills-sync deregistrations from the registrations key ([d5948b4](https://github.com/minipuft/claude-prompts-mcp/commit/d5948b4879fc2667238db526b4764db37e56afbe))
* **runtime:** record what each edit produced and refuse incomplete snapshots ([39ae9dd](https://github.com/minipuft/claude-prompts-mcp/commit/39ae9dda00f31949a529683b4bf2d502babbc15a))
* **runtime:** runtime state is written under the runtime root ([#327](https://github.com/minipuft/claude-prompts-mcp/issues/327)) ([a844497](https://github.com/minipuft/claude-prompts-mcp/commit/a844497e5621a22e3a47de4c222ee443a5f4288f))
* **runtime:** stop cpm rollback from deleting the fields a snapshot does not carry ([1fc8f42](https://github.com/minipuft/claude-prompts-mcp/commit/1fc8f4200c108a0510ff8f7a26d4c9bdc897ef5e))
* **scripts:** add contracts self-test to generate-contracts.ts ([29c76e9](https://github.com/minipuft/claude-prompts-mcp/commit/29c76e9b1a7ebe2a5bc06ccaca430e09b3a30ed5))
* **scripts:** regenerating a ratchet baseline cannot raise a ceiling ([#321](https://github.com/minipuft/claude-prompts-mcp/issues/321)) ([998c12b](https://github.com/minipuft/claude-prompts-mcp/commit/998c12b34d4aa45a2f51284b84dcda4114bb5cfc))
* **scripts:** removing lint debt requires lowering the ratchet ceiling ([#326](https://github.com/minipuft/claude-prompts-mcp/issues/326)) ([dd105ea](https://github.com/minipuft/claude-prompts-mcp/commit/dd105eaf27d93b6ca7a2dc1374a70f9ae8552b8d))
* **server:** execute the security review — bound every path from authored content to code ([#249](https://github.com/minipuft/claude-prompts-mcp/issues/249)) ([7826298](https://github.com/minipuft/claude-prompts-mcp/commit/78262981da1229f7dceb7e5063ebfcd44f0648cc))
* **server:** harden the HTTP transport to the MCP spec ([#245](https://github.com/minipuft/claude-prompts-mcp/issues/245)) ([15d27a5](https://github.com/minipuft/claude-prompts-mcp/commit/15d27a503c45af300a3a62fa0b63a1f4868a61c7))
* **server:** skills sync over MCP follows the workspace a server was started with ([#303](https://github.com/minipuft/claude-prompts-mcp/issues/303)) ([d59bffa](https://github.com/minipuft/claude-prompts-mcp/commit/d59bffaf817e8f6bf42b94cfae427cc2cd12047c))
* **server:** skills sync reads resources from the workspace a server was given ([#301](https://github.com/minipuft/claude-prompts-mcp/issues/301)) ([267bc61](https://github.com/minipuft/claude-prompts-mcp/commit/267bc6101e20231fc77987beb090ffad11899529))
* **tests:** a test run leaves no trace outside its temp directories ([#320](https://github.com/minipuft/claude-prompts-mcp/issues/320)) ([7a24676](https://github.com/minipuft/claude-prompts-mcp/commit/7a246763811d362ce9dec1122e2a9d347d06e35c))
* **tests:** cover the two new tool parameters and align gates with merged reality ([d156038](https://github.com/minipuft/claude-prompts-mcp/commit/d156038e22507e1c18460da85bdcd7e76892ffd9))
* **tests:** make the path-resolver test hermetic against the operator's environment ([#251](https://github.com/minipuft/claude-prompts-mcp/issues/251)) ([3186883](https://github.com/minipuft/claude-prompts-mcp/commit/318688312e3ed80d82af30f969c1b726ef48db7a))
* **tests:** the flaky e2e suite waits for its server before deleting its workspace ([#260](https://github.com/minipuft/claude-prompts-mcp/issues/260)) ([d9726fd](https://github.com/minipuft/claude-prompts-mcp/commit/d9726fd7e12133e4112baf271f0430e61f3dccab))


### Changed

* **chains:** remove unreachable cap-widening branch from workflow-ir validator ([e557766](https://github.com/minipuft/claude-prompts-mcp/commit/e55776644d6fd212e3f2c1bfd7ac1e0730981cd0))
* **execution:** delegate to the host's general agent; stop shipping chain-executor ([#248](https://github.com/minipuft/claude-prompts-mcp/issues/248)) ([5c2d7f1](https://github.com/minipuft/claude-prompts-mcp/commit/5c2d7f181e5c7bb085fa9142740c08fa5e47e3d1))
* **gates:** delete the superseded criteria validator, name coverage for what it does ([779e3e8](https://github.com/minipuft/claude-prompts-mcp/commit/779e3e84d9c667edb0373163b89bb879f14539bd))
* **gates:** gate definitions load parsed, one settings type, one check line ([#306](https://github.com/minipuft/claude-prompts-mcp/issues/306)) ([4f28604](https://github.com/minipuft/claude-prompts-mcp/commit/4f28604b0836d76d5c548608aa32a38a5fc932db))
* **mcp-tools:** drop unreachable saveVersion failure branch ([d2d7724](https://github.com/minipuft/claude-prompts-mcp/commit/d2d77245d02e25a19e90a622c5ce37cc9b2a3b9c))
* **prompts:** consolidate plan execution around workflow-IR compilation ([f62a185](https://github.com/minipuft/claude-prompts-mcp/commit/f62a185ba8fd6bdf10707ddd5d8480c99ffb0833))
* **server:** delete the unreachable did-you-mean duplicate ([c17864d](https://github.com/minipuft/claude-prompts-mcp/commit/c17864d95180f48ae8dda85e1ba3b62e7b956019))


### Documentation

* **docs:** add frontmatter to the F17 implementation notes ([8adf66d](https://github.com/minipuft/claude-prompts-mcp/commit/8adf66d0e6963efa99acfee1d1b73233a4df5e04))
* **docs:** archive the script-tools notes its plan retirement left behind ([e01b403](https://github.com/minipuft/claude-prompts-mcp/commit/e01b4038c1b5503ee9b1832f9bf944c35ec5e7ec))
* **docs:** close out adaptive-chain-runtime residuals sweep plan ([dbaa450](https://github.com/minipuft/claude-prompts-mcp/commit/dbaa45082703367f976205ba731154988b607eb6))
* **docs:** close the prompt-surface plan's stale markers and growth rows ([42f30fc](https://github.com/minipuft/claude-prompts-mcp/commit/42f30fc79ac049216ac156a9921108be07463878))
* **docs:** document uniform version semantics and the tool that stops a run ([03aa4e7](https://github.com/minipuft/claude-prompts-mcp/commit/03aa4e74e0b0464a9be1cd84502c5bdbb378f216))
* **docs:** draft the cross-client chain handoff plan (interview 2A) with T0 measurements ([5af7e88](https://github.com/minipuft/claude-prompts-mcp/commit/5af7e8891eb4b8548dcf7d29a1f495b7c7793217))
* **docs:** finish the plan sweep and repoint every technical-debt reference ([2aa7592](https://github.com/minipuft/claude-prompts-mcp/commit/2aa7592441e6034805d4c5bc94c64d1f1e3dac11))
* **docs:** flip both re-measurement falsifiers in the test roadmap ([3d47a8d](https://github.com/minipuft/claude-prompts-mcp/commit/3d47a8d4acfb41ada234e2d3e6da9a87c5e118e0))
* **docs:** four P4 rows rejoin the table their gates read ([#274](https://github.com/minipuft/claude-prompts-mcp/issues/274)) ([3b39cae](https://github.com/minipuft/claude-prompts-mcp/commit/3b39cae8268557c89dff048312fe6ba5078437a3))
* **docs:** make PR and commit boundaries explicit, and enforce the template ([#250](https://github.com/minipuft/claude-prompts-mcp/issues/250)) ([9bf1d09](https://github.com/minipuft/claude-prompts-mcp/commit/9bf1d09b05c403fcde5c771026fa74d960ce809e))
* **docs:** open the adoption-conversion plan with a weekly traffic ledger ([6d99b7c](https://github.com/minipuft/claude-prompts-mcp/commit/6d99b7cf36a4d9f1ee4e64a4e2b5427612eac617))
* **docs:** open the contract-layer ledger and the chain-integrity plan ([fe0e21f](https://github.com/minipuft/claude-prompts-mcp/commit/fe0e21f61d6b92d56e9bccd4350d6c4a17997d09))
* **docs:** phase 4 carries a dispatch table and two corrected premises ([#269](https://github.com/minipuft/claude-prompts-mcp/issues/269)) ([ac398cb](https://github.com/minipuft/claude-prompts-mcp/commit/ac398cb949f3ffa7f921161550a096b853dfe7c4))
* **docs:** plan the framework resource lifecycle fixes with the operator's rulings ([e97d82e](https://github.com/minipuft/claude-prompts-mcp/commit/e97d82effb25f66d7918a7e15fce336f3b377b56))
* **docs:** queue residuals plan for retirement as reference ([2ba8771](https://github.com/minipuft/claude-prompts-mcp/commit/2ba87715603776c9e51eb2cde49879689afbc3d7))
* **docs:** record F15 — skills-sync.yaml was tracked and self-contradictory ([dba185b](https://github.com/minipuft/claude-prompts-mcp/commit/dba185bf6039248a11c1b82942a1804b8aa4585d))
* **docs:** record settability-parity row status and six owner decisions ([b1d5d23](https://github.com/minipuft/claude-prompts-mcp/commit/b1d5d23230264523226e4b765577a9cd5c9f65ae))
* **docs:** record the F17 root cause and answer two of its open questions ([0130fa6](https://github.com/minipuft/claude-prompts-mcp/commit/0130fa69d511846dbfefc224f26da4fafb6063f7))
* **docs:** record the owner rulings that scope the remaining skills-sync rows ([85d2ff3](https://github.com/minipuft/claude-prompts-mcp/commit/85d2ff3d0119fb8fbfc67a69b5d75070cd221fde))
* **docs:** record the schema-bump exclusion zone in the framework lifecycle plan ([7500eb8](https://github.com/minipuft/claude-prompts-mcp/commit/7500eb8e36b70db2a53a4672946d2e895e942e6e))
* **docs:** record the skills-sync export fidelity plan and its rulings ([f999b04](https://github.com/minipuft/claude-prompts-mcp/commit/f999b04ba90e2cb4a1f4d975380181e80f4e56f3))
* **docs:** restore subagent-delegation-contract plan to active ([30b941d](https://github.com/minipuft/claude-prompts-mcp/commit/30b941dfbefc92b4b14b840f7f50b47344b7ae91))
* **docs:** retire delegation-contract plan and test roadmap as reference ([1e485dc](https://github.com/minipuft/claude-prompts-mcp/commit/1e485dc32e7eaa2278af8b8db2fecd7406b7b82b))
* **docs:** retire fifteen completed plans to reference and archive ([d17dfe1](https://github.com/minipuft/claude-prompts-mcp/commit/d17dfe1fd2bc8576cbf827a56322ceeeb1161b21))
* **docs:** retire the acquisition-recovery plan with its traffic scorecard ([87f566f](https://github.com/minipuft/claude-prompts-mcp/commit/87f566f8751b2e139032a31860fb546b8428bf55))
* **docs:** retire the resource-versioning plan to reference, promote F17 out ([9051c45](https://github.com/minipuft/claude-prompts-mcp/commit/9051c45af844e4a32a6f28237a5a09c66263255b))
* **docs:** retire the semantic module governance plan to reference ([#243](https://github.com/minipuft/claude-prompts-mcp/issues/243)) ([697148f](https://github.com/minipuft/claude-prompts-mcp/commit/697148f64c72c8b4337d77659ebc5e3ed3ae5186))
* **docs:** retire the skills-sync export-fidelity plan to reference ([fb0c70b](https://github.com/minipuft/claude-prompts-mcp/commit/fb0c70b00300b2b3b65aef2e61bff558f56e3d72))
* **docs:** stamp OQ-F1 resolved so a retired plan stops reading as open ([f8a6bfe](https://github.com/minipuft/claude-prompts-mcp/commit/f8a6bfe6e6a232e0d53d8532deca34b521f7441c))
* **docs:** stamp the 2A plan's resolved sections with the linter's marker ([6a6372d](https://github.com/minipuft/claude-prompts-mcp/commit/6a6372d8242b2da6a6bb6e7dfa08c7556fdada59))
* **docs:** stamp the rulings table so no open question reads as unresolved ([7cc94b5](https://github.com/minipuft/claude-prompts-mcp/commit/7cc94b57dd8ca10c3fc909bb2a4c861c69083fb6))
* **docs:** stamp two implementation notes done so retirement can proceed ([b78ec47](https://github.com/minipuft/claude-prompts-mcp/commit/b78ec475ec0b7e37a3266031b3d0a22ffb055a1c))
* **docs:** tell every session to work in a worktree ([#247](https://github.com/minipuft/claude-prompts-mcp/issues/247)) ([0b5ab85](https://github.com/minipuft/claude-prompts-mcp/commit/0b5ab854f7c2c82960106f48720117f90ec463d2))
* **docs:** the adoption implementation notes carry plan frontmatter ([#267](https://github.com/minipuft/claude-prompts-mcp/issues/267)) ([c951834](https://github.com/minipuft/claude-prompts-mcp/commit/c951834e45b11b85d9d78d1ae81b0b5e6973cf1c))
* **docs:** the contract-layer ledger records the chain-integrity landing and the findings handoff ([f76f621](https://github.com/minipuft/claude-prompts-mcp/commit/f76f621dd38d8f9a511c2d67b8a3767f6d51f585))
* **docs:** the environment-gaps plan holds 5.0.0 with every downstream fix live ([#280](https://github.com/minipuft/claude-prompts-mcp/issues/280)) ([e52e8eb](https://github.com/minipuft/claude-prompts-mcp/commit/e52e8eb1d0e9ee59e3879fd86e00eeccd4cfca15))
* **docs:** the unreleased changelog speaks to the consumer ([#261](https://github.com/minipuft/claude-prompts-mcp/issues/261)) ([f455c8b](https://github.com/minipuft/claude-prompts-mcp/commit/f455c8bd77fec77ff7d2ec1b4ecba90a357a7074))
* **frameworks:** retire the Stage 09b name and close the phase-guard plan ([655642d](https://github.com/minipuft/claude-prompts-mcp/commit/655642d5819aaf130e45abbc8dc6ef07c96deff8))
* queue adaptive-chain-runtime residuals plan for retirement ([e7fb1f4](https://github.com/minipuft/claude-prompts-mcp/commit/e7fb1f4868f62fa23777162c42876e316292c7c6))
* **resources:** apply the do-or-kill row lifecycle to the delegation plan ([dc97413](https://github.com/minipuft/claude-prompts-mcp/commit/dc97413bc941557ad13e253cc474978ae0438dcf))
* **resources:** convert filed-not-fixed findings into owned rows (D.1/D.2) ([32bd77c](https://github.com/minipuft/claude-prompts-mcp/commit/32bd77c311e172f1e4edb581419eccd2348900ce))
* **resources:** correct the framework lifecycle plan's G2 before dispatch ([07c6364](https://github.com/minipuft/claude-prompts-mcp/commit/07c63640249c304760a46de27a6b4f2e82456567))
* **resources:** flip the integration claim from inferred to observed ([913f00b](https://github.com/minipuft/claude-prompts-mcp/commit/913f00b09097a89617c96778f1440dbb9a6f69a9))
* **resources:** record the framework lifecycle live drive and close every tier row ([088d01d](https://github.com/minipuft/claude-prompts-mcp/commit/088d01d47d01b87f2d03d4eed408ce4886cc101e))
* **resources:** record the honesty audit that found this fix incomplete ([0fd0f21](https://github.com/minipuft/claude-prompts-mcp/commit/0fd0f21c2ed16c59b86dec1845fdee255833f937))
* **resources:** record the run cancelled by its own documentation ([6a476c8](https://github.com/minipuft/claude-prompts-mcp/commit/6a476c801d987f7226ec95f8fb080becb4930953))
* **resources:** record what today's green run did and did not observe ([ca7839c](https://github.com/minipuft/claude-prompts-mcp/commit/ca7839ca5a2c6064f87fbee1d23c287e3ee70bd4))
* **resources:** retire a deviation row that outlived what it described ([0cad516](https://github.com/minipuft/claude-prompts-mcp/commit/0cad51699a044d61c2dad79028a4ee26667ea8ba))
* **resources:** retire the framework lifecycle plan and re-home what outlives it ([bf3c9ac](https://github.com/minipuft/claude-prompts-mcp/commit/bf3c9ac64df9e2dc0fc78c0882eb035f16878bf5))
* **resources:** retire the sub-agent delegation contract plan ([#240](https://github.com/minipuft/claude-prompts-mcp/issues/240)) ([f2e685f](https://github.com/minipuft/claude-prompts-mcp/commit/f2e685f4b72f04aba4e6ebf8c01da11e2fc5cea2))
* restore subagent-delegation-contract plan to active ([8dc4d8a](https://github.com/minipuft/claude-prompts-mcp/commit/8dc4d8a54167140cb2e66150a562d65a196e3b64))
* retire delegation-contract plan and test roadmap as reference ([f2c0f12](https://github.com/minipuft/claude-prompts-mcp/commit/f2c0f128131c7b435112b5152640fa709dee49aa))
* **scripts:** commit the script-tools verification deviation log ([5f6a0f1](https://github.com/minipuft/claude-prompts-mcp/commit/5f6a0f1bd9a589ffb38b696d008cb1e6b0cdfc00))
* **scripts:** rule Q1-Q3 and record the template-injection finding ([acfa935](https://github.com/minipuft/claude-prompts-mcp/commit/acfa935381130a637fb5e23d6817213bd61ea941))

## [Unreleased]

### ⚠ BREAKING CHANGES

- **`resource_manager`'s gate parameter `gate_type` is renamed to `type`, and `gate_type` now writes the gate.yaml key of the same name.** The parameter published as `gate_type` never wrote `gate_type` — it wrote the gate.yaml key `type` (`validation` | `guidance`), which left the real `gate_type` key (`framework` | `category` | `custom`, the classification the loader filters framework gates on) with no parameter at all, because its name was taken. Every gate parameter is now named for the key it writes: send the validation/guidance value as `type`, and the classification as `gate_type`. A client still sending `gate_type: "validation"` is rejected by the schema rather than silently reinterpreted. No alias was added — one would ship a parameter already scheduled for deletion.
- **`resource_manager`'s `format` parameter is removed.** It was declared in the schema and the contract and forwarded by the router, and read by nothing, so `format: "json"` returned the same markdown as `format: "text"`. Measured with a positive control: the identical probe against `detail` finds two real read sites. Callers passing it were already getting the default output and now get a schema rejection instead of a silent no-op. `prompt_engine`'s `format` is a different parameter and is unchanged.
- **`resource_manager`'s `resource_type` gains `category`.** Adding a member widens the published union of the tool surface, which this repository's Public API Contract prices as breaking.

- **`session cancel` moved from `system_control` to `prompt_engine`.** Call `prompt_engine(chain_id:"chain-x#1", cancel:true)` instead of `system_control(action:"session", operation:"cancel")`, which now refuses and names the replacement; `list`, `inspect`, and `clear` remain on `system_control`.
- **`resource_manager` gains a `source_workspace` parameter for cross-workspace history.** It is honored only by `history` and `compare`; every other action, including `rollback`, refuses it, because a snapshot from another workspace may not exist locally and version numbers are per-workspace. The refusal is deliberate: silently scoping back to local would leave the caller believing they had restored the other workspace's version.
- **A gate criterion that cannot be enforced now fails to load instead of auto-passing.** `shell_verify` without a `shell_command`, and `script_tool` without a `script_tool_id`, now fail gate loading with a message naming the missing field.
- **A script tool with an unrecognized file extension is now refused instead of silently run as a shell script.** Auto-detection covers `.py`, `.js`, `.mjs`, `.cjs`, `.ts`, `.sh`, and `.bash`; declare `runtime:` explicitly (including `runtime: shell`) to use any other extension.
- **A script tool's `env` can no longer set keys that control which interpreter it resolves to, and its `workingDir` must stay inside the tool's own directory.** Refused keys: `PATH`, `LD_*`, `DYLD_*`, `NODE_OPTIONS`, `PYTHONPATH`, `PYTHONHOME`, `PYTHONSTARTUP`, `BASH_ENV`, `ENV`, `IFS`, `SHELLOPTS`, `BASHOPTS`, `PERL5LIB`, `PERL5OPT`, `RUBYLIB`, `RUBYOPT`; ordinary variables are unaffected. The same restriction applies to a gate's `shell_env`, and a new `MCP_SHELL_VERIFY_ALLOWED_DIRS` environment variable bounds a gate's `shell_working_dir`.
- **A gate's `shell_command` must now be an argv array; a bare string fails to load.** Replace `shell_command: "npm test"` with `shell_command: ["npm", "test"]` (the shipped `test-suite` gate is already migrated); the inline `:: verify:"..."` operator is unchanged and still takes a string.
- **`category_ship_status` is removed from the prompt mutation receipt.** It always reported `true` because the `.gitignore` file it checked no longer exists; the receipt still reports `resource_root`, the path the write actually landed in.
- **`dry_run` is removed from `resource_manager`; previewing is now `action: "preview"` with a required `preview_action`.** Replace `action:"update", dry_run:true` with `action:"preview", preview_action:"update"`, and likewise for `"delete"` and `"rollback"`. A preview no longer needs `confirm: true` — as a flag on `delete` it did, because the confirmation guard reads the action, so seeing what a deletion would cost meant confirming the deletion first. `preview_action` sent without `action:"preview"` is refused rather than ignored, and the response's `structuredContent` now reports `action:"preview"` with `preview_action` instead of `dry_run: true`. Removing a parameter narrows the reachable-shape union of the MCP tool surface, which this repository's Public API Contract prices as breaking.
- **`preview_action:"update"` is accepted for prompts only.** Gates and frameworks support `"delete"` and `"rollback"`; asking to preview a gate or framework update is refused by name. This closes a defect rather than adding a restriction — `dry_run` was forwarded to those two handlers and nothing there read it, so a preview of a gate or framework update performed the update and returned a success receipt.
- **`skills_sync` merges `dry_run` and `preview` into `preview` plus `preview_detail`.** Replace `dry_run: true` with `preview: true` (all four commands), and the old `preview: true` on `pull` with `preview: true, preview_detail: "diff"`. The two were never synonyms: one planned the run, the other rendered unified diffs and worked on `pull` alone. The `--dry-run` CLI flag becomes `--preview`, `--preview-detail diff` replaces the old `--preview`, and the `--json` run summary reports `preview` where it reported `dryRun`.
- **The prompt authority HTTP route `POST /api/v1/authority/prompts/{id}/dry-run` is now `/preview`.** Request and response bodies are unchanged; only the path moved, so the removed vocabulary does not survive on the surface an operator reads most often.
- **`chain_step_operation: "replace"` is removed; use `"update"` instead.** The `replace` member returned the step array untouched and let a supplied `chain_steps` array overlay, which is exactly what omitting the parameter already does — so omit it to replace the whole array. In its place, `"update"` edits one step at `chain_step_index` by overlaying `chain_step_data` onto it, leaving unnamed fields alone. Removing an enum member narrows the reachable-shape union of the MCP tool surface, which this repository's Public API Contract prices as breaking.
- **`system_control config` no longer serves `get`, `set`, `reset`, or `restore`.** A request naming any of them — or naming no operation at all — is now refused by name instead of silently answered with a dump of the whole configuration, which is what a malformed or unrecognized request used to fall back to. What remains: `list` (the whole loaded configuration), `keys` (the keys the packaged schema declares), and `validate` (the load-time schema result, or a per-key candidate check via the nested `config: { key, value, operation: "validate" }`). This is not "configuration cannot be written over MCP": `system_control(action:"gates"|"framework", operation:"enable"|"disable", persist:true)` still records that one setting in `config.json` — what is gone is a caller naming an arbitrary key or value to write, or restoring a backup. Set a key with `cpm config set <key> <value>`, reset with `cpm config reset --force`, and read one key with `cpm config get <key>` for now — the MCP `get` operation is held rather than dropped, and returns once a generated config shape can resolve most of the schema's declared keys.
- **A path setting the server cannot use now stops it instead of starting it on something you did not ask for.** This covers a config path set with `MCP_CONFIG_PATH` or `--config` that is missing, a directory, unreadable, or malformed JSON; a workspace set with `MCP_WORKSPACE` or `--workspace`, or a resources directory set with `MCP_RESOURCES_PATH`, that does not exist or is not a directory; and a `config.json` inside your workspace that is unreadable, a directory, malformed JSON, or not a JSON object. Each used to produce a server that started and served anyway: a named config file was ignored, a missing workspace was silently created, a missing resources directory quietly served the bundled prompts instead of yours, and a broken workspace `config.json` was replaced by built-in defaults. The server now exits non-zero before serving anything, on STDIO and Streamable HTTP, and prints to stderr which variable or flag was set, its value, the path it resolved to, what is wrong with it, and what removing it would fall back to. Fix the path, or unset the variable (or drop the flag). A workspace with no `config.json` still uses the packaged one, an empty value still counts as unset, and `MCP_RUNTIME_ROOT` is still created when missing. The `skills-sync` CLI refuses a missing `MCP_RESOURCES_PATH` the same way.

- **`config.json` now requires `"version": 5`, and several settings moved or left the schema.** `frameworks.systemPromptFrequency`, `frameworks.systemPromptTarget`, `frameworks.gateGuidanceFrequency`, `frameworks.gateGuidanceTarget`, `frameworks.styleGuidance`, `frameworks.styleGuidanceFrequency`, and `frameworks.styleGuidanceTarget` are replaced by nested `frameworks.injection.{systemPrompt,gateGuidance,styleGuidance}.{enabled,frequency,target}`; `advanced.sessions` moves to a root `chainSessions`; and `versioning.max_versions` / `versioning.auto_version` are renamed `versioning.maxVersions` / `versioning.autoVersion`. `server.version`, `gates.enforcePendingVerdict`, and `resources.prompts.defaultRegistration` are gone outright, and are refused as unrecognized keys rather than silently ignored. The `analysis` section is not exempt from this reshape: it is removed from the 5.0 schema entirely, not merely deprecated — see the dedicated `analysis` entry below. Add `"version": 5` and move each setting to its new home; a file with no overrides needs only `{"$schema": "./config.schema.json", "version": 5}`.
- **`server.transport` no longer selects the transport; `--transport=streamable-http` does.** A `config.json` still setting `server.transport` to anything but `"stdio"` now refuses to start the server, naming the value it found and the `--transport` flag to pass instead; a value left at `"stdio"`, or the key omitted entirely, draws only the ordinary schema warning. Remove `server.transport` from your file and launch with `--transport=streamable-http` (or `--transport=both`).
- **`cpm config set` and `system_control(action:"config", operation:"keys")` now work from the 5.0 key set.** Every key this reshape moved or removed — `server.transport`, `gates.enforcePendingVerdict`, `resources.prompts.defaultRegistration`, the snake_case `versioning` keys, and `advanced.sessions` — is no longer settable or listed; both surfaces now read the same generated table `config.schema.json` does, so a key that left the schema leaves both at once instead of drifting out of sync.
- **A framework's `toolDescriptions` text is now appended after each tool's own description instead of replacing it.** Every tool now serves its own description first — its actions, parameters and syntax — followed by the active framework's `toolDescriptions.<tool>.description` under an `ACTIVE FRAMEWORK [<type>]:` heading; `parameters` text for `prompt_engine` and `system_control` is appended to each parameter's own description the same way. If your framework's `description` was written as a full replacement for the tool's text, it now appears after the tool's own text, so the model reads both. Rewrite it as the framework's own guidance only: say what the framework adds, and don't restate the tool's name, actions, resource types, parameters or syntax. A copy of those goes stale when the tool changes. No YAML key changed, so a framework that loaded before still loads.
- **The `analysis` config section is removed from `config.json`'s 5.0 shape, not just deprecated.** It is gone from the `Config` type and the generated schema. A `config.json` with no `version` key that still carries `analysis.semanticAnalysis.llmIntegration.*` is read as a 4.x file: the section is dropped on load and named in the one-time translation notice, which points to the `%judge` modifier or `gates.evaluation.defaultMode` for model-graded gate evaluation instead. A file already declaring `"version": 5` that still carries `analysis` is flagged by the ordinary schema check, the same as any other key the schema no longer declares.
- **`gates.directory` is removed from `config.json`.** It was never read — the gates directory always comes from the resources path, not this key. A `config.json` with no `version` key that still sets it is read as a 4.x file: the key is dropped on load and named in the one-time translation notice. A file already declaring `"version": 5` that still sets it is flagged by the ordinary schema check instead.

### Added

- **`resource_manager` manages prompt categories.** `resource_type: "category"` accepts the same actions every other resource kind does, and writes the `category.yaml` that gives a category its name, its description, and the two MCP defaults its prompts inherit. All five of those fields had readers in the loader long before anything could write one, so the only way to set them was to edit a file by hand — which this project's first rule forbids. The resource is the declaration, not the directory of prompts around it: `create` succeeds on a directory that already holds prompts, and `delete` removes the declaration while every prompt keeps serving under a name derived from the directory again. Nothing validates a `category.yaml` when it is read, so the write is where it is checked — a malformed document is refused and rolled back rather than degrading silently to defaults.
- **A prompt that fails to load can now be repaired by the only tool allowed to author it.** The loader records refused prompt files instead of discarding them, so `resource_manager` can reach a file that is on disk and not in the catalog: `inspect` names the file, its root, its category and the load error where it previously answered `Prompt not found`, `list` reports every quarantined file, and `update` writes the repair back to the file that is actually broken, clearing the record once it loads.
- **A broken prompt in your workspace no longer silences the one it was overriding, and no longer does so quietly.** A workspace file that fails to load leaves the bundled definition serving, exactly as before — but `list`, `inspect` and the startup log now say which root supplied the definition, which nearer file was skipped, and why. An edit that is inert is visible rather than invisible.
- **A gate or framework that fails to load can now be repaired by the only tool allowed to author it.** The same treatment prompts got: the gate and framework loaders record refused files instead of discarding them, so `inspect` names the file, its root and the load error where it answered `Gate '<id>' not found`, `list` reports every refused file, and `update` writes the whole definition back to the file that is actually broken and says whether it loads afterwards. `reload` now names the loader's own reason instead of telling you to check whether a file exists in the one case where it provably does. A broken copy in your workspace still leaves the bundled gate or framework serving — and now says so, names the root that is answering by path, and says when repairing the file would change what serves. Both surfaces say it: `inspect` on the shadowed id, and the quarantine section of `list`, which previously announced no shadow for a framework at all. Nothing in a refused file's own body is returned: it is the content that failed validation.

- **The prompts the README tells you to run now ship with the server.** `tech_evaluation_chain`, `research_chain` and `review` were the first command of every documented install path but were not in the package, so a fresh install answered each with prompt-not-found. They ship now, along with the eight prompts their chains call, in `development` and a new `analysis` category: 51 bundled prompts across 9 categories.
- **A gate's `severity` and `enforcement_mode` are now settable through `resource_manager`.** Both were in the gate file schema and read at runtime, but no tool parameter reached them, so every tool-authored gate silently took the loader defaults (`severity: medium`, enforcement derived from severity) and no caller could tell. Omitting them still preserves whatever an existing gate declares, so updates that say nothing about severity continue to leave it alone. The gate file key `gate_type` (`framework` | `category` | `custom`) stays unauthorable for now: the parameter named `gate_type` already writes the different key `type`, and resolving that collision is a rename.
- **A framework's eleven advanced fields are now declared in the tool schema instead of riding an open passthrough.** `framework_gates`, `template_suggestions`, `framework_elements`, `argument_suggestions`, `judge_prompt`, `processing_steps`, `execution_steps`, `execution_type_enhancements`, `template_enhancements`, `execution_flow`, and `quality_indicators` were all accepted and written before this change — they were simply absent from the published schema, so no client could discover one without reading server source. `framework_gates` was the sharpest case: creating a framework requires it, so the cheapest way to learn its shape was to omit it and read the error. Each parameter now carries its element shape and names which file it lands in (`framework.yaml` or `phases.yaml`). Existing calls are unaffected; a payload whose element shape was already wrong is now refused at the call rather than at load, where it was rejected anyway.
- **`inspect` now reports the gate and framework fields that `create` and `update` write.** A gate's `severity` and `enforcement_mode`, and a framework's eleven advanced fields, were settable but had no read-back: the only way to confirm what a write landed was to open the file on disk. Each is shown only when the resource actually declares it, so a gate that says nothing about severity shows no severity line rather than the loader's default — the distinction that makes the reported value worth reading. Framework values are summarized rather than dumped: arrays as counts, strings as a truncated first line.
- **`resource_manager` can now remove a prompt field, not just set or preserve one.** `unset: ["system_message", "tools", …]` clears any of 11 fields. Supplying a value sets it and omitting it preserves it, so removal previously had no spelling at all — `system_message: ""` wrote an empty body rather than dropping the key, and fields the writer carries forward off disk could not be cleared. Unsetting `system_message` also deletes `system-message.md`. `name`, `category`, `description` and `user_message_template` are refused by name: they stay settable, but a prompt missing one does not load.
- **`tool_operation` separates unbinding a script tool from deleting it.** Omit it and a supplied `tools` array replaces the binding, leaving dropped ids' `tools/{id}/` files on disk; `"add"` unions with what is already bound; `"remove"` names ids in `tool_ids`, unbinds them and deletes their directories, which is why it alone requires `confirm: true`. `tool_ids` without `tool_operation: "remove"` is refused rather than ignored.
- **A blocking unknown can now interrupt a running chain instead of quietly bending it.** A `blocking:true` observation returns a structured `chain_interrupt` (the unknown, affected steps, remaining plan, and accepted exits) in the response; declaring `budget.pauseOnBlocking` instead holds the run until `gate_action:"resume"` or `gate_action:"accept_alternative"` clears it.
- **`prompt_engine(remainder:…)` lets you rewrite the rest of a running chain.** Submit replacement Workflow IR nodes with `mode:"replace"` (discards everything after the current node) or `mode:"append"` (extends it); a `chain_id` call whose `command` starts with `-->` is the string-shorthand equivalent. Each contributed node accepts only `{id, promptId, stepName, args, delegated}`, and the server allows one accepted remainder per unknown id plus a per-run ceiling.
- **`delegated` and `inlineGateCriteria` are now declarable step fields.** A chain step can request context isolation without naming a model tier (`delegated: true`), and can carry raw `::` gate tokens instead of only pre-resolved `inlineGateIds`; both reach YAML `chainSteps`, the Workflow IR, and `-->` chains.
- **YAML chains, the Workflow IR, and `-->` chains now share one step schema.** A YAML chain step can declare `args` (merged over the run's arguments for that step) and chain-level `edges` (dependency constraints) and `budget`; a step's `delegation` field is no longer read at runtime and is now stripped rather than rejected, so existing YAML keeps loading.
- **`GET /api/v1/catalog/prompts` now lists the full served prompt catalog.** Previously only the per-prompt route existed, so a catalog adapter could fetch a prompt it already knew the id for but never discover one; the new route returns metadata-only summaries under a `prompts` key and honors the same read-token guard as the item route.
- **`prompt_engine.inputs` now accepts typed structured arguments.** Nested objects, arrays, quotes, and backslashes bypass the command-string grammar entirely; precedence is inline arguments, then `inputs`, then legacy `options`, then prompt defaults.
- **Prompt creation now supports preview, write receipts, and optimistic concurrency.** `resource_manager(resource_type:"prompt", action:"validate")` runs creation validation without writing; successful create/update calls return the affected files and current version, and `expected_version` refuses a stale update before it writes.
- **Two new reference-grounded creative workflows are available.** `>>design_muse` turns supplied visual evidence into an approval-gated design direction; `>>practice_capture` preserves references, techniques, reactions, and unknowns, defaulting to evidence retention only — framework promotion is never applied without confirmation.
- **Source modules can now declare a `module.yaml` descriptor naming their boundary and lifecycle.** A guarded scaffold creates new boundaries, and validation checks descriptor coverage and catalog drift against structured JSON output.
- **`prompt_engine(chain_id, handoff:true)` hands a running chain off to another client.** It mints a single-use token; calling `prompt_engine(claim_token:"…")` from another client (Codex, OpenCode, a different Claude Code conversation) transfers ownership and resumes the run in the same call.
- **`script_tool` gate criteria now actually execute.** Declare `script_tool_id` naming a registered script tool; gate review runs it with JSON on stdin and reads back `{ passed, reason?, details? }`, so the script's own explanation appears in review feedback instead of a generic message.
...
- **The server now checks `config.json` against its schema at startup — and the published package ships that schema, which was previously missing from the tarball.** Before, an installed server had nothing to validate against, and a `$schema` reference in `config.json` pointed at nothing. The check reads the schema shipped beside the server, not the config's own `$schema` line, so a config loaded from elsewhere via `MCP_CONFIG_PATH` is still checked. A mismatch never stops the server: it prints one stderr warning naming the file, the section, and the key, e.g. `/gates: must NOT have additional properties (enabld)`, and stays quiet on repeated warnings for an unchanged file; a schema that can't be read is reported as not checked, never as an invalid config. `system_control` with `action: "config", operation: "validate"` sent without a `config` object now reports this same check — matched, each mismatch, schema unreadable, or not checked — instead of returning the entire configuration.
- **`config.schema.json`, and the table `cpm config set` / `system_control(operation:"keys")` validate against, are both generated from one TypeScript source.** `npm run generate:config-schema` derives the JSON Schema and a settable-key table — 67 dotted leaves, each carrying its type, enum, and numeric bounds (`server.port` accepts 1024–65535, for example) — from the `ConfigFile` type, and `npm run validate:config-schema` fails whichever one has drifted from it. A bound the old hand-written switch enforced is now read from the schema instead of duplicated in code.
- **The shipped `server/config.json` now holds only `$schema` and `version`.** Every setting's default now lives in code rather than in the file a fresh install copies, so deleting a key from your own `config.json` reverts it to the built-in default, not to whatever the packaged file happened to carry.
- **`hooks.expandedOutput` still defaults to `true` when `config.json` says nothing.** The shipped file previously carried that default explicitly; now that it ships with no `hooks` section, the hook's own fallback matches what every install already saw.
- **`system_control(action:"config", operation:"get")` returns one key's effective value and where it came from.** Send `config: { operation: "get", key: "<dot.path>" }`; the reply reports the value as JSON and a `source` — `file` (the config file sets it), `default` (the built-in default), `environment` (an environment variable overrides the file and default), or `deferred` (nothing has resolved it yet). An undeclared key, or a `get` with no key, is refused by name rather than answered with a listing. See [Config Operations](docs/reference/mcp-tools.md#config-operations).

### Changed

- **A gate, framework or style in your workspace now wins over the same id in the server's resources directory, the way a prompt already did.** Prompts have followed "same id in your workspace wins" since the overlay was introduced, and the resource index agreed — but gates, frameworks and styles resolved the other way, so `<workspace>/gates/code-quality/` was loaded, indexed, and then quietly outranked by `<workspace>/resources/gates/code-quality/`. All five resource kinds now use one order: workspace overlays first, then the writable resources directory, then the definitions shipped with the server. What a write lands in is unchanged, so a write to the resources directory for an id an overlay also defines will not change what is served — remove the overlay copy, or edit it instead.
- **The bundled `deep_analysis` chain now asks how far its findings can be trusted.** Its synthesis step adds a critical assessment — the strength of the evidence behind each key insight, the limits or likely bias of its sources, and whether it rests on evidence or inference — and its implications name who the findings affect.
- **Delegated chain steps now run on the host's default general agent; the plugin no longer ships `agents/chain-executor.md`.** With no `agentType` declared, a Claude Code handoff names `general-purpose`; other clients (Codex, Gemini, OpenCode, Cursor) get no `agent_type` parameter and use their own default instead. Agent names now pass through exactly as written — **migration**: a chain that explicitly set `agentType: chain-executor` should remove the key or name a real host agent.
...
- **The `config.json` shipped with the package now logs at `info` instead of `debug`.** Set `LOG_LEVEL=debug` for verbose logs. It also drops the `gates.mode` key, which no runtime reader ever consulted.
- **A `config.json` with no `version` key is read as a 4.x file and translated to the 5.0 shape in memory, on load.** The seven flat `frameworks.*Frequency`/`*Target`/`styleGuidance` keys become nested `frameworks.injection.{systemPrompt,gateGuidance,styleGuidance}.{enabled,frequency,target}`, `advanced.sessions.*` becomes root `chainSessions.*`, `versioning.max_versions`/`auto_version` become `maxVersions`/`autoVersion`, and every `<section>.mode: "on"|"off"` becomes its boolean; `server.version`, `server.transport` (when `"stdio"`), `gates.enforcePendingVerdict`, `resources.prompts.defaultRegistration`, and the whole `analysis` section are dropped. The file on disk is never rewritten and no action is needed — the server prints one notice per process naming every rename and drop. A file already declaring `"version": 5` is read as written. This translation is removed in 6.0.0.
- **`system_control(action:"config", operation:"list")` now shows every effective value, defaults included**, not just what the file overrides.

### Fixed

- **`skills-sync` and the prompt delete lookup skip the directories the server skips.** `export` treated a `_`-prefixed directory as a category and a category-level `tools/` as a prompt, and reported each as a skipped prompt. `clone` copied a skill's `resources/tools/` or `_drafts/` step into the new prompt, where `tools/` holds script tools. The prompt delete lookup also listed `_`-prefixed files and `tool.yaml` as prompts. A new `validate:prompt-walks` check finds every function that walks a prompts tree and fails when its module does not use the shared skip rules.
- **`cpm list` and `cpm validate` now see only the prompts the server serves.** `cpm` read any directory under a category as a prompt, so it listed and validated prompts under `_drafts/`, under a category's `tools/` directory, and under top-level `backup/` or `node_modules/`. It also listed a `prompt.yaml` placed directly in a top-level directory, which the server treats as a category, not a prompt. A broken prompt in any of those places no longer fails `cpm validate`. `cpm` now uses the server's own rules instead of a copy.
- **Every part of the server now agrees on which top-level directories are prompt categories.** The server never served prompts under a top-level `backup/` directory, but the resource index, the startup change comparison, `validate:prompts`, `skills-sync export` and `cpm` all treated `backup/` as a category, so its prompts were indexed, announced, validated and exported. A top-level `node_modules/` was the opposite case: the server served it as a category but never watched it for changes. Neither directory is a category anywhere now, and neither is a `.`- or `_`-prefixed one. `validate:prompt-walks` now also fails when a module that walks a prompts tree does not use the shared category rule.
- **A prompt category named `tools` is now indexed and announced, and prompts under a `_`-prefixed directory no longer are.** The resource index and the startup change comparison treated `tools` as reserved at the top of the prompts directory too, so a whole category the server serves was missing from both. The index also walked into directories such as `_drafts/`, which the server never serves, and indexed what it found there. `validate:prompts` now skips the same directories the server skips, so a file under a prompt's `tools/` or under `_drafts/` can no longer fail CI as a prompt.
- **Three architecture checks that read as enforcement now enforce something.** `validate:arch` stated in its own header — and two source comments cited by name — that nothing may import the server's composition root, and no rule expressed it: a planted `modules/` file importing `runtime/` cruised with zero errors while the control that should fail did fail, so the harness was working and the rule was simply missing. `no-imports-into-runtime` now forbids that direction, and the three MCP tool files that were reaching into the composition root to record resource changes read the change log through a contract in `shared/` instead; the log itself, and what it records, are unchanged. `cli-shared-no-runtime` — cited by the CLI barrel's own header and by the CLI guide since the CLI split, and never written — now exists, and checks the whole reachable closure rather than the first hop, which is what keeps the server out of the CLI bundle. `no-runtime-state-direct-access` was deleted rather than repaired: it aimed at a directory the server writes at run time, so no module could ever match it, and two of its three exemptions named files that no longer exist.
- **A prompt you edit in a workspace overlay, or in the bundled catalog, now reloads.** The catalog is assembled from the bundled tree, your primary prompts directory and every workspace overlay, but only the primary directory was watched — so an edit to a prompt that lives in either of the others was never noticed, and the previous version kept being served until a restart. Every root the catalog is assembled from is now watched, using the same resolution the loader itself uses. An overlay directory that does not exist when the server starts is still not picked up until a restart.
- **Editing a framework's files now reloads it, and deleting its folder now releases it, instead of both waiting for a restart.** Every framework file change was watched and then discarded: the reload event was built without the framework id it needed, so the handler refused it with `missing frameworkId, skipping`. An edited `framework.yaml` kept serving its previous guidance, and a framework whose folder you removed stayed selected and kept rendering. Both now take effect within about a second — an edit re-reads the definition, and a removal moves the selection to the framework named by `frameworks.defaultFramework`. Files a framework carries that are not YAML, such as `system-prompt.md`, also reach the reload now instead of logging a warning and stopping.
- **Repairing or breaking a style now takes effect without restarting the server.** The server kept two separate style loaders — one whose record of unloadable files decided what `resource_index` contains, and one the style hot reload actually refreshed. The two agreed the moment the server started and diverged from the first edit afterwards, so fixing a broken `style.yaml` left it missing from the index until a restart, and breaking a working one left it listed there as though nothing had happened. Anything reading that index, including the Python hooks, was offered a style the server cannot load or denied one it can. There is one style loader now, and the hot reload refreshes it. A loaded style also records which root it was served from, the way gates and frameworks already did, so a style shadowed by a copy in another root can be named by path rather than by id alone.
- **Hot reload now watches your prompts folder itself, not the folder above it.** It watched the parent of the prompts folder instead — your whole `resources/` folder, or with the older `<workspace>/prompts` layout the entire workspace, `logs/` and `runtime-state/` included — and polled everything in it. Every other resource type has its own watcher, so edits to prompts, gates, frameworks and styles still reload as before. Editing a framework file now triggers one prompt reload instead of two.
- **A resources folder you create while the server runs is now picked up, and something you create then delete right away no longer lingers.** A workspace overlay folder (`<workspace>/prompts`, `gates`, `frameworks` or `styles`) that did not exist at startup was ignored until a restart; it is now read and watched as soon as it appears. And when a create wrote the first file into a folder that did not exist yet, deleting that entry within about a second went unnoticed — a framework created, selected and deleted stayed selected, and a gate stayed listed. Once the watcher reaches a new folder, the server now checks what it holds against the disk and drops anything that is gone.
- **`system_control(action:"changes")` now records edits in your workspace overlay folders, not just your primary ones.** A resource present in two folders is recorded once, for the copy that is served, so editing a copy another folder overrides no longer logs a change that did nothing. A nested chain-step prompt is logged under the same id the startup comparison uses. The bundled catalog is still not tracked unless it is your primary folder.

- **A resource file that fails to load in the root you edit is no longer silent because a higher root answers.** Gates, frameworks and styles stopped reading roots once one of them served an id, so a malformed file below the winner was never opened: no warning, no `list` entry, no repair target. That used to hide a broken workspace overlay; since overlays started outranking the writable root it hid the file an operator is most likely to have just edited — a broken `<workspace>/resources/gates/foo` behind a legacy `<workspace>/gates/foo` was invisible on every surface. Every root holding the id is now read and every refusal recorded, while the id is still served by the first root that yields a valid definition, unchanged.

- **`inspect` on a category declared in another root now says whether your update would actually displace it.** It said an update "writes your own copy under <your resources root>, which then takes precedence". A write lands in your resources root, and a workspace overlay outranks that root — and an overlay is exactly where a category declaration in "another root" most often lives, because overlays are read last and win. So for the overlay case the sentence was false in both halves. It now names the root that wins: either your copy would take precedence over the bundled tree, or the overlay outranks your resources root and the category would still be declared from there, so edit it where it lives.
- **Repairing a prompt whose broken file is in another root no longer reports the repaired prompt as absent.** A prompt write always lands in your own resources root, so repairing a malformed file that lives in the bundled tree leaves that file exactly as broken while a working copy appears in yours. The response called that "Still quarantined — the prompt remains absent from the catalog", and the second half was false: the prompt was being served, from the copy the same call had just written. It now distinguishes the three outcomes the gate and framework repairs already did — repaired, refused-file-in-another-root, still-refused — and says which root serves the id afterwards, read back from the loader rather than assumed. Two bundled-resource refusals that said your copy "takes precedence" now say what it takes precedence over.

- **A repair no longer tells you your copy is serving when it is not.** Repairing a quarantined gate or framework answered, verbatim, that it "wrote `<path>`, which takes precedence, so `<id>` now serves your copy" — and a repair writes the root a `resource_manager` write lands in, which workspace overlays outrank. Both halves could be wrong at once: the write takes precedence over nothing, and the id can go on being answered by an overlay. The response now reads which root actually served the id back off the loader after the reload, and says either that your copy is what answers, naming the root, or that `<other root>` still outranks it and the copy you just wrote is not what answers. `inspect` and `list` drop the same assumption: a refused file behind a served one is "another file", not "a nearer file", and repairing it changes what serves only if its root outranks the one answering. The three places that documented their repair target as matching root precedence now say what is actually true of it — a repair goes to the root you can write to.

- **The startup warning for a prompt file that failed to load now names the root that is answering instead of it.** It said `(another root is serving this id — your edit is not live)`, which leaves the operator to work out whether the bundled tree, the primary root or an overlay took over — the fact that decides whether repairing the file changes what serves. It now names that root by path, exactly as `resource_manager`'s `list` and `inspect` already did, and a refused file that shadows nothing still carries no such clause.
- **An edit to a chain's step prompt is now reported in the change log.** The startup comparison stopped descending as soon as a directory held a `prompt.yaml`, on the reasoning that such a directory is a prompt rather than a folder of them — but a chain directory is both, holding its own definition alongside its steps. Fifteen step prompts ship below that line (every step of `deep_analysis`, `implementation_plan`, `quick_decision` and `scaffold_project`), so editing one outside the server produced no `added`, no `modified` and no `removed` at the next start. They are now reported, under the same qualified id the server serves them by (`deep_analysis/deep_dive`), rather than under a bare step name nothing answers. The first start after upgrading reports each of them once as an external addition — they are entering the change log for the first time — and nothing is reported as removed.
- **`create` no longer overwrites a gate file the server refused to load.** `create` checked the gate registry, and a refused file is absent from it — so creating a gate whose id already existed on disk in a broken state rewrote that file and reported plain success, never mentioning that anything had been there. That was one of several dead ends while a quarantined gate was unreachable by every verb; once `update` learned to repair one, `create` was the remaining door and the only one that gave no signal. It now refuses, naming the refused file, its load error, and the `update` verb that repairs it. A gate the registry does know still refuses with the message it always did, and an id nothing on disk claims still creates normally. Frameworks were affected in a narrower shape, closed by the next entry.
- **`create` no longer writes over a framework whose only file is a broken one in another root.** Unlike gates, framework creation did consult the filesystem — but only the directory it writes into, so a framework whose sole file is a refused one in the bundled tree or another resource root was invisible to all three of its existence checks. `create` then wrote a fresh copy that took over the id, reported plain success, and never mentioned the broken file, which stayed exactly as broken. It now refuses, naming the refused file, its load error, and the `update` verb that repairs it. A framework the server already serves still refuses with the message it always did, one whose broken file is in the writable root still refuses with the directory check's message, and an id nothing on disk claims still creates normally.
- **Repairing a quarantined gate or framework now records a version.** Both repair paths deliberately wrote no version row, on the sound reasoning that a file which never loaded has no prior state to diff against — but `version_history` is durable and nothing regenerates it, so the first recorded version of a repaired resource was its *second* edit. A repair now records the state it produced as version 1, described as a repair and carrying no diff, and the response says what was recorded instead of saying that nothing was. The prior broken content is never recorded: it is the content that failed validation.
- **A hot reload no longer re-publishes the gates, frameworks and styles the server refused to load.** The index that Python hooks read withholds a file its loader refused, and did so correctly at startup — but the re-sync that runs after every hot reload was handed the prompt loader's refusal record alone, so the first reload put every refused gate, framework and style back into `resource_index` for a hook to hand to `prompt_engine`, which rejects them. The refusal record for all four kinds is now assembled once, at startup, and the reload reads that same one. A file repaired between two reloads still appears at the next sync, as it did before.

- **The resource index and the change log no longer report resources the server cannot load.** Both ran their own filesystem walk and accepted any file whose YAML parsed, which is considerably more than the loaders accept — so `resource_index`, the table the Python hooks read, could name a prompt or gate that `prompt_engine` then rejects, and the startup change log recorded an external `added` for a file that never entered the catalog. Each now reads what the loaders refused: the index withholds such a file and deletes a stale row for one, reporting it as **refused** rather than as removed, because a file still sitting on disk has not been deleted; the change log records no event at all and keeps the file's key live, so a repair is later reported as the `added` or `modified` it actually is. A refused file's absence is a correct index, not a missing one, and the startup warning says so by name and path.
- **A prompt written as a single `{category}/{id}.yaml` file is now indexed.** The index walk only ever looked at directories, so the second of the two supported prompt layouts reached no row at all — such a prompt loaded and served while being invisible to every hook that reads the index, including prompt suggestion. Nested single-file prompts are indexed under the qualified id the server answers to (`chain/step`, not `step`), and where both spellings of one id exist the directory form wins, which is what the loader already did. `category.yaml`, a script tool's `tool.yaml`, `_`-prefixed files and anything at the prompts root are still correctly absent.
- **The startup change log no longer reports a category declaration as a new prompt.** Its filesystem walk called any `.yaml` file in a prompts directory a single-file prompt, so `category.yaml` — which ships under `guidance/` — was announced at every startup as an externally added prompt with the id `category`, and so would a script tool's `tool.yaml` or a stray file at the prompts root. The rule for which filenames are a prompt is now the prompt loader's own, shared rather than copied, so the three walks that ask the question agree on the answer. A genuine `{category}/{id}.yaml` prompt is still reported.
- **A script tool whose definition fails to load is no longer offered as an available tool.** Its index row was deliberately kept so the removal sweep would not report it as deleted — a file that fails validation is still on disk — but `skills-sync` reads the same rows, so the tool stayed published while being unusable. It is now reported as **refused**: no index row, absent from the tool listing, and still not counted as removed. It is no longer counted as a sync error either, because nothing failed on the indexer's side; look for it on the refusal line, which names the tool directory to repair. A valid tool under the same prompt is unaffected.
- **A response style the server cannot load is no longer listed in the resource index.** Prompts, gates and frameworks already withheld a file their loader refused; styles were the fourth kind the index walks and the only one with no such record, so a `style.yaml` that failed validation left the catalog and stayed in the table the Python hooks read. It is now reported as **refused** by name and path at startup, and a stale row for one is deleted. The startup inventory line for styles also changes meaning as a result: it counts the styles that actually load, not the directories that exist, so a broken style now shows up as a smaller number rather than not at all.
- **An `update` on a prompt that could not load reported success and wrote the wrong file.** It answered `✅ Prompt Updated`, created a *second* prompt under `general/`, printed a move that never happened, and left the broken file untouched. The repair now lands on the file that is actually broken.
- **Two framework fields were written to disk and never read back.** `judge_prompt` was loaded from its file and dropped before the framework was reassembled, and `execution_type_enhancements` was written to `phases.yaml` and read by nothing — so an update carrying either could not preserve it. Both now survive the round trip. The mapping they were missing from is now a declared field table rather than fifteen near-identical blocks, so a future field cannot be omitted the same silent way.
- **A category's metadata written into a workspace was discarded when the same category also shipped with the server.** The catalog merged categories by display *name* and let the bundled copy win, so a personal `category.yaml` was written correctly, loaded correctly, and then dropped. Categories now follow the same rule prompts do — same id, your copy wins — and two categories that merely share a display name no longer evict each other.
- **The HTTP `create_category` endpoint required a name and a description and wrote neither.** It created an empty directory and let the loader invent both. It now writes the declaration it was given, through the same path the MCP tool uses, and to the resources root the server actually serves from rather than the one named in the config file.

- **Enabling or disabling the framework system now reports a save that failed instead of answering with success.** The toggle updated memory and then wrote to the database, but the write's failure was caught and discarded inside the framework state store, so `system_control(action:"framework", operation:"enable"|"disable")` answered `✅ Framework System Enabled` while the database still held the old value, and the setting reverted at the next restart. The same toggle applied from `config.json` was started and never waited for, so its failure was reported to no one. The write is now awaited and its failure surfaces: the tool answers with the error, and a toggle applied from configuration at startup stops the server with that error rather than serving state it failed to record.
- **A Streamable HTTP request whose server cannot be built now reports why.** Every HTTP request is served by a freshly built server instance. When building one failed, the request answered `Internal server error` and nothing said what went wrong unless the failing step happened to log for itself — a failure while binding resources or prompts left no trace anywhere, so consecutive failing requests produced an opaque body and a silent log. The cause is now written to the server log, naming the stage that failed (tools, resources, or prompts). The request itself still fails: a half-built instance is never served, because a tool surface missing the tools that failed to bind reads as success to every client. Requests after it are unaffected, as they were before.
- **A create or update-prompt reply no longer suggests a gate that does not exist.** `ContentAnalyzer` suggested `basic_validation` on every analysis, and the prompt-analysis failure fallback suggested `execution_validation` — neither has ever named a gate under `resources/gates/`. Both analyzers have no gate registry to check a name against, so both now suggest none; `GateAnalyzer`'s registry-backed recommendation (for example `content-structure`), which the same reply also carries, is unaffected.
- **A `skills_sync` reply now states what the run did, instead of ending in progress lines with no summary.** `export`, `sync`, `diff`, and `pull` discarded the machine-readable report the run itself produced, so a caller — including on a preview — could not tell how many resources loaded or files were written without inspecting folders directly, and a preview's "nothing was written" outcome was never stated. The response now leads with resources loaded, files written (0 and labeled preview when previewing), a per-client write breakdown, directories pruned, drifted-resource counts for `diff`, and a failure count, all read from the run's own report.
- **Switching to a framework you created now works, and the selection survives a restart and the framework's removal.** The framework state kept its own list of frameworks, which held only the ones shipped with the package, so it could not resolve a framework created with `resource_manager` or stored in your workspace. After switching to one, every later request over Streamable HTTP failed with `Internal server error` until the server restarted, `tools/list` included, and over STDIO every prompt failed with `Active framework ... not found`. A restart replaced a workspace framework selection with the first framework available. The framework state and the tools now share one list, so a created or workspace framework can be switched to and stays selected across a restart. Deleting the active framework with `resource_manager` now selects the framework named by `frameworks.defaultFramework` and saves that selection, and the same default replaces a saved selection whose framework no longer exists at startup. The framework named by `frameworks.defaultFramework` cannot be deleted, and nothing is removed, until that setting names another framework. A server whose saved selection and `frameworks.defaultFramework` both name no registered framework refuses to start, with an error naming that setting, instead of selecting another framework. A change to `frameworks.defaultFramework` while the server runs applies to that fallback and to the delete refusal without a restart.
- **A prompt hot reload now picks up a `skills-sync.yaml` export change, instead of only a restart.** A prompt newly registered for export stayed in `prompts/list` until the server restarted, and one unregistered stayed hidden just as long: the filesystem-watch reload path refreshed prompt content and re-registered every prompt, but never recomputed which ones `skills-sync.yaml` exports — only startup, and a manual full refresh, did that. The exported set is now recomputed on that path too, so editing `skills-sync.yaml` and then touching a prompt file (or waiting for the next unrelated one) is enough; editing `skills-sync.yaml` alone still needs one of those, since it is not itself watched.
- **`@id` now applies a framework created while the server runs.** The command parser recognized only the framework ids that existed at startup, so after `resource_manager` created a framework, `@<id> >>prompt` was read as literal text and failed with a parse error until the server restarted. The parser now checks the current framework set on every command: a new framework applies at once, and after a framework is deleted `@<id>` is literal text again.
- **A server started over STDIO now writes nothing to stderr on a clean start.** A startup trace of about a dozen `DEBUG:` lines printed on every launch regardless of verbosity; it now prints only with `--verbose` or `--debug-startup`. Two warnings that described no real problem are also gone: `Invalid framework state, falling back to defaults` on a first start with no saved framework state, and `ChainSessionStore: no DatabasePort provided, persistence disabled` from a chain session store that went on to persist anyway.
- **Warnings and errors now reach stderr when the server runs over STDIO.** The logger previously printed nothing to the console under STDIO unless `CI` or `NODE_ENV=test` was set, so a warning or error your server raised existed only in its log file. Warnings and errors now go to stderr; info and debug messages follow them there only with `--verbose` or `--debug-startup`. Nothing is written to stdout, and CI and HTTP output are unchanged. The `config.json` schema warning goes through the same logger now, so its line starts with `[WARN]` like every other warning.
- **Tools are now described by their contract descriptions in every install.** The tool description loader looked for the generated descriptions in a directory that exists in neither the repository nor the published package, so every server silently fell back to built-in descriptions instead. The descriptions are now bundled with the server, so the fallback no longer triggers.
- **Skills sync now reads the prompts and gates in your workspace, not only the ones the package ships.** Every command read resources and `skills-sync.yaml` from the package folder, so with `MCP_WORKSPACE` set, as the Claude Code plugin sets it, exporting a workspace prompt or gate found nothing, and `clone` wrote into the folder a plugin update replaces. Sources now resolve the way the server resolves them: the bundled resources with the workspace layered over them, a workspace entry replacing a bundled one with the same identity, and `MCP_RESOURCES_PATH` taking precedence over the workspace. `skills-sync.yaml` is read from the workspace when it holds one, else from the package, and is no longer looked for beside an `MCP_RESOURCES_PATH` directory. `clone` writes into the workspace, `patch` writes under `MCP_RUNTIME_ROOT` when it is set, and `pull` refuses to write an edit into a bundled resource while a workspace or resources directory is configured, naming the file. The server's `prompts/list` reads the same file, so a prompt registered in the workspace's `skills-sync.yaml` is no longer listed beside its skill. An export reads `gates.harnessCovers` from the `config.json` the server reads, so it leaves out the same reminders the runtime does.
- **A server started with `--workspace` and no `MCP_WORKSPACE` now finds its own workspace's skills-sync sources over MCP.** `system_control`'s `skills_sync` action re-resolved its directories from the environment on every call instead of reading the server's own path resolver, so `--workspace` alone reported the package's `skills-sync.yaml` and could not see a resource the workspace held, while setting `MCP_WORKSPACE` worked correctly. `status` and `export` now resolve through the running server the same way every other resource type does; the standalone CLI (`npm run skills:export` and friends), which has no running server to read from, is unaffected.
- **`readme_improver` and `documentation_change` get their documentation gates back.** Gates now attach by the artifacts a prompt declares (B13), and neither bundled prompt declared any, so `information-placement`, `product-positioning-fidelity`, `prose-hygiene`, and `semantic-discoverability` silently stopped reaching them — a documentation prompt with no declared artifacts got none of the checks meant for documentation. Both now declare `artifacts.produces: [docs, readme]`, which they always write regardless of `doc_type`, and picked up `api-documentation` the same way. `code-quality`'s gate definition dropped its `prompt_categories` list, which had been unreachable since gates started attaching by artifact.
- **`system_control` now receives every parameter its actions read.** Its input schema declared 11 fields and dropped the rest before the action ran, so a `skills_sync` export with `preview: true` wrote skills for every registered client instead of previewing, and `client` did not narrow it. An `injection` override answered `Invalid injection type: undefined`. `config` restore and `maintenance` restart refused even with `confirm: true`, and the `changes`, `analytics` history and `execution_history` filters ran on their defaults. Each parameter is now declared with its type. The `changes` resource type filter is `resource_type`, the name `skills_sync` already uses; the documented `resourceType` never reached the action. `resource_manager` now declares `full_restart`, and its prompt `guide` receives `goal` and `include_legacy`, which it read but was never sent.
- **Creating a prompt, a gate or a framework now records the created state as version 1.** `resource_manager(action:"create")` previously saved no version at all, so its receipt reported `current_version: 0`, and the first `update` had to invent a "prior live state" bridge row for a resource that had never been recorded — a reader saw version 0, then a bridge, then 2. `create` now saves version 1 as part of the same write, so its receipt reports `current_version: 1`, `action:"history"` shows one row with a create description, and the first `update` saves version 2 directly with no bridge. The bridge row is unchanged for a resource created before this fix, or edited out of band, where the latest recorded snapshot genuinely differs from the live state.
- **A framework error now names the directory the framework actually resolves from, not the server's own install directory.** `reload`, and a `create` whose rollback could not finish, reported a path built from the server's package root — so with a workspace or personal resources library configured, the message pointed at a directory the framework never occupied. Each now resolves through the same writable and bundled roots the framework loader itself reads.
- **Updating a prompt stored as a single file (`{category}/{id}.yaml`) no longer leaves it defined twice.** The writer only ever produces the directory layout (`{id}/prompt.yaml` plus message files), and its lookup for a prompt already on disk skipped single-file matches, so an update wrote a fresh `{id}/` directory beside the untouched file — one definition updated, one going stale, both on disk, both served. An update of a single-file prompt now converts it to directory layout in the same write: the flat file is deleted, the directory is written, and the preview names the deleted file alongside the files it creates. The lookup scans every category, not just the update's target one, so a single-file prompt updated with a new `category` in the same call is converted and relocated together — before this, that combination left the flat file under its old category and a new directory under the new one, served under both.
- **A prompt update preview now shows the change the update makes, in the files it makes it in.** For a prompt stored as a directory — `prompt.yaml` beside `user-message.md` and `system-message.md` — the preview rendered the prompt's fields as a single YAML file named after the prompt, with long template lines rewrapped, while the update itself changed one line of `user-message.md`. The preview, the diff the update reports and the change counts saved in version history now all come from the write itself: each file is named by its path under the resources root, lines appear as the file holds them, a created file is diffed from `/dev/null`, a deleted one to it, and files the update leaves untouched are left out. A prompt rollback preview does the same.
- **A gate or framework update, and its rollback preview, now show the change in the files it is made in.** A gate is stored as `gate.yaml` beside `guidance.md`, and a framework as `framework.yaml` beside `system-prompt.md`, `phases.yaml` and `judge-prompt.md`, but the diff an update reported and the one a rollback preview showed rendered the saved fields as a single YAML file named after the resource — so a guidance edit appeared as a change to `gate.yaml`, and `guidance.md` never appeared at all. Both now come from the write itself, as prompt previews already do: each file is named by its path under the resources root, lines appear as the file holds them, and files the write leaves untouched are left out. The change counts saved in version history follow the same diff.
- **A framework update now writes its phases and judge-prompt content to the file names `framework.yaml` itself declares**, instead of always `phases.yaml` and `judge-prompt.md`. A framework can name its companion files with `phasesFile` and `judgePromptFile`, and the loader follows those names — but an update that touched phases or `judge_prompt` silently rewrote the declaration to the default names, orphaning the declared file with stale content, and an update that touched neither still wrote a stray default-named file beside the declared one while the declaration (and the loader) kept pointing at the original. The update diff and rollback preview now name whichever file each write actually lands in, and a declared name that resolves outside the framework's own folder is refused rather than joined.
- **Updating a gate without touching its guidance no longer rewrites `guidance.md` one byte short.** `resource_manager(resource_type:"gate", action:"update")` omitting `guidance` falls back to the gate's loaded guidance text, and that text had lost its file's trailing newline on load — so an update that changed only `activation` (or `description`, or anything else) rewrote `guidance.md` without it, and the repo's own commit-time formatting check then blocked on a file the caller never asked to change. Loading `guidance.md` now keeps its exact bytes, the same way the prompt and framework loaders already keep `system-message.md`, `user-message.md`, `system-prompt.md`, and `judge-prompt.md` verbatim. `create`, `update`, and `rollback` all write `guidance.md` through the same path, which now appends a trailing newline to guidance that lacks one instead of leaving it off — restoring a version recorded before this fix, whose guidance was already `.trim()`'d and unrecoverable byte-for-byte, now reconstructs the same one-newline ending every shipped gate has rather than reproducing the missing-newline defect a second way.
- **A gate update naming only `guidance` no longer rewrites `gate.yaml`.** `resource_manager(resource_type:"gate", action:"update")` planned and wrote `gate.yaml` on every update regardless of which fields were supplied, so a guidance-only edit re-serialized it into the writer's own key order and dropped any hand-authored comment — invisible in a hand-authored gate whose `gate.yaml` had never been touched, since a gate the tool itself had written re-serialized back to the same bytes. `gate.yaml` and `guidance.md` are now two independently-scoped writes: a field that lives in `gate.yaml` is what puts `gate.yaml` in scope, `guidance` is what puts `guidance.md` in scope, and the reported diff names only the file that actually changed. `create` and `rollback` still write the whole gate, both files.
- **A prompt you create through the Claude Code plugin, and its version history, now survive a plugin update.** Created prompts, gates and frameworks, gate and framework settings, argument history and version history were all saved inside the plugin's install folder, which Claude Code replaces on every update. The plugin now keeps them in the plugin data folder Claude Code preserves across updates, and its hooks read them from there. Nothing already saved in an install folder is moved: a prompt created before this release stays in that version's `server/resources/prompts/` folder, and creating it again with `resource_manager` saves it in the data folder.
- **A server given a workspace now saves the prompts, gates and frameworks you create inside that workspace, even when the workspace starts empty.** A workspace set with `MCP_WORKSPACE` or `--workspace` that had no `resources/prompts/` folder, or no `resources/gates/` or `resources/frameworks/`, sent each new resource of that type into the package's own folder, where the next reinstall or update deleted it. New resources now go to `<workspace>/resources/<type>/`, created by the first write; the bundled resources still load underneath, a workspace resource with the same id still wins, and an edit to a file in that folder reloads without a restart. A workspace that already keeps a type in the older `<workspace>/prompts/` layout keeps writing there. An explicit `MCP_RESOURCES_PATH` and a server with no workspace set write where they did before.
- **Disabling gates through `system_control` now survives a restart.** While gates are off, `prompt_engine` stops advertising `gates`, `gate_verdict` and `gate_action`, which is how a client saves the tokens those parameters cost — but the next server start advertised all three again, because the toggle was saved for your workspace and startup never read it back. A server now reads every saved toggle when it starts, on STDIO and Streamable HTTP. A toggle saved before workspace isolation, under no workspace, is adopted into the launch workspace once, if that workspace has no toggle of its own.
- **Deleting a framework the package ships is now refused, and names where it lives.** `resource_manager(resource_type:"framework", action:"delete")` refused only four of the eight bundled frameworks; `focus`, `liquescent`, `radiant` and `verify` were removed from the package tree in a default install, and the tool reported success. A comment claimed a second guard covered them, but that guard is reachable only when the framework directory is absent, which is never true for the ids it named. The same four also loaded as operator-created rather than built-in, so every surface reading `isBuiltIn` misreported them. Frameworks you created yourself are still deletable; `validate:shipped-frameworks` now fails when the declared set and the shipped tree disagree in either direction.
- **A failed resource update no longer leaves a version row describing a file that was never written.** The write and the version record describing it ran as two sequential steps in every prompt, gate and framework update and rollback, so a failure between them left one without the other — and `version_history` is durable, so a row pointing at a state no file ever held was permanent. Both halves now commit together: a failed write records nothing, and a failed record restores the files byte-for-byte. `validate:mutation-atomicity` fails when a version record is added outside a write transaction.
- **Prompt-id normalization now has a single implementation.** Two copies previously disagreed on nested chain-step ids (`deep_analysis/initial_scan` collapsed to `deep_analysisinitial_scan` in one of them); `npm run validate:prompts` now enforces the id convention (`snake_case` for `>>`/`-->` command ids, `kebab-case` for everything else) and accepts a `--root` flag to check an external prompt library.
- **Creating a prompt with a spaced category no longer reports failure for a write that succeeded.** `My Category` still writes to `my-category/` on disk as before; the post-write check now compares both sides through the same normalization instead of the caller's raw value against the reloaded slug.
- **A failed prompt mutation no longer opens with a success headline.** The response now chooses its headline after verification runs, so a failed write leads with the failure and the reason instead of `✅ **Prompt Created**` followed by a buried error.
- **The claims-conformance test suite no longer writes into the packaged resource tree.** A resources-path default previously outranked the test's isolated workspace, leaving real files behind on an otherwise-passing run; a teardown check now fails any run that adds a file to the packaged tree.
- **`npm run validate:prompts` now catches prompts that fail schema validation instead of silently dropping them from the catalog at load.** It also accepts `--root` to check a library outside the repository, and it found (and fixed) two gates in the bundled `practice_capture` prompt that had never loaded.
- **Editing a bundled prompt or framework no longer destroys the parts of it that aren't fields (chain steps, tool files).** An update now copies the resource's whole source subtree into your own resources root before editing, and the receipt notes that the resource has forked and will no longer track bundled updates; `inspect` reports where a loaded resource came from and whether editing it would fork a copy.
- **Deleting a bundled prompt, gate, or framework no longer reports a false "not found."** `delete` now states where the resource actually lives and that updating it (not deleting) is how to change it; deleting your own copy still succeeds and now says the bundled version is being served again.
- **A caller-supplied `category` or resource `id` can no longer write outside the resources root via a value like `../../ESCAPED`.** Every join built from payload data now resolves through a containment check, and `npm run validate:contained-resource-writes` enforces it on every run so a new resource type can't ship unguarded.
- **A chain step named `.` or `..` can no longer scaffold outside its parent prompt's own directory.** Naming it in a prompt's very first `chain_steps` — before any directory existed to catch it — wrote a stray `prompt.yaml` into the parent's category folder and reported success; naming it in an update to an existing chain was silently dropped instead, also reporting success. Both are now refused by name before anything is written.
- **The e2e test suite no longer inherits your local environment.** All server spawns now route through one helper that fully controls the child's environment, so a developer's `MCP_RESOURCES_PATH` can no longer leak a personal prompt library into what a test asserts against.
- **Startup now checks that the SQLite resource index matches the live catalog and reports any mismatch.** Previously the index could silently diverge — including reporting zero gates, frameworks, and styles — which broke every Python hook that reads it and made bundled, executable prompts answer `Unknown prompt`.
- **A prompt id defined in two categories now reports the conflict by file path instead of silently dropping one definition.** The warning distinguishes an intentional override (matching category — the overlay contract) from a real conflict (same id, different category).
- **The startup inventory line now names what it excluded**, e.g. `prompts: 120 (16 categories, 3 invalid)` instead of a bare success count.
- **Two internal prompt-count structures that disagreed on catalog size now use the same key.** A consumer's reported prompt count no longer depends on which internal structure it happened to read.
- **A workspace resource directory now overlays the bundled resources instead of replacing them.** Previously, adding even one workspace framework, prompt, or style directory could silently drop the rest of the bundled catalog (or crash startup entirely); a same-id workspace entry still wins, matching what `--help` has always described.
- **The startup inventory now names the bundled base it merged with your workspace**, e.g. `gates: 25 — <workspace>/resources/gates`.
- **Skills Sync now refuses to write through a symlinked skill directory** instead of letting one client's export silently overwrite another's through the link. It names the real target the symlink resolves to.
- **`resource_manager` responses now include machine-readable `structuredContent`** for prompt validation drafts, inspect metadata, write receipts, and version conflicts, not just text.
- **Reloading a prompt no longer creates a spurious extra version from argument key reordering alone.** Version comparison now ignores JSON key order.
- **Claude Code plugin hooks now resolve Python 3 reliably on Windows, macOS, and Linux**, including a stock python.org Windows install that lacks the `python3.exe` WindowsApps alias. ([#224](https://github.com/minipuft/claude-prompts-mcp/issues/224))
- **A chain running in one client (Claude Code, Codex, OpenCode) can no longer surface inside another client's conversation.** Hooks now key chain recovery to the chain the conversation itself recorded, instead of scanning for the newest active chain across every server sharing the same `state.db`.
- **Hook session rows now clear once their chain run resolves**, instead of surviving until a 24-hour sweep and letting compact recovery re-inject an already-resolved gate.
- **Attaching a gate to a prompt call no longer breaks prompts whose arguments are all optional or defaulted.** `>>reference_demo :: code-quality` previously failed with "Missing required field: text" while `>>reference_demo` alone worked; both paths now resolve default arguments the same way.
- **A chain step can now reach its own prompt's script tools.** Script lookup already checked a prompt-local `tools/{id}/` before the shared workspace location, but the chain renderer never passed the information needed to use it; it now does.
- **A prompt's `tools/*/tool.yaml` script-tool manifest is no longer validated against the prompt schema**, which previously logged an unavoidable error on every startup.
- **A phase guard no longer blocks on a section header the executing prompt was never told to produce.** A header the prompt doesn't declare is now advisory only; a declared-and-missing header still enforces as before.
- **Gate and framework version history now records the state each edit produced**, matching how prompt version history already worked — the state currently on disk is now always rollback-reachable.
- **Rollback now restores the recorded snapshot exactly instead of merging in current values.** A version whose snapshot can't fully rebuild the resource is refused, naming the missing fields, and the response now names anything a rollback intentionally leaves untouched (category-inherited fields, framework fields the version never recorded, script tools under `tools/{id}/`).
- **A rollback that gets refused no longer writes a version row.** Validation now happens before the file write, so a persistence failure leaves nothing on disk.
- **A gate or framework edit no longer writes a spurious extra version row on every save.**
- **`cpm rollback` no longer destroys resource fields on gates.** It previously wrote the recorded snapshot as the entire file, deleting `pass_criteria`, `retry_config`, `activation`, and `guidanceFile`; the snapshot is now merged over what's on disk, and `cpm rollback style` is refused (styles have no version history).
- **`resource_manager framework update description:"…"` now actually writes the new description**, instead of accepting and reporting the value while silently discarding it.
- **A script tool's declared interpreter fallbacks (e.g. `python3` then `python`, `bash` then `sh`) now work**, instead of always trying only the first name in the list and failing with a bare `ENOENT` on hosts that only have the second.
...
- **Claude Code now receives the write receipt, preview notice, and other readable summaries it was silently dropping.** On 2.1.272, a tool result carrying both `content` text and `structuredContent` reaches the model as `structuredContent` alone, so `resource_manager`'s `validate`, `create`, `preview`, `update`, and `inspect` results — draft-valid notices, write receipts, "nothing written" previews, saved-version confirmations, and next-step guidance — never reached the model when they carried structured data too. Every such result now mirrors its readable text into `structuredContent.message`. A result with `content` text only is unaffected.
- **`config.json`'s schema now rejects an undeclared key in every section, not just at the top level.** A typo one level down — `gates.enabld`, `server.prot` — previously passed schema validation, and the server silently used the default for the setting you thought you'd set. Keys `cpm config set` can already write are declared in the schema, some marked deprecated, so a config the CLI wrote still validates.
- **A workspace script tool and a workspace style are now found.** Both were resolved from the server's own package directory regardless of `MCP_WORKSPACE`, so a script under `<workspace>/resources/scripts/{id}/` — the tier `docs/guides/script-tools.md` documents — answered "not found", and a style under `<workspace>/resources/styles/{id}/` — where `cpm create style` writes — matched nothing and rendered no guidance, silently. Both now resolve the way prompts, gates and frameworks already do; a workspace style overlays the bundled styles rather than replacing them, so `#analytical` and the rest keep working alongside your own.
- **Editing a style's `guidance.md`, a framework's guidance, or a workspace script while the server is running now reaches the next call, instead of needing a restart.** Style hot reload was built but never wired in at all, so `#style_id` kept serving the guidance loaded at startup; framework hot reload was wired but watched only the bundled tree, so a workspace framework's edits were silently dropped once a workspace was configured; and the workspace-tier scripts folder (`resources/scripts/`, loadable since the previous release) was never watched, so an edited script kept running its old version until something else happened to trigger a reload. All three now watch every directory their loader reads, workspace and bundled alike, matching how gates already did.
- **The diff shown after a category update, and the one a category rollback preview shows, described a file that was never written.** Both were rendered from the recorded fields rather than from what the write puts on disk, so they missed everything a write to `category.yaml` replaces — a hand-authored comment, the author's key order, the way a long description is wrapped — and the preview's diff could not even be applied to the file it named. Both now read the writer's own plan, as the prompt, gate and framework diffs already did, so they name `category.yaml` and exactly the lines that change in it. The prompts the category holds are never named, because a category write never touches them.
- **`system_control` now advertises which twelve values `action` accepts, and `resource_manager` documents the five parameters it accepted without ever listing them.** `action` was published as a plain string, so a client had to mine the twelve valid values out of prose and learned an invalid one only from the error it came back with; it is now an enum the client can read and check against before it calls. On `resource_manager`, `chain_step_operation`, `chain_step_index`, `chain_step_data`, `chain_step_order` and `subject` were live parameters missing from the tool's own documentation, `preview_action` was documented as a free string when only three values are accepted, and `action` was documented without `preview`. Every parameter of all three tools is now described with the type it actually takes, and a test holds them to it.
- **The tool descriptions a client reads now list every action, whichever framework is active.** A framework's tool text used to replace the tool's own description, so under CAGEERF, the default framework, `resource_manager` named 8 of the 15 actions it accepts and said nothing about validating a draft before creating it, and under FOCUS, LIQUESCENT or VERIFY it named none. The same replacement dropped `system_control`'s action list and `prompt_engine`'s syntax reference, and several bundled frameworks still taught the deprecated `@Framework` form. Each tool's description now comes first, with the active framework's guidance appended under an `ACTIVE FRAMEWORK [TYPE]:` heading. A custom framework's `toolDescriptions` text is appended the same way, so write only what the framework adds.
- **Chain runs and execution history are now recorded under the launch workspace on an ordinary launch, instead of only when `--identity-mode` was passed.** The pipeline read identity settings only after that flag set an explicit `identity.mode`; without it, a run's continuity scope fell back to the shared `default` bucket even though the server had already derived a workspace id from the project directory. Framework and gate state already scoped to that derived id — chain runs and execution history now match them.
- **A verification loop's state now lives in your runtime directory, so a plugin update no longer erases it and the Stop hook can find it.** `:: verify:"…" loop:true` records its progress in `verify-state.db`, which the server wrote into its own package directory while `state.db` went to `MCP_RUNTIME_ROOT`. Under the Claude Code plugin that meant the install directory, which every plugin update replaces, and the Stop hook looked for the file in a place that disagreed with the server as soon as a workspace `config.json` existed. The server now writes it beside `state.db` — `${CLAUDE_PLUGIN_DATA}/runtime-state/` for plugin users, `$MCP_RUNTIME_ROOT/runtime-state/` otherwise — and the hook reads it from there.

### Security

- **`script_tool` gate criteria no longer execute through a shell.** The declared tool id is now resolved through the same script-tool registry the inline `{{script:}}` path uses, instead of being passed to the process helper as a raw command string. A check that can't run — unknown id, no registry wired, or a tool requiring confirmation a gate has no way to grant — now fails closed instead of reporting a false pass.
- **`{{script:id}}` inline references now honor a tool's `confirm: true` setting.** Previously an inline reference ran the tool unconditionally even when it required approval; approve one run by naming the tool as `tool:<id>` in the invocation arguments. **Prompt authors:** a template referencing a `confirm: true` tool now aborts the render instead of running it — set `confirm: false` on tools that need no approval.
- **Script output is no longer evaluated as template syntax.** Output is now inserted as literal text, closing a path where a script emitting `{{`, `{%`, or `{#` could read other arguments, inject structure into the prompt, or hang the render.

### Documentation

- **PR bodies and squash merges now show the actual change instead of every commit message.** The PR template gains a required `## Demonstration` section for `feat`/`fix`/`perf`/`refactor` changes, `npm run pr:body` pre-fills it from the branch, and `commitlint` warns on titles that name session activity instead of the merged outcome.
...
- **One command now runs every PR gate before you open a PR: `npm run pr:check`.** It runs all four gating steps the `PR Conventions` workflow runs -- the two positive controls, the body check, and `commitlint` on the title -- so a green local run predicts a green check. Previously the documented local route covered only the body, and the body checker does not read the title beyond its type, so a title that CI would reject passed every instruction a contributor was given. A parity test reads the workflow file and fails when the two sets of steps diverge. `npm run pr:body` moves from `server/` to the repo root, where `pr:check` and `commitlint` already live, so the whole flow runs in one directory.
- **`MCP_RUNTIME_ROOT` is now documented in `--help` and the handbook.** It independently pins `runtime-state/` and relative `logs/` regardless of `MCP_WORKSPACE`, so you can point `MCP_WORKSPACE` at a personal resource library while keeping `state.db` and logs elsewhere; `--help` also now notes that `MCP_RESOURCES_PATH` alone does not enable resource overlays.
