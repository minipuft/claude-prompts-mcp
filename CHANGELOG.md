...
- **Pre-emission output contract skeleton in chain step prompts**: When a chain step runs under an active methodology framework (CAGEERF, ReACT, 5W1H, SCAMPER), the renderer now prepends a "## Required Output Structure" block listing the required section headers, length expectations, and forbidden-term hints derived from the methodology's `phases.yaml`. The same SSOT already drives post-emission phase-guard validation (stage 09b); pre-emitting it eliminates the round-trip where a model is judged against a contract it never saw. Skeleton also appears in gate-review prompts so retries see the contract. Reuses existing `processingSteps[].section_header` + `guards` declarations — no methodology authoring change needed.
