 ...
 */
function inferAnalysisMode(llmConfig) {
    ...
    if (llmConfig.enabled && llmConfig.endpoint) {
        ...
        if (llmConfig.endpoint.includes('localhost') ||
...
export class ConfigManager extends EventEmitter {
    ...
    getFrameworksConfig() {
        return {
            enableSystemPromptInjection: this.config.frameworks?.enableSystemPromptInjection ??
                DEFAULT_FRAMEWORKS_CONFIG.enableSystemPromptInjection,
            enableMethodologyGates: this.config.frameworks?.enableMethodologyGates ??
                DEFAULT_FRAMEWORKS_CONFIG.enableMethodologyGates,
            enableDynamicToolDescriptions: this.config.frameworks?.enableDynamicToolDescriptions ??
                DEFAULT_FRAMEWORKS_CONFIG.enableDynamicToolDescriptions,
            systemPromptReinjectionFrequency: this.config.frameworks?.systemPromptReinjectionFrequency ??
    ...
    getGatesConfig() {
        ...
        return {
            ...
            templatesDirectory: gatesConfig.templatesDirectory ?? DEFAULT_GATES_CONFIG.templatesDirectory,
    ...
    getInjectionConfig() {
        ...
        if (legacyFrequency !== undefined) {
            logger.warn('[ConfigManager] DEPRECATION: frameworks.systemPromptReinjectionFrequency is deprecated. ' +
    ...
    getResolvedPromptsFilePath(overridePath) {
        ...
        let resolvedPath = overridePath ?? process.env['MCP_PROMPTS_CONFIG_PATH'] ?? this.getPromptsFilePath();
        if (!path.isAbsolute(resolvedPath)) {
            const baseDir = overridePath || process.env['MCP_PROMPTS_CONFIG_PATH']
                ? defaultBaseDir
                : path.dirname(this.configPath);
    ...
    validateAnalysisConfig(analysisConfig) {
        ...
        const validModes = ['structural', 'semantic'];
        const mode = semanticAnalysis.mode && validModes.includes(semanticAnalysis.mode)
            ? semanticAnalysis.mode
    ...
    haveFrameworkConfigsChanged(a, b) {
        return (a.enableSystemPromptInjection !== b.enableSystemPromptInjection ||
