...
export class JudgeSelectionStage extends BasePipelineStage {
    constructor(promptsProvider, gateLoader, configManager, logger, frameworksProvider) {
        super(logger);
...
`;
        ...
        const responseLines = [
            ...
            '```',
            `prompt_engine({`,
            `  command: "${escapedCommand}${operatorContext.hasFrameworkOperator ? '' : ' @<CAGEERF|ReACT|5W1H|SCAMPER>'} :: <gate_id or criteria> #style(<analytical|procedural|creative|reasoning>)"`,
            `})`,
            '```',
            '',
            '**Notes:**',
