...
const LEGACY_SESSION_ID_ERROR = "session_id is no longer supported. Use 'chain_id' from the response footer to resume.";
const LEGACY_GATE_VALIDATION_ERROR = "gate_validation has been retired. Use 'gates' parameter to specify quality gates.";
const LEGACY_API_VALIDATION_ERROR = "api_validation has been removed. Use 'gates' parameter to specify quality gates.";
const LEGACY_LLM_VALIDATION_ERROR = "llm_validation is experimental and not yet implemented. Use 'gates' parameter with gate_verdict for gate-based validation. See docs/enhanced-gate-system.md for current gate features.";
const LEGACY_GUIDED_PARAM_ERROR = "guided parameter is deprecated. Use %judge (alias %guided) in the command to trigger judge mode.";
const LEGACY_SELECTION_PARAM_ERROR = 'selected_framework/selected_gates/selected_style parameters are deprecated. Use @framework, ::gates, and #style(<id>) inline in the command.';
const LEGACY_TIMEOUT_PARAM_ERROR = 'timeout parameter has been removed. The pipeline manages timing; rely on defaults or chain-level controls.';
export class McpToolRequestValidator {
    ...
    static validate(raw) {
        try {
            ...
            if (raw &&
                typeof raw === 'object' &&
                'gate_validation' in raw &&
                raw.gate_validation !== undefined) {
    ...
    static validatePartial(partialRequest) {
        ...
        if (partialRequest &&
            ...
            partialRequest.gate_validation !== undefined) {
            recordParameterIssue('prompt_engine', 'gate_validation', LEGACY_GATE_VALIDATION_ERROR);
            throw new Error(LEGACY_GATE_VALIDATION_ERROR);
        }
        if (partialRequest &&
            'api_validation' in partialRequest &&
            partialRequest.api_validation !== undefined) {
            recordParameterIssue('prompt_engine', 'api_validation', LEGACY_API_VALIDATION_ERROR);
            throw new Error(LEGACY_API_VALIDATION_ERROR);
        }
        if (partialRequest &&
            'llm_validation' in partialRequest &&
            partialRequest.llm_validation !== undefined) {
            recordParameterIssue('prompt_engine', 'llm_validation', LEGACY_LLM_VALIDATION_ERROR);
            throw new Error(LEGACY_LLM_VALIDATION_ERROR);
        }
