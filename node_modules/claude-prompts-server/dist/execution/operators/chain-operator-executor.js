...
export class ChainOperatorExecutor {
    constructor(logger, convertedPrompts, gateGuidanceRenderer, getFrameworkContext, promptGuidanceService) {
        this.logger = logger;
    ...
    async renderGateReviewStep(input, stepPrompts, chainContext, additionalGateIds, inlineGuidanceText) {
        ...
        if (targetStep) {
            ...
            if (convertedPrompt) {
                ...
                const renderedTemplate = this.renderTemplate(convertedPrompt, templateContext, targetStep.promptId);
                originalContent = [
    ...
    async renderNormalStep(input, stepPrompts, chainContext) {
        ...
        if (currentStepIndex === 0) {
            ...
            const storedOutput = this.getStoredStepResult(chainContext, stepPrompts[previousStepIndex].stepNumber);
            if (storedOutput) {
        ...
        const renderedTemplate = this.renderTemplateString(templateToRender, templateContext, step.promptId);
        const lines = [];
    ...
    renderTemplate(convertedPrompt, templateContext, promptId) {
        return this.renderTemplateString(convertedPrompt.userMessageTemplate, templateContext, promptId);
    }
    renderTemplateString(templateString, templateContext, promptId) {
        try {
