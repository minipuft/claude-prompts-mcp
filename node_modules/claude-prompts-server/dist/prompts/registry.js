...
export class PromptRegistry {
     ...
     */
    async processTemplateDirect(template, args, specialContext = {}, toolsEnabled = false) {
    ...
    async executePromptLogic(promptData, args) {
        try {
            ...
            if (effectivelyEmptyArgs && promptData.onEmptyInvocation === 'return_template') {
                this.logger.info(`Prompt '${promptData.name}' invoked without arguments and onEmptyInvocation is 'return_template'. Returning template info.`);
                ...
                if (promptData.arguments && promptData.arguments.length > 0) {
                    ...
                    responseText += `\nExample usage: >>${promptData.id || promptData.name} ${promptData.arguments.map((arg) => `${arg.name}=\"value\"`).join(' ')}`;
                }
                else {
            ...
            userMessageText = await this.processTemplateDirect(userMessageText, args, { previous_message: previousMessageContext }, promptData.tools || false);
     ...
     */
    areArgumentsEffectivelyEmpty(promptArgs, providedArgs) {
        if (!providedArgs ||
    ...
    async executePromptDirectly(promptId, args, prompts) {
        try {
            ...
            if (missingArgs.length > 0) {
                this.logger.info(`Missing arguments for '${promptId}': ${missingArgs.join(', ')}. Will attempt to use conversation context.`);
            ...
            // Using direct processing since TemplateProcessor was consolidated
            const userMessageText = await this.processTemplateDirect(convertedPrompt.userMessageTemplate, args, { previous_message: this.conversationManager.getPreviousMessage() }, convertedPrompt.tools || false);
