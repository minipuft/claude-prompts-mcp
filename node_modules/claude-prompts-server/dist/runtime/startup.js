...
export class ServerRootDetector {
     ...
     * Test strategies with optimized flow
     */
    async testStrategies(strategies, isVerbose, shouldShowOutput) {
        ...
        for (let i = 0; i < strategies.length; i++) {
            ...
            try {
                ...
                if (shouldShowOutput) {
                    ...
                    if (isVerbose) {
                        console.error(`  Strategy #${i + 1}/${strategies.length} (${strategy.priority} priority)`);
                ...
                if (i === lastHighPriorityIndex && !isVerbose && lastHighPriorityIndex >= 0) {
                    if (shouldShowOutput) {
                        console.error(`⚠️  High-priority detection strategies failed. Trying fallback methods...`);
                        console.error(`💡 Tip: Set MCP_SERVER_ROOT environment variable for guaranteed detection`);
        ...
        throw new Error(`Unable to auto-detect server root directory after testing ${strategies.length} strategies.\n\n` +
            `SOLUTION OPTIONS:\n` +
     ...
     */
    generateTroubleshootingInfo(attemptedPaths) {
        return `
