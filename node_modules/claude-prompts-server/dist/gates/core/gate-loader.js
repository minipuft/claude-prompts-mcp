...
export class GateLoader {
    constructor(logger, gatesDirectory, temporaryGateRegistry) {
    ...
    async loadGate(gateId) {
        try {
            ...
            if (loadResult.gate.id !== gateId) {
                this.logger.error(`Gate ID mismatch in file ${gateFile}: expected ${gateId}, got ${loadResult.gate.id}`);
                return null;
            }
    ...
    async listAvailableGates() {
        try {
            ...
            const gateFiles = files.filter((file) => file.endsWith('.yaml') || file.endsWith('.yml') || file.endsWith('.json'));
     ...
     */
    async parseGateFile(filePath) {
        try {
            const result = await this.resourceLoader.load(filePath, {
                kind: 'auto',
                useCache: true,
            });
            ...
            if (validation.success !== true || validation.data === undefined) {
                this.logger.error(`Invalid gate definition in ${filePath}: ${validation.errors?.join('; ')}`);
                return null;
            }
            return {
