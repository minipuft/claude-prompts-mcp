// @lifecycle deprecated - Use data-driven YAML definition at methodologies/5w1h/ instead
/**
...
export class FiveW1HMethodologyGuide extends BaseMethodologyGuide {
    constructor() {
        super(...arguments);
        this.frameworkId = '5w1h';
        this.frameworkName = '5W1H Framework';
        this.methodology = '5W1H';
        this.version = '1.0.0';
    }
    /**
    ...
    enhanceWithMethodology(prompt, context) {
        ...
        return {
            ...
            templateSuggestions,
            enhancementMetadata: this.createEnhancementMetadata(0.9, '5W1H methodology ensures comprehensive analysis through systematic questioning'),
        };
    }
    /**
    ...
    validateMethodologyCompliance(prompt) {
        ...
        if (questions.who)
            strengths.push('Stakeholder consideration present');
        else
            improvement_areas.push('Identify stakeholders and people involved');
        if (questions.what)
            strengths.push('Objective definition evident');
        else
            improvement_areas.push('Define specific objectives and deliverables');
        if (questions.when)
            strengths.push('Timing consideration included');
        else
            improvement_areas.push('Establish timing and scheduling requirements');
        if (questions.where)
            strengths.push('Context awareness demonstrated');
        else
            improvement_areas.push('Consider location and contextual factors');
        if (questions.why)
            strengths.push('Purpose and motivation addressed');
        else
            improvement_areas.push('Explore underlying purposes and motivations');
        if (questions.how)
            strengths.push('Implementation approach considered');
        else
            improvement_areas.push('Plan specific methods and implementation approaches');
...
Use systematic questioning to ensure comprehensive coverage and uncover hidden requirements. Address each question thoroughly to build complete understanding and effective solutions.`;
    ...
    getToolDescriptions() {
        return {
            prompt_engine: {
                description: '🚀 PROMPT ENGINE [5W1H]: Operators: %judge menu, @framework, ::gates, #style(<id>), --> chain. Use `>>guide <question>` for targeted tips on each W; gate verdicts via `gate_verdict`, step outputs via `user_response`.',
            },
            prompt_manager: {
                description: '📝 PROMPT MANAGER [5W1H GUIDE]: Lifecycle actions surface Who/What/When/Where/Why/How cues. Use `action:"guide" goal:"who"` to bias recommendations toward stakeholder discovery verbs.',
                parameters: {
                    action: 'Use the guide goal text (e.g., `goal:"who"`) to bias recommendations toward stakeholder discovery verbs.',
                },
            },
            system_control: {
                description: '⚙️ SYSTEM CONTROL [5W1H-ENHANCED]: `>>help` highlights Who/What/When/Where/Why/How considerations for framework switches, gate management, and analytics. Use `topic:"why"` to explain operation context.',
                parameters: {
                    action: 'Guide topic filters (e.g., `topic:"why"`) explain why a specific system operation matters before running it.',
                },
            },
        };
    }
    ...
    getJudgePrompt() {
        return {
            systemMessage: `You are a 5W1H methodology expert specializing in comprehensive analysis through systematic questioning.
...
Select resources that ensure thorough coverage of all six questions for comprehensive understanding.`,
            userMessageTemplate: `Analyze this task using 5W1H systematic questioning:
...
//# sourceMappingURL=5w1h-guide.js.map