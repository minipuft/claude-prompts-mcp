// @lifecycle deprecated - Use data-driven YAML definition at methodologies/cageerf/ instead
/**
...
export class CAGEERFMethodologyGuide extends BaseMethodologyGuide {
    constructor() {
        super(...arguments);
        this.frameworkId = 'cageerf';
        this.frameworkName = 'C.A.G.E.E.R.F Framework';
        this.methodology = 'CAGEERF';
        this.version = '2.0.0';
    }
    /**
    ...
    enhanceWithMethodology(prompt, context) {
        ...
        return {
            ...
            templateSuggestions,
            enhancementMetadata: this.createEnhancementMetadata(0.9, 'CAGEERF methodology provides systematic approach to complex problem solving'),
        };
    }
    /**
    ...
    validateMethodologyCompliance(prompt) {
        ...
        if (phases.context.present) {
            if (phases.context.quality > 0.7)
                strengths.push('Strong contextual awareness and environmental understanding');
            else if (phases.context.quality > 0.4)
                strengths.push('Basic context awareness present - could be enhanced');
            else
                improvement_areas.push('Context present but lacks depth - need comprehensive situational analysis');
        }
        else {
        ...
        if (phases.analysis.present) {
            if (phases.analysis.quality > 0.7)
                strengths.push('Systematic analytical approach with multiple perspectives');
            else if (phases.analysis.quality > 0.4)
                strengths.push('Analytical thinking evident - could be more systematic');
            else
                improvement_areas.push('Analysis present but lacks systematic depth');
        }
        else {
        ...
        if (phases.goals.present) {
            if (phases.goals.quality > 0.7)
                strengths.push('Well-defined, specific and measurable objectives');
            else
                strengths.push('Goals mentioned - ensure they are specific and measurable');
        }
        else {
        ...
        if (phases.execution.present) {
            if (phases.execution.quality > 0.7)
                strengths.push('Comprehensive implementation approach with practical steps');
            else
                strengths.push('Execution approach mentioned - could be more detailed');
        }
        else {
            improvement_areas.push('Missing practical implementation approach');
        }
        if (phases.evaluation.present) {
            if (phases.evaluation.quality > 0.7)
                strengths.push('Robust evaluation criteria and success metrics');
            else
                strengths.push('Evaluation mentioned - strengthen success criteria');
        }
        else {
            improvement_areas.push('Missing evaluation criteria and success metrics');
        }
        if (phases.refinement.present) {
            if (phases.refinement.quality > 0.7)
                strengths.push('Strong continuous improvement and iteration process');
            else
                strengths.push('Refinement mentioned - enhance feedback loops');
        }
        else {
            improvement_areas.push('Missing refinement and continuous improvement process');
        }
        if (phases.framework.present) {
            if (phases.framework.quality > 0.7)
                strengths.push('Strong systematic framework application');
            else
                strengths.push('Framework awareness present - strengthen systematic application');
        }
        else {
...
Ensure each phase builds logically on the previous phases while maintaining focus on practical, actionable outcomes. Consider stakeholder perspectives, resource constraints, and environmental factors throughout the methodology application.`;
    ...
    getToolDescriptions() {
        return {
            prompt_engine: {
                description: '🚀 PROMPT ENGINE [CAGEERF]: Operators: %judge menu, @framework, ::gates, #style(<id>), --> chain. Use `>>guide <topic>` for phase-aware help; gate verdicts via `gate_verdict`, step outputs via `user_response`.',
            },
            prompt_manager: {
                description: '📝 PROMPT MANAGER [CAGEERF GUIDE]: Lifecycle operations emit Context→Analysis→Goals cues and use the metadata-driven `action:"guide"` to direct authors toward the right verb. Legacy creation aliases have been removed; use action:"create" only.',
                parameters: {
                    action: 'Use `guide` to see which lifecycle action best fits the current phase (e.g., Context research vs. Execution updates).',
                },
            },
            system_control: {
                description: '⚙️ SYSTEM CONTROL [CAGEERF-ACTIVE]: `>>help` routes here for methodology-aware guidance. Summaries highlight which C.A.G.E.E.R.F phase is currently enforced across the server plus pending analytics work.',
                parameters: {
                    action: 'Use `guide` with `topic:"framework"` to view phase-specific operations before switching.',
                },
            },
        };
    }
    ...
    getJudgePrompt() {
        return {
            systemMessage: `You are a CAGEERF methodology expert selecting enhancement resources.
...
Select resources that support this structured, phase-based approach.`,
            userMessageTemplate: `Analyze the task through the CAGEERF methodology lens and select enhancement resources.
...
//# sourceMappingURL=cageerf-guide.js.map