...
export class ChainExecutor {
    // Phase 4: Legacy cleanup - Advanced gate orchestration removed
    constructor(conversationManager, lightweightGateSystem, frameworkManager, frameworkStateManager, responseFormatter, chainSessionManager) {
     ...
     */
    detectChainManagementCommand(command) {
     ...
     */
    parseKeyValueParams(paramString) {
        const params = {};
    ...
    async handleChainManagementCommand(chainCommand) {
        ...
        try {
            switch (chainCommand.action) {
                ...
                default:
                    return this.responseFormatter.formatErrorResponse(`Unknown chain management action: ${chainCommand.action}`);
            }
        }
        catch (error) {
     ...
     */
    async executeChainWithDualSupport(convertedPrompt, promptArgs, enableGates, options = { enableGates: true }) {
        ...
        return await this.generateChainInstructions(convertedPrompt, convertedPrompt.chainSteps || [], enableGates, options);
    }
    /**
     * Handles chain management operations
     */
    async executeChainManagement(action, parameters, options) {
        ...
        try {
            switch (action) {
                ...
                default:
                    return this.responseFormatter.formatErrorResponse(`Unknown chain management action: ${action}`);
            }
        }
        catch (error) {
     ...
     */
    async generateChainInstructions(prompt, steps, enableGates, options = { enableGates: true }) {
        logger.debug(`🔍 [Chain Debug] generateChainInstructions called for: ${prompt.id}`);
        ...
        try {
            if (!steps || steps.length === 0) {
                return this.responseFormatter.formatErrorResponse(`Chain ${prompt.id} has no steps defined`);
            }
            ...
            if (temporaryGateRegistry && allTemporaryGates.length > 0) {
                ...
                for (const tempGateDef of allTemporaryGates) {
                    try {
                        ...
                        const gateSource = tempGateDef.source || 'manual';
            ...
            const frameworkContext = activeFramework ?
                this.frameworkManager.generateExecutionContext(prompt, { userPreference: activeFramework.methodology }) :
                null;
     ...
     */
    formatStepCommand(promptId, stepArgs) {
     ...
     */
    async handleValidateCommand(target, parameters) {
        logger.debug('🔍 [Chain Validation] Validating chain', { target, parameters });
        try {
            if (!target) {
                return this.responseFormatter.formatErrorResponse('Chain ID is required for validation');
            }
     ...
     */
    async handleListChainsCommand(parameters) {
     ...
     */
    async getGateInfo(target) {
        logger.debug('🚪 [Gate Info] Getting gate information', { target });
        try {
            if (!target) {
                return this.responseFormatter.formatErrorResponse('Chain ID is required for gate information');
            }
            const gateInfo = {
                status: 'available',
                gates: [
     ...
     */
    async generateMetadataSection(prompt, steps, enableGates) {
