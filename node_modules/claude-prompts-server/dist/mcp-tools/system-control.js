...
export class ConsolidatedSystemControl {
    ...
    async switchFramework(args) {
        ...
        if (currentState.activeFramework === framework) {
            return this.createMinimalSystemResponse(`ℹ️ Framework '${framework}' is already active. No change needed.`, 'switch_framework');
        }
        const request = {
    ...
    async listMethodologiesAction(args) {
        ...
        if (methodologyIds.length === 0) {
            return this.createMinimalSystemResponse(`📋 **No Methodologies Found**\n\n` +
                `Ensure YAML files exist in \`methodologies/<id>/methodology.yaml\`.`, 'list_methodologies');
        }
    ...
    async handleConfigGet(key) {
        ...
        const value = config
            ? key
                .split('.')
                .reduce((obj, k) => (obj && obj[k] !== undefined ? obj[k] : undefined), config)
            : undefined;
