 ...
 *
 * Every module holding a walk must value-import `isReservedPromptDirectoryName` from
 * `prompt-layout`. The marker vocabulary is read from `prompt-layout.ts` itself, so a predicate
 * added there becomes a marker here without anyone editing this file.
 *
 ...
 *      adopts one. `skills-sync`'s `collectPromptDirs` was exactly that before P4.48.
 ...
 * prompts tree.
 *
 * ONE DECLARED EXCEPTION. `cli/src/lib/workspace.ts` belongs to the standalone `cpm` package, which
 * cannot import server source. The exception is anchored by path. It fails closed when the file
 * disappears, and it fails as stale when the file stops being a walk or starts importing the
 * predicate.
 *
...
const REQUIRED_IMPORT = 'isReservedPromptDirectoryName';
...
/** Walks that may not import the predicate, each with the reason. Keyed by repo-relative path. */
const EXCEPTIONS: ReadonlyMap<string, string> = new Map([
  [
    'cli/src/lib/workspace.ts',
    'the standalone `cpm` CLI is a separate package (`cli/package.json`, Node >=18.18) and ' +
      'bundles no server source, so it cannot import `prompt-layout.ts`',
  ],
]);
...
export interface ModuleReport {
  ...
  readonly importsPredicate: boolean;
}
...
export function analyzeModule(
  ...
  const importsPredicate = [...imports.entries()].some(
    ([, binding]) =>
      binding.importedName === REQUIRED_IMPORT && isLayoutSpecifier(binding.specifier)
  );

  return { walks, listers, markerArgumentCalls, importsPredicate };
}
...
function checkTree(): TreeResult {
  ...
  if (!layoutNames.has(REQUIRED_IMPORT)) {
    problems.push(`${LAYOUT_MODULE} no longer exports \`${REQUIRED_IMPORT}\``);
  }
  ...
  for (const walk of walks) {
    if (reports.get(walk.file)?.importsPredicate === true || EXCEPTIONS.has(walk.file)) continue;
    problems.push(
      `${walk.file}:${walk.line} \`${walk.name}\` walks a prompts tree, but the module does not ` +
        `import \`${REQUIRED_IMPORT}\` from prompt-layout`
    );
  }
    ...
    if (report === undefined) {
      problems.push(
        `exception \`${file}\` names no scanned file. Delete the entry, or update the path if the ` +
          `file moved (reason on record: ${reason})`
      );
    } else if (!walksByFile.has(file)) {
      problems.push(`exception \`${file}\` is stale: the file no longer walks a prompts tree`);
    } else if (report.importsPredicate) {
      problems.push(`exception \`${file}\` is stale: the file now imports \`${REQUIRED_IMPORT}\``);
    }
  }
  ...
  if (problems.length > 0) {
    ...
    console.error(
      `\nEvery prompts-tree walk must skip what the loader skips. Take the rules from ` +
        `\`${LAYOUT_MODULE}\` (\`isIgnoredPromptEntryName\`, \`${REQUIRED_IMPORT}\`, ` +
        `\`isSingleFilePromptName\`). Do not restate them. A walk that truly cannot import ` +
        `server source needs an entry in EXCEPTIONS with its reason.`
    );
    return 1;
  }
  console.log(
    `✅ Prompt walks: ${filesScanned} files scanned, ${walks.length} prompts-tree walks found, ` +
      `each in a module that imports \`${REQUIRED_IMPORT}\` or a declared exception:`
  );
  for (const walk of walks) {
    const note = EXCEPTIONS.has(walk.file) ? ' (exception)' : '';
...
const LAYOUT_IMPORT = `import { isReservedPromptDirectoryName } from '#shared/utils/prompt-layout.js';\n`;
...
function crossFileCase(markerLiteral: string): boolean {
  const layoutNames = new Set([REQUIRED_IMPORT]);
...
function selfTest(): number {
  ...
  record(
    layoutNames.has(REQUIRED_IMPORT) && layoutNames.has('isSingleFilePromptName'),
  ...
  for (const testCase of selfTestCases()) {
    ...
    const reports = report.walks.length > 0 && !report.importsPredicate;
