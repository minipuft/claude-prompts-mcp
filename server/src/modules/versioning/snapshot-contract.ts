...
/**
 * The description a version row carries for the state a write produced, by SURFACE.
 *
 ...
 * row contract rather than a label each call site is free to phrase.
 *
 * It was a pair of CONSTANTS until 2026-09-21, and `cpm` reused them, so every `cpm`-written row
 * said `Created via resource_manager`. That is a plain untruth in the one sentence a history is
 * consulted for — what changed this, and can I undo it — and the two surfaces undo differently.
 * A function rather than a second pair of constants because the SHAPE of the sentence is what the
 * era convention reads; two pairs would let the shapes drift while each stayed correct alone.
 *
