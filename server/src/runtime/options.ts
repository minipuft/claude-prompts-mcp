...
// Re-exported, not redefined: `cli-shared/version-history.ts` must derive the SAME
// scope id as the server or the CLI and the server write `version_history` under
// different tenants. It cannot import this module (that edge would close a cycle), so
// the definition moved to `shared/utils/project-scope.ts` and both layers read it there.
export {
  deriveProjectScopeId,
  type ProjectScopeDerivation,
  type ProjectScopeSource,
} from '#shared/utils/project-scope.js';
