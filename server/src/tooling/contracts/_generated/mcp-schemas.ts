...
export const promptEngineSchema = z.object({
  ...
  gate_verdict: z.string().refine((v) => /^(?:GATE_REVIEW:\s*(?:PASS|FAIL)\s*[-:]\s*.+|GATE\s+(?:PASS|FAIL)\s*[-:]\s*.+|(?:PASS|FAIL)\s*[-:]\s*.+)$/i.test(v), 'Gate verdict must follow one of: "GATE_REVIEW: PASS/FAIL - reason", "GATE PASS/FAIL - reason", or minimal "PASS/FAIL - reason" (param only)').optional(),
...
export const resourceManagerSchema = z.object({
  ...
  action: z.enum(['create', 'update', 'delete', 'reload', 'list', 'inspect', 'analyze_type', 'analyze_gates', 'guide', 'switch', 'history', 'rollback', 'compare', 'clear']),
