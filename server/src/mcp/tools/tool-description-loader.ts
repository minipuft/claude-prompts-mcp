...
export class ToolDescriptionLoader extends EventEmitter {
   ...
   * so there is no "not found" branch to fall back from. `createDefaultToolDescriptionMap()`
   * remains as the per-tool safety net in `getDescription()`/`getParameterDescription()` for a
   * tool key the generated file omits, and as the `synchronize()` catch-all for a genuinely
   * unexpected failure (e.g. a malformed framework YAML thrown from `buildActiveConfig`) —
   * neither of those is a file-existence question anymore.
   */
