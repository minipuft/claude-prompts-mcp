...
class ContainmentServer {
  ...
  /**
   * `kill()` only SIGNALS; it does not wait. Returning immediately let the workspace `rm` race the
   * server's own log flush, so teardown failed with `ENOTEMPTY: rmdir '<ws>/runtime/logs'` while
   * every assertion in the suite had passed — a red suite reporting a green test list. Awaiting the
...
describe('a caller-supplied segment cannot steer a write out of the resources root', () => {
  ...
  afterAll(async () => {
    await server?.stop();
    // `maxRetries` covers a file handle the OS has not released yet, even after the exit.
