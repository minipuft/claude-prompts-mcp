...
| `script_tool`          | script_tool_id, script_tool_input, script_tool_timeout, script_tool_working_dir              | **Not enforced** — Stage 20 runs `shell_verify` criteria only, so a declared `script_tool` criterion executes nothing. `GateValidator` has a correct runner (resolves the id to a registered tool, fails closed) that no production caller reaches. Prefer `shell_verify`. |
