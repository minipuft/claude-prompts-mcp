...
| `framework_compliance` | framework, min_compliance_score, quality_indicators                                          | **Declarative** — GateValidator auto-passes it. Stage 19 enforces phase guards from `phases.yaml` independently                                                                                                                                                                                                       |
...
| `script_tool`          | script_tool_id, script_tool_input, script_tool_timeout, script_tool_working_dir              | **Enforced** — resolves `script_tool_id` against the registered script tools and runs THAT tool with JSON via stdin, parsing a `{passed, reason?}` verdict. Runs beside `shell_verify` in gate review. Fails closed when it cannot run; a missing `script_tool_id` is refused at load. The id is not a shell command. |
