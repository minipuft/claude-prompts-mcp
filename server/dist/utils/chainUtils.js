...
export function normalizeChainId(chainId) {
    ...
    if (!CHAIN_ID_PATTERN.test(normalized)) {
        throw new ValidationError(`Invalid chain ID "${normalized}": only letters, numbers, hyphen, and underscore are allowed`);
    }
    return normalized;
}
/**
 ...
 */
export function hasValidChainSteps(prompt) {
 ...
 */
export function getChainInfo(prompt) {
    const steps = prompt.chainSteps;
    return {
 ...
 */
export function isValidChain(prompt) {
