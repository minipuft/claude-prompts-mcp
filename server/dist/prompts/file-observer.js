...
export class FileObserver extends EventEmitter {
    constructor(logger, config, configManager) {
        super();
     ...
     */
    async watchDirectory(directoryPath, category) {
        if (!this.isStarted) {
     ...
     */
    async unwatchDirectory(directoryPath) {
     ...
     */
    handleFileEvent(eventType, directoryPath, filename, category) {
        if (!filename) {
            return;
        }
     ...
     */
    async emitFileChangeEvent(event) {
     ...
     */
    async classifyRenameEvent(filePath) {
        try {
     ...
     */
    handleWatcherError(directoryPath, error) {
     ...
     */
    isConfigFile(filename, fullPath) {
     ...
     */
    isMethodologyFile(filename, fullPath) {
        const ext = path.extname(filename).toLowerCase();
        // Must be a YAML file
        if (ext !== '.yaml' && ext !== '.yml') {
    ...
    analyzeFrameworkImpact(event) {
        ...
        const performanceImpact = event.isConfigFile ? 'high' : 'low';
     ...
     */
    enableFrameworkIntegration(options = {}) {
 ...
 */
export function createFileObserver(logger, config, configManager) {
