...
export class PromptRegistry {
     ...
     */
    async processTemplateDirect(template, args, specialContext = {}) {
        const { processTemplate } = await import('../utils/jsonUtils.js');
        return processTemplate(template, args, specialContext);
    }
    constructor(logger, mcpServer, configManager, conversationManager) {
        // templateProcessor removed - functionality consolidated into UnifiedPromptProcessor
        this.registeredPromptIds = new Set(); // Track registered prompt IDs to prevent duplicates
        this.logger = logger;
     ...
     */
    registerIndividualPrompts(prompts) {
        try {
            ...
            for (const prompt of prompts) {
                ...
                try {
                    this.mcpServer.registerPrompt(prompt.id, {
                        title: prompt.id,
                    ...
                    if (error.message?.includes('already registered')) {
                        ...
                        this.logger.warn(`Prompt '${prompt.id}' already registered in MCP SDK, skipping re-registration`);
     ...
     */
    async executePromptLogic(promptData, args) {
        try {
            ...
            if (isChainPrompt(promptData) && promptData.chainSteps && promptData.chainSteps.length > 0) {
                this.logger.info(`Prompt '${promptData.name}' is a chain with ${promptData.chainSteps.length} steps. NOT automatically executing the chain.`);
     ...
     */
    async executePromptDirectly(promptId, args, prompts) {
        try {
            ...
            // Using direct processing since TemplateProcessor was consolidated
            const userMessageText = await this.processTemplateDirect(convertedPrompt.userMessageTemplate, args, { previous_message: this.conversationManager.getPreviousMessage() });
