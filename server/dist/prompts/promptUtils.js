 ...
 */
export function resolvePromptFilePath(promptFile, configFilePath, categoryFolder) {
 ...
 */
export async function readPromptFile(promptFilePath) {
    try {
 ...
 */
export async function modifyPromptSection(promptId, sectionName, newContent, configPath) {
    try {
        const messages = [];
        ...
        for (const importPath of promptsConfig.imports) {
            try {
                ...
                if (categoryPromptsFile.prompts && Array.isArray(categoryPromptsFile.prompts)) {
                    ...
                    const foundIndex = categoryPromptsFile.prompts.findIndex((p) => p.id === promptId);
                    if (foundIndex !== -1) {
                        ...
                        messages.push(`✅ Found prompt '${promptId}' in category file: ${path.basename(categoryFilePath)}`);
                        break;
                    }
                }
            }
 ...
 */
export async function performTransactionalFileOperations(operations, rollbacks) {
    // Validate inputs
 ...
 */
export async function findAndDeletePromptFile(promptId, baseDir) {
    try {
 ...
 */
export async function findPromptFile(promptId, baseDir) {
    try {
