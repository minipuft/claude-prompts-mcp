 ...
 */
export function buildGateConfigurationSection(gateConfiguration) {
...
export function buildChainStepsSection(chainSteps) {
    if (!chainSteps?.length) {
        return '';
    }
    const lines = ['', '## Chain Steps', ''];
...
export function buildPromptBaseContent(data) {
    const lines = [`# ${data.name}`, '', '## Description', data.description, ''];
 ...
 */
export function buildPromptMarkdownContent(data) {
...
//# sourceMappingURL=prompt-writer.js.map