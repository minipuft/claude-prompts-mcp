...
import { createCategoryManager } from './category-manager.js';
import { validatePromptYaml } from './prompt-schema.js';
...
export class PromptLoader {
    constructor(logger, config = {}) {
        ...
        if (this.debug) {
            this.logger.info(`[PromptLoader] Initialized with caching ${this.enableCache ? 'enabled' : 'disabled'}`);
        }
    }
    /**
    ...
    async loadCategoryPrompts(configPath) {
        try {
            ...
            if (!promptsConfig.categories) {
                this.logger.warn(`⚠️ Config file ${configPath} does not have a 'categories' array. Initializing it.`);
                promptsConfig.categories = [];
            }
            if (!promptsConfig.imports || !Array.isArray(promptsConfig.imports)) {
                this.logger.warn(`⚠️ Config file ${configPath} does not have a valid 'imports' array. Initializing it.`);
            ...
            for (const importPath of promptsConfig.imports) {
                ...
                try {
                    ...
                    try {
                        ...
                        this.logger.info(`   🔧 Creating empty prompts file for ${importPath} due to parsing error.`);
                    ...
                    if (!categoryPromptsFile.prompts) {
                        this.logger.warn(`   ⚠️ Import file ${importPath} does not have a 'prompts' array. Initializing it.`);
                        categoryPromptsFile.prompts = [];
                        await safeWriteFile(fullImportPath, JSON.stringify(categoryPromptsFile, null, 2), 'utf8');
                    }
                    else if (!Array.isArray(categoryPromptsFile.prompts)) {
                        this.logger.warn(`   ⚠️ Import file ${importPath} has an invalid 'prompts' property (not an array). Resetting it.`);
                        categoryPromptsFile.prompts = [];
                        await safeWriteFile(fullImportPath, JSON.stringify(categoryPromptsFile, null, 2), 'utf8');
                    }
                    this.logger.info(`   📊 Found ${categoryPromptsFile.prompts.length} prompts in this import`);
                    ...
                    const categoryPrompts = categoryPromptsFile.prompts
                        ...
                        if (!prompt.id || !prompt.name || !prompt.file) {
                            this.logger.warn(`   ⚠️ Skipping invalid prompt ${index + 1} in ${importPath}: missing required properties (id: ${!!prompt.id}, name: ${!!prompt.name}, file: ${!!prompt.file})`);
                            return null;
                        }
                    ...
                    if (beforeCount !== afterCount) {
                        this.logger.warn(`   ⚠️ ${beforeCount - afterCount} prompts were filtered out due to validation issues`);
                    }
                    this.logger.info(`   ✅ Successfully processed ${afterCount} valid prompts from ${importPath}`);
            ...
            for (const category of promptsConfig.categories) {
                ...
                for (const yamlPrompt of yamlPrompts) {
                    ...
                    if (jsonPromptIds.has(yamlPrompt.id)) {
                        if (this.debug) {
                            this.logger.debug(`   ⏭️ Skipping YAML prompt ${yamlPrompt.id} (JSON version exists)`);
                        }
                        yamlPromptsSkipped++;
                        continue;
                    }
            ...
            allPrompts = allPrompts.map((prompt) => {
                ...
                if (category?.registerWithMcp !== undefined) {
                    return { ...prompt, _categoryRegisterWithMcp: category.registerWithMcp };
                }
                return prompt;
            });
    ...
    async loadFromDirectories(promptsDir) {
        ...
        for (const categoryEntry of categoryDirs) {
            ...
            if (existsSync(categoryYamlPath)) {
                try {
                    categoryMeta = loadYamlFileSync(categoryYamlPath);
                }
                catch (e) {
            ...
            for (const prompt of yamlPrompts) {
                ...
                if (category.registerWithMcp !== undefined) {
                    prompt._categoryRegisterWithMcp =
        ...
        this.logger.info(`✅ [PromptLoader] Loaded ${allPrompts.length} prompts from ${categories.length} categories`);
     ...
     */
    async loadPromptFile(filePath, basePath) {
        ...
        if (filePath.endsWith('.yaml')) {
            ...
            const promptPath = filePath.endsWith('/prompt.yaml') || filePath.endsWith('\\prompt.yaml')
        ...
        try {
                ...
                try {
                    ...
                    if (gateConfigContent) {
                        ...
                        if (Array.isArray(parsedConfig)) {
                            ...
                            const normalizedGateConfiguration = {};
                            ...
                            const inlineGateDefinitions = this.normalizeInlineGateDefinitions(parsedConfig.inline_gate_definitions);
                ...
                if (!chainContent) {
                    ...
                    for (const match of stepMatches) {
                        ...
                        if (!promptId || !stepName) {
                            this.logger.warn(`Skipping invalid chain step ${stepNumber ?? 'unknown'} in ${filePath}: missing promptId or stepName`);
                            continue;
                        }
                        const step = {
                        ...
                        if (inputMappingStr) {
                            try {
                                ...
                                this.logger.warn(`Invalid input mapping in chain step ${stepNumber} of ${filePath}: ${e}`);
                            }
                        }
                        if (outputMappingStr) {
                            try {
                                ...
                                this.logger.warn(`Invalid output mapping in chain step ${stepNumber} of ${filePath}: ${e}`);
                            }
                        }
            ...
            if (this.enableCache) {
                ...
                if (this.debug) {
                    this.logger.debug(`[PromptLoader] Cached prompt file: ${filePath} (cache size: ${this.promptFileCache.size})`);
                }
            }
            return result;
        }
     ...
     */
    discoverYamlPrompts(categoryDir) {
        if (!existsSync(categoryDir)) {
            return [];
        }
    ...
    loadYamlPrompt(promptPath) {
        ...
        if (this.enableCache && this.promptFileCache.has(cacheKey)) {
            ...
            const yamlData = loadYamlFileSync(yamlPath);
            return {
        ...
        try {
            ...
            if (!validation.valid) {
                this.logger.error(`[PromptLoader] Invalid YAML in ${yamlPath}: ${validation.errors.join(', ')}`);
                this.stats.loadErrors++;
                return null;
            }
            if (validation.warnings.length > 0 && this.debug) {
                this.logger.warn(`[PromptLoader] Warnings for ${promptId}: ${validation.warnings.join(', ')}`);
            }
        ...
        if (yamlData.userMessageTemplateFile) {
            ...
            this.logger.error(`[PromptLoader] No userMessageTemplate or userMessageTemplateFile in ${yamlPath}`);
            this.stats.loadErrors++;
            return null;
        }
        const loadedContent = {
        ...
        const normalizedGateConfiguration = (() => {
            ...
            const inlineGateDefinitions = this.normalizeInlineGateDefinitions(configuration.inline_gate_definitions);
            if (inlineGateDefinitions) {
        ...
        if (this.enableCache) {
            ...
            if (this.debug) {
                this.logger.debug(`[PromptLoader] Cached YAML prompt: ${promptId} (cache size: ${this.promptFileCache.size})`);
            }
        }
        return {
            promptData: this.yamlToPromptData(yamlData, relativeFilePath),
    ...
    normalizeInlineGateDefinitions(definitions) {
        ...
        for (const rawDefinition of definitions) {
            ...
            if (context && typeof context === 'object') {
                inlineDefinition.context = context;
            }
     ...
     */
    yamlToPromptData(yaml, filePath) {
        const promptData = {
            id: yaml.id,
            name: yaml.name,
            ...
            arguments: yaml.arguments?.map((arg) => {
                const normalizedArg = {
                    name: arg.name,
        ...
        if (yaml.gateConfiguration) {
            ...
            const inlineGateDefinitions = this.normalizeInlineGateDefinitions(yaml.gateConfiguration.inline_gate_definitions);
            if (inlineGateDefinitions) {
                normalizedGateConfiguration.inline_gate_definitions = inlineGateDefinitions;
            }
            if (Object.keys(normalizedGateConfiguration).length > 0) {
     ...
     */
    loadAllYamlPrompts(categoryDir) {
