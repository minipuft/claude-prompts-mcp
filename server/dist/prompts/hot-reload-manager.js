...
export class HotReloadManager {
    constructor(logger, categoryManager, config, configManager) {
        ...
        const debounceMs = this.config.debounceMs ?? DEFAULT_HOT_RELOAD_CONFIG.debounceMs ?? 500;
        const watchPromptFiles = this.config.watchPromptFiles ?? DEFAULT_HOT_RELOAD_CONFIG.watchPromptFiles ?? true;
        const watchConfigFiles = this.config.watchConfigFiles ?? DEFAULT_HOT_RELOAD_CONFIG.watchConfigFiles ?? true;
        const recursive = this.config.recursive ?? DEFAULT_HOT_RELOAD_CONFIG.recursive ?? true;
        const ignoredPatterns = this.config.ignoredPatterns ?? DEFAULT_HOT_RELOAD_CONFIG.ignoredPatterns ?? [];
        const maxRetries = this.config.maxRetries ?? DEFAULT_HOT_RELOAD_CONFIG.maxRetries ?? 3;
        const retryDelayMs = this.config.retryDelayMs ?? DEFAULT_HOT_RELOAD_CONFIG.retryDelayMs ?? 1000;
     ...
     */
    setGateReloadCallback(callback) {
    ...
    async handleMethodologyFileChange(event) {
        ...
        this.logger.info(`🔧 Methodology file change detected: ${event.type} - ${event.filename}` +
     ...
     */
    mapToChangeOperation(fileChangeType) {
