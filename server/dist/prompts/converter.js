...
import { createScriptToolDefinitionLoader, } from '../scripts/core/script-definition-loader.js';
 ...
 */
function resolveRegisterWithMcp(promptData, globalRegisterWithMcp) {
...
export class PromptConverter {
    constructor(logger, loader, globalRegisterWithMcp) {
        this.logger = logger;
     ...
     */
    setGlobalRegisterWithMcp(value) {
        if (value !== undefined) {
            ...
            try {
                ...
                if (promptData.tools && promptData.tools.length > 0) {
                    try {
                        ...
                        const loadedTools = this.scriptToolLoader.loadToolsForPrompt(promptDir, promptData.tools, promptData.id);
                        if (loadedTools.length > 0) {
                            convertedPrompt.scriptTools = loadedTools;
                            this.logger.debug(`[PromptConverter] Loaded ${loadedTools.length} script tools for '${promptData.id}'`);
                        }
                        // Store promptDir for script reference resolution ({{script:id}} syntax)
                        convertedPrompt.promptDir = promptDir;
                    }
                    catch (toolLoadError) {
                        this.logger.warn(`[PromptConverter] Failed to load script tools for '${promptData.id}':`, toolLoadError);
    ...
    validateConvertedPrompt(prompt) {
        ...
        if (prompt.userMessageTemplate) {
            ...
            if (prompt.userMessageTemplate.includes('{{#if') ||
                ...
                prompt.userMessageTemplate.includes('{{/unless')) {
                errors.push(`Handlebars syntax detected in template. This system uses Nunjucks syntax.\n` +
            ...
            const orphanedPlaceholders = placeholders.filter((placeholder) => !argumentNames.includes(placeholder) && !this.isSpecialPlaceholder(placeholder));
