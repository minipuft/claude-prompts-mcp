...
export class PromptMatcher {
     ...
     */
    async matchesFilters(prompt, filters, classification) {
        // Debug logging
     ...
     */
    matchesIntent(prompt, classification, intent) {
     ...
     */
    matchesTextSearch(prompt, classification, searchText) {
        ...
        return searchWords.every((word) => {
            return (searchable.includes(word) ||
                ...
                searchable
                    .split(/\s+/)
                    .some((searchableWord) => searchableWord.startsWith(word) || word.startsWith(searchableWord.slice(0, 3))));
        });
    }
    /**
     * Calculate relevance score for search results ordering
     */
    calculateRelevanceScore(prompt, classification, filters) {
        let score = 0;
     ...
     */
    findSimilarPrompts(targetPrompt, allPrompts, limit = 5) {
     ...
     */
    generateSearchSuggestions(partialQuery, allPrompts) {
        const suggestions = [];
