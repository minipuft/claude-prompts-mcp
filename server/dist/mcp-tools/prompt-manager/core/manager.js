...
export class ConsolidatedPromptManager {
    ...
    async createPrompt(args) {
        ...
        const refValidation = refValidator.validate(typedArgs.id, typedArgs.user_message_template, typedArgs.system_message);
    ...
    async updatePrompt(args) {
        ...
        const typedArgsForRef = args;
        const hasTemplateChange = typeof typedArgsForRef.user_message_template === 'string' ||
            typeof typedArgsForRef.system_message === 'string';
        if (hasTemplateChange) {
            const typedPromptData = promptData;
            const refValidator = new PromptReferenceValidator(this.convertedPrompts);
            const refValidation = refValidator.validate(typedArgsForRef.id, typedPromptData.userMessageTemplate, typedPromptData.systemMessage);
            if (!refValidation.valid) {
                const errorDetails = refValidation.errors
                    .map((e) => `• **${e.type}**: ${e.details}`)
                    .join('\n');
        ...
        if (beforeContent !== null &&
            ...
            const versionResult = await this.versionHistoryService.saveVersion(promptDir, 'prompt', promptData.id, beforeContent, {
            ...
            if (versionResult.success) {
                ...
                this.logger.warn(`Failed to save version for prompt ${promptData.id}: ${versionResult.error}`);
            }
        }
        const result = await this.fileOperations.updatePromptImplementation(promptData);
        ...
        for (const [category, prompts] of Object.entries(groupedByCategory)) {
            ...
            for (const { prompt, classification } of prompts) {
                ...
                if (prompt.arguments?.length > 0) {
                    result += `   🔧 **Args**: ${prompt.arguments.map((arg) => arg.name).join(', ')}\n`;
                }
            }
        }
    ...
    async inspectPrompt(args) {
        ...
        if (prompt.arguments?.length) {
            response += `🔧 **Arguments**: ${prompt.arguments.map((arg) => arg.name).join(', ')}\n`;
        }
    ...
    async guidePromptActions(args) {
        ...
        if (quickReference.length > 0) {
            ...
            quickReference.forEach((action) => {
                ...
                sections.push(`- \`${action.id}\` (${this.describeActionStatus(action)}) — Required: ${argsText}`);
            });
        }
        ...
        sections.push('💡 Use `resource_manager(resource_type:"prompt", action:"<id>", ...)` with the required arguments above.');
        return {
    ...
    async handleHistory(args) {
        ...
        if (!prompt) {
            return {
                content: [{ type: 'text', text: `Prompt not found: ${id}` }],
                isError: true,
            };
        }
        const promptsDir = this.configManager.getPromptsDirectory();
        ...
        if (!history || history.versions.length === 0) {
            return {
                content: [
                    {
                        type: 'text',
                        text: `📜 No version history for prompt '${id}'\n\n` +
    ...
    async handleRollback(args) {
        ...
        if (!confirm) {
            return {
                content: [
                    {
                        type: 'text',
                        text: `⚠️ Rollback requires confirmation.\n\n` +
        ...
        const promptDir = path.join(promptsDir, effectiveCategory, id);
        // Perform rollback
        const result = await this.versionHistoryService.rollback(promptDir, 'prompt', id, version, currentPrompt);
        if (!result.success) {
            return {
        ...
        return {
            content: [
                {
                    type: 'text',
                    text: `✅ Prompt '${id}' rolled back to version ${version}\n\n` +
    ...
    async handleCompare(args) {
        ...
        const effectiveCategory = (prompt.category ?? 'general').toLowerCase().replace(/\s+/g, '-');
        const promptDir = path.join(promptsDir, effectiveCategory, id);
        const result = await this.versionHistoryService.compareVersions(promptDir, from_version, to_version);
        if (!result.success) {
            return {
                content: [{ type: 'text', text: `❌ Compare failed: ${result.error}` }],
                isError: true,
            };
        }
        // Generate diff between versions
        const diffResult = this.textDiffService.generateObjectDiff(result.from.snapshot, result.to.snapshot, `${id}/prompt.yaml`);
