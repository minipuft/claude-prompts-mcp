...
import { discoverYamlPromptsInCategory, findYamlPromptInCategory, hasYamlPromptsInCategory, deleteYamlPrompt, } from '../../../prompts/category-maintenance.js';
...
export class FileOperations {
    ...
    async updatePromptImplementation(promptData) {
        ...
        const { exists: promptExists, paths } = await this.createOrUpdateYamlPrompt(promptData, effectiveCategory, promptsDir);
    ...
    async deletePromptImplementation(id) {
        ...
        let deletedFromCategoryDir = null;
        let deletedFromCategoryId = null;
        ...
        if (deletedFromCategoryDir && deletedFromCategoryId) {
            ...
            if (!hasRemainingPrompts) {
                ...
                const nonMetadataEntries = entries.filter((e) => e.name !== 'category.yaml' && !e.name.startsWith('.'));
     ...
     */
    discoverCategoryDirectories(promptsDir) {
     ...
     */
    async createOrUpdateYamlPrompt(promptData, effectiveCategory, promptsDir) {
        const promptDir = path.join(promptsDir, effectiveCategory, promptData.id);
        const paths = [];
        ...
        if (promptData.tools && promptData.tools.length > 0) {
            promptYamlData['tools'] = promptData.tools.map((t) => t.id);
        }
        // Write prompt.yaml
     ...
     */
    async createOrUpdateTools(promptDir, tools, promptId) {
        const messages = [];
        const paths = [];
        ...
        try {
            ...
            for (const categoryDir of categoryDirs) {
                ...
                for (const prompt of prompts) {
                    const promptYamlPath = prompt.format === 'directory' ? path.join(prompt.path, 'prompt.yaml') : prompt.path;
                    try {
