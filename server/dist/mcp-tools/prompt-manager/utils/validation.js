...
export function validateToolDefinitions(tools) {
    ...
    for (const tool of tools) {
        ...
        if (tool.id && !/^[a-z][a-z0-9_-]*$/.test(tool.id)) {
            errors.push(`Tool ID '${tool.id}' must start with lowercase letter and contain only lowercase alphanumeric, underscores, or hyphens`);
        }
        // Duplicate ID check
        if (tool.id) {
        ...
        if (tool.runtime && !validRuntimes.includes(tool.runtime)) {
            errors.push(`Tool '${tool.id}' has invalid runtime: '${tool.runtime}'. Valid: ${validRuntimes.join(', ')}`);
        }
        ...
        if (tool.trigger && !validTriggers.includes(tool.trigger)) {
            errors.push(`Tool '${tool.id}' has invalid trigger: '${tool.trigger}'. Valid: ${validTriggers.join(', ')}`);
        }
