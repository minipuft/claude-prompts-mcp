...
export function validateConfigInput(key, value) {
    switch (key) {
        case 'server.port': {
...
export class SafeConfigWriter {
    constructor(logger, configManager, configPath) {
        this.logger = logger;
     ...
     */
    async updateConfigValue(key, value, options) {
        try {
     ...
     */
    applyConfigChange(config, key, value) {
 ...
 */
export function createSafeConfigWriter(logger, configManager, configPath) {
