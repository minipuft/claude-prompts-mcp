 ...
 */
import { CONFIG_RESTART_REQUIRED_KEYS, CONFIG_VALID_KEYS, createSafeConfigWriter, validateConfigInput, } from './config-utils.js';
import { INJECTION_TYPES, INJECTION_TYPE_DESCRIPTIONS, DECISION_SOURCE_DESCRIPTIONS, isSessionOverrideManagerInitialized, getSessionOverrideManager, initSessionOverrideManager, } from '../execution/pipeline/decisions/injection/index.js';
import { getDefaultRuntimeLoader } from '../frameworks/methodology/index.js';
import { SYSTEM_CONTROL_ACTION_IDS, systemControlMetadata, } from '../tooling/action-metadata/definitions/system-control.js';
...
function createStructuredResponse(content, second, third) {
    let metadata;
    let isError = false;
...
export class ConsolidatedSystemControl {
    constructor(logger, mcpServer, onRestart) {
     ...
     */
    setMCPToolsManager(mcpToolsManager) {
     ...
     */
    setChainSessionManager(chainSessionManager) {
     ...
     */
    setGateGuidanceRenderer(renderer) {
     ...
     */
    createMinimalSystemResponse(text, action) {
        const now = Date.now();
        ...
        const frameworkEnabled = systemHealth?.frameworkSystemEnabled ?? frameworkState?.frameworkSystemEnabled ?? false;
        const activeFramework = frameworkState?.activeFramework ?? systemHealth?.activeFramework ?? 'unknown';
        const availableFrameworks = systemHealth?.availableFrameworks ??
     ...
     */
    async persistGateConfig(enabled) {
     ...
     */
    async persistFrameworkConfig(enabled) {
        if (!this.safeConfigWriter) {
            return '⚠️ Persistence skipped (config writer unavailable).';
        }
        const keys = [
    ...
    async handleAction(args, extra) {
        ...
        try {
            if (!isSystemControlActionId(action)) {
                ...
                throw new Error(`Unknown action: ${action}. Valid actions: ${SYSTEM_CONTROL_ACTION_IDS.join(', ')}`);
            }
            ...
            const status = error instanceof Error && /Unknown action/i.test(error.message) ? 'unknown' : 'failure';
    ...
    async getSystemStatus(args) {
        ...
        if (include_metrics) {
            ...
            response += `- Success Rate: ${this.systemAnalytics.totalExecutions > 0
                ? Math.round((this.systemAnalytics.successfulExecutions / this.systemAnalytics.totalExecutions) *
                    100)
                : 0}%\n`;
    ...
    async switchFramework(args) {
        ...
        const result = await this.frameworkManager.switchFramework(args.framework, args.reason || `User requested switch to ${args.framework}`);
        if (!result.success) {
     ...
     */
    async inspectMethodology(args) {
        const methodologyId = args.methodology_id?.toLowerCase();
        const runtimeLoader = getDefaultRuntimeLoader();
        if (!methodologyId) {
            // List available methodologies
            const available = runtimeLoader.discoverMethodologies();
            return this.createMinimalSystemResponse(`📋 **Available Methodologies**\n\n` +
                `Use \`operation:"inspect" methodology_id:"<id>"\` to inspect a specific methodology.\n\n` +
                `Available: ${available.join(', ')}`, 'inspect_methodology');
        }
        const definition = runtimeLoader.loadMethodology(methodologyId);
        if (!definition) {
            const available = runtimeLoader.discoverMethodologies();
            return this.createMinimalSystemResponse(`❌ **Methodology Not Found**: \`${methodologyId}\`\n\n` +
                `Available methodologies: ${available.join(', ')}`, 'inspect_methodology');
        }
        ...
        if (definition.methodologyGates?.length) {
            ...
            definition.methodologyGates.slice(0, 3).forEach((gate) => {
     ...
     */
    async listMethodologiesAction(args) {
        const runtimeLoader = getDefaultRuntimeLoader();
        const methodologyIds = runtimeLoader.discoverMethodologies();
        if (methodologyIds.length === 0) {
            return this.createMinimalSystemResponse(`📋 **No Methodologies Found**\n\n` +
                `Ensure YAML files exist in \`resources/methodologies/<id>/methodology.yaml\`.`, 'list_methodologies');
        }
    ...
    async enableFrameworkSystem(args) {
        if (!this.frameworkStateManager) {
            throw new Error('Framework state manager not initialized');
        }
        const currentState = this.frameworkStateManager.getCurrentState();
        if (currentState.frameworkSystemEnabled) {
            return this.createMinimalSystemResponse(`ℹ️ Framework system is already enabled.`, 'enable_framework_system');
        }
        // Enable framework system (for now, just return success message since the method may not exist)
        try {
            await this.frameworkStateManager.enableFrameworkSystem?.(args.reason || 'User requested to enable framework system');
        }
        catch (error) {
    ...
    async disableFrameworkSystem(args) {
        if (!this.frameworkStateManager) {
            throw new Error('Framework state manager not initialized');
        }
        const currentState = this.frameworkStateManager.getCurrentState();
        if (!currentState.frameworkSystemEnabled) {
            return this.createMinimalSystemResponse(`ℹ️ Framework system is already disabled.`, 'disable_framework_system');
        }
        // Disable framework system (for now, just return success message since the method may not exist)
        try {
            await this.frameworkStateManager.disableFrameworkSystem?.(args.reason || 'User requested to disable framework system');
        }
        catch (error) {
            // Method may not exist, that's ok for now
        }
    ...
    async enableGateSystem(args) {
        ...
        if (currentState.enabled) {
            return this.createMinimalSystemResponse(`ℹ️ Gate system is already enabled.`, 'enable_gate_system');
        }
        await this.gateSystemManager.enableGateSystem(args.reason || 'User requested to enable gate system');
        const persistenceNotes = [];
        if (args.persist) {
        ...
        const response = `✅ **Gate System Enabled**\n\n` +
     ...
     * Disable gate system
     */
    async disableGateSystem(args) {
        if (!this.gateSystemManager) {
            throw new Error('Gate system manager not initialized');
        }
        const currentState = this.gateSystemManager.getCurrentState();
        if (!currentState.enabled) {
            return this.createMinimalSystemResponse(`ℹ️ Gate system is already disabled.`, 'disable_gate_system');
        }
        await this.gateSystemManager.disableGateSystem(args.reason || 'User requested to disable gate system');
        const persistenceNotes = [];
        if (args.persist) {
        ...
        const response = `⚠️ **Gate System Disabled**\n\n` +
     ...
     */
    async getGateSystemStatus() {
        if (!this.gateSystemManager) {
            return this.createMinimalSystemResponse(`❌ Gate system manager not available.`, 'gate_system_status');
        }
        const currentState = this.gateSystemManager.getCurrentState();
     ...
     */
    async getGateSystemHealth() {
        if (!this.gateSystemManager) {
            return this.createMinimalSystemResponse(`❌ Gate system manager not available.`, 'gate_system_health');
        }
        const health = this.gateSystemManager.getSystemHealth();
    ...
    async listAvailableGates(searchQuery) {
        ...
        if (filteredGates.length === 0) {
            ...
            return this.createMinimalSystemResponse(`📋 **Available Quality Gates**\n\n${noResultsMsg}`, 'gate_list');
        }
        const lines = ['📋 **Available Quality Gates**', ''];
        if (searchQuery) {
    ...
    async resetMetrics(args) {
        if (!args.confirm) {
            return this.createMinimalSystemResponse("❌ Metrics reset cancelled. Set 'confirm: true' to reset all switching performance metrics.", 'reset_metrics');
        }
    ...
    formatTrendContext(trend) {
        ...
        if (trend.executionMode)
            context += ` (${trend.executionMode})`;
        if (trend.success !== undefined)
            context += trend.success ? ' ✓' : ' ✗';
        return context;
    }
    ...
    async restoreConfig(args) {
        ...
        if (!args.confirm) {
            return this.createMinimalSystemResponse("❌ Restore cancelled. Set 'confirm: true' to restore configuration.", 'restore_config');
        }
        try {
            if (!this.safeConfigWriter) {
            ...
            if (result.success) {
                return this.createMinimalSystemResponse('✅ Configuration restored successfully.', 'restore_config');
            }
            else {
    ...
    async manageConfig(args) {
        ...
        if (!this.configManager) {
            return createStructuredResponse('❌ **Configuration Manager Unavailable**', { operation: 'config', error: 'config_manager_unavailable' }, true);
        }
        try {
            if (!configRequest) {
    ...
    async restartServer(args) {
        if (!args.confirm) {
            return this.createMinimalSystemResponse("❌ Restart cancelled. Set 'confirm: true' to perform a full system restart.", 'restart_server');
        }
        ...
        if (this.onRestart) {
            ...
            return this.createMinimalSystemResponse(`🔄 Server restart initiated.\n\n**Reason**: ${reason}\n**Status**: Restarting in 1 second...`, 'restart_server');
        }
        else {
            return this.createMinimalSystemResponse('⚠️ Restart callback not configured. Server cannot be restarted automatically.', 'restart_server');
        }
    }
    async handleConfigList() {
        const config = this.configManager?.getConfig();
        return this.createMinimalSystemResponse(`📋 **Current Configuration**\n\`\`\`json\n${JSON.stringify(config, null, 2)}\n\`\`\``, 'config_list');
    }
    ...
    async handleConfigValidate(key, value) {
        if (!this.configManager)
            throw new Error('Config manager unavailable');
        const validation = validateConfigInput(key, value);
        return this.createMinimalSystemResponse(validation.valid
            ? `✅ Configuration valid for **${key}**`
            : `❌ Invalid configuration for **${key}**: ${validation.error}`, 'config_validate');
    }
}
/**
...
class FrameworkActionHandler extends ActionHandler {
    async execute(args) {
        ...
        switch (operation) {
            ...
            default:
                throw new Error(`Unknown framework operation: ${operation}. Valid operations: switch, list, enable, disable, inspect, list_methodologies`);
        }
    }
}
/**
...
class GateActionHandler extends ActionHandler {
    async execute(args) {
        ...
        switch (operation) {
            ...
            default:
                throw new Error(`Unknown gates operation: ${operation}. Valid operations: enable, disable, status, health, list`);
        }
    }
}
/**
...
class GuideActionHandler extends ActionHandler {
    async execute(args) {
        ...
        const operations = systemControlMetadata.data.operations.filter((operation) => includePlanned || operation.status !== 'planned');
        const filtered = topic.length > 0
            ? operations.filter((operation) => operation.category.toLowerCase().includes(topic) ||
...
class MaintenanceActionHandler extends ActionHandler {
    ...
    async getSystemStatus(args) {
        ...
        return this.responseFormatter.formatPromptEngineResponse(response, formatterContext, {
            ...
            metadata: {
                ...
                availableFrameworks: this.frameworkManager?.listFrameworks().map((f) => f.name) || [],
                uptime: (now - this.startTime) / 1000,
                memoryUsage: process.memoryUsage ? process.memoryUsage() : undefined,
                serverHealth: systemHealth.status,
    ...
    async getAnalytics(args) {
        ...
        response += `**Gate Adoption Rate**: ${this.systemControl.systemAnalytics.totalExecutions > 0
            ? Math.round((this.systemControl.systemAnalytics.gateValidationCount /
    ...
    async getSystemHealth(args) {
        ...
        response += `**Framework Switch Success Rate**: ${health.switchingMetrics.totalSwitches > 0
            ? Math.round((health.switchingMetrics.successfulSwitches / health.switchingMetrics.totalSwitches) *
                100)
            : 100}%\n`;
    ...
    async manageConfig(args) {
        ...
        if (!this.configManager) {
            return createStructuredResponse('❌ **Configuration Manager Unavailable**\n\n' +
                'ConfigManager is not initialized. This indicates a system initialization issue.\n' +
                'Configuration management requires proper system startup.', { operation: 'config', error: 'config_manager_unavailable' }, true);
        }
        try {
    ...
    async handleConfigGet(key) {
        ...
        if (value === undefined) {
            return createStructuredResponse(`❌ **Configuration Key Not Found**\n\n` +
                `The key \`${key}\` does not exist in the configuration.\n\n` +
                `Use the \`list\` operation to see available configuration keys.`, { operation: 'config_get', key, error: 'key_not_found' }, true);
        }
    ...
    async handleConfigSet(key, value) {
        ...
        if (!this.safeConfigWriter) {
            return createStructuredResponse(`⚠️ **Configuration Writing Unavailable**\n\n` +
    ...
    validateConfigValue(key, value) {
        ...
        const inferredType = validation.valueType ??
    ...
    isValidConfigKey(key) {
        return CONFIG_VALID_KEYS.includes(key);
    }
    /**
     * Check if config key requires restart
     */
    requiresRestart(key) {
        return CONFIG_RESTART_REQUIRED_KEYS.includes(key);
    }
    getValidationSuggestion(key) {
        const suggestions = {
    ...
    async enableFrameworkSystem(args) {
        try {
            if (!this.systemControl.frameworkStateManager) {
                return createStructuredResponse('❌ Framework State Manager not available', {
     ...
     */
    async getFrameworkSystemStatus(args) {
        try {
            if (!this.systemControl.frameworkStateManager) {
                return createStructuredResponse('❌ Framework State Manager not available', {
    ...
    handleError(error, context) {
        ...
        return this.responseFormatter.formatErrorResponse(error instanceof Error ? error : String(error), formatterContext, {
            metadata: {
                tool: 'system_control',
                operation: context,
            },
...
class InjectionActionHandler extends ActionHandler {
    async execute(args) {
        ...
        switch (operation) {
            ...
            default:
                return this.createMinimalSystemResponse(`❌ Unknown injection operation: ${operation}\n\n` +
                    'Valid operations:\n' +
                    '- `status` - Show injection configuration and active overrides\n' +
                    '- `override` - Set a session override for an injection type\n' +
                    '- `reset` - Clear all session overrides', 'injection_error');
        }
    }
    /**
        ...
        if (isSessionOverrideManagerInitialized()) {
            ...
            if (status.activeOverrides === 0) {
                ...
                for (const override of status.overrides) {
                    const enabledStr = override.enabled === true
                        ? '✅ Enabled'
     ...
     */
    setInjectionOverride(args) {
        const type = args.type;
        const enabled = args.enabled;
        const scope = args.scope || 'session';
        const scopeId = args.scope_id;
        const expiresInMs = args.expires_in_ms;
        // Validate type
        if (!type || !INJECTION_TYPES.includes(type)) {
            return this.createMinimalSystemResponse(`❌ Invalid injection type: \`${type}\`\n\n` +
                `Valid types: ${INJECTION_TYPES.map((t) => `\`${t}\``).join(', ')}`, 'injection_override_error');
        }
        // Validate enabled is provided
        if (enabled === undefined) {
            return this.createMinimalSystemResponse('❌ Missing `enabled` parameter.\n\n' +
                'Specify `enabled:true` to enable injection or `enabled:false` to disable.', 'injection_override_error');
        }
     ...
     */
    resetInjectionOverrides() {
        if (!isSessionOverrideManagerInitialized()) {
            return this.createMinimalSystemResponse('✅ **No Active Overrides**\n\nSession override manager was not initialized. Nothing to reset.', 'injection_reset');
        }
        const manager = getSessionOverrideManager();
        const count = manager.clearAllOverrides();
        return this.createMinimalSystemResponse(`✅ **Injection Overrides Cleared**\n\n` +
            `Cleared ${count} override${count === 1 ? '' : 's'}.\n\n` +
            '_Injection decisions will now use default configuration._', 'injection_reset');
    }
}
/**
...
class SessionActionHandler extends ActionHandler {
    async execute(args) {
        ...
        switch (operation) {
            ...
            default:
                throw new Error(`Unknown session operation: ${operation}. Valid operations: list, clear, inspect`);
        }
    }
    async listSessions(args) {
        ...
        if (sessions.length === 0) {
            return this.createMinimalSystemResponse('📭 **No Active Sessions**\n\nThere are currently no active chain sessions.', 'session_list');
        }
    ...
    async clearSession(args) {
        ...
        if (wasSessionCleared) {
            return this.createMinimalSystemResponse(`✅ **Session Cleared**: \`${sessionId}\`\n\nAll state and artifacts for this session have been removed.`, 'session_clear');
        }
        // Try clearing all sessions for this chain
        await manager.clearSessionsForChain(sessionId);
        return this.createMinimalSystemResponse(`✅ **Chain Sessions Cleared**: \`${sessionId}\`\n\nAll sessions and history associated with chain ID \`${sessionId}\` have been removed.`, 'session_clear');
    }
    async inspectSession(args) {
        ...
        const varNames = Object.keys(context).filter((k) => !['chain_run_id', 'chain_id', 'current_step', 'total_steps', 'execution_order'].includes(k));
...
function isSystemControlActionId(value) {
    return SYSTEM_CONTROL_ACTION_IDS.includes(value);
}
/**
 * Create consolidated system control
 */
export function createConsolidatedSystemControl(logger, mcpServer, onRestart) {
