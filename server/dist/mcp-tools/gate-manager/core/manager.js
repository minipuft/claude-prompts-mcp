...
export class ConsolidatedGateManager {
    ...
    async handleAction(args, _context) {
        ...
        try {
            ...
            return this.createErrorResponse(`Error in gate_manager: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    // ============================================================================
    // Action Handlers
    ...
    async handleCreate(args) {
        ...
        return this.createSuccessResponse(`✅ Gate '${id}' created successfully\n\n` +
    ...
    async handleUpdate(args) {
        ...
        if (this.versionHistoryService.isAutoVersionEnabled() && !skipVersion) {
            ...
            const diffForVersion = this.textDiffService.generateObjectDiff(beforeState, afterState, `${id}/gate.yaml`);
            const diffSummary = `+${diffForVersion.stats.additions}/-${diffForVersion.stats.deletions}`;
            const versionResult = await this.versionHistoryService.saveVersion(gateDir, 'gate', id, beforeState, {
        ...
        // Generate diff view
        const diffResult = this.textDiffService.generateObjectDiff(beforeState, afterState, `${id}/gate.yaml`);
        let response = `✅ Gate '${id}' updated successfully\n\n` +
            `📁 Files updated:\n${result.paths?.map((p) => `  - ${p}`).join('\n')}\n\n`;
    ...
    async handleDelete(args) {
        ...
        try {
            ...
            return this.createErrorResponse(`Failed to delete gate directory: ${error instanceof Error ? error.message : String(error)}`);
        }
        ...
        await this.onRefresh?.();
        return this.createSuccessResponse(`✅ Gate '${id}' deleted successfully\n\n` +
    ...
    async handleList(args) {
        ...
        if (gates.length === 0) {
            return this.createSuccessResponse(`📋 No gates found${enabled_only ? ' (enabled only)' : ''}\n\n` +
        ...
        const gateList = gates
            ...
            .join('\n');
        return this.createSuccessResponse(`📋 Gates (${gates.length} total)\n\n` +
            `${gateList}\n\n` +
            `📊 Registry Stats:\n` +
    ...
    async handleInspect(args) {
        ...
        return this.createSuccessResponse(`🚦 Gate: ${gate.name}\n\n` +
            `📋 Details:\n` +
            `  - ID: ${gate.gateId}\n` +
    ...
    async handleHistory(args) {
        ...
        if (!history || history.versions.length === 0) {
            return this.createSuccessResponse(`📜 No version history for gate '${id}'\n\n` +
    ...
    async handleRollback(args) {
        ...
        // Perform rollback
        const result = await this.versionHistoryService.rollback(gateDir, 'gate', id, version, currentState);
        if (!result.success) {
        ...
        const gateData = {
            ...
            type: snapshot['type'] ?? existingGate.type ?? 'validation',
        ...
        // Reload the gate
        await this.gateManager.reload(id);
        return this.createSuccessResponse(`✅ Gate '${id}' rolled back to version ${version}\n\n` +
    ...
    async handleCompare(args) {
        ...
        const gatesDir = this.getGatesDirectory();
        const gateDir = path.join(gatesDir, id);
        const result = await this.versionHistoryService.compareVersions(gateDir, from_version, to_version);
        if (!result.success) {
            return this.createErrorResponse(`Compare failed: ${result.error}`);
        }
        // Generate diff between versions
        const diffResult = this.textDiffService.generateObjectDiff(result.from.snapshot, result.to.snapshot, `${id}/gate.yaml`);
 ...
 */
export function createConsolidatedGateManager(deps) {
