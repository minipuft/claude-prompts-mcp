...
export type PromptManagerAction = ({
    ...
    action: 'list';
} & Pick<BasePromptManagerArgs, 'filter' | 'format'>) | ({
    action: 'inspect';
    id: string;
} & Pick<BasePromptManagerArgs, 'detail' | 'format'>);
/**
...
export type SystemControlAction = ({
    action: 'status';
} & Pick<BaseSystemControlArgs, 'include_history' | 'include_metrics' | 'operation'>) | ({
    action: 'framework';
    framework?: string;
} & Pick<BaseSystemControlArgs, 'reason' | 'show_details' | 'operation'>) | ({
    action: 'analytics';
} & Pick<BaseSystemControlArgs, 'include_history' | 'reset_analytics' | 'limit' | 'confirm' | 'operation'>) | ({
    action: 'config';
    config?: ConfigObject;
    backup_path?: string;
} & Pick<BaseSystemControlArgs, 'confirm' | 'operation'>) | ({
    action: 'maintenance';
} & Pick<BaseSystemControlArgs, 'reason' | 'confirm' | 'operation'>);
/**
