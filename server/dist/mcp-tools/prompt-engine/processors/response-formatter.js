...
function normalizeToolResponse(payload, defaultIsError = false) {
    if (payload && typeof payload === 'object') {
        ...
        if (Array.isArray(obj['content'])) {
            const normalized = {
                content: obj['content'],
                isError: obj['isError'] ?? defaultIsError,
            };
            if (obj['structuredContent'] !== undefined) {
                normalized.structuredContent = obj['structuredContent'];
            }
        ...
        if (typeof obj['message'] === 'string') {
            const structuredContent = obj['structuredContent'];
            return {
                content: [{ type: 'text', text: obj['message'] }],
                isError: obj['isError'] ?? defaultIsError,
                ...(structuredContent !== undefined && { structuredContent }),
            };
        }
...
export class ResponseFormatter {
    ...
    formatPromptEngineResponse(response, executionContext = {
        ...
        try {
            ...
            const includeStructuredContent = executionContext.executionType === 'chain' &&
