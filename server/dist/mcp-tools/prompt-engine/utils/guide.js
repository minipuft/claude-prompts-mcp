...
export function renderPromptEngineGuide(goal) {
    ...
    const fullParameterCatalog = promptEngineMetadata.data.parameters.filter((param) => param.status !== 'deprecated');
    const sections = [];
    sections.push('🧭 **Prompt Engine Guide**');
    sections.push('Executes prompts, templates, and chains with framework awareness plus advanced gate controls. Metadata-driven output keeps this description synchronized with the tool implementation.');
    sections.push(normalizedGoal
    ...
    if (parameterList.length > 0) {
        ...
        parameterList.forEach((param) => {
            const issues = param.issues && param.issues.length > 0
                ? ` Issues: ${param.issues.map((issue) => issue.summary).join(' • ')}`
                : '';
            sections.push(`- \`${param.name}\` (${describePromptEngineStatus(param.status)}) — ${param.description}${issues}`);
        });
    }
    if (commandList.length > 0) {
        sections.push('### Built-in Commands');
        commandList.forEach((cmd) => {
            sections.push(`- \`${cmd.id}\` (${describePromptEngineStatus(cmd.status)}) — ${cmd.description}`);
        });
    }
    ...
    if (fullParameterCatalog.length > 0) {
        ...
        fullParameterCatalog.forEach((param) => {
            sections.push(`- \`${param.name}\` (${describePromptEngineStatus(param.status)}) — ${param.description}`);
        });
    }
    sections.push('💡 Tip: Use the unified `gates` parameter to specify validation criteria. The response footer will include a `gate_verdict` format reminder when gates are active.');
    return sections.join('\n\n');
}
function selectGuideParameters(goal) {
    const normalizedGoal = goal.toLowerCase();
    const parameters = promptEngineMetadata.data.parameters.filter((param) => param.status !== 'deprecated');
    if (!normalizedGoal) {
...
function computeParameterScore(param, goal) {
...
function describePromptEngineStatus(status) {
    switch (status) {
        case 'working':
            return '✅ Working';
        case 'planned':
...
function formatUsagePattern(pattern) {
    ...
    const notes = pattern.notes && pattern.notes.length > 0
 ...
 */
function renderGatesGuide() {
    const sections = [];
    sections.push('🔐 **Gates Syntax Guide**');
    sections.push('Quality gates enforce validation criteria during prompt execution. ' +
