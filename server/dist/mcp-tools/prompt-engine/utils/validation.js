...
export class EngineValidator {
     ...
     */
    validateArguments(convertedPrompt, promptArgs) {
        const errors = [];
        const warnings = [];
        let score = 100;
        ...
        const expectedArgs = convertedPrompt.arguments.map((arg) => arg.name);
     ...
     */
    isValidArgumentType(value, expectedType) {
     ...
     */
    async validateWithGates(convertedPrompt, promptArgs, suggestedGates = [], processedContent) {
        try {
            ...
            for (const gateName of suggestedGates) {
                try {
                    ...
                    const primaryMessage = gateResult.errors?.[0]?.message ?? (gateResult.valid ? 'Gate passed' : 'Gate failed');
                    results.push({
                        gate: gateName,
