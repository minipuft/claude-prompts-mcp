...
export class CategoryExtractor {
     ...
     */
    extractCategoryFromPattern(promptId) {
        const patterns = [
     ...
     */
    selectGatesWithPrecedence(categoryResult, frameworkGates = [], fallbackGates = ['content-structure']) {
     ...
     */
    selectGatesWithEnhancedPrecedence(categoryResult, frameworkGates = [], fallbackGates = ['content-structure'], temporaryGates = [], enhancedConfig) {
        const { category, gateConfiguration } = categoryResult;
        const categoryGates = CategoryExtractor.getCategoryGateMapping()[category] || [];
 ...
 */
export function extractPromptCategory(prompt, logger) {
    const extractor = new CategoryExtractor(logger);
    return extractor.extractCategory(prompt);
}
//# sourceMappingURL=category-extractor.js.map