 ...
 */
export function detectChainManagementCommand(command) {
    if (!command) {
        return null;
    }
...
export class ChainManagementService {
    constructor(initialPrompts, sessionManager, responseFormatter, gateSystem) {
        this.sessionManager = sessionManager;
        this.responseFormatter = responseFormatter;
        this.gateSystem = gateSystem;
        this.promptLookup = new Map();
    ...
    async tryHandleCommand(command) {
        ...
        try {
            switch (match.action) {
                ...
                default:
                    return this.responseFormatter.formatErrorResponse(`Unknown chain management action: ${match.action}`);
            }
        }
        catch (error) {
            const message = error instanceof Error ? error.message : String(error);
            return this.responseFormatter.formatErrorResponse(`Chain management command failed: ${message}`);
        }
    }
    async handleValidate(target, parameters) {
        ...
        const session = this.sessionManager.getSessionByChainIdentifier(target, { includeDormant: true }) ??
        ...
        if (session) {
            ...
            const metadata = context['chain_metadata'];
            const lines = [
                `## 🔁 Active Chain Run`,
        ...
        return this.responseFormatter.formatErrorResponse(`Chain "${target || '<unspecified>'}" not found${scope}.`);
    }
    async handleList(parameters) {
        const limitParam = parameters['limit'];
        const limit = limitParam ? Number(limitParam) : 20;
        const activeSessions = this.sessionManager.listActiveSessions(limit);
        if (activeSessions.length === 0) {
            return this.responseFormatter.formatPromptEngineResponse('No active chain sessions detected.', undefined, { metadata: { type: 'chain_list', count: 0 } });
        }
        const lines = ['## 📋 Active Chain Sessions'];
        activeSessions.forEach((summary) => {
            lines.push(`- **${summary.chainId}** → resume with \`chain_id=${summary.chainId}\` (archive run ${summary.sessionId})` +
    ...
    async handleGates(target, parameters) {
        const prompt = this.findPrompt(target);
        const session = this.sessionManager.getSessionByChainIdentifier(target, { includeDormant: true }) ??
            (target ? this.sessionManager.getSession(target) : undefined);
        if (!prompt && !session) {
            return this.responseFormatter.formatErrorResponse(`No chain prompt or session found for "${target}".`);
        }
        const sections = ['## 🔒 Chain Gate Overview'];
        if (prompt) {
    ...
    buildPromptGateSummary(prompt) {
        ...
        if (enhanced?.inline_gate_definitions && enhanced.inline_gate_definitions.length > 0) {
            lines.push(`- Inline Gate Definitions: ${enhanced.inline_gate_definitions.map((gate) => gate.name).join(', ')}`);
        }
    ...
    buildSessionGateSummary(session) {
        ...
        const lines = [
            ...
            `- Chain-Scoped Temporary Gates: ${tempGates.length ? tempGates.map((gate) => gate.name).join(', ') : 'none'}`,
        ];
        if (session.pendingGateReview) {
            lines.push(`- Pending Gate Review: ${session.pendingGateReview.gateIds?.join(', ') || 'unspecified'}`);
        }
        return lines.join('\n');
    }
}
function parseKeyValueParams(paramString) {
    const params = {};
