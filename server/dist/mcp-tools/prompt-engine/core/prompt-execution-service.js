...
import { PromptReferenceResolver, ScriptReferenceResolver, } from '../../../execution/reference/index.js';
...
import { createStyleManager } from '../../../styles/index.js';
...
export class PromptExecutionService {
    constructor(logger, mcpServer, promptManager, configManager, semanticAnalyzer, conversationManager, textReferenceManager, gateManager, mcpToolsManager, promptGuidanceService) {
        ...
        this.lightweightGateSystem = new LightweightGateSystem(gateProvider, gateValidator, temporaryGateRegistry);
        ...
        this.gateGuidanceRenderer = createGateGuidanceRenderer(logger, {
            ...
            temporaryGateRegistry: this.lightweightGateSystem.getTemporaryGateRegistry?.() ?? temporaryGateRegistry,
    ...
    updateData(_promptsData, convertedPrompts) {
        ...
        this.scriptReferenceResolver = new ScriptReferenceResolver(this.logger, scriptLoader, scriptExecutor);
        this.chainOperatorExecutor = this.createChainOperatorExecutor();
        this.resetPipeline();
    }
    ...
    async executePromptCommand(args, extra) {
        ...
        const chainIdValue = args.chain_id ?? (shouldTreatAsResumeOnly ? chainIdFromCommand : undefined);
        const request = {
        ...
        try {
            switch (targetTool) {
                ...
                case 'prompt_engine_invalid_command':
                    return this.responseFormatter.formatErrorResponse('Commands must start with a real prompt id after `>>`. Use resource_manager(resource_type:"prompt", action:"list") to find valid ids before executing.');
                default:
                    break;
            }
    ...
    createChainOperatorExecutor() {
        return new ChainOperatorExecutor(this.logger, this.convertedPrompts, this.gateGuidanceRenderer, this.resolveFrameworkContextForPrompt.bind(this), this.promptGuidanceService, this.referenceResolver, this.scriptReferenceResolver);
    }
    ...
    buildPromptExecutionPipeline() {
        ...
        const scriptExecutionStage = new ScriptExecutionStage(scriptExecutor, toolDetectionService, executionModeService, this.logger);
        ...
        const judgeSelectionStage = new JudgeSelectionStage(() => this.convertedPrompts, this.lightweightGateSystem.gateLoader, this.configManager, this.logger, undefined, // frameworksProvider - uses default methodology loader
        this.styleManager ?? null);
        const promptGuidanceStage = new PromptGuidanceStage(this.promptGuidanceService ?? null, this.styleManager ?? null, this.logger);
        const gateStage = new GateEnhancementStage(this.createGateService(), temporaryGateRegistry, () => this.configManager.getFrameworksConfig(), this.gateReferenceResolver, () => this.frameworkManager, this.logger, () => this.analyticsService, () => this.configManager.getGatesConfig(), this.lightweightGateSystem.gateLoader, () => this.gateManager // Provider for registry-based gate selection
        );
...
export function createPromptExecutionService(logger, mcpServer, promptManager, configManager, semanticAnalyzer, conversationManager, textReferenceManager, gateManager, mcpToolsManager, promptGuidanceService) {
    return new PromptExecutionService(logger, mcpServer, promptManager, configManager, semanticAnalyzer, conversationManager, textReferenceManager, gateManager, mcpToolsManager, promptGuidanceService);
}
