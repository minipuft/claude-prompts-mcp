...
export class MethodologyFileService {
     ...
     */
    async loadExistingMethodology(id) {
        const methodologyDir = this.getMethodologyDir(id);
        const methodologyPath = join(methodologyDir, 'methodology.yaml');
        ...
        try {
            const methodology = await loadYamlFile(methodologyPath);
            ...
            if (phasesFileRef !== undefined && phasesFileRef !== null) {
                ...
                if (existsSync(phasesPath)) {
                    const loadedPhases = await loadYamlFile(phasesPath);
     ...
     */
    toMethodologyCreationData(id, existing) {
        ...
        const systemGuidance = systemPrompt ?? (typeof rawSystemGuidance === 'string' ? rawSystemGuidance : undefined);
        ...
        if (rawToolDescriptions !== undefined && rawToolDescriptions !== null) {
            data.tool_descriptions = rawToolDescriptions;
        }
        ...
        const rawMethodologyGates = methodology['methodologyGates'] ?? phasesSource['methodology_gates'];
        ...
        const rawQualityIndicators = phasesSource['qualityIndicators'] ?? phasesSource['quality_indicators'];
        const rawTemplateEnhancements = phasesSource['templateEnhancements'] ?? phasesSource['template_enhancements'];
        const rawExecutionFlow = phasesSource['executionFlow'] ?? phasesSource['execution_flow'];
        const rawMethodologyElements = methodology['methodologyElements'] ?? phasesSource['methodology_elements'];
        const rawArgumentSuggestions = methodology['argumentSuggestions'] ?? phasesSource['argument_suggestions'];
        const rawTemplateSuggestions = methodology['templateSuggestions'] ?? phasesSource['template_suggestions'];
        if (Array.isArray(rawPhases)) {
            data.phases = rawPhases;
        }
        if (Array.isArray(rawMethodologyGates)) {
            data.methodology_gates = rawMethodologyGates;
        }
        if (Array.isArray(rawProcessingSteps)) {
            data.processing_steps = rawProcessingSteps;
        }
        if (Array.isArray(rawExecutionSteps)) {
            data.execution_steps = rawExecutionSteps;
        }
        if (rawQualityIndicators !== undefined && rawQualityIndicators !== null) {
            data.quality_indicators = rawQualityIndicators;
        }
        ...
        if (rawMethodologyElements !== undefined && rawMethodologyElements !== null) {
            data.methodology_elements = rawMethodologyElements;
        }
        if (Array.isArray(rawArgumentSuggestions)) {
            data.argument_suggestions = rawArgumentSuggestions;
        }
        if (Array.isArray(rawTemplateSuggestions)) {
            data.template_suggestions = rawTemplateSuggestions;
        }
        return data;
    }
    /**
     ...
     */
    async writeMethodologyFiles(data, existingData) {
        ...
        try {
            ...
            const finalMethodologyData = existingData !== undefined && existingData !== null
            ...
            const originalMethodologyContent = existingData !== undefined && existingData !== null
            ...
            if (needsPhasesFile) {
                ...
                const finalPhasesData = existingPhases !== null && hasNewPhasesData
                ...
                if (Object.keys(finalPhasesData).length > 0) {
                    ...
                    const originalPhasesContent = existingPhases !== null ? serializeYaml(existingPhases, { sortKeys: false }) : null;
    ...
    buildMethodologyYamlData(data) {
        ...
        yamlData['version'] ?? (yamlData['version'] = '1.0.0');
        return yamlData;
    }
    /**
     * Build phases.yaml data from input (only sets defined fields)
     */
    buildPhasesYamlData(data) {
        const phasesData = {};
...
//# sourceMappingURL=methodology-file-service.js.map