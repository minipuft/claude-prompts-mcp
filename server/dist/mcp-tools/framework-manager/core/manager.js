...
export class ConsolidatedFrameworkManager {
     ...
     */
    async createMethodologyAtomic(id, methodologyData) {
        const normalizedId = id.toLowerCase();
        const registry = this.frameworkManager.getMethodologyRegistry();
    ...
    validateMethodology(data) {
        ...
        const level = score >= 80 ? 'full' : score >= 50 ? 'standard' : 'incomplete';
     ...
     */
    createValidationErrorResponse(id, validation) {
     ...
     */
    formatValidationSuccess(id, validation, paths) {
     ...
     */
    async handleAction(args, _context) {
        const { action } = args;
        try {
            switch (action) {
                case 'create':
            ...
            return this.createErrorResponse(`Error in framework_manager: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    // ============================================================================
    // Action Handlers
    ...
    async handleCreate(args) {
        ...
        const derivedType = methodology !== undefined && methodology !== ''
            ? methodology
        ...
        if (exists.inAnySource) {
            return this.createErrorResponse(`Methodology '${id}' already exists in: ${exists.sources.join(', ')}. Use update action to modify.`);
        }
        ...
        return this.createSuccessResponse(this.formatValidationSuccess(id, validation, result.paths ?? []));
    }
    async handleUpdate(args) {
        ...
        if (existingFramework === undefined) {
            return this.createErrorResponse(`Methodology '${id}' not found. Use create action to add new methodology.`);
        }
        ...
        if (existingData === null) {
            return this.createErrorResponse(`Failed to load methodology files for '${id}'. Files may be corrupted.`);
        }
        ...
        if (args.name !== undefined)
            methodologyData.name = args.name;
        ...
        if (args.enabled !== undefined)
            methodologyData.enabled = args.enabled;
        ...
        if (this.versionHistoryService.isAutoVersionEnabled() && !skipVersion) {
            ...
            const diffForVersion = this.textDiffService.generateObjectDiff(beforeState, afterState, `${id}/methodology.yaml`);
            const diffSummary = `+${diffForVersion.stats.additions}/-${diffForVersion.stats.deletions}`;
            const versionResult = await this.versionHistoryService.saveVersion(methodologyDir, 'methodology', id, beforeState, {
        ...
        await this.onRefresh?.();
        // Generate diff view
        const diffResult = this.textDiffService.generateObjectDiff(beforeState, afterState, `${id}/methodology.yaml`);
        let response = `✅ Methodology '${id}' updated successfully\n\n` +
    ...
    async handleDelete(args) {
        ...
        if (confirm !== true) {
            return this.createErrorResponse(`⚠️ Delete requires confirmation.\n\nTo delete methodology '${id}', set confirm: true`);
        }
        ...
        if (builtInMethodologies.includes(id.toLowerCase())) {
            return this.createErrorResponse(`Cannot delete built-in methodology '${id}'. Only custom methodologies can be deleted.`);
        }
        ...
        try {
            ...
            return this.createErrorResponse(`Failed to delete methodology directory: ${error instanceof Error ? error.message : String(error)}`);
        }
        ...
        // Trigger refresh for any dependent systems
        await this.onRefresh?.();
        return this.createSuccessResponse(`✅ Methodology '${id}' deleted successfully\n\n` +
    ...
    async handleList(args) {
        ...
        if (frameworks.length === 0) {
            return this.createSuccessResponse(`📋 No methodologies found${enabled_only ? ' (enabled only)' : ''}\n\n` +
        ...
        const frameworkList = frameworks
            ...
            .join('\n');
        const activeInfo = activeFramework !== undefined
            ? `\n📍 Active Framework: ${activeFramework.type !== '' ? activeFramework.type : activeFramework.id}`
            : '\n📍 No active framework';
        return this.createSuccessResponse(`📋 Methodologies (${frameworks.length} total)\n\n` + `${frameworkList}\n` + `${activeInfo}`);
    }
    async handleInspect(args) {
        ...
        return this.createSuccessResponse(`Methodology: ${framework.name}\n\n` +
            `Details:\n` +
            `  ID: ${framework.id}\n` +
            `  Type: ${framework.type}\n` +
            `  Status: ${activeStatus}\n` +
    ...
    async handleSwitch(args) {
        ...
        try {
            ...
            return this.createErrorResponse(`Failed to switch framework: ${error instanceof Error ? error.message : String(error)}`);
        }
        if (!success) {
            return this.createErrorResponse(`Failed to switch to framework '${targetFramework.name}'. Check server logs for details.`);
        }
        ...
        return this.createSuccessResponse(`✅ Switched to framework '${targetFramework.name}'${reasonText}\n\n` +
    ...
    async handleHistory(args) {
        ...
        if (!history || history.versions.length === 0) {
            return this.createSuccessResponse(`📜 No version history for methodology '${id}'\n\n` +
    ...
    async handleRollback(args) {
        ...
        if (confirm !== true) {
            return this.createErrorResponse(`⚠️ Rollback requires confirmation.\n\n` +
        ...
        // Perform rollback
        const result = await this.versionHistoryService.rollback(methodologyDir, 'methodology', id, version, currentState);
        if (!result.success) {
        ...
        const methodologyData = {
            ...
            enabled: snapshot['enabled'] ?? existingData.methodology['enabled'],
        ...
        // Trigger refresh
        await this.onRefresh?.();
        return this.createSuccessResponse(`✅ Methodology '${id}' rolled back to version ${version}\n\n` +
    ...
    async handleCompare(args) {
        ...
        if (from_version === undefined || to_version === undefined) {
            return this.createErrorResponse('Both from_version and to_version are required for compare action');
        }
        // Check if methodology exists
        const framework = this.frameworkManager.getFramework(id);
        if (framework === undefined) {
        ...
        const methodologyDir = this.fileService.getMethodologyDir(id);
        const result = await this.versionHistoryService.compareVersions(methodologyDir, from_version, to_version);
        if (!result.success) {
            return this.createErrorResponse(`Compare failed: ${result.error}`);
        }
        // Generate diff between versions
        const diffResult = this.textDiffService.generateObjectDiff(result.from.snapshot, result.to.snapshot, `${id}/methodology.yaml`);
        let response = `📊 **Version Comparison**: ${id}\n\n` +
 ...
 */
export function createConsolidatedFrameworkManager(deps) {
