...
const DEFAULT_TOOL_DESCRIPTION_ENTRIES = [
    ...
    [
        'system_control',
        {
            description: '⚙️ SYSTEM CONTROL: Framework switching, gate management, analytics. Actions: status|framework|gates|analytics|config|maintenance|guide. Use `>>help` for system-wide guidance.',
    ...
    [
        'resource_manager',
        {
            description: '📦 RESOURCE MANAGER: Unified CRUD for prompts, gates, and methodologies. resource_type: prompt|gate|methodology. Actions: create|update|delete|list|inspect|reload + analyze_type|analyze_gates|guide (prompt only) + switch (methodology only).',
...
export class ToolDescriptionManager extends EventEmitter {
    constructor(logger, configManager) {
        ...
        this.configPath = path.join(serverRoot, 'src', 'tooling', 'contracts', '_generated', 'tool-descriptions.contracts.json');
        ...
        this.frameworksConfigListener = (newConfig) => {
            ...
            this.logger.info(`Tool description manager feature toggle updated (dynamicDescriptions: ${this.frameworksConfig.dynamicToolDescriptions})`);
    ...
    preloadMethodologyDescriptions() {
        try {
            ...
            this.logger.info(`✅ Pre-loaded tool descriptions for ${this.methodologyDescriptions.size} methodologies from YAML (SOT)`);
        }
        catch (error) {
    ...
    async loadBaseConfig() {
        ...
        this.logger.warn(`[ToolDescriptionManager] Generated tool-descriptions.contracts.json not found at ${this.configPath}. ` +
    ...
    buildActiveConfig(baseConfig, activeContext) {
        ...
        const dynamicDescriptionsEnabled = this.frameworksConfig.dynamicToolDescriptions &&
    ...
    async maybePersistActiveConfig(config) {
        try {
            ...
            this.logger.info(`💾 Wrote active tool descriptions to ${this.activeConfigPath} (framework: ${config.activeFramework || 'n/a'}, methodology: ${config.activeMethodology || 'n/a'})`);
            return true;
        }
        catch (error) {
    ...
    async synchronizeActiveConfig(reason, options) {
        try {
            ...
            this.logger.info(`✅ Synchronized tool descriptions (${reason}); source=${base.source}, framework=${activeContext.activeFramework || 'n/a'}, methodology=${activeContext.activeMethodology || 'n/a'}`);
