...
import { DEFAULT_VERSIONING_CONFIG, } from '../types/index.js';
import { DEFAULT_INJECTION_CONFIG, } from '../execution/pipeline/decisions/injection/index.js';
...
const DEFAULT_FRAMEWORKS_CONFIG = {
...
const DEFAULT_GATES_CONFIG = {
    enabled: true,
    definitionsDirectory: 'gates',
    enableMethodologyGates: true,
};
const DEFAULT_CHAIN_SESSION_CONFIG = {
...
export class ConfigManager extends EventEmitter {
    ...
    getLoggingConfig() {
        ...
        if (envLogLevel) {
            ...
            if (validLevels.includes(normalizedLevel)) {
                ...
                console.warn(`Invalid LOG_LEVEL environment variable: "${envLogLevel}". ` +
    ...
    getFrameworksConfig() {
        ...
        return {
            dynamicToolDescriptions: frameworksConfig?.dynamicToolDescriptions ??
                DEFAULT_FRAMEWORKS_CONFIG.dynamicToolDescriptions,
            injection: {
                systemPrompt: {
                    enabled: frameworksConfig?.injection?.systemPrompt?.enabled ??
                        DEFAULT_FRAMEWORKS_CONFIG.injection?.systemPrompt?.enabled ??
                        true,
                    frequency: frameworksConfig?.injection?.systemPrompt?.frequency ??
                        DEFAULT_FRAMEWORKS_CONFIG.injection?.systemPrompt?.frequency ??
                        2,
                },
                styleGuidance: frameworksConfig?.injection?.styleGuidance ??
    ...
    getGatesConfig() {
        ...
        return {
            ...
            definitionsDirectory: gatesConfig.definitionsDirectory ?? DEFAULT_GATES_CONFIG.definitionsDirectory,
            enableMethodologyGates: gatesConfig.enableMethodologyGates ?? DEFAULT_GATES_CONFIG.enableMethodologyGates,
        };
    }
    /**
     * Get chain session lifecycle configuration
     */
    getChainSessionConfig() {
        const chainConfig = this.config.chainSessions ?? {};
        return {
            sessionTimeoutMinutes: chainConfig.sessionTimeoutMinutes ?? DEFAULT_CHAIN_SESSION_CONFIG.sessionTimeoutMinutes,
            reviewTimeoutMinutes: chainConfig.reviewTimeoutMinutes ?? DEFAULT_CHAIN_SESSION_CONFIG.reviewTimeoutMinutes,
            cleanupIntervalMinutes: chainConfig.cleanupIntervalMinutes ?? DEFAULT_CHAIN_SESSION_CONFIG.cleanupIntervalMinutes,
        };
    }
    /**
     ...
     */
    getVersioningConfig() {
        const versioningConfig = this.config.versioning ?? {};
        return {
     ...
     */
    getResolvedPromptsFilePath(overridePath) {
    ...
    validateAndSetDefaults() {
        ...
        if (!this.config.chainSessions) {
            ...
            this.config.chainSessions = {
                sessionTimeoutMinutes: this.config.chainSessions.sessionTimeoutMinutes ??
                    DEFAULT_CHAIN_SESSION_CONFIG.sessionTimeoutMinutes,
                reviewTimeoutMinutes: this.config.chainSessions.reviewTimeoutMinutes ??
                    DEFAULT_CHAIN_SESSION_CONFIG.reviewTimeoutMinutes,
                cleanupIntervalMinutes: this.config.chainSessions.cleanupIntervalMinutes ??
     ...
     */
    validateAnalysisConfig(analysisConfig) {
        const semanticAnalysis = analysisConfig.semanticAnalysis || {};
        // Build LLM integration config
        const llmIntegration = {
            enabled: semanticAnalysis.llmIntegration?.enabled ??
                DEFAULT_ANALYSIS_CONFIG.semanticAnalysis.llmIntegration.enabled,
            apiKey: semanticAnalysis.llmIntegration?.apiKey ??
                DEFAULT_ANALYSIS_CONFIG.semanticAnalysis.llmIntegration.apiKey,
            endpoint: semanticAnalysis.llmIntegration?.endpoint ??
                DEFAULT_ANALYSIS_CONFIG.semanticAnalysis.llmIntegration.endpoint,
            model: semanticAnalysis.llmIntegration?.model ??
                DEFAULT_ANALYSIS_CONFIG.semanticAnalysis.llmIntegration.model,
            maxTokens: semanticAnalysis.llmIntegration?.maxTokens ??
                DEFAULT_ANALYSIS_CONFIG.semanticAnalysis.llmIntegration.maxTokens,
            temperature: semanticAnalysis.llmIntegration?.temperature ??
    ...
    async handleExternalConfigChange() {
        await this.loadConfig();
        this.emit('configChanged', this.getConfig());
    }
    emitConfigChange(previousFrameworks) {
    ...
    haveFrameworkConfigsChanged(a, b) {
        return (a.dynamicToolDescriptions !== b.dynamicToolDescriptions ||
