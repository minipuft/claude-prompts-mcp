...
export class GateHotReloadCoordinator {
    ...
    async handleGateChange(event) {
        ...
        if (this.config.debug) {
            this.logger.debug(`Processing gate hot reload for: ${gateId} (changeType: ${event.changeType ?? 'unknown'})`);
        }
     ...
     */
    async handleGateDeletion(gateId) {
        try {
 ...
 */
export function createGateHotReloadRegistration(logger, registry, loader, config) {
    ...
    return {
        ...
        handler: (event) => coordinator.handleGateChange(event),
        coordinator,
    };
}
/**
 ...
 */
export function createGateHotReloadCoordinator(logger, registry, loader, config) {
