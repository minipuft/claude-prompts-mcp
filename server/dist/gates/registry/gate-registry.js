...
export class GateRegistry {
    ...
    async initialize() {
        ...
        try {
            ...
            this.logger.info(`GateRegistry initialized with ${this.guides.size} guides in ${loadTime.toFixed(1)}ms`);
        }
        catch (error) {
        ...
        try {
            ...
            if (this.config.validateOnRegistration) {
                ...
                if (!validationResult.valid) {
                    this.logger.warn(`Guide validation failed for ${guide.gateId}: ${validationResult.errors.join(', ')}`);
                    return false;
                }
            }
     ...
     */
    setGuideEnabled(gateId, enabled) {
        const normalizedId = gateId.toLowerCase();
        const entry = this.guides.get(normalizedId);
        if (!entry) {
 ...
 */
export async function createGateRegistry(logger, config) {
