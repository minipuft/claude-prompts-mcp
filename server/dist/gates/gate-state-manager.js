...
export class GateSystemManager extends EventEmitter {
    constructor(logger, stateDirectory) {
        super();
        this.logger = logger;
        ...
        const resolvedBaseDir = stateDirectory ||
     ...
     */
    async enableGateSystem(reason = 'User request') {
     ...
     */
    async disableGateSystem(reason = 'User request') {
        if (!this.currentState.enabled) {
     ...
     */
    recordValidation(success, executionTime) {
        const metrics = this.currentState.validationMetrics;
 ...
 */
export function createGateSystemManager(logger, stateDirectory) {
    return new GateSystemManager(logger, stateDirectory);
}
//# sourceMappingURL=gate-state-manager.js.map