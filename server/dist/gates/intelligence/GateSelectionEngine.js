...
export class GateSelectionEngine {
    constructor(logger, configManager) {
        ...
        this.frameworksConfigListener = (newConfig) => {
            ...
            this.logger.info(`Gate selection engine feature toggles updated (methodologyGates: ${this.frameworksConfig.enableMethodologyGates})`);
        };
     ...
     */
    mergeGateSelections(...selections) {
     ...
     */
    generateSelectionReasoning(criteria, primaryGates, semanticGates) {
        const reasoning = [];
        if (criteria.framework) {
     ...
     */
    calculateSelectionConfidence(criteria, selectedGates) {
     ...
     */
    estimateExecutionTime(selectedGates, criteria) {
     ...
     */
    determineFallbackGates(criteria) {
...
GateSelectionEngine.METHODOLOGY_GATES = new Set([
 ...
 */
export function createGateSelectionEngine(logger, configManager) {
    return new GateSelectionEngine(logger, configManager);
}
// Interfaces are already exported via declaration above
//# sourceMappingURL=GateSelectionEngine.js.map