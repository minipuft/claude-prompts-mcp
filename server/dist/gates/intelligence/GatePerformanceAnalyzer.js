...
export class GatePerformanceAnalyzer {
     ...
     */
    recordGateExecution(gateId, executionTime, success) {
    ...
    calculatePerformanceScore(metrics) {
        ...
        return (metrics.successRate * successWeight + speedScore * speedWeight + usageScore * usageWeight);
    }
    /**
    ...
    generateOptimizationRecommendations(allMetrics) {
        ...
        if (slowGates.length > 0) {
            recommendations.push(`Optimize slow gates: ${slowGates.map((g) => g.gateId).join(', ')} (>${300}ms avg)`);
        }
        // Check for low success rates
        const unreliableGates = allMetrics.filter((m) => m.successRate < 0.8);
        if (unreliableGates.length > 0) {
            recommendations.push(`Improve reliability of: ${unreliableGates.map((g) => g.gateId).join(', ')} (<80% success)`);
        }
        // Check for unused gates
        const underusedGates = allMetrics.filter((m) => m.usageCount < 5);
        if (underusedGates.length > 0) {
            recommendations.push(`Review gate relevance: ${underusedGates.map((g) => g.gateId).join(', ')} (low usage)`);
        }
 ...
 */
export function createGatePerformanceAnalyzer(logger) {
    return new GatePerformanceAnalyzer(logger);
}
//# sourceMappingURL=GatePerformanceAnalyzer.js.map