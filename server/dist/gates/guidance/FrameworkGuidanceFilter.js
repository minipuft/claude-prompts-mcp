 ...
 */
function resolveFrameworks(frameworks) {
    return frameworks && frameworks.length > 0 ? frameworks : [];
}
function matchesFrameworkLine(line, framework) {
    const trimmed = line.trimStart().toLowerCase();
    return trimmed.startsWith(`- ${framework.toLowerCase()}:`);
}
function matchesAnyFramework(line, frameworks) {
 ...
 */
export function filterFrameworkGuidance(guidance, activeFramework, frameworkNames) {
    const frameworks = resolveFrameworks(frameworkNames);
    if (frameworks.length === 0) {
        return guidance;
    }
    const resolvedFrameworkName = frameworks.find((framework) => framework.toLowerCase() === activeFramework.toLowerCase()) ??
        activeFramework;
    ...
    if (filteredLines.some((line) => matchesFrameworkLine(line, resolvedFrameworkName))) {
        ...
        result = result.replace(`- ${resolvedFrameworkName}: `, `**${resolvedFrameworkName} Methodology Guidelines:**\n- `);
        return result;
    }
    return guidance;
}
 ...
 */
export function hasFrameworkSpecificContent(guidance, frameworkNames) {
    const frameworks = resolveFrameworks(frameworkNames);
    if (frameworks.length === 0) {
        return false;
    }
 ...
 */
export function getFrameworksInGuidance(guidance, frameworkNames) {
    const frameworks = resolveFrameworks(frameworkNames);
    if (frameworks.length === 0) {
        return [];
    }
...
//# sourceMappingURL=FrameworkGuidanceFilter.js.map