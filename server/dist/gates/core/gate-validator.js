...
export class GateValidator {
    constructor(logger, gateLoader, llmConfig) {
     ...
     */
    async validateGate(gateId, context) {
        ...
        try {
            ...
            this.logger.debug(`Gate validation complete: ${gateId} - ${passed ? 'PASSED' : 'FAILED'} (${checks.length} checks)`);
            return result;
        }
        catch (error) {
     ...
     */
    async validateGates(gateIds, context) {
        const startTime = Date.now();
        const results = [];
     ...
     */
    async runValidationCheck(criteria, _context) {
        try {
            ...
            this.logger.debug(`[GATE VALIDATOR] Check type '${criteria.type}' auto-passed (string-based validation removed)`);
            return {
                type: criteria.type,
                passed: true,
                score: 1.0,
     ...
     */
    async runLLMSelfCheck(criteria) {
     ...
     */
    generateRetryHints(gate, checks) {
        const hints = [];
        ...
        for (const check of failedChecks) {
            if (check.type === 'llm_self_check') {
                ...
                if (check.details?.['feedback'] !== undefined) {
                    hints.push(check.details['feedback']);
                }
            }
        }
 ...
 */
export function createGateValidator(logger, gateLoader, llmConfig) {
    return new GateValidator(logger, gateLoader, llmConfig);
}
//# sourceMappingURL=gate-validator.js.map