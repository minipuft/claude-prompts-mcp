 ...
 */
export function buildReviewInstructions(prompts) {
 ...
 */
export function composeReviewPrompt(prompts, previousResponse, retryHints = [], timestamps) {
    ...
    const retryBlock = retryHints.length > 0
        ? ['Retry Hints:', ...retryHints.map((hint) => `- ${hint.trim()}`)].join('\n')
        : '';
    const previousResponseBlock = previousResponse && previousResponse.trim().length > 0
    ...
    if (executionContext && Object.keys(executionContext.originalArgs).length > 0) {
        ...
        Object.entries(executionContext.originalArgs).forEach(([key, value]) => {
            ...
            const truncatedValue = String(value).length > 200 ? String(value).substring(0, 200) + '...' : String(value);
