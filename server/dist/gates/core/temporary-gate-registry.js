...
export class TemporaryGateRegistry {
    ...
    createTemporaryGate(definition, scopeId) {
        ...
        const gateId = definition.id && this.isValidCustomId(definition.id)
            ? definition.id
        ...
        const now = Date.now();
        const { id: _unusedId, expires_at, scope_id, pass_criteria, context, target_step_number, apply_to_steps, ...defWithoutId } = definition;
        const tempGate = {
            id: gateId,
    ...
    convertToLightweightGate(tempGate) {
        const lightweight = {
            id: tempGate.id,
            name: tempGate.name,
        ...
        if (tempGate.pass_criteria !== undefined) {
            lightweight.pass_criteria = tempGate.pass_criteria;
        }
        return lightweight;
    }
    /**
     ...
     */
    cleanupChainExecution(chainExecutionId) {
     ...
     */
    cleanupExecutionScope(executionId) {
    ...
    performCleanup() {
        ...
        if (this.temporaryGates.size >= this.maxMemoryGates) {
            const gates = Array.from(this.temporaryGates.values()).sort((a, b) => a.created_at - b.created_at);
     ...
     */
    associateWithScope(gateId, scope, scopeId) {
        const scopeKey = `${scope}:${scopeId}`;
     ...
     */
    removeFromScope(gateId, scope, scopeId) {
        const scopeKey = `${scope}:${scopeId}`;
        const scopeInfo = this.scopeManagement.get(scopeKey);
        if (scopeInfo) {
    ...
    isValidCustomId(id) {
        ...
        if (id.startsWith('temp_') && /^temp_\d+_[a-z0-9]+$/.test(id)) {
            this.logger.warn(`[TEMP GATE REGISTRY] Rejecting user ID that matches auto-generated pattern: ${id}`);
            return false;
        }
 ...
 */
export function createTemporaryGateRegistry(logger, options) {
    return new TemporaryGateRegistry(logger, options);
}
//# sourceMappingURL=temporary-gate-registry.js.map