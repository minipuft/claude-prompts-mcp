...
import { validateGateSchema, } from './gate-schema.js';
...
export class GateDefinitionLoader {
        ...
        try {
            ...
            if (this.validateOnLoad) {
                ...
                if (!validation.valid) {
                    ...
                    console.error(`[GateDefinitionLoader] Validation failed for '${id}':`, validation.errors.join('; '));
                    return undefined;
                }
     ...
     */
    inlineReferencedFiles(definition, gateDir) {
        ...
        if (definition.guidanceFile) {
            ...
            if (existsSync(guidancePath)) {
                try {
                    ...
                    console.warn(`[GateDefinitionLoader] Failed to inline guidance from ${guidancePath}:`, error);
                }
            }
            else {
 ...
 */
export function createGateDefinitionLoader(config) {
 ...
 */
export function getDefaultGateDefinitionLoader() {
    if (!defaultLoader) {
 ...
 */
export function resetDefaultGateDefinitionLoader() {
    defaultLoader = null;
}
//# sourceMappingURL=gate-definition-loader.js.map