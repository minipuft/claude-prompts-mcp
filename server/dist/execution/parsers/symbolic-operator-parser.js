...
export class SymbolicCommandParser {
    ...
    parseChainOperator(command) {
        ...
        const steps = stepStrings.map((stepStr, index) => {
            ...
            this.logger.debug(`[parseChainOperator] Step ${index + 1}: promptId="${promptId}", args="${stepMatch[2]?.trim() ?? ''}"`);
            return {
                promptId,
    ...
    parseParallelOperator(command) {
        ...
        const prompts = promptStrings.map((promptStr, index) => {
            ...
            if (!promptId) {
                throw new ValidationError(`Invalid parallel prompt format: missing prompt ID in ${promptStr}`);
            }
            return {
                promptId,
    ...
    generateExecutionPlan(detection, basePromptId, baseArgs) {
        const steps = [];
        let frameworkOverride;
        let finalValidation;
        let styleSelection;
        const frameworkOp = detection.operators.find((op) => op.type === 'framework');
        if (frameworkOp) {
        ...
        const styleOp = detection.operators.find((op) => op.type === 'style');
        if (styleOp) {
            styleSelection = styleOp.normalizedId;
        }
        const chainOp = detection.operators.find((op) => op.type === 'chain');
        if (chainOp) {
    ...
    buildParseResult(command, operators, basePromptId, baseArgs) {
...
export function createSymbolicCommandParser(logger) {
    return new SymbolicCommandParser(logger);
}
//# sourceMappingURL=symbolic-operator-parser.js.map