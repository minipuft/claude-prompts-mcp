...
import { createSymbolicCommandParser } from './symbolic-operator-parser.js';
...
export class UnifiedCommandParser {
    ...
    extractModifier(command) {
        ...
        if (!modifier) {
            throw new ValidationError(`Unknown execution modifier "%${modifierKey}". Supported modifiers: %clean, %judge, %lean, %framework.`);
        }
    ...
    async parseCommand(command, availablePrompts) {
        ...
        const { command: commandWithoutModifier, modifier: modifierToken } = this.extractModifier(trimmed);
            ...
            if (strategy.canHandle(normalized)) {
                try {
                    ...
                    if (result) {
                        ...
                        if (modifierToken) {
                            const hasExistingModifier = result.modifiers?.clean === true ||
                                result.modifiers?.judge === true ||
                                result.modifiers?.lean === true ||
                                result.modifiers?.framework === true;
                            if (hasExistingModifier) {
                                throw new ValidationError('Only one %modifier is allowed per command. Remove the additional modifier.');
                            }
    ...
    createSymbolicCommandStrategy() {
        return {
            ...
            parse: (command) => {
                ...
                cleanCommand = cleanCommand.replace(/\s+(::|=)\s*(?:[a-z][a-z0-9_-]*:["'][^"']+["']|[a-z][a-z0-9_-]*\([^)]+\)|["'][^"']+["']|[^\s"']+)/gi, '');
    ...
    createJsonCommandStrategy() {
        return {
            ...
            parse: (command) => {
                ...
                const { command: innerCommand, modifier: modifierToken } = this.extractModifier(String(actualCommand));
