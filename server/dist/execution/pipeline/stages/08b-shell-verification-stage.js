...
import { SHELL_VERIFY_DEFAULT_MAX_ITERATIONS, } from '../../../gates/shell/index.js';
...
export class ShellVerificationStage extends BasePipelineStage {
    constructor(shellVerifyExecutor, logger, runtimeStateDir) {
        super(logger);
        this.shellVerifyExecutor = shellVerifyExecutor;
        this.name = 'ShellVerification';
     ...
     */
    async handleGateAction(context, gateAction, pending) {
        switch (gateAction) {
            case 'retry':
            ...
            case 'abort':
                ...
                context.state.gates.pendingShellVerification = undefined;
                context.diagnostics.info(this.name, 'User chose to abort after shell verification failure', {
                    gateId: pending.gateId,
                    attemptCount: pending.attemptCount,
                });
     ...
     */
    setBounceBackResponse(context, result, pending) {
        const errorOutput = (result.stderr !== '' ? result.stderr : null) ??
     ...
     */
    setEscalationResponse(context, result, pending) {
     ...
     */
    formatBounceBackMessage(result, pending, errorOutput) {
        const lines = [
        ...
        lines.push('', '### Error Output', '```', errorOutput, '```', '', 'Please fix the issues and submit again.');
        return lines.join('\n');
    }
    /**
     * Format escalation message after max attempts.
     */
    formatEscalationMessage(result, pending, errorOutput) {
        const lines = [
        ...
        lines.push('', '### Recent Error Output', '```', errorOutput, '```', '', 'Use `gate_action` parameter to decide:', '', '- **retry**: Reset attempt count and try again', '- **skip**: Bypass verification and continue', '- **abort**: Stop execution');
        return lines.join('\n');
    }
    /**
     * Truncate output for display.
     */
    truncateForDisplay(output, maxLength) {
     ...
     */
    async writeVerifyActiveState(context, pending) {
        const { shellVerify } = pending;
        const state = {
 ...
 */
export function createShellVerificationStage(shellVerifyExecutor, logger, runtimeStateDir) {
    return new ShellVerificationStage(shellVerifyExecutor, logger, runtimeStateDir);
}
//# sourceMappingURL=08b-shell-verification-stage.js.map