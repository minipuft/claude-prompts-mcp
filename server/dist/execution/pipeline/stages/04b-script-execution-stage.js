...
export class ScriptExecutionStage extends BasePipelineStage {
    constructor(scriptExecutor, toolDetectionService, executionModeService, logger) {
        super(logger);
    ...
    async execute(context) {
        ...
        for (const match of autoApproveMatches) {
            ...
            if (validationResult.valid) {
                ...
                if (validationResult.warnings.length > 0) {
                    ...
                    context.diagnostics.info(this.name, `Tool '${tool.id}' validated with warnings: ${validationResult.warnings.join('; ')}`);
                }
                ...
                context.diagnostics.warn(this.name, `Tool '${tool.id}' validation failed: ${validationResult.errors.join('; ')}`);
            }
        }
        // Filter normal tools by execution mode (auto/manual/confirm)
        const filterResult = this.executionModeService.filterByExecutionMode(normalMatches, prompt.scriptTools, prompt.id);
        ...
        if (toolsPendingConfirmation.length > 0) {
            context.diagnostics.info(this.name, `Confirm mode tools pending: ${toolsPendingConfirmation.join(', ')}`);
        }
        // Execute ready tools (normal flow - excludes already-executed autoApprove tools)
        await this.executeReadyTools(context, filterResult.readyForExecution, prompt.scriptTools, scriptResults);
        ...
        if (filterResult.requiresConfirmation) {
            scriptsState.confirmationRequired = this.executionModeService.buildConfirmationResponse(filterResult, prompt.id);
        }
     ...
     */
    separateAutoApproveTools(matches, toolMap) {
        const autoApproveMatches = [];
        const normalMatches = [];
     ...
     */
    async executeReadyTools(context, matches, tools, results) {
     ...
     */
    async executeScriptTool(context, tool, inputs) {
        const prompt = context.parsedCommand?.convertedPrompt;
        const request = {
            toolId: tool.id,
 ...
 */
export function createScriptExecutionStage(scriptExecutor, toolDetectionService, executionModeService, logger) {
    return new ScriptExecutionStage(scriptExecutor, toolDetectionService, executionModeService, logger);
}
//# sourceMappingURL=04b-script-execution-stage.js.map