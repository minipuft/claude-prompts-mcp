...
export class RequestNormalizationStage extends BasePipelineStage {
    constructor(chainManagementService, toolRouter, logger) {
        super(logger);
        this.chainManagementService = chainManagementService;
        this.toolRouter = toolRouter;
        this.name = 'RequestNormalization';
    }
    async execute(context) {
        ...
        if (!command && !hasResumeIdentifier) {
            context.setResponse(this.buildErrorResponse(`❌ Error: Missing required parameters.
...
Note: When continuing a chain, the 'command' parameter is optional - the system will restore the execution plan from the chain session.`));
        ...
        if (context.mcpRequest.force_restart && context.mcpRequest.chain_id) {
            context.setResponse(this.buildErrorResponse(`❌ Error: Conflicting parameters detected.
...
- Remove one of these parameters and try again`));
    ...
    async tryHandleChainCommand(command, context) {
    ...
    async tryRouteCommand(command, context) {
        if (!this.toolRouter) {
            return false;
        }
        ...
        try {
            const response = await this.toolRouter(routing.targetTool, routing.translatedParams, routing.originalCommand ?? command);
            context.setResponse(response);
...
//# sourceMappingURL=00-request-normalization-stage.js.map