...
export class JudgeSelectionStage extends BasePipelineStage {
    constructor(promptsProvider, gateLoader, configManager, logger, frameworksProvider, styleManager) {
        super(logger);
    ...
    async execute(context) {
        ...
        try {
            ...
            const totalResources = resources.styles.length + resources.frameworks.length + resources.gates.length;
            if (totalResources === 0) {
     ...
     */
    applyInlineStyleSelection(context) {
        const styleFromCommand = context.parsedCommand?.styleSelection ?? context.parsedCommand?.executionPlan?.styleSelection;
        if (!styleFromCommand) {
            return false;
        }
     ...
     */
    shouldTriggerJudgePhase(context) {
    ...
    getOperatorContext(context) {
        ...
        if (finalValidation && 'parsedCriteria' in finalValidation) {
            const chainGates = finalValidation.parsedCriteria ?? [];
            inlineGates = [...new Set([...inlineGates, ...chainGates])];
        }
        const styleSelection = parsedCommand?.styleSelection ?? parsedCommand?.executionPlan?.styleSelection;
     ...
     */
    getCleanCommandForDisplay(context) {
     ...
     */
    getActiveMethodologyJudgePrompt(context) {
     ...
     */
    buildJudgeResponse(resources, context) {
...
`;
        ...
        const responseLines = [
            ...
            '',
            `${frameworkInstructions}${operatorContext.hasFrameworkOperator ? '1' : '2'}. **Style** (recommended): Select a response style matching the task type`,
            ...
            '',
            `${operatorContext.hasFrameworkOperator ? '2' : '3'}. **Gates** (optional): Select quality gates to ensure specific aspects`,
            ...
            '```',
            `prompt_engine({`,
            `  command: "${escapedCommand}${operatorContext.hasFrameworkOperator ? '' : ' @<framework>'} :: <gate_id or criteria> #<analytical|procedural|creative|reasoning>"`,
            `})`,
            '```',
            '',
            '**Notes:**',
     ...
     */
    buildOperatorContextHeader(operatorContext) {
        const parts = [];
     ...
     */
    isFrameworkPromptId(id) {
        const normalized = id.toLowerCase();
        return (normalized.includes('cageerf') ||
     ...
     */
    async collectFrameworkResources() {
     ...
     */
    mapMethodologyToFrameworkResource(definition) {
     ...
     */
    formatResourceMenuForClaude(resources, operatorContext) {
        const sections = [];
        // Response Styles section (always shown)
        if (resources.styles.length > 0) {
            sections.push('#### Response Styles');
            sections.push(resources.styles.map((r) => `- **${r.id}**: ${r.description || r.name}`).join('\n'));
        }
        ...
        if (!operatorContext.hasFrameworkOperator && resources.frameworks.length > 0) {
            ...
            sections.push(resources.frameworks.map((r) => `- **${r.id}**: ${r.description || r.name}`).join('\n'));
        }
...
//# sourceMappingURL=06a-judge-selection-stage.js.map