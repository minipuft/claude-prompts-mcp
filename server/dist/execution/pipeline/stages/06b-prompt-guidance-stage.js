...
export class PromptGuidanceStage extends BasePipelineStage {
    constructor(promptGuidanceService, styleManager, logger) {
        super(logger);
        this.promptGuidanceService = promptGuidanceService;
        this.styleManager = styleManager;
        this.name = 'PromptGuidance';
    }
     ...
     */
    hasClientSelections(context) {
        const operatorStyle = context.parsedCommand?.executionPlan?.styleSelection;
        return Boolean(context.state.framework.clientOverride ||
     ...
     */
    applyStyleEnhancement(context, style) {
        ...
        if (context.hasChainCommand()) {
            ...
            for (const step of steps) {
                if (step.convertedPrompt) {
                    const enhancedSystemMessage = this.enhanceWithStyle(step.convertedPrompt.systemMessage, styleGuidance);
     ...
     */
    enhanceWithStyle(systemMessage, styleGuidance) {
    ...
    async applyGuidanceToChain(context) {
        const steps = context.requireChainSteps();
        let applied = 0;
        for (const step of steps) {
    ...
    async applyGuidance(prompt, context, promptId = prompt.id) {
        try {
            ...
            const includeSystemPrompt = (injectionDecision?.inject ?? true) && context.state.framework.systemPromptApplied !== true;
            ...
            const availableResources = context.state.framework.availableResources;
...
//# sourceMappingURL=06b-prompt-guidance-stage.js.map