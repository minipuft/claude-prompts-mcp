...
export class GateEnhancementStage extends BasePipelineStage {
    constructor(gateService, temporaryGateRegistry, frameworksConfigProvider, gateReferenceResolver, frameworkManagerProvider, logger, metricsProvider, gatesConfigProvider, gateLoader, gateManagerProvider) {
        super(logger);
        ...
        this.gateManagerProvider = gateManagerProvider;
     ...
     */
    ensureDefaultMethodologyGate(gateIds, gatesConfig, activeFrameworkId) {
        ...
        this.logger.debug('[GateEnhancementStage] ensureDefaultMethodologyGate - adding framework-compliance');
     ...
     */
    async enhanceSinglePrompt(gateContext, context, registeredGates, gatesConfig) {
        const executionPlan = context.executionPlan;
        if (executionPlan === undefined) {
            return;
        }
     ...
     */
    async enhanceChainSteps(gateContext, context, registeredGates, gatesConfig) {
        const gateService = this.requireGateService();
        ...
        for (const step of steps) {
            ...
            const registryGates = registrySelectionContext.promptCategory !== undefined
     ...
     */
    selectRegistryGates(context) {
        const gateManager = this.gateManagerProvider?.();
        if (!gateManager) {
            this.logger.warn('[GateEnhancementStage] GateManager unavailable - no automatic gate selection');
            return [];
        }
        try {
    ...
    normalizeGateInput(gate, isChainExecution = false, currentStep = 1) {
        ...
        const normalizeType = (type) => {
            const validTypes = ['validation', 'guidance'];
    ...
    async registerTemporaryGates(context) {
        ...
        const resolvedGateIds = new Set(); // Track all resolved gates (canonical or processed)
        const createdIds = [];
        ...
        const seenStringInputs = new Set();
        const seenGateSignatures = new Set();
                ...
                if (typeof rawGate === 'string') {
                    ...
                    if (this.gateReferenceResolver) {
                        ...
                        if (resolution.referenceType === 'registered') {
                            ...
                            this.logger.debug('[GateEnhancementStage] Resolved string gate to canonical definition', { input: trimmed, gateId: resolution.gateId });
