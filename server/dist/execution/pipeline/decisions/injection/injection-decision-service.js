...
export class InjectionDecisionService {
    ...
    computeDecision(input) {
        ...
        if (input.currentStep !== undefined && resolved.config.frequency) {
            const frequencyDecision = this.checkFrequency(input.injectionType, input.currentStep, input.totalSteps, resolved.config.frequency, timestamp, resolved.source, target);
     ...
     */
    applyTargetFilter(decision, executionContext) {
        ...
        const matches = (target === 'steps' && executionContext === 'step') ||
     ...
     */
    checkFrequency(type, currentStep, totalSteps, frequency, timestamp, source, target = 'both') {
