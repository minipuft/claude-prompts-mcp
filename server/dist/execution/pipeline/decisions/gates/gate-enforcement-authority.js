...
export class GateEnforcementAuthority {
    ...
    parseVerdict(raw, source) {
        ...
        for (const { regex, priority } of GateEnforcementAuthority.VERDICT_PATTERNS) {
            ...
            if (match) {
                ...
                if (!rationale) {
                    this.logger.warn(`[GateEnforcementAuthority] Verdict detected but missing rationale: "${raw.substring(0, 50)}..."`);
                ...
                return {
                    verdict: verdictValue.toUpperCase(),
                    rationale,
                    raw,
                    source,
     ...
     */
    resolveEnforcementMode(configuredMode) {
     ...
     */
    async recordOutcome(sessionId, verdict, enforcementMode = 'blocking') {
        ...
        if (verdict.verdict === 'FAIL') {
            ...
            switch (enforcementMode) {
                ...
                case 'advisory':
                    ...
                    this.logger.warn(`[GateEnforcementAuthority] Gate FAIL in advisory mode - continuing: ${verdict.rationale}`);
                ...
                case 'informational':
                    ...
                    this.logger.debug(`[GateEnforcementAuthority] Gate FAIL in informational mode - logged only: ${verdict.rationale}`);
                    await this.chainSessionManager.clearPendingGateReview(sessionId);
                    return {
                        status: 'cleared',
                        nextAction: 'continue',
     ...
     */
    async resolveAction(sessionId, action) {
        switch (action) {
            case 'retry':
     ...
     */
    async setPendingReview(sessionId, review) {
     ...
     * @param sessionId - Session to update
     */
    async clearPendingReview(sessionId) {
        await this.chainSessionManager.clearPendingGateReview(sessionId);
    }
    /**
...
GateEnforcementAuthority.VERDICT_PATTERNS = [
...
//# sourceMappingURL=gate-enforcement-authority.js.map