 ...
 */
function isGateReviewInput(input) {
    return input.executionType === 'gate_review';
}
/**
 * Type guard for normal step input
 */
function isNormalStepInput(input) {
    return input.executionType === 'normal';
}
export class ChainOperatorExecutor {
    constructor(logger, convertedPrompts, 
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    gateGuidanceRenderer, getFrameworkContext, promptGuidanceService, referenceResolver, scriptReferenceResolver) {
        this.logger = logger;
        this.convertedPrompts = convertedPrompts;
        this.gateGuidanceRenderer = gateGuidanceRenderer;
        this.getFrameworkContext = getFrameworkContext;
     ...
     */
    async renderGateReviewStep(input, stepPrompts, chainContext, additionalGateIds, inlineGuidanceText) {
        ...
        const { gateIds: gateIdsToRender, explicitGateIds } = this.collectReviewGateIds(pendingGateReview, additionalGateIds, reviewStep);
        const hasInlineGateFocus = explicitGateIds.length > 0 ||
            gateIdsToRender.some((gateId) => this.isInlineGateId(gateId)) ||
            Boolean(inlineGuidanceText);
        const callToAction = 'Use the resume shortcut below and respond via gate_verdict as `GATE_REVIEW: PASS` or `GATE_REVIEW: FAIL - reason` to resume the workflow.';
        ...
        if (targetStep) {
            ...
            const convertedPrompt = targetStep.convertedPrompt ||
            ...
            if (convertedPrompt) {
                ...
                const stepArgs = this.normalizeStepArgs(chainContext['currentStepArgs'] ??
                    targetStep?.args ??
                    {});
                const templateContext = { ...chainContext, ...stepArgs };
                const renderedTemplate = await this.renderTemplate(convertedPrompt, templateContext, targetStep.promptId);
                originalContent = [
        ...
        const attemptSummary = `**Review Attempts:** ${Math.min(attemptCount + 1, // Display as 1-indexed (Attempt 1 of 2, not 0 of 2)
        maxAttempts)}/${maxAttempts}`;
        const supplementalSections = [attemptSummary];
        if (hasInlineGateFocus) {
            supplementalSections.push('**Inline Gate Priority:** These inline criteria triggered the review. Fix them before checking framework standards.');
        }
        ...
        if (pendingGateReview?.retryHints && pendingGateReview.retryHints.length > 0) {
            const hintHeading = hasInlineGateFocus
                ? '**Inline Fix Guidance:**'
                : '**Improvements Needed:**';
            supplementalSections.push(`${hintHeading}\n` +
     ...
     */
    async renderNormalStep(input, stepPrompts, chainContext) {
        ...
        if (!step) {
            throw new Error(`Step at index ${currentStepIndex} is undefined in chain of length ${stepPrompts.length}`);
        }
        ...
        const convertedPrompt = step.convertedPrompt || this.convertedPrompts.find((p) => p.id === step.promptId);
        if (!convertedPrompt) {
        ...
        const stepArgs = this.normalizeStepArgs(chainContext['currentStepArgs'] ?? step?.args ?? {});
        ...
        if (step.inputMapping) {
            for (const [semanticName, sourceVar] of Object.entries(step.inputMapping)) {
                if (templateContext[sourceVar] !== undefined) {
                    ...
                    this.logger.debug(`[SymbolicChain] Applied inputMapping: ${semanticName} <- ${sourceVar}`);
                }
            }
        }
        ...
        if (this.promptGuidanceService && templateContext['previous_step_output']) {
            ...
            if (previousOutputStr.trim().startsWith('{')) {
                ...
                if (parseResult.success &&
                    ...
                    this.logger.info(`[SymbolicChain] Applying runtime enhancement based on Judge result for step ${step.stepNumber}`);
                    try {
                        templateToRender = await this.promptGuidanceService.applyRuntimeEnhancement(templateToRender, parseResult.data, this.convertedPrompts);
                    }
                    catch (error) {
        ...
        const renderedTemplate = await this.renderTemplateString(templateToRender, templateContext, step.promptId);
        const lines = [];
    ...
    renderSimpleGateGuidance(gateIds, inlineGuidanceText) {
        ...
        const hasInlineGuidance = inlineGateIds.length > 0 ||
     ...
     */
    isGateGuidanceEnabled(chainContext) {
     ...
     */
    isFrameworkInjectionEnabledForGates(chainContext) {
        const injectionState = chainContext['injectionState'];
     ...
     */
    shouldSuppressFrameworkForSteps(chainContext) {
        const injectionState = chainContext['injectionState'];
        if (!injectionState?.systemPrompt) {
    ...
    async buildFrameworkGuidance(step) {
    ...
    async resolveFrameworkContext(step) {
        if (!step) {
            return null;
        }
    ...
    renderChainMetadataSection(chainContext, totalSteps) {
        // Chain metadata banner removed to reduce redundant instructions.
        return null;
    }
    isInlineGateId(gateId) {
        if (!gateId) {
            return false;
        }
    ...
    getStoredStepResult(chainContext, stepNumber) {
        if (!chainContext) {
            return undefined;
        }
        const stepResults = chainContext['step_results'];
        if (stepResults) {
    ...
    buildManualReviewBody(pendingReview) {
        if (!pendingReview) {
            return null;
        }
        ...
        if (pendingReview.prompts && pendingReview.prompts.length > 0) {
            const composed = composeReviewPrompt(pendingReview.prompts, pendingReview.previousResponse, pendingReview.retryHints ?? []);
            return composed.combinedPrompt;
        }
        return null;
    }
    normalizeStepArgs(argsInput) {
        if (!argsInput || typeof argsInput !== 'object') {
            return {};
        }
        return { ...argsInput };
    }
    async renderTemplate(convertedPrompt, templateContext, promptId) {
        return this.renderTemplateString(convertedPrompt.userMessageTemplate, templateContext, promptId);
    }
    async renderTemplateString(templateString, templateContext, promptId) {
        try {
            ...
            if (this.referenceResolver || this.scriptReferenceResolver) {
                const result = await processTemplateWithRefs(templateString, templateContext, {}, this.referenceResolver, { scriptResolver: this.scriptReferenceResolver });
    ...
    getPromptDisplayName(step) {
        return step.convertedPrompt?.name || step.promptId;
    }
    buildChainSummary(stepPrompts) {
        if (stepPrompts.length === 0) {
            return '(no steps)';
        }
        return stepPrompts.map((step) => this.getPromptDisplayName(step)).join(' → ');
    }
    resolveReviewStep(stepPrompts, chainContext, pendingReview) {
        if (stepPrompts.length === 0) {
            return undefined;
        }
        ...
        const promptMetadataIndex = pendingReview.prompts
    ...
    extractStepIndexFromContext(chainContext) {
    ...
    extractStepIndexFromMetadata(metadata) {
    ...
    clampStepIndex(index, totalSteps) {
    ...
    collectReviewGateIds(pendingReview, additionalGateIds, reviewStep) {
        const gateSet = new Set();
        const explicitSet = new Set();
        const addGate = (gateId, explicit = false) => {
    ...
    extractInlineGateIdsFromMetadata(metadata) {
        if (!metadata || typeof metadata !== 'object') {
            return [];
        }
...
//# sourceMappingURL=chain-operator-executor.js.map