...
export class MaxDepthExceededError extends PromptReferenceError {
    constructor(promptId, depth, maxDepth) {
        super(`Maximum reference depth (${maxDepth}) exceeded at prompt "${promptId}" (current depth: ${depth})`, promptId);
        this.depth = depth;
...
export class ScriptExecutionError extends PromptReferenceError {
    constructor(promptId, toolId, originalError) {
        super(`Script execution failed for tool "${toolId}" in prompt "${promptId}": ${originalError.message}`, promptId);
        this.toolId = toolId;
...
export class ReferenceRenderError extends PromptReferenceError {
    constructor(promptId, originalError) {
