...
import { InvalidFieldAccessError, InvalidScriptIdError, InvalidScriptOutputError, ScriptExecutionFailedError, ScriptNotRegisteredError, } from './script-reference-errors.js';
 ...
 */
const SCRIPT_REFERENCE_PATTERN = /\{\{script:([a-zA-Z0-9_-]+)(?:\.([a-zA-Z0-9_]+))?((?:\s+[a-zA-Z_][a-zA-Z0-9_]*=(?:'[^']*'|"[^"]*"|\d+(?:\.\d+)?|true|false))*)\s*\}\}/g;
/**
 * Resolves {{script:id}} references in templates.
 */
export class ScriptReferenceResolver {
    constructor(logger, scriptLoader, scriptExecutor, options) {
        this.logger = logger;
     ...
     */
    async preResolve(template, context, promptDir) {
        ...
        this.logger.debug(`[ScriptReferenceResolver] Found ${references.length} script reference(s) to resolve`);
        ...
        diagnostics.resolutionTimeMs = Date.now() - startTime;
        this.logger.debug(`[ScriptReferenceResolver] Resolved ${diagnostics.scriptsResolved} script(s) in ${diagnostics.resolutionTimeMs}ms`);
        return {
            resolvedTemplate,
            scriptResults: allScriptResults,
            diagnostics,
     ...
     */
    async resolveScriptReference(ref, context, promptDir) {
        ...
        if (!executionResult.success) {
            throw new ScriptExecutionFailedError(ref.scriptId, executionResult.exitCode, executionResult.stderr, executionResult.error !== undefined && executionResult.error !== ''
                ? new Error(executionResult.error)
                : undefined);
        }
        // 7. Parse output (must be JSON for field access)
        let parsedOutput = executionResult.output;
        ...
        if (typeof parsedOutput === 'object' &&
            ...
            const innerOutput = parsedOutput.output;
        ...
        let outputValue = parsedOutput;
        if (ref.fieldAccess !== undefined && ref.fieldAccess !== '') {
            if (typeof parsedOutput !== 'object' || parsedOutput === null) {
                throw new InvalidScriptOutputError(ref.scriptId, executionResult.stdout, 'Output must be a JSON object for field access');
            }
     ...
     */
    detectScriptReferences(template) {
        const references = [];
        let match;
        // Reset regex state
        ...
        while ((match = SCRIPT_REFERENCE_PATTERN.exec(template)) !== null) {
            ...
            references.push({
                ...
                inlineArgs: argsString !== undefined && argsString !== ''
    ...
    validateScriptId(id) {
        ...
        if (!/^[a-zA-Z0-9_-]+$/.test(id)) {
            throw new InvalidScriptIdError(id, 'Script ID must contain only alphanumeric characters, underscores, and hyphens');
        }
...
//# sourceMappingURL=script-reference-resolver.js.map