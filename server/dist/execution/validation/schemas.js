...
const temporaryGateObjectSchema = z
    ...
    const hasContent = Boolean((value.criteria && value.criteria.length > 0) ||
...
export const gateSpecificationSchema = z.union([
    ...
    errorMap: (issue, ctx) => {
        if (issue.code === z.ZodIssueCode.invalid_union) {
            return {
                message: 'Gate specification must be a string ID, custom check object, or full gate definition',
            };
        }
        return { message: ctx.defaultError };
    },
