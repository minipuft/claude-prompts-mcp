...
export class ChainSessionManager {
    ...
    async updateSessionState(sessionId, stepNumber, stepResult, stepMetadata) {
        ...
        this.logger?.debug(`[StepLifecycle] Step ${stepNumber} ${isPlaceholder ? 'rendered as placeholder' : 'response captured'}, ` +
    ...
    async completeStep(sessionId, stepNumber, options) {
        ...
        this.logger?.debug(`[StepLifecycle] Step ${stepNumber} marked COMPLETED${isPlaceholder ? ' (placeholder)' : ''}, ` +
     ...
     */
    async advanceStep(sessionId, stepNumber) {
        const session = this.activeSessions.get(sessionId);
        if (!session) {
            this.logger?.warn(`[StepLifecycle] Cannot advance step for non-existent session: ${sessionId}`);
            return false;
        }
        // Only advance if we're at or before this step (prevent double-advancement)
        if (session.state.currentStep > stepNumber) {
            this.logger?.debug(`[StepLifecycle] Step ${stepNumber} already passed, currentStep is ${session.state.currentStep}`);
            return true;
        }
        ...
        session.state.lastUpdated = Date.now();
        session.lastActivity = Date.now();
        this.logger?.debug(`[StepLifecycle] Advanced from step ${stepNumber} to step ${session.state.currentStep}`);
        await this.saveSessions();
        return true;
    }
    /**
    ...
    listActiveSessions(limit = 50) {
        ...
        for (const session of this.activeSessions.values()) {
            ...
            const promptId = session.blueprint?.parsedCommand?.convertedPrompt?.id ??
    ...
    getDormantSessionForChain(chainId) {
        const sessionIds = this.chainSessionMapping.get(chainId);
        if (!sessionIds) {
            return undefined;
        }
    ...
    getDormantSessionForBaseChain(baseChainId) {
        const normalized = this.extractBaseChainId(baseChainId);
        const history = this.baseChainMapping.get(normalized) ?? [];
        for (let idx = history.length - 1; idx >= 0; idx -= 1) {
            const runChainId = history[idx];
