...
export class ArgumentHistoryTracker {
     ...
     */
    constructor(logger, maxEntriesPerChain = 50, persistencePath) {
        this.logger = logger;
        /** Chain ID to entries mapping */
        this.chainHistory = new Map();
        /** Session ID to chain ID mapping */
        this.sessionToChain = new Map();
        ...
        this.logger.debug(`ArgumentHistoryTracker initialized (maxEntriesPerChain: ${this.maxEntriesPerChain}, persistence: ${this.persistenceEnabled})`);
    }
    /**
    ...
    trackExecution(options) {
        ...
        if (chainEntries.length > this.maxEntriesPerChain) {
            ...
            this.logger.debug(`Removed oldest entry ${removed?.entryId} from chain ${chainId} (limit: ${this.maxEntriesPerChain})`);
        }
        // Update session-to-chain mapping
        if (sessionId) {
            this.sessionToChain.set(sessionId, chainId);
        }
        this.logger.debug(`Tracked execution: chainId=${chainId}, promptId=${promptId}, step=${stepNumber}, entryId=${entryId}`);
        // Persist to file
        if (this.persistenceEnabled) {
            this.saveToFile();
        }
     ...
     */
    buildReviewContext(sessionId, currentStepNumber) {
        const history = this.getSessionHistory(sessionId);
        if (history.length === 0) {
            const reviewContext = {
                originalArgs: {},
    ...
    loadFromFile() {
        try {
            ...
            this.logger.info(`Loaded argument history from file: ${stats.totalChains} chains, ${stats.totalEntries} entries`);
        }
        catch (error) {
...
//# sourceMappingURL=argument-history-tracker.js.map