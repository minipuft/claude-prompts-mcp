...
export class MethodologyTracker extends EventEmitter {
     ...
     */
    async setMethodologySystemEnabled(enabled, reason = 'Manual toggle') {
        ...
        if (previousState !== enabled) {
            this.emit('methodology-system-toggled', enabled, reason);
        }
    }
    /**
     * Get switch history
    ...
    async readPersistedState() {
        try {
            ...
            if (error?.code !== 'ENOENT') {
                this.logger.warn(`Failed to read methodology state from ${this.config.stateFilePath}: ${error instanceof Error ? error.message : String(error)}`);
            }
            return null;
        }
    }
    ...
    updateConfig(config) {
        ...
        if (oldConfig.healthCheckIntervalMs !== this.config.healthCheckIntervalMs &&
 ...
 */
export async function createMethodologyTracker(logger, config) {
    const tracker = new MethodologyTracker(logger, config);
    await tracker.initialize();
    return tracker;
}
//# sourceMappingURL=methodology-tracker.js.map