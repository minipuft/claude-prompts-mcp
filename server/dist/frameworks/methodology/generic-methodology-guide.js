 ...
 */
import { BaseMethodologyGuide, } from '../types/methodology-types.js';
...
import { convertTemplateSuggestions, convertMethodologyGates, convertProcessingSteps, } from '../utils/template-enhancer.js';
/**
...
export class GenericMethodologyGuide extends BaseMethodologyGuide {
        ...
        return {
            ...
            qualityGuidance: {
                ...
                completenessChecks: elements
                    ? [`Ensure all ${this.methodology} phases are addressed`].concat(elements.requiredSections.map((s) => `Verify ${s.toLowerCase()} is complete`))
                    : [],
    ...
    enhanceWithMethodology(prompt, context) {
        ...
        const methodologyGates = this.definition.methodologyGates
        ...
        return {
            ...
            methodologyGates,
            templateSuggestions,
            enhancementMetadata: this.createEnhancementMetadata(0.9, `${this.methodology} methodology provides systematic approach`),
        };
    }
    /**
    ...
    validateMethodologyCompliance(prompt) {
        ...
        if (!qualityIndicators || Object.keys(qualityIndicators).length === 0) {
            ...
            const hasMethodologyMention = combinedText.toLowerCase().includes(this.methodology.toLowerCase()) ||
 ...
 */
export function createGenericGuide(definition) {
    return new GenericMethodologyGuide(definition);
}
//# sourceMappingURL=generic-methodology-guide.js.map