...
import { validateMethodologySchema, } from './methodology-schema.js';
...
export class RuntimeMethodologyLoader {
    ...
    loadMethodology(id) {
        ...
        try {
            ...
            const definition = loadYamlFileSync(entryPath, {
                required: true,
            });
            if (!definition) {
                return undefined;
            ...
            if (this.validateOnLoad) {
                ...
                if (!validation.valid) {
                    ...
                    console.error(`[RuntimeMethodologyLoader] Validation failed for '${id}':`, validation.errors.join('; '));
                    return undefined;
                }
                if (validation.warnings.length > 0) {
                    console.warn(`[RuntimeMethodologyLoader] Warnings for '${id}':`, validation.warnings.join('; '));
                }
            }
            // Cache result
            if (this.enableCache) {
            ...
            try {
                if (existsSync(pkgPath)) {
                    ...
                    if (pkg.name === 'claude-prompts') {
                        ...
                        if (existsSync(resourcesMethodologiesPath) &&
     ...
     */
    inlineReferencedFiles(definition, methodologyDir) {
 ...
 */
export function createRuntimeMethodologyLoader(config) {
 ...
 */
export function resetDefaultRuntimeLoader() {
    defaultLoader = null;
}
//# sourceMappingURL=runtime-methodology-loader.js.map