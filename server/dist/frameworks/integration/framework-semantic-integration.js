...
export class FrameworkSemanticIntegration {
    constructor(frameworkManager, frameworkStateManager, semanticAnalyzer, logger, config = {}) {
        // Framework switching state management
        this.lastFrameworkSwitch = new Map();
        this.frameworkUsageHistory = new Map();
     ...
     */
    async analyzeWithFrameworkIntegration(prompt, userFrameworkPreference, includePromptGuidance) {
        ...
        try {
            ...
            }, userFrameworkPreference, semanticAnalysis);
            ...
            const frameworkContext = this.selectOptimalFramework(prompt, enhancedCriteria, semanticAnalysis);
            ...
            const result = {
                ...
                recommendations: this.generateIntegratedRecommendations(semanticAnalysis, frameworkContext, alignment),
            };
            ...
            this.updateFrameworkUsage(frameworkContext.selectedFramework.id, performance.now() - startTime, alignment.overallAlignment);
            this.logger.info(`Framework integration completed: ${frameworkContext.selectedFramework.name} ` +
     ...
     */
    async evaluateFrameworkSwitch(prompt, currentResult) {
        ...
        const bestAlternative = alternatives.find((alt) => !this.config.blacklistedFrameworks.includes(alt.id) && alt.id !== currentFramework.id);
        if (!bestAlternative) {
        ...
        return {
            ...
            expectedImprovement: this.estimateImprovementPotential(currentFramework, bestAlternative, currentResult.semanticAnalysis),
        };
    }
     ...
     */
    enhanceFrameworkCriteria(baseCriteria, userPreference, semanticAnalysis) {
        ...
        if (semanticAnalysis) {
            ...
            if (semanticAnalysis.analysisMetadata.mode === 'minimal' ||
                ...
                if (!userPreference && semanticAnalysis.frameworkRecommendation.requiresUserChoice) {
                    ...
                    this.logger.info('Minimal analysis mode - framework selection requires user choice or default');
                }
            }
     ...
     */
    selectOptimalFramework(prompt, criteria, semanticAnalysis) {
        ...
        this.logger.debug(`Framework selection: ${frameworkContext.selectedFramework.name} ` +
     ...
     */
    validateFrameworkAlignment(semanticAnalysis, frameworkContext) {
        const framework = frameworkContext.selectedFramework;
        // Calculate alignment scores
        const confidenceAlignment = this.calculateConfidenceAlignment(semanticAnalysis.confidence, frameworkContext.metadata.confidence);
        const complexityMatch = this.calculateComplexityMatch(semanticAnalysis.complexity, framework);
        const executionTypeCompatibility = this.calculateExecutionTypeCompatibility(semanticAnalysis.executionType, framework);
     ...
     */
    generateAlternativeFrameworks(criteria, currentContext, semanticAnalysis) {
     ...
     */
    generateIntegratedRecommendations(semanticAnalysis, frameworkContext, alignment) {
        return {
    ...
    calculateConfidenceAlignment(semanticConfidence, frameworkConfidence) {
        // How well do the confidence scores align?
        const difference = Math.abs(semanticConfidence - frameworkConfidence);
        return Math.max(0, 1 - difference);
    }
    calculateComplexityMatch(semanticComplexity, framework) {
    ...
    calculateExecutionTypeCompatibility(executionType, framework) {
        // Check if execution type is compatible with framework
        if (framework.applicableTypes.length === 0)
            return 1.0; // Framework supports all types
        return framework.applicableTypes.includes(executionType) ? 1.0 : 0.6;
    }
    // Additional helper methods
    generateExecutionApproach(semanticAnalysis, frameworkContext) {
    ...
    getFrameworkSpecificGates(framework) {
        const gates = [];
    ...
    generateOptimizationSuggestions(semanticAnalysis, frameworkContext, alignment) {
        const suggestions = [];
     ...
     */
    estimateProcessingTime(semanticAnalysis) {
     ...
     */
    estimateMemoryUsage(semanticAnalysis) {
    ...
    updateFrameworkUsage(frameworkId, processingTime, alignmentScore) {
    ...
    generateUsageRecommendations(insights) {
        ...
        const mostUsedFramework = frameworkUsage.reduce((best, current) => current.usageCount > best.usageCount ? current : best);
        const leastUsedFramework = frameworkUsage.reduce((worst, current) => current.usageCount < worst.usageCount ? current : worst);
    ...
    estimateImprovementPotential(currentFramework, alternativeFramework, semanticAnalysis) {
    ...
    async createNonFrameworkIntegratedResult(prompt, startTime) {
     ...
     */
    setPromptGuidanceService(promptGuidanceService) {
     ...
     */
    async applySemanticGuidedEnhancement(prompt, semanticAnalysis, frameworkContext) {
    ...
    createFallbackIntegratedResult(prompt, startTime) {
 ...
 */
export async function createFrameworkSemanticIntegration(frameworkManager, frameworkStateManager, logger, semanticAnalyzer, config) {
    return new FrameworkSemanticIntegration(frameworkManager, frameworkStateManager, semanticAnalyzer, logger, config);
}
//# sourceMappingURL=framework-semantic-integration.js.map