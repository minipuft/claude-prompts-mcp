 ...
 */
export function assessPhaseQuality(text, indicators) {
 ...
 */
export function detectPhase(text, indicators) {
    const textLower = text.toLowerCase();
    // Check keyword presence
    const hasKeywords = indicators.keywords?.some((keyword) => textLower.includes(keyword.toLowerCase())) ?? false;
 ...
 */
export function validateCompliance(text, qualityIndicators, options = {}) {
    const opts = { ...DEFAULT_OPTIONS, ...options };
    const phaseNames = Object.keys(qualityIndicators);
    // Detect all phases
    const phaseDetections = {};
    ...
    const qualitySum = Object.values(phaseDetections).reduce((sum, p) => sum + (p.present ? p.quality : 0), 0);
    const totalPhases = phaseNames.length;
    const complianceScore = totalPhases > 0
    ...
    for (const [phaseName, detection] of Object.entries(phaseDetections)) {
        ...
        if (detection.present) {
            if (detection.quality > 0.7) {
                ...
                strengths.push(`${capitalizedPhase} awareness present - could be enhanced with more detail`);
            }
            else {
                improvementAreas.push(`${capitalizedPhase} present but lacks depth - strengthen systematic approach`);
            }
        }
        else {
