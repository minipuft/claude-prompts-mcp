...
import { validateScriptToolSchema, } from './script-schema.js';
...
export class ScriptToolDefinitionLoader {
     ...
     */
    loadTool(promptDir, toolId, promptId) {
    ...
    discoverTools(promptDir) {
        ...
        try {
            ...
            if (this.debug) {
                process.stderr.write(`[ScriptToolDefinitionLoader] Failed to discover tools in ${toolsDir}: ${String(error)}\n`);
            }
            return [];
        }
    }
     ...
     */
    loadToolsForPrompt(promptDir, toolIds, promptId) {
        const results = [];
        for (const toolId of toolIds) {
            ...
            if (tool) {
                ...
                process.stderr.write(`[ScriptToolDefinitionLoader] Tool '${toolId}' not found for prompt '${promptId}'\n`);
            }
        }
        return results;
    }
     ...
     */
    loadAllToolsForPrompt(promptDir, promptId) {
     ...
     */
    clearToolCache(promptDir, toolId) {
        const normalizedId = toolId.toLowerCase();
     ...
     */
    loadFromToolDir(toolDir, toolId, promptId) {
        try {
            ...
            // Load main tool.yaml
            const yamlDefinition = loadYamlFileSync(entryPath, {
                required: true,
            });
            if (!yamlDefinition) {
                return undefined;
            ...
            if (this.validateOnLoad) {
                ...
                if (!validation.valid) {
                    ...
                    process.stderr.write(`[ScriptToolDefinitionLoader] Validation failed for '${toolId}': ${validation.errors.join('; ')}\n`);
                    return undefined;
                }
                if (validation.warnings.length > 0 && this.debug) {
                    process.stderr.write(`[ScriptToolDefinitionLoader] Warnings for '${toolId}': ${validation.warnings.join('; ')}\n`);
                }
            }
            ...
            if (this.debug) {
                process.stderr.write(`[ScriptToolDefinitionLoader] Loaded: ${loadedTool.name} (${toolId}) for prompt ${promptId}\n`);
            }
            return loadedTool;
        }
        catch (error) {
            this.stats.loadErrors++;
            if (this.debug) {
                process.stderr.write(`[ScriptToolDefinitionLoader] Failed to load '${toolId}': ${String(error)}\n`);
            }
            return undefined;
        }
    }
     ...
     */
    loadReferencedFiles(toolDir, definition) {
        ...
        if (existsSync(schemaPath)) {
            try {
                ...
                process.stderr.write(`[ScriptToolDefinitionLoader] Failed to parse schema.json: ${String(error)}\n`);
            }
        }
        else if (this.debug) {
        ...
        if (existsSync(descPath)) {
            try {
                ...
                if (this.debug) {
                    process.stderr.write(`[ScriptToolDefinitionLoader] Loaded description from ${descPath}\n`);
                }
            }
            catch (error) {
                process.stderr.write(`[ScriptToolDefinitionLoader] Failed to read description.md: ${String(error)}\n`);
            }
        }
     ...
     */
    resolveExecutionConfig(yamlConfig) {
        if (!yamlConfig) {
 ...
 */
export function createScriptToolDefinitionLoader(config) {
 ...
 */
export function getDefaultScriptToolDefinitionLoader() {
    if (!defaultLoader) {
 ...
 */
export function resetDefaultScriptToolDefinitionLoader() {
    defaultLoader = null;
}
//# sourceMappingURL=script-definition-loader.js.map