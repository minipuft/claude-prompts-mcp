 ...
 * ```
 */
export class WorkspaceScriptLoader {
    constructor(config) {
        ...
        if (this.debug) {
            process.stderr.write(`[WorkspaceScriptLoader] Initialized with workspace scripts path: ${this.workspaceScriptsPath}\n`);
        }
    }
    /**
     ...
     */
    scriptExists(scriptId, promptDir) {
        ...
        if (promptDir !== undefined && promptDir !== '') {
            if (this.promptLocalLoader.toolExists(promptDir, normalizedId)) {
                if (this.debug) {
                    process.stderr.write(`[WorkspaceScriptLoader] Script '${scriptId}' found in prompt-local: ${promptDir}\n`);
                }
                return true;
            }
        }
        ...
        if (existsSync(workspaceToolYaml)) {
            if (this.debug) {
                process.stderr.write(`[WorkspaceScriptLoader] Script '${scriptId}' found in workspace: ${this.workspaceScriptsPath}\n`);
            }
            return true;
        }
        return false;
     ...
     */
    loadScript(scriptId, promptDir) {
        const normalizedId = scriptId.toLowerCase();
        // 1. Try prompt-local first
        if (promptDir !== undefined && promptDir !== '') {
            const promptLocalTool = this.promptLocalLoader.loadTool(promptDir, normalizedId, 'inline-script');
            if (promptLocalTool !== undefined) {
                if (this.debug) {
                    process.stderr.write(`[WorkspaceScriptLoader] Loaded script '${scriptId}' from prompt-local\n`);
                }
        ...
        if (workspaceTool !== undefined) {
            if (this.debug) {
                process.stderr.write(`[WorkspaceScriptLoader] Loaded script '${scriptId}' from workspace\n`);
            }
     ...
     */
    getSearchedPaths(scriptId, promptDir) {
        const normalizedId = scriptId.toLowerCase();
        const paths = [];
        if (promptDir !== undefined && promptDir !== '') {
    ...
    discoverWorkspaceScripts() {
        ...
        try {
            ...
            if (this.debug) {
                process.stderr.write(`[WorkspaceScriptLoader] Failed to discover workspace scripts: ${String(error)}\n`);
            }
            return [];
        }
    }
    ...
    loadWorkspaceScript(scriptId) {
        ...
        try {
            ...
            if (this.validateOnLoad) {
                ...
                if (!validation.valid) {
                    if (this.debug) {
                        process.stderr.write(`[WorkspaceScriptLoader] Invalid tool.yaml for '${scriptId}': ${validation.errors.join(', ')}\n`);
                    }
                    return undefined;
                }
            }
            ...
            const tool = {
                ...
                execution: yaml.execution !== undefined
                    ? {
            ...
            if (this.debug) {
                process.stderr.write(`[WorkspaceScriptLoader] Failed to load workspace script '${scriptId}': ${String(error)}\n`);
            }
            return undefined;
        }
    }
     ...
     */
    loadReferencedFiles(scriptDir, yaml) {
        const result = {};
        ...
        if (existsSync(schemaPath)) {
            try {
                ...
                result.inputSchema = JSON.parse(schemaContent);
            }
            catch {
...
//# sourceMappingURL=workspace-script-loader.js.map