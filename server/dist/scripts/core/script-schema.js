...
export const TriggerTypeSchema = z
    ...
    if (val === 'parameter_match') {
        ...
        console.warn("[ScriptSchema] DEPRECATED: 'parameter_match' trigger is deprecated. Use 'schema_match' instead.");
        return 'schema_match';
    }
    return val;
});
...
export const ExecutionConfigSchema = z
    ...
    mode: ExecutionModeSchema.optional().transform((val) => {
        if (val !== undefined && val !== 'auto') {
            // eslint-disable-next-line no-console -- Intentional deprecation warning
            console.warn(`[ScriptSchema] DEPRECATED: 'mode: ${val}' is deprecated. ` +
    ...
    confidence: z
        ...
        if (val !== undefined) {
            // eslint-disable-next-line no-console -- Intentional deprecation warning
            console.warn('[ScriptSchema] DEPRECATED: numeric confidence is deprecated and ignored. ' +
 ...
 */
export function validateScriptToolSchema(definition, expectedId) {
    const errors = [];
    const warnings = [];
    // Parse with Zod
    ...
    if (expectedId !== undefined &&
        ...
        warnings.push(`Tool ID '${normalized.id}' does not match expected directory name '${expectedId}'`);
    }
    ...
    if (rawExec !== null && rawExec !== undefined && typeof rawExec === 'object') {
        ...
        if (exec['confidence'] !== undefined) {
            warnings.push("Numeric 'confidence' field is deprecated and ignored. Use 'trigger' and 'strict' instead.");
        }
 ...
 */
export function normalizeExecutionConfig(raw) {
 ...
 */
export function isValidScriptToolYaml(value) {
