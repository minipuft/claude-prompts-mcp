...
export class PendingConfirmationTracker {
     ...
     */
    recordPending(promptId, toolId, inputs) {
        ...
        if (this.debug) {
            // eslint-disable-next-line no-console
            console.error(`[PendingConfirmationTracker] Recorded pending: ${key} (hash: ${inputsHash.slice(0, 8)}...)`);
        }
     ...
     */
    checkAndClearPending(promptId, toolId, inputs) {
        const key = this.makeKey(promptId, toolId);
 ...
 */
export function getDefaultPendingConfirmationTracker() {
    defaultTracker ?? (defaultTracker = new PendingConfirmationTracker());
    return defaultTracker;
}
/**
 * Reset the default tracker (useful for testing).
 */
export function resetDefaultPendingConfirmationTracker() {
    defaultTracker = null;
}
//# sourceMappingURL=pending-confirmation-tracker.js.map