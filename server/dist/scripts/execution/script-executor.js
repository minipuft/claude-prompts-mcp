...
export class ScriptExecutor {
    ...
    async execute(request, tool) {
        ...
        if (!validation.valid) {
            return this.createErrorResult(startTime, `Input validation failed: ${validation.errors.join('; ')}`, -1);
        }
     ...
     */
    normalizeJsonStringInputs(inputs, schema) {
        ...
        for (const [key, value] of Object.entries(normalized)) {
            ...
            if (expectedTypes.includes('array') || expectedTypes.includes('object')) {
                ...
                if (trimmed === '' && !required.has(key)) {
                    ...
                    if (this.debug) {
                        console.error(`[ScriptExecutor] Removed empty string for optional '${key}' (expects ${expectedTypes.join(' | ')})`);
                    }
                    continue;
                }
                ...
                if (trimmed.startsWith('[') || trimmed.startsWith('{')) {
                    try {
                        ...
                        if (this.debug) {
                            console.error(`[ScriptExecutor] Normalized JSON string for '${key}': ${trimmed.substring(0, 50)}...`);
                        }
                    }
                    catch {
                        // Invalid JSON - leave as string, validation will catch the type error
                        if (this.debug) {
                            console.error(`[ScriptExecutor] Failed to parse JSON string for '${key}': ${trimmed.substring(0, 50)}...`);
                        }
                    }
                }
            }
    ...
    resolveRuntime(tool) {
        ...
        if (detected) {
            if (this.debug) {
                console.error(`[ScriptExecutor] Auto-detected runtime '${detected}' from extension '${ext}'`);
            }
            return detected;
        }
     ...
     */
    createErrorResult(startTime, error, exitCode) {
        return {
            success: false,
            output: null,
            stdout: '',
 ...
 */
export function createScriptExecutor(config) {
 ...
 */
export function getDefaultScriptExecutor() {
    if (!defaultExecutor) {
 ...
 */
export function resetDefaultScriptExecutor() {
