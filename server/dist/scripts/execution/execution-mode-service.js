 ...
 */
import { getDefaultPendingConfirmationTracker, } from './pending-confirmation-tracker.js';
/**
 * Execution Mode Service
 *
...
export class ExecutionModeService {
     ...
     */
    filterByExecutionMode(matches, tools, promptId) {
        const readyForExecution = [];
        const skippedManual = []; // Legacy field, always empty now
        ...
        for (const match of matches) {
            ...
            if (isExplicit) {
                ...
                if (this.debug && needsConfirmation) {
                    // eslint-disable-next-line no-console
                    console.error(`[ExecutionModeService] Tool '${tool.id}' confirmation bypassed via explicit arg`);
                }
                continue;
            }
            ...
            if (needsConfirmation) {
                ...
                const autoApproved = this.confirmationTracker.checkAndClearPending(promptId, match.toolId, match.extractedInputs);
                if (autoApproved) {
                    readyForExecution.push(match);
                    if (this.debug) {
                        // eslint-disable-next-line no-console
     ...
     */
    buildConfirmationResponse(filterResult, promptId) {
        ...
        return {
            ...
            message: `The following tools require confirmation before execution: ${toolNames}. ` +
     ...
     */
    buildPendingConfirmation(tool, match, promptId) {
     ...
     */
    logManualOverride(toolId) {
        // eslint-disable-next-line no-console
        console.warn(`[ExecutionModeService] WARN: Tool '${toolId}' confirmation bypassed via explicit arg`);
    }
}
/**
 * Factory function with default configuration.
 */
export function createExecutionModeService(config) {
 ...
 */
export function getDefaultExecutionModeService() {
    defaultService ?? (defaultService = new ExecutionModeService());
    return defaultService;
}
/**
 * Reset the default service (useful for testing).
 */
export function resetDefaultExecutionModeService() {
    defaultService = null;
}
//# sourceMappingURL=execution-mode-service.js.map