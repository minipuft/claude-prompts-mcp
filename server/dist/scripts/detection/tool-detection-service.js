...
export class ToolDetectionService {
     ...
     */
    detectTools(input, args, availableTools) {
        const matches = [];
        ...
        if (this.debug && matches.length > 0) {
            console.error('[ToolDetectionService] Detected tools:', matches.map((m) => `${m.toolId} (${m.matchReason}, confirm=${m.requiresConfirmation}, explicit=${m.explicitRequest})`));
        }
        return matches;
    }
    /**
     ...
     */
    extractExplicitToolRequests(args) {
     ...
     */
    matchToolWithTrigger(input, args, tool, execution, isExplicit) {
     ...
     */
    findArgValue(args, propName, propSchema) {
        // Direct match
        if (propName in args) {
     ...
     */
    matchToolBySchema(args, tool, strict) {
        const schema = tool.inputSchema;
        ...
        if (strict) {
            ...
            if (missingParams.length > 0) {
                if (this.debug) {
                    console.error(`[ToolDetectionService] Tool '${tool.id}' strict mode: missing params: ${missingParams.join(', ')}`);
                }
                return null;
            }
     ...
     */
    createMatch(tool, priority, matchReason, args, matchedParams, missingParams) {
        return {
            toolId: tool.id,
 ...
 */
export function createToolDetectionService(config) {
 ...
 */
export function getDefaultToolDetectionService() {
    if (!defaultService) {
 ...
 */
export function resetDefaultToolDetectionService() {
    defaultService = null;
}
