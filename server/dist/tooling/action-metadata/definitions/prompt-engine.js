...
import { prompt_engineCommands, prompt_engineParameters, } from '../../../tooling/contracts/_generated/prompt_engine.generated.js';
import { contractToCommandDescriptors, contractToParameterDescriptors, } from '../../contracts/adapter.js';
const promptEngineContract = {
    tool: 'prompt_engine',
    version: 1,
    summary: 'Execute prompts/chains with inline operators. Steps must start with `>>prompt_id` (or `/prompt_id`); place modifiers first: `%judge @Framework #style :: gates`.',
...
const generatedParameters = contractToParameterDescriptors(promptEngineContract).map((param) => {
...
const parameterDescriptors = [...generatedParameters];
...
const usagePatterns = [
    {
        ...
        summary: 'Show correct operator order: modifiers first, prompt ids on every step, quoted free text after the prompt id.',
        sampleCommand: [
            'prompt_engine({',
    ...
    },
    {
        ...
        summary: 'Use the `::` operator to add validation criteria directly in commands as natural language. Simple, flexible, and requires no setup.',
        sampleCommand: [
    ...
    },
    {
        ...
        summary: 'Use the new `gates` parameter to specify all types of validation in a single array. Accepts gate IDs, simple checks, and full gate definitions.',
        sampleCommand: [
...
export const promptEngineMetadata = {
    tool: 'prompt_engine',
