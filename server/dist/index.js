...
import { resolveRuntimeLaunchOptions } from './runtime/options.js';
 ...
 */
function setupHealthMonitoring(runtimeOptions) {
    if (!logger)
        return;
...
function initWorkspace(targetPath) {
    try {
        ...
        if ((existsSync(promptsDir) && readdirSync(promptsDir).length > 0) ||
    ...
    try {
        ...
        try {
            ...
            if (isVerbose && process.platform === 'win32') {
                ...
                debugLog(`DEBUG: Environment keys: ${Object.keys(process.env)
        ...
        if (!initialHealth) {
            ...
            throw new Error('Initial health validation failed - application may not be properly initialized. ' +
                'Health details: ' +
...
export function getDetailedDiagnostics() {
    ...
    try {
        ...
        return {
            available: false,
            reason: `Error collecting diagnostics: ${error instanceof Error ? error.message : String(error)}`,
            timestamp: new Date().toISOString(),
        };
    }
}
