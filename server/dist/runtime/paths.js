...
export class PathResolver {
     ...
     */
    getMethodologiesPath() {
        if (this.cache.methodologies)
            return this.cache.methodologies;
        let resolved;
        let source;
        // 1. CLI flag (highest priority)
     ...
     */
    logResolution(name, resolved, source) {
        if (this.debug) {
 ...
 */
export function parsePathCliOptions(args) {
    const options = {};
 ...
 */
export function validatePathCliOptions(options) {
    const errors = [];
 ...
 */
export function createPathResolver(args, packageRoot, debug = false) {
