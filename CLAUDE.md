...
- **Environment (secrets)**: `MCP_CATALOG_READ_TOKEN` gates executable prompt detail/history/compare; `MCP_CATALOG_WRITE_TOKEN` independently gates dry-run/apply/rollback and every legacy `/api/v1/tools/*` mutation. Either unset value refuses its surface (`503`), equal read/write values disable writes, and authentication runs before prompt lookup/body parsing. `MCP_HTTP_ALLOWED_ORIGINS` is a comma-separated allowlist for requests carrying `Origin`; absent `Origin` remains valid for server-side clients. HTTP transport only — STDIO reads none of these values.
