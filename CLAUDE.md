...
- **Environment (secrets)**: `MCP_CATALOG_READ_TOKEN` gates `GET /api/v1/catalog/prompts/:promptId`, the only REST route serving executable template content. It is listed apart from the path variables because it is the one env var that is a **credential**: unset is not a default but a refusal (`503`), and the route authenticates before it looks a prompt up, so a wrong token cannot be used to probe which prompt ids exist. HTTP transport only — STDIO never reads it.
