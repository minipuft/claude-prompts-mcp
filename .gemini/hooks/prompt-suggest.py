#!/usr/bin/env python3
"""
BeforeModel hook: Context injection for gemini-prompts extension.

Detects and provides guidance for:
- `>>prompt_id` - Prompt invocation with args, types, tool call
- `>>a --> >>b` - Chain syntax with step info
- `:: 'criteria'` - Inline gate syntax (reminds Gemini of responsibility)
- Active chain state - Shows current step, pending gates

Output: Rich context injected for Gemini to act on.
...
def main():
    ...
    # Gemini CLI BeforeModel hook provides the prompt differently
    # Try various field names for compatibility
    user_message = (
    ...
    if output_lines:
        output = "\n".join(output_lines)
        # Gemini CLI hook response format
        hook_response = {
