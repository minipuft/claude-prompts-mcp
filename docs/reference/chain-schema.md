...
| `await`              | `string`  | No       | `node` (the default: the run waits for this step) or `run` (the step is **detached**: its worker runs in a sub-agent while the run moves on, and its result reports later). `run` implies `delegated`. See [Detached steps](../concepts/chains-lifecycle.md#detached-steps-await-run).        |
