...
| `framework_compliance` | Nothing. GateValidator has no branch for this type, so it hits the auto-pass default. Stage 09b does inspect the response against the active framework's `phases.yaml` (section headers, `min_length`, `forbidden_terms`) — but it triggers on `phases.yaml` guards, not on this value. | Declarative only              | Documenting intent. To actually enforce sections, define guards in the framework's `phases.yaml` |
...
| Enforce required framework sections (e.g., CAGEERF Context/Analysis/Goals) | Guards in the framework's `phases.yaml`                  | Stage 09b inspects section headers + length + forbidden terms deterministically. Zero LLM cost; clear "Improvements Needed" feedback on failure. Setting `pass_criteria.type: framework_compliance` does **not** switch this on. |
