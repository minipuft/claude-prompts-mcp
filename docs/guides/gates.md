...
| `methodology_compliance` | Pipeline stage 09b inspects the response against the active framework's `phases.yaml` (section headers, `min_length`, `forbidden_terms`). Failure injects "Improvements Needed" feedback. | Hard structural enforcement   | Required framework sections (e.g., "## Context must exist with ≥100 chars") |
...
| Enforce required framework sections (e.g., CAGEERF Context/Analysis/Goals) | `methodology_compliance` (via phase guards)              | Stage 09b inspects section headers + length + forbidden terms deterministically. Zero LLM cost; clear "Improvements Needed" feedback on failure.                                  |
