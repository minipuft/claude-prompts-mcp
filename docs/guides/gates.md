...
| `script_tool`          | **Nothing on any live path.** Stage 20 runs `shell_verify` criteria and no others, so a declared `script_tool` criterion is never executed during a run. `GateValidator` does have a runner — it resolves `script_tool_id` against the registered script tools, runs THAT tool with JSON stdin, and fails closed when it cannot — but nothing in production calls it. | Not enforced                  | (none today — use `shell_verify`; see the warning below)                                         |

> [!WARNING]
> **`script_tool` declares an intent nothing acts on.** Stage 20 (`20-gate-review-stage.ts`) is the
> only live consumer of `pass_criteria`, and it filters for `shell_verify`. A gate declaring
> `script_tool` therefore clears review without running anything and without reporting that it did
> not run. The runner in `GateValidator` is correct and tested — it resolves the id to a registered
> tool rather than shelling it, and fails closed — but its whole call chain
> (`EngineValidator.validateWithGates` → `LightweightGateSystem.validateContent` →
> `GateValidator.validateGates`) has no production caller. Either wire the type into gate review or
> retire it; accepted-but-inert is the failure mode this table exists to prevent.
...
| Invoke a registered script tool with typed JSON input                      | `shell_verify` (not `script_tool`)                       | `script_tool` is accepted by the schema but executed by nothing — see the taxonomy table. Until it is wired into gate review, a `script_tool` criterion is silently inert, which is worse than declaring no gate at all.                                                                                                         |
