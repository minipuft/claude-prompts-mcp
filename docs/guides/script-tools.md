...
{% if tool_builder_result %} ## Created Successfully
...
```
resources/prompts/examples/create_framework/
├── prompt.yaml
├── user-message.md
└── tools/
    └── framework_builder/
...
| `>>create_framework` | Frameworks    | `resources/prompts/examples/create_framework/` |
...
Full process capabilities by design. The trust boundary is _who writes the script_, not _what the script can do_.
