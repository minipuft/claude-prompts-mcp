...
**Related**: [MCP Tooling Guide](mcp-tools.md) for command syntax, [Chains](chains.md) for multi-step gate validation.
...
| `GateDefinitionLoader` | `gates/core/gate-definition-loader.ts` | YAML + MD loading with caching and guidance inlining. |
| `RuntimeGateLoader` | `gates/core/runtime-gate-loader.ts` | Path resolution for npx/npm compatibility, guidance.md inlining. |
...
| Component | Location | Purpose |
| --- | --- | --- |
| `GateLoader` | `gates/core/gate-loader.ts` | Legacy loader, delegates to RuntimeGateLoader. |
| `GateValidator` | `gates/core/gate-validator.ts` | Orchestrates validation (string checks auto-pass; LLM validation pending). |
| `GateStateManager` | `gates/gate-state-manager.ts` | Persists gate enable/disable state and validation metrics. |
...
| Pipeline stages | `execution/pipeline/stages/02,05,10-*` | Inline extraction, gate enhancement, gate review. |
...
Edit files in `server/gates/{id}/`, and changes are hot-reloaded without server restart.

### Predefined Gates

| Gate ID | gate_type | Activation |
| --- | --- | --- |
...
- `gate_type` — Classification: `framework`, `category`, or `custom`
...
1. Create directory: `server/gates/{gate-id}/`
...
1. Edit `server/gates/{gate-id}/gate.yaml` or `guidance.md`
