...
| F2  | ~~**`framework_gates: false` is inert on the live path.** The planner's framework-gate filter reads `convertedPrompt.enhancedGateConfiguration`, and `enhancedGateConfiguration` has **no writer anywhere in the repo** — it is declared once and read in five places. A prompt's YAML flag lands in `gateConfiguration.framework_gates`, which that filter does not consult.~~ **Superseded (as of 2026-09-25):** the flag is live. `GateSetResolver` reads `gateConfiguration.framework_gates` and applies the `framework-gates-opt-out` veto, which removes the active framework's gates up to rank 60 (`prompt-config`), like `exclude`; since #394 a prompt carrying the flag also declares no framework section headers (R22). The framework's system prompt is still injected. | ~~filter at `execution/planning/execution-planner.ts:154`; declaration `execution/types.ts:73`; readers `execution-planner.ts:154,548,560` + `chain-session-router.ts:267`; writers: none~~ now: veto at `gates/services/gate-set-resolver.ts:219` (input read at `:501`); sections at `frameworks/declared-sections.ts:86`; `enhancedGateConfiguration` has no remaining reader |
